...
const activeRequestStatuses = new Set(["ready", "link_creating", "waiting", "authorized"]);
const paidRequestStatuses = new Set(["paid", "fiscalizing", "fiscalized", "posted"]);
...
function csrfToken() {
  ...
  const match = cookies.find((part) =>
    part.startsWith("__Host-arthello_csrf=") || part.startsWith("arthello_csrf="),
  );
...
function filteredObligations() {
  ...
  return state.obligations.filter((item) => {
    ...
    return [item.purpose, item.studentCrmId, item.contractId, branch?.branchName]
      .filter(Boolean)
...
function catalogFor(branchCrmId, legalEntityId) {
  return state.catalog.find((item) => item.branchCrmId === branchCrmId && item.legalEntityId === legalEntityId) ?? null;
}

function requestForObligation(obligationId) {
  return state.requests.find((item) => item.obligationId === obligationId) ?? null;
}
...
function stats() {
  ...
  const open = state.obligations.filter((item) => item.status === "open" && item.confirmedPaidKopecks < item.amountKopecks);
  const outstanding = open.reduce((sum, item) => sum + Math.max(0, item.amountKopecks - item.confirmedPaidKopecks), 0);
  const overdue = open.filter((item) => item.dueDate && item.dueDate < today).length;
  const waiting = state.requests.filter((item) => activeRequestStatuses.has(item.status)).length;
  const receipts = state.requests.filter((item) => item.receiptStatus && !["fiscalized", "done", "sent"].includes(item.receiptStatus)).length;
...
function mobileNavMarkup() {
  const items = [["overview","⌂","Обзор"],["obligations","₽","Начисления"],["requests","↗","Платежи"],["automation","↻","Авто"]];
  ...
  app.innerHTML = `
    <div class="app-shell">
      <aside class="sidebar">
        ...
        <div class="sidebar-user"><div class="avatar">${escapeHtml(initials(state.user?.name))}</div><div class="sidebar-user-copy"><strong>${escapeHtml(state.user?.name)}</strong><span>${escapeHtml(roleLabel(state.user?.role))}</span></div><button class="logout-icon" type="button" data-action="logout" aria-label="Выйти">↪</button></div>
      </aside>
      <main class="main">
        <header class="mobile-topbar"><div class="brand-lockup"><div class="brand-mark">A</div><div class="brand-copy"><strong>ArtHello Pay</strong><span>${escapeHtml(roleLabel(state.user?.role))}</span></div></div><button class="icon-button" type="button" data-action="logout" aria-label="Выйти">↪</button></header>
    ...
    <section>
      ...
      <div class="workspace-grid">
        ...
        <div class="side-stack">
          <article class="quick-card"><h3>Маршрутизация</h3><p>Филиал автоматически определяет правильного получателя.</p><div class="quick-list">${state.catalog.slice(0,5).map((item) => `<div class="quick-line"><span>${escapeHtml(item.branchName)}</span><strong>${escapeHtml(item.legalEntityName)}</strong></div>`).join("") || `<div class="empty-state" style="padding:18px 0"><strong>Маршруты не найдены</strong></div>`}</div></article>
...
function renderObligationRows(items, compact = false) {
  if (!items.length) return `<div class="empty-state"><div class="empty-icon">₽</div><strong>Начислений пока нет</strong><p>Создайте первую оплату — получатель будет выбран автоматически по филиалу.</p></div>`;
  return `<div class="data-list">${items.map((item) => {
    const branch = catalogFor(item.branchCrmId, item.legalEntityId);
    const request = requestForObligation(item.id);
    const outstanding = Math.max(0, item.amountKopecks - item.confirmedPaidKopecks);
    return `<div class="data-row">
      ...
      <div class="data-secondary"><strong>${escapeHtml(item.studentCrmId ? `Ученик ${item.studentCrmId}` : "Клиент")}</strong><span>${escapeHtml(branch?.legalEntityName || "Получатель определён системой")}</span></div>
  ...
  return `
    <section>
      ...
      <article class="panel">
        ...
        ${items.length ? `<div class="data-list">${items.map((item) => {
          const branch = catalogFor(item.branchCrmId, item.legalEntityId);
          return `<div class="data-row">
        ...
        }).join("")}</div>` : `<div class="empty-state"><div class="empty-icon">↗</div><strong>Платёжных ссылок пока нет</strong><p>Ссылка появится после создания начисления и проверки маршрута.</p></div>`}
      </article>
    </section>`;
}
...
function branchOptions() {
  return state.catalog.map((item) => `<option value="${escapeHtml(item.branchCrmId)}" data-legal="${escapeHtml(item.legalEntityId)}" ${state.modal?.branchCrmId === item.branchCrmId ? "selected" : ""}>${escapeHtml(item.branchName)} · ${escapeHtml(item.legalEntityName)}</option>`).join("");
}
    ...
    <div class="modal-backdrop" data-action="close-modal">
      <section class="modal" role="dialog" aria-modal="true" aria-labelledby="create-title" data-modal-stop>
        ...
        <form id="create-payment-form">
          ...
          <footer class="modal-footer"><button class="secondary-button" type="button" data-action="close-modal">Отмена</button><button class="primary-button" type="submit" ${!selected || modal.submitting ? "disabled" : ""}>${modal.submitting ? "Создаём…" : "Создать ссылку"}</button></footer>
        </form>
      </section>
    </div>`;
}
...
function renderCustomerResults() {
  if (state.customerLoading) return `<div class="empty-state" style="padding:20px"><div class="spinner" style="width:24px;height:24px;margin-bottom:10px"></div><p>Ищем клиентов…</p></div>`;
  if (!state.customerResults.length) return `<div class="empty-state" style="padding:20px"><strong>Введите фамилию или выберите клиента</strong><p>Показываются только клиенты выбранного филиала.</p></div>`;
  return state.customerResults.map((item, index) => `<button class="customer-option" type="button" data-action="select-customer" data-customer-index="${index}"><span class="customer-avatar">${escapeHtml(initials(item.studentName))}</span><span class="customer-copy"><strong>${escapeHtml(item.studentName)}</strong><span>${escapeHtml(item.payerName || "Семья")} · ${escapeHtml(item.payerPhone || item.payerEmail || "без контакта")}</span></span><span>→</span></button>`).join("");
}
...
async function loadCustomers(branchCrmId, query = "") {
  ...
  renderShell();
  try {
    state.customerResults = await api(`/api/payments/customers?branchCrmId=${encodeURIComponent(branchCrmId)}&q=${encodeURIComponent(query)}`);
  } catch (error) {
...
async function performLogin(form) {
  ...
  try {
    const session = await api("/api/auth/login", { method: "POST", body: JSON.stringify({ login, password }) });
...
async function createPayment(form) {
  ...
  const catalog = state.catalog.find((item) => item.branchCrmId === branchCrmId);
  const rawAmount = String(formData.get("amount") ?? "").replace(/\s/g, "").replace(",", ".");
  ...
  try {
    ...
    const obligation = await api("/api/payments/obligations", { method: "POST", body: JSON.stringify(payload) });
    const preview = await api(`/api/payments/obligations/${encodeURIComponent(obligation.id)}/requests/preview`, { method: "POST" });
    if (preview.decision?.action !== "ISSUE") {
      state.modal = { step: "success", obligation, warning: preview.decision?.reason || "Ссылка не создана: начисление требует проверки." };
      await loadWorkspace();
      renderShell();
      return;
    }
    if (!preview.route?.ready) {
      state.modal = { step: "success", obligation, warning: `Маршрут оплаты ещё не готов: ${preview.route?.reason || "требуется настройка владельца"}.` };
      await loadWorkspace();
      renderShell();
      return;
    }
    const request = await api(`/api/payments/obligations/${encodeURIComponent(obligation.id)}/requests`, {
      method: "POST",
...
async function cancelRequest(id) {
  try {
    await api(`/api/payments/requests/${encodeURIComponent(id)}/cancel`, { method: "POST" });
    state.modal = null;
    await loadWorkspace();
    renderShell();
...
async function renderPublicPayment() {
  ...
  try {
    const payment = await api(`/api/payments/public/${encodeURIComponent(match[1])}`);
    const paid = paidRequestStatuses.has(payment.status) || ["fiscalized", "posted"].includes(payment.status);
    app.innerHTML = `
        ...
        <section class="public-main">
          <article class="payment-card">
            ...
            <div class="payment-card-body">
              ...
              ${paid ? `<div class="payment-paid"><strong>Оплачено</strong><span>${payment.receiptUrl ? "Электронный чек уже сформирован." : "Платёж подтверждён. Чек появится здесь после фискализации."}</span></div>${payment.receiptUrl ? `<a class="secondary-button full-button" style="margin-top:12px;text-decoration:none;display:grid;place-items:center" href="${escapeHtml(payment.receiptUrl)}" target="_blank" rel="noopener">Открыть чек</a>` : ""}` : payment.canPay ? `
...
app.addEventListener("change", (event) => {
  const target = event.target;
  if (target instanceof HTMLSelectElement && target.matches("[data-status-filter]")) {
    state.statusFilter = target.value;
    renderShell();
  }
  if (target instanceof HTMLSelectElement && target.matches("[data-create-branch]")) {
    if (!state.modal) return;
...
app.addEventListener("click", (event) => {
  ...
  if (name === "close-modal") {
    if (raw.closest("[data-modal-stop]") && raw === action.closest("[data-modal-stop]")) return;
    state.modal = null;
    renderShell();
  }
  ...
  if (name === "change-customer" && state.modal) {
    state.modal.selectedCustomer = null;
    renderShell();
    void loadCustomers(state.modal.branchCrmId, state.modal.customerQuery || "");
  }
  if (name === "copy-link") void copyText(action.getAttribute("data-link") || "");
  if (name === "cancel-request") void cancelRequest(action.getAttribute("data-id") || "");
  if (name === "request-menu") {
    const item = state.requests.find((request) => request.id === action.getAttribute("data-id"));
    if (item) { state.modal = { type: "actions", kind: "request", item }; renderShell(); }
  }
  if (name === "obligation-menu") {
    const item = state.obligations.find((obligation) => obligation.id === action.getAttribute("data-id"));
    if (item) { state.modal = { type: "actions", kind: "obligation", item }; renderShell(); }
  }
});

app.addEventListener("click", (event) => {
  const target = event.target;
  if (target instanceof Element && target.matches("[data-modal-stop]")) event.stopPropagation();
});
