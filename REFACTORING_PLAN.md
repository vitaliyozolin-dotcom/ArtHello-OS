...
5. **[BEH, частично выполнено 2026-09-08]** 2.5 В `lib/db` module-level env-throw заменён на `createDb(env)` и ленивые совместимые exports; `alphaCrmClient` остаётся отдельным следующим изменением.
6. 2.6 D-номер о политике доступа к БД: drizzle builder по умолчанию; `db.execute(sql)` — допустим для отчётности; `pool.query` запрещён (энфорс lint-правилом фазы 1); PGlite — только в `scripts/`.
