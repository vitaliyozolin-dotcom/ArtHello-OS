...
4. **[реализовано 2026-09-10, D-105; ожидает exact CI]** Materialized `deploy/v44/src` и `deploy/v52/src` проходят собственный `tsc` с типами, генерируемыми закреплённым Wrangler; прежние 27 сиротских tests вошли в полный School suite. Matrix-job `school-source` собирает application stage обоих Dockerfile и тем самым выполняет typecheck, lint, build и все 135 School test-файлов на GitHub-hosted runner без production capability.
