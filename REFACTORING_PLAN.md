...
1. **[частично] [DEL]** Пять cleanup-блоков архивировали и удалили 35 завершённых либо поглощённых workflow: D060–D069, recovery R2–R12, ранние School diagnostics/cutover, legacy RU D059, R12 verifier, D092 artifact inspector, accepted R17 controller и standalone hosted production diagnostic. Параллельный approved-catalog importer довёл набор до 15; D116 снизил его до 14, D120 — до 13. Ratchet запрещает незаявленный рост выше 13. Blob/run provenance сохранён в `docs/workflow-archive-2026-09-09.md`; незавершённые и постоянные checks оставлены активными.
...
3. **[завершено 2026-09-10, D-099] [BEH]** Де-тарболизация School (R6/R6b/R9; не переписывание — нормализация version control): v44/v52 archive+override+patch деревья материализованы в `deploy/v44/src` и `deploy/v52/src`; исходные и canonical tree SHA закреплены в `deploy/school-source-manifest.json`. Точная старая сборочная цепочка воспроизведена без файлового diff, обе materialized Docker-сборки прошли, старые чанки и patch/override transport удалены. Одна неисполняемая evidence-копия сохранена по старому пути для frozen R13 contract. Постоянный hash-ratchet исполняется в `test:refactoring`.
4. **[завершено 2026-09-10, D-105/D-107; exact Quality 34477515417 PASS]** Materialized `deploy/v44/src` и `deploy/v52/src` проходят собственный `tsc` с типами, генерируемыми закреплённым Wrangler; прежние 27 сиротских tests вошли в полный School suite. D-107 явно исключает только воспроизводимые outputs `tsconfig.tsbuildinfo`/`.wrangler` из source identity и защищает это regression. Matrix-job `school-source` собирает application stage обоих Dockerfile и тем самым выполняет typecheck, lint, build и все 135 School test-файлов на GitHub-hosted runner без production capability. Exact SHA `2acc22559ca11f784c07ad6b2a859ee907c87306` прошёл оба School jobs, общий test/coverage, secret-scan и immutable provenance.
5. **[частично, D-119]** Единая ssh-конвенция: активный deploy и оба manual v44 fallback требуют заранее проверенный known_hosts; TOFU удалён. Локальный PostgreSQL backup стал атомарным, проверяет читаемость custom archive и переживает отдельный сбой через bounded retry. Off-host provider, alert destination и утверждённые RPO/RTO не заданы владельцем и остаются явным внешним блокером R17.
6. **[завершено в текущих доказуемых границах; D-106/D-119/D-120]** `test:full` включает `test:postgres`, importer/sandbox suite, весь последовательный legacy glob и четыре Python production-consumer contracts без прежнего повторного запуска Node-файлов через несколько aliases. Migration-twin test явно пропускается без создаваемого PostgreSQL gate артефакта и исполняется в своём gate. API policy-core сохраняет ratchet 93.32/70.51/92.92; поведенческий browser-startup scope — 100/78/90. `docs/test-strategy.md` запрещает новые assertions по incidental prose/formatting и разделяет behavioral, structural policy и frozen recovery contracts. Сохраняемые source inspections доказывают отрицательные architecture/migration/import инварианты и не объявляются behavioral coverage; frozen tests удаляются только вместе с последним recovery consumer.
