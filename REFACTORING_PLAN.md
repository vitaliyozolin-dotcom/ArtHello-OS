...
4. **[реализовано 2026-09-10, D-105/D-107; ожидает повторный exact CI]** Materialized `deploy/v44/src` и `deploy/v52/src` проходят собственный `tsc` с типами, генерируемыми закреплённым Wrangler; прежние 27 сиротских tests вошли в полный School suite. Exact D-106 School matrix jobs PASS, но общий Quality выявил загрязнённый локальным `tsconfig.tsbuildinfo` manifest hash. D-107 явно исключает только воспроизводимые outputs `tsconfig.tsbuildinfo`/`.wrangler` из source identity и добавляет regression. Matrix-job `school-source` собирает application stage обоих Dockerfile и тем самым выполняет typecheck, lint, build и все 135 School test-файлов на GitHub-hosted runner без production capability.
