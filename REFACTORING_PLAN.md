...
1. **[частично] [DEL]** Четыре cleanup-блока архивировали и удалили 34 завершённых workflow: D060–D069, recovery R2–R12, ранние School diagnostics/cutover, legacy RU D059, R12 verifier, D092 artifact inspector и accepted R17 controller. Параллельный approved-catalog importer довёл набор до 15; D116 снизил его до 14 и ratchet запрещает незаявленный рост выше 14. Blob/run provenance сохранён в `docs/workflow-archive-2026-09-09.md`; незавершённые и постоянные checks оставлены активными.
...
6. **[частично; D-106/D-119]** Тестовая стратегия: `test:full` включает `test:postgres`, importer/sandbox suite и весь последовательный legacy glob `scripts/test/*.test.mjs`; migration-twin test явно пропускается без создаваемого PostgreSQL gate артефакта и исполняется в своём gate. API policy-core сохраняет ratchet 93.32/70.51/92.92; отдельный поведенческий browser-startup scope добавляет фактически измеренный ratchet 100/78/90. Spent release contracts пока сохранены как recovery anti-drift evidence; массовая замена остальных осознанных source-policy assertions требует самостоятельных эквивалентных behavioral contracts.
