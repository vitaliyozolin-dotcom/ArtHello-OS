...
import { retireR15Browser } from '../../deploy/browser/retire-r15-browser.mjs';

const SOURCE = '4a0713b4a7d87f132e49836fe0ce9ca9258bc1ec';
const NEXT = 'a'.repeat(40);
const TREE = 'ac3fcaac06acf33bf9ee32e438d0c040adb38fd7';
const BROWSER = 'sha256:6ab75a9ba76c0df8439fd2f4a532191bd4f86fc641eaf7c50a4e2be1385b5977';
...
const FINGERPRINTS = {
  [BROWSER]: '65c1a6965f1873326f4cd8868122b2bb31d1048860111edd04e81f2f822d351e',
  [APP]: 'a28b64b3a57f96eb8e23c18d888d033f70d90f92b2ef98df5cc7b07c920d153a',
};
const ARCHIVES = {
  '/actions/artifacts/10139358225': { id: 10139358225, name: `server-browser-${SOURCE}-34445017241-1`, size_in_bytes: 640796077, digest: 'sha256:c80713c5d61a29c0c6d8b9e9860f14029f6ff96ba264b11f445bde9854eadc7a', expired: false, expires_at: '2026-09-11T08:01:35Z', workflow_run: { id: 34445017241, repository_id: 1311964413, head_repository_id: 1311964413, head_branch: 'main', head_sha: SOURCE } },
};
...
test('only verified R15 browser is retired; app, backup and volumes remain outside the command surface', async () => {
  const f = fixture();
  const result = await retireR15Browser(f.dependencies);
  assert.deepEqual(result, { kind: 'r15-browser-retirement', result: 'ready', browser: 'retired', capacity: { scratchAvailableKiB: 9000000, scratchRequiredKiB: 3348735, dockerAvailableKiB: 9000000, dockerRequiredKiB: 7995084 } });
...
test('invalid size/source/temp inputs reject before any Docker mutation', async () => {
  for (const environment of [{ ARCHIVE_BYTES: '0' }, { ARCHIVE_BYTES: 'PRIVATE' }, { ARCHIVE_BYTES: '2e3' }, { EXPANDED_BYTES: '640' }, { EXPANDED_BYTES: String(101 * 1024 ** 3) }, { CHECKED_SOURCE_SHA: SOURCE }, { CHECKED_SOURCE_SHA: 'bad' }, { RUNNER_TEMP: 'relative' }, { RUNNER_TEMP: '/tmp\nPRIVATE' }]) {
    ...
    assert.equal((await retireR15Browser(f.dependencies)).result, 'blocked');
    assert.deepEqual(f.removals, []);
  }
});
...
for (const artifactId of [10139358225]) test(`artifact ${artifactId} must match every immutable recovery field and fresh expiry`, async () => {
...
test('only unique own-repository self digests are accepted, independently from runtime fingerprint', async () => {
  ...
  assert.equal((await retireR15Browser(f.dependencies)).result, 'ready');
});
...
test('recovery archive is freshly rechecked immediately before retirement', async () => {
  ...
  f.dependencies.readGitHub = async path => {
    ...
    if (path === '/actions/artifacts/10139358225' && ++archives > 1) value.expired = true;
    return value;
  };
  const result = await retireR15Browser(f.dependencies);
  assert.equal(result.result, 'blocked');
...
test('CLI has no target/force/general mode and suppresses argument/exception details', () => {
  const script = fileURLToPath(new URL('../../deploy/browser/retire-r15-browser.mjs', import.meta.url));
