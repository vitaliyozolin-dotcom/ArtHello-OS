...
import {
  normalizeRuntimeEnv,
  renderCentralRoute,
} from "../../deploy/atlas-runtime-recovery-d183.mjs";
...
test("D183 production workflow is manual, protected, Atlas-only and independently verified", () => {
  const workflow = read(".github/workflows/deploy-diaries-d133.yml");
  const release = read("deploy/release-atlas-d183.sh");

  assert.match(workflow, /workflow_dispatch:/);
  assert.match(workflow, /DEPLOY D183 TO PRODUCTION/);
  ...
  assert.match(workflow, /Verify exact D183 from an independent network/);
  assert.doesNotMatch(
    workflow,
  ...
  for (const marker of [
    "ATLAS_D183_PREDECESSOR=VERIFIED",
    "ATLAS_D183_BACKUP=VERIFIED",
    "ATLAS_D183_CENTRAL_SSO_OPEN=VERIFIED",
    "ATLAS_D183_OWNER_SSO=VERIFIED",
    "ATLAS_D183_ROLLBACK=STARTED",
    "ATLAS_D183_PRODUCTION=VERIFIED",
  ])
