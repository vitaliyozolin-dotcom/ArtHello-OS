...
import { retireD144Browser } from '../../deploy/browser/retire-d144-browser.mjs';

const SOURCE = '1fc6433206c2e3462cd383e6812747a373102317';
const NEXT = 'a'.repeat(40);
const TREE = 'c66b66c268dcb137147fd0d1e933d44f69ac4d2c';
const BROWSER = 'sha256:a2f6aa28d907b2a9ce1d5de7c871e0af32ece7fda696cd70957c15f8da51a7c4';
const APP = 'sha256:d17040f46e3515de2b16a20bc5cb787cfa3f9e702cb780f5f7a0d002a0d801f0';
const BROWSER_TAG = 'arthello-e2e:' + SOURCE;
const APP_TAGS = ['arthello-direct:' + SOURCE, 'arthello-v52-verify:' + SOURCE];
const FINGERPRINTS = {
  [BROWSER]: '268c28e75c1b954dfe18066e209c8a52c5bd19be57e7c084134dcb4496cf4a0c',
  [APP]: '821990e025610869fc4361d0969aee2477ff67ceaf3907683f23f794bad29fdc',
};
const ARCHIVES = {
  '/actions/artifacts/10196489198': { id: 10196489198, name: `server-browser-${SOURCE}-34593160135-1`, size_in_bytes: 640788295, digest: 'sha256:996b59d29be50592f16879b31826513651ce28f1948b47483000f228db7b7e34', expired: false, expires_at: '2026-09-13T11:17:27Z', workflow_run: { id: 34593160135, repository_id: 1311964413, head_repository_id: 1311964413, head_branch: 'main', head_sha: SOURCE } },
};
...
function fixture(options = {}) {
  ...
  const dependencies = {
    environment: { CHECKED_SOURCE_SHA: NEXT, ARCHIVE_BYTES: '640788153', EXPANDED_BYTES: '1585953792', RUNNER_TEMP: '/owned runner/temp', ...options.environment },
...
test('only verified D144 browser is retired; app, backup and volumes remain outside the command surface', async () => {
  const f = fixture();
  const result = await retireD144Browser(f.dependencies);
  assert.deepEqual(result, { kind: 'd144-browser-retirement', result: 'ready', browser: 'retired', capacity: { scratchAvailableKiB: 9000000, scratchRequiredKiB: 3348692, dockerAvailableKiB: 9000000, dockerRequiredKiB: 7995041 } });
...
test('invalid size/source/temp inputs reject before any Docker mutation', async () => {
  for (const environment of [{ ARCHIVE_BYTES: '0' }, { ARCHIVE_BYTES: 'PRIVATE' }, { ARCHIVE_BYTES: '2e3' }, { EXPANDED_BYTES: '640' }, { EXPANDED_BYTES: String(101 * 1024 ** 3) }, { CHECKED_SOURCE_SHA: SOURCE }, { CHECKED_SOURCE_SHA: 'bad' }, { RUNNER_TEMP: 'relative' }, { RUNNER_TEMP: '/tmp\nPRIVATE' }]) {
    ...
    assert.equal((await retireD144Browser(f.dependencies)).result, 'blocked');
    assert.deepEqual(f.removals, []);
  }
});
...
for (const artifactId of [10196489198]) test(`artifact ${artifactId} must match every immutable recovery field and fresh expiry`, async () => {
...
test('only unique own-repository self digests are accepted, independently from runtime fingerprint', async () => {
  ...
  assert.equal((await retireD144Browser(f.dependencies)).result, 'ready');
});
...
test('recovery archive is freshly rechecked immediately before retirement', async () => {
  ...
  f.dependencies.readGitHub = async path => {
    ...
    if (path === '/actions/artifacts/10196489198' && ++archives > 1) value.expired = true;
    return value;
  };
  const result = await retireD144Browser(f.dependencies);
  assert.equal(result.result, 'blocked');
...
test('CLI has no target/force/general mode and suppresses argument/exception details', () => {
  const script = fileURLToPath(new URL('../../deploy/browser/retire-d144-browser.mjs', import.meta.url));
