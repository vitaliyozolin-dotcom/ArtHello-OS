...
SOURCE_DIR = HERE if (HERE / 'repair.py').exists() else HERE.parent.parent / 'deploy/school/sso-relay-r5'
...
class RepairTest(unittest.TestCase):
    ...
    def test_full_mount_permutation_is_stable_without_mutating_or_dropping_entries(self):
        mounts = [
            {'Type': 'bind', 'Source': '/config', 'Destination': '/config', 'RW': False,
             'Propagation': 'rprivate', 'UnknownDockerField': {'values': [2, 1]}},
            {'Type': 'volume', 'Name': 'school-data', 'Source': '/data', 'Destination': '/data',
             'Driver': 'local', 'Mode': 'ro', 'RW': False, 'Propagation': ''},
        ]
        self.engine.school['Mounts'] = copy.deepcopy(mounts)
        baseline = r.school_fingerprint(self.engine.school)
        self.assertEqual(self.engine.school['Mounts'], mounts)
        self.engine.school['Mounts'].reverse()
        self.assertEqual(r.school_fingerprint(self.engine.school), baseline)
        self.assertEqual(self.engine.school['Mounts'], list(reversed(mounts)))
        self.engine.school['Mounts'].append(copy.deepcopy(mounts[0]))
        self.assertNotEqual(r.school_fingerprint(self.engine.school), baseline, 'duplicate mounts must remain significant')

    def test_each_mount_field_and_unknown_nested_value_remains_fingerprint_significant(self):
        original = {'Type': 'bind', 'Source': '/config', 'Destination': '/config', 'RW': False,
                    'Mode': 'ro', 'Propagation': 'rprivate', 'UnknownDockerField': {'values': [2, 1]}}
        self.engine.school['Mounts'] = [copy.deepcopy(original)]
        baseline = r.school_fingerprint(self.engine.school)
        for key, value in [('Type', 'volume'), ('Source', '/other'), ('Destination', '/different'),
                           ('RW', True), ('Mode', 'rw'), ('Propagation', 'rshared'),
                           ('UnknownDockerField', {'values': [1, 2]}), ('NewField', 'must-not-be-ignored')]:
            with self.subTest(key=key):
                self.engine.school['Mounts'] = [dict(copy.deepcopy(original), **{key: value})]
                self.assertNotEqual(r.school_fingerprint(self.engine.school), baseline)

    def test_config_hostconfig_and_network_arrays_are_not_canonicalized(self):
        school = self.engine.school
        school['Config']['Env'] = ['FIRST=one', 'SECOND=two']
        school['HostConfig']['Mounts'] = [{'Target': '/first'}, {'Target': '/second'}]
        school['NetworkSettings']['Networks'][r.BACKEND]['Aliases'] = ['first', 'second']
        baseline = r.school_fingerprint(school)
        for parent, key in [(school['Config'], 'Env'), (school['HostConfig'], 'Mounts'),
                            (school['NetworkSettings']['Networks'][r.BACKEND], 'Aliases')]:
            parent[key].reverse()
            self.assertNotEqual(r.school_fingerprint(school), baseline)
            parent[key].reverse()
            self.assertEqual(r.school_fingerprint(school), baseline)

    def test_create_and_readonly_retry_survive_mount_order_changes_between_inspects(self):
        self.engine.school['Mounts'].append({'Source': '/school-config', 'Destination': '/config', 'RW': False})
        inspect = self.engine.inspect
        calls = 0
        def permuted(kind, name, optional=False):
            nonlocal calls
            value = inspect(kind, name, optional)
            if kind == 'container' and name in (r.SCHOOL, SID):
                calls += 1
                if calls % 2: value['Mounts'].reverse()
            return value
        with patch.object(r, 'inspect', side_effect=permuted):
            first = self.invoke()
            self.engine.mutations.clear()
            second = self.invoke()
        self.assertEqual(first['mode'], 'created')
        self.assertEqual(second['mode'], 'verified-existing')
        self.assertEqual(first['activatedAtUtc'], second['activatedAtUtc'])
        self.assertEqual(self.engine.mutations, [])
        self.assertGreaterEqual(calls, 4)

    def test_genuine_mount_change_refuses_existing_repair_without_mutation(self):
        self.invoke()
        self.engine.mutations.clear()
        self.engine.school['Mounts'][0]['Source'] = '/different-live-data'
        with self.assertRaisesRegex(r.Refused, 'configuration_backup_mismatch'):
            self.invoke()
        self.assertEqual(self.engine.mutations, [])
