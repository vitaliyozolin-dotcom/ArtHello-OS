...
import { retireR13Browser } from '../../deploy/browser/retire-r13-browser.mjs';
...
const BROWSER = 'sha256:43ee8d5da7fe768ac5883b407beb64eeaae071b3209e41ac65aace42ad707be5';
...
const FINGERPRINTS = {
  [BROWSER]: 'da3879c09cfa263d7a3192ab566bef914179328a93dcc3ef65121b220896bee3',
  [APP]: 'a28b64b3a57f96eb8e23c18d888d033f70d90f92b2ef98df5cc7b07c920d153a',
};
const ARCHIVES = {
  '/actions/artifacts/10099592667': { id: 10099592667, name: `server-browser-${SOURCE}-34340461537-1`, size_in_bytes: 640803216, digest: 'sha256:cd441f458efd0f25d62ae60942dbaab9713016597b446d6e75fcd3e9106a2b27', expired: false, expires_at: '2026-09-11T08:01:35Z', workflow_run: { id: 34340461537, repository_id: 1311964413, head_repository_id: 1311964413, head_branch: 'main', head_sha: SOURCE } },
};
...
test('only accepted R13 browser is retired; app, backup and volumes remain outside the command surface', async () => {
  const f = fixture();
  const result = await retireR13Browser(f.dependencies);
  assert.deepEqual(result, { kind: 'r13-browser-retirement', result: 'ready', browser: 'retired', capacity: { scratchAvailableKiB: 9000000, scratchRequiredKiB: 3348735, dockerAvailableKiB: 9000000, dockerRequiredKiB: 7995084 } });
...
test('invalid size/source/temp inputs reject before any Docker mutation', async () => {
  for (const environment of [{ ARCHIVE_BYTES: '0' }, { ARCHIVE_BYTES: 'PRIVATE' }, { ARCHIVE_BYTES: '2e3' }, { EXPANDED_BYTES: '640' }, { EXPANDED_BYTES: String(101 * 1024 ** 3) }, { CHECKED_SOURCE_SHA: SOURCE }, { CHECKED_SOURCE_SHA: 'bad' }, { RUNNER_TEMP: 'relative' }, { RUNNER_TEMP: '/tmp\nPRIVATE' }]) {
    ...
    assert.equal((await retireR13Browser(f.dependencies)).result, 'blocked');
    assert.deepEqual(f.removals, []);
  }
});
...
for (const artifactId of [10099592667]) test(`artifact ${artifactId} must match every immutable recovery field and fresh expiry`, async () => {
...
test('only unique own-repository self digests are accepted, independently from runtime fingerprint', async () => {
  ...
  assert.equal((await retireR13Browser(f.dependencies)).result, 'ready');
});
...
test('recovery archive is freshly rechecked immediately before retirement', async () => {
  ...
  f.dependencies.readGitHub = async path => {
    ...
    if (path === '/actions/artifacts/10099592667' && ++archives > 1) value.expired = true;
    return value;
  };
  const result = await retireR13Browser(f.dependencies);
  assert.equal(result.result, 'blocked');
...
test('CLI has no target/force/general mode and suppresses argument/exception details', () => {
  const script = fileURLToPath(new URL('../../deploy/browser/retire-r13-browser.mjs', import.meta.url));
