...
import { retireR12Browser } from '../../deploy/browser/retire-r12-browser.mjs';

const SOURCE = '77f26ec9bcba7233f39d5e8cb9f59c276bc8c1ed';
const NEXT = 'a'.repeat(40);
const TREE = 'ac3fcaac06acf33bf9ee32e438d0c040adb38fd7';
const BROWSER = 'sha256:e07d75e5964218b4382d98e255c5f17829dc6a4429ccef3060fb833d4cc3b61f';
...
const FINGERPRINTS = {
  [BROWSER]: '3bd2ca4a24dad705752258568b501414a640cdca95cfd51d36c951eb1c6bbf30',
  [APP]: 'a28b64b3a57f96eb8e23c18d888d033f70d90f92b2ef98df5cc7b07c920d153a',
};
const ARCHIVES = {
  '/actions/artifacts/10094111549': { id: 10094111549, name: `server-browser-${SOURCE}-34326582961-1`, size_in_bytes: 640811234, digest: 'sha256:b4b05bbddb30dbd6a0745a6d5e54f9f1b6199a4e5166d12a511fd2b27aedf821', expired: false, expires_at: '2026-09-11T08:01:35Z', workflow_run: { id: 34326582961, repository_id: 1311964413, head_repository_id: 1311964413, head_branch: 'main', head_sha: SOURCE } },
};
...
test('only accepted R12 browser is retired; app, backup and volumes remain outside the command surface', async () => {
  const f = fixture();
  const result = await retireR12Browser(f.dependencies);
  assert.deepEqual(result, { kind: 'r12-browser-retirement', result: 'ready', browser: 'retired', capacity: { scratchAvailableKiB: 9000000, scratchRequiredKiB: 3348735, dockerAvailableKiB: 9000000, dockerRequiredKiB: 7995084 } });
...
test('invalid size/source/temp inputs reject before any Docker mutation', async () => {
  for (const environment of [{ ARCHIVE_BYTES: '0' }, { ARCHIVE_BYTES: 'PRIVATE' }, { ARCHIVE_BYTES: '2e3' }, { EXPANDED_BYTES: '640' }, { EXPANDED_BYTES: String(101 * 1024 ** 3) }, { CHECKED_SOURCE_SHA: SOURCE }, { CHECKED_SOURCE_SHA: 'bad' }, { RUNNER_TEMP: 'relative' }, { RUNNER_TEMP: '/tmp\nPRIVATE' }]) {
    ...
    assert.equal((await retireR12Browser(f.dependencies)).result, 'blocked');
    assert.deepEqual(f.removals, []);
  }
});
...
for (const artifactId of [10094111549]) test(`artifact ${artifactId} must match every immutable recovery field and fresh expiry`, async () => {
...
test('only unique own-repository self digests are accepted, independently from runtime fingerprint', async () => {
  ...
  assert.equal((await retireR12Browser(f.dependencies)).result, 'ready');
});
...
test('recovery archive is freshly rechecked immediately before retirement', async () => {
  ...
  f.dependencies.readGitHub = async path => {
    ...
    if (path === '/actions/artifacts/10094111549' && ++archives > 1) value.expired = true;
    return value;
  };
  const result = await retireR12Browser(f.dependencies);
  assert.equal(result.result, 'blocked');
...
test('CLI has no target/force/general mode and suppresses argument/exception details', () => {
  const script = fileURLToPath(new URL('../../deploy/browser/retire-r12-browser.mjs', import.meta.url));
