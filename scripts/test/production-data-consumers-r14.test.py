"""R14 public consumer proof with real durable validators and bounded FakeDocker."""
import copy
...
class PublicConsumerTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix='r14-public-consumers-', dir=Path.home())
        self.addCleanup(self.temp.cleanup)
        self.f = donor.HeldR14(Path(self.temp.name).resolve())
        c = self.f.context
    ...
    def test_both_accepted_ancestors_remain_readonly_and_individually_pinned(self):
        older,fixture=self.historical()
        retired=donor.controller_tests.ControllerTests.retained_r12(fixture)
        before=copy.deepcopy(self.f.backup.docker.objects),fixture.historical_path.read_bytes(),fixture.live_historical_path.read_bytes()
        self.assertEqual(self.observe()['liveId'],self.f.candidate['Id'])
        self.assertEqual(before,(self.f.backup.docker.objects,fixture.historical_path.read_bytes(),fixture.live_historical_path.read_bytes()))
        with patch.object(consumers.controller,'LIVE_CONTEXT_SHA256','0'*64),self.assertRaises(REFUSED):
            self.observe()
...
class R14LauncherTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.fixture.environment['EXPECTED_LIVE_SOURCE_SHA'] = 'd44137d8342b7eacec510f9f70ffeba6c3bf4f3a'
        ...
        replacements = [
            ...
            ("sourceSha='a'*40", "sourceSha='d44137d8342b7eacec510f9f70ffeba6c3bf4f3a'"),
    ...
    def new_observations(self):
        ...
        observations = [(index, args) for index, (name, args) in enumerate(calls)
                        if name == 'python3' and 'scripts/production-data-consumers-r14.py' in args]
        for _, arguments in observations:
            self.assertEqual(arguments[:2], ['-I', 'scripts/production-data-consumers-r14.py'])
            self.assertEqual(arguments[arguments.index('--expected-release') + 1], self.fixture.environment['EXPECTED_LIVE_SOURCE_SHA'])
        return observations

    def test_fixed_r14_launcher_observes_before_and_after_complete_or_incomplete_probe(self):
    ...
    def test_initial_r14_refusal_has_no_legacy_fallback_or_database_probe(self):
    ...
    def test_final_r14_proof_only_drift_blocks_unchanged_legacy_identity_fields(self):
