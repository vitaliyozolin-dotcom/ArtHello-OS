...
test -s deploy/school/sso-relay/relay.mjs
...
docker create --name "$relay" --network "$network_school" --network-alias "$hostname" \
  ...
  --mount "type=bind,src=$work,dst=/fixtures,readonly" \
  --mount "type=bind,src=$(pwd)/deploy/school/sso-relay/relay.mjs,dst=/relay/relay.mjs,readonly" \
...
printf 'SCHOOL_RELAY_HOSTED_DOCKER_DNS_TLS_UID443_ACL=VERIFIED\n'
