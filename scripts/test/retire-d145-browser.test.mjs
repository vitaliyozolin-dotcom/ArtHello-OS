...
import { retireD145Browser } from '../../deploy/browser/retire-d145-browser.mjs';

const SOURCE = 'c848acd0c4efae04ad83a80ad20ad144325e9728';
const NEXT = 'a'.repeat(40);
const TREE = '8c54e63fe05de1351e42ba09cd98fcc26babe6b6';
const BROWSER = 'sha256:9c57b128a54eec71a88516512a21ac28dea3f53782c5f4e385dd3a6096d58dfd';
const APP = 'sha256:0fc0dd446c097acd128a6652d6f8f1bdd52227e93fa0cae055042a28340b2da3';
const BROWSER_TAG = 'arthello-e2e:' + SOURCE;
const APP_TAGS = ['arthello-direct:' + SOURCE, 'arthello-v52-verify:' + SOURCE];
const FINGERPRINTS = {
  [BROWSER]: '01e7affd1693882763b8546af85c15038e0eef3f02ec6b261259949aec047244',
  [APP]: 'fcc3fe71614c2ccaff5d8a8f4e99d8afadc44d73304e019ca2a124c9ee07d451',
};
const ARCHIVES = {
  '/actions/artifacts/10261836819': { id: 10261836819, name: `server-browser-${SOURCE}-34595178841-1`, size_in_bytes: 640801336, digest: 'sha256:d3a6623577671aa9efd7e9c43a7fb9f2bd59823bc690d5db6fbabb499cb2ef04', expired: false, expires_at: '2026-09-13T11:42:32Z', workflow_run: { id: 34595178841, repository_id: 1311964413, head_repository_id: 1311964413, head_branch: 'main', head_sha: SOURCE } },
};
...
function fixture(options = {}) {
  ...
  const dependencies = {
    environment: { CHECKED_SOURCE_SHA: NEXT, ARCHIVE_BYTES: '640801194', EXPANDED_BYTES: '1585953792', RUNNER_TEMP: '/owned runner/temp', ...options.environment },
...
test('only verified D145 browser is retired; app, backup and volumes remain outside the command surface', async () => {
  const f = fixture();
  const result = await retireD145Browser(f.dependencies);
  assert.deepEqual(result, { kind: 'd145-browser-retirement', result: 'ready', browser: 'retired', capacity: { scratchAvailableKiB: 9000000, scratchRequiredKiB: 3348717, dockerAvailableKiB: 9000000, dockerRequiredKiB: 7995066 } });
...
test('invalid size/source/temp inputs reject before any Docker mutation', async () => {
  for (const environment of [{ ARCHIVE_BYTES: '0' }, { ARCHIVE_BYTES: 'PRIVATE' }, { ARCHIVE_BYTES: '2e3' }, { EXPANDED_BYTES: '640' }, { EXPANDED_BYTES: String(101 * 1024 ** 3) }, { CHECKED_SOURCE_SHA: SOURCE }, { CHECKED_SOURCE_SHA: 'bad' }, { RUNNER_TEMP: 'relative' }, { RUNNER_TEMP: '/tmp\nPRIVATE' }]) {
    ...
    assert.equal((await retireD145Browser(f.dependencies)).result, 'blocked');
    assert.deepEqual(f.removals, []);
  }
});
...
for (const artifactId of [10261836819]) test(`artifact ${artifactId} must match every immutable recovery field and fresh expiry`, async () => {
  const mutations = [x => { x.id++; }, x => { x.name += '-other'; }, x => { x.digest = 'sha256:' + 'f'.repeat(64); }, x => { x.size_in_bytes++; }, x => { x.expired = true; }, x => { x.expires_at = '2026-09-11T10:59:59Z'; }, x => { x.expires_at = 'invalid'; }, x => { x.workflow_run.id++; }, x => { x.workflow_run.head_sha = NEXT; }, x => { x.workflow_run.repository_id++; }, x => { x.workflow_run.head_repository_id++; }, x => { x.workflow_run.head_branch = 'feature'; }];
...
test('only unique own-repository self digests are accepted, independently from runtime fingerprint', async () => {
  ...
  assert.equal((await retireD145Browser(f.dependencies)).result, 'ready');
});
...
test('recovery archive is freshly rechecked immediately before retirement', async () => {
  ...
  f.dependencies.readGitHub = async path => {
    ...
    if (path === '/actions/artifacts/10261836819' && ++archives > 1) value.expired = true;
    return value;
  };
  const result = await retireD145Browser(f.dependencies);
  assert.equal(result.result, 'blocked');
...
test('CLI has no target/force/general mode and suppresses argument/exception details', () => {
  const script = fileURLToPath(new URL('../../deploy/browser/retire-d145-browser.mjs', import.meta.url));
