...
import { retireR14Browser } from '../../deploy/browser/retire-r14-browser.mjs';

const SOURCE = 'd44137d8342b7eacec510f9f70ffeba6c3bf4f3a';
const NEXT = 'a'.repeat(40);
const TREE = 'ac3fcaac06acf33bf9ee32e438d0c040adb38fd7';
const BROWSER = 'sha256:19a8671fd03ba31bcfc3ddc7d479dceeabcfbcb836b61bfa7fda17fbf8f77f1a';
...
const FINGERPRINTS = {
  [BROWSER]: '666c2af60d83da845ca39e8930b659fd70e7a9030eb5060ef5ba90031fa24f40',
  [APP]: 'a28b64b3a57f96eb8e23c18d888d033f70d90f92b2ef98df5cc7b07c920d153a',
};
const ARCHIVES = {
  '/actions/artifacts/10119716232': { id: 10119716232, name: `server-browser-${SOURCE}-34391105865-1`, size_in_bytes: 640801625, digest: 'sha256:0835c0a93f375c847d52d9e5178b4f5570e450cba1a61407b976536a46872102', expired: false, expires_at: '2026-09-11T08:01:35Z', workflow_run: { id: 34391105865, repository_id: 1311964413, head_repository_id: 1311964413, head_branch: 'main', head_sha: SOURCE } },
};
...
test('only verified R14 browser is retired; app, backup and volumes remain outside the command surface', async () => {
  const f = fixture();
  const result = await retireR14Browser(f.dependencies);
  assert.deepEqual(result, { kind: 'r14-browser-retirement', result: 'ready', browser: 'retired', capacity: { scratchAvailableKiB: 9000000, scratchRequiredKiB: 3348735, dockerAvailableKiB: 9000000, dockerRequiredKiB: 7995084 } });
...
test('invalid size/source/temp inputs reject before any Docker mutation', async () => {
  for (const environment of [{ ARCHIVE_BYTES: '0' }, { ARCHIVE_BYTES: 'PRIVATE' }, { ARCHIVE_BYTES: '2e3' }, { EXPANDED_BYTES: '640' }, { EXPANDED_BYTES: String(101 * 1024 ** 3) }, { CHECKED_SOURCE_SHA: SOURCE }, { CHECKED_SOURCE_SHA: 'bad' }, { RUNNER_TEMP: 'relative' }, { RUNNER_TEMP: '/tmp\nPRIVATE' }]) {
    ...
    assert.equal((await retireR14Browser(f.dependencies)).result, 'blocked');
    assert.deepEqual(f.removals, []);
  }
});
...
for (const artifactId of [10119716232]) test(`artifact ${artifactId} must match every immutable recovery field and fresh expiry`, async () => {
...
test('only unique own-repository self digests are accepted, independently from runtime fingerprint', async () => {
  ...
  assert.equal((await retireR14Browser(f.dependencies)).result, 'ready');
});
...
test('recovery archive is freshly rechecked immediately before retirement', async () => {
  ...
  f.dependencies.readGitHub = async path => {
    ...
    if (path === '/actions/artifacts/10119716232' && ++archives > 1) value.expired = true;
    return value;
  };
  const result = await retireR14Browser(f.dependencies);
  assert.equal(result.result, 'blocked');
...
test('CLI has no target/force/general mode and suppresses argument/exception details', () => {
  const script = fileURLToPath(new URL('../../deploy/browser/retire-r14-browser.mjs', import.meta.url));
