...
import { retireR17Browser } from '../../deploy/browser/retire-r17-browser.mjs';

const SOURCE = 'ff8559254faaedade63a9ee7567a45686d08c13a';
const NEXT = 'a'.repeat(40);
const TREE = '6aeb7e6879b786a434d12f026f765bcaf9acb5a4';
const BROWSER = 'sha256:c158743b8877db1648f5048fdda69cce962d325285630a100c1e451829ab9e99';
const APP = 'sha256:34402014063a05c81754716f46b3f9059297f1d21d37da7e5f00b1eb8f7fdd46';
const BROWSER_TAG = 'arthello-e2e:' + SOURCE;
const APP_TAGS = ['arthello-direct:' + SOURCE, 'arthello-v52-verify:' + SOURCE];
const FINGERPRINTS = {
  [BROWSER]: 'e05306e01e58c2b51330a7a1403ac0c798ee89e687a1c834c557d13964d03822',
  [APP]: 'a97c538504d68613ed29d9de1230cf2ab23fc905207d6806d24a35f1d3e79bff',
};
const ARCHIVES = {
  '/actions/artifacts/10159561231': { id: 10159561231, name: `server-browser-${SOURCE}-34495273615-1`, size_in_bytes: 640794811, digest: 'sha256:e5770042a713f2a24ef9cd2f8b0f7b167e83b9811fc79e1e83ecde30481546c8', expired: false, expires_at: '2026-09-12T15:24:45Z', workflow_run: { id: 34495273615, repository_id: 1311964413, head_repository_id: 1311964413, head_branch: 'main', head_sha: SOURCE } },
};
...
function fixture(options = {}) {
  ...
  const dependencies = {
    environment: { CHECKED_SOURCE_SHA: NEXT, ARCHIVE_BYTES: '640794669', EXPANDED_BYTES: '1585953792', RUNNER_TEMP: '/owned runner/temp', ...options.environment },
...
test('only verified R17 browser is retired; app, backup and volumes remain outside the command surface', async () => {
  const f = fixture();
  const result = await retireR17Browser(f.dependencies);
  assert.deepEqual(result, { kind: 'r17-browser-retirement', result: 'ready', browser: 'retired', capacity: { scratchAvailableKiB: 9000000, scratchRequiredKiB: 3348705, dockerAvailableKiB: 9000000, dockerRequiredKiB: 7995054 } });
...
test('invalid size/source/temp inputs reject before any Docker mutation', async () => {
  for (const environment of [{ ARCHIVE_BYTES: '0' }, { ARCHIVE_BYTES: 'PRIVATE' }, { ARCHIVE_BYTES: '2e3' }, { EXPANDED_BYTES: '640' }, { EXPANDED_BYTES: String(101 * 1024 ** 3) }, { CHECKED_SOURCE_SHA: SOURCE }, { CHECKED_SOURCE_SHA: 'bad' }, { RUNNER_TEMP: 'relative' }, { RUNNER_TEMP: '/tmp\nPRIVATE' }]) {
    ...
    assert.equal((await retireR17Browser(f.dependencies)).result, 'blocked');
    assert.deepEqual(f.removals, []);
  }
});
...
for (const artifactId of [10159561231]) test(`artifact ${artifactId} must match every immutable recovery field and fresh expiry`, async () => {
...
test('only unique own-repository self digests are accepted, independently from runtime fingerprint', async () => {
  ...
  assert.equal((await retireR17Browser(f.dependencies)).result, 'ready');
});
...
test('recovery archive is freshly rechecked immediately before retirement', async () => {
  ...
  f.dependencies.readGitHub = async path => {
    ...
    if (path === '/actions/artifacts/10159561231' && ++archives > 1) value.expired = true;
    return value;
  };
  const result = await retireR17Browser(f.dependencies);
  assert.equal(result.result, 'blocked');
...
test('CLI has no target/force/general mode and suppresses argument/exception details', () => {
  const script = fileURLToPath(new URL('../../deploy/browser/retire-r17-browser.mjs', import.meta.url));
