...
import { retireR10Images } from '../../deploy/browser/retire-r10-images.mjs';

const SOURCE = '2e57dd22c6ff1cec1fcad0bd11af479e465c00bb';
const NEXT = 'a'.repeat(40);
const TREE = '445762d319e5c792d97617e8a4515045bc3b4126';
const BROWSER = 'sha256:44ef654e88c3e92d4e1a1d967b5054594b0b7ed96928819111ba52fee28d937d';
const APP = 'sha256:93a918799b751f973d2a921b3c7ddd6e8caf7024ca36ac8687e881b9223772c6';
const BROWSER_TAG = 'arthello-e2e:' + SOURCE;
const APP_TAGS = ['arthello-direct:' + SOURCE, 'arthello-v52-verify:' + SOURCE];
const FINGERPRINTS = {
  [BROWSER]: '9efafc066c918c5d68188e4eb3bc23c2feba82eb02875442d3db1072bafc1696',
  [APP]: '70c8d791f7d0fffaec5b990136b0049595d0fdda3798e9b3913ccaa35f0b215d',
};
const ARCHIVES = {
  '/actions/artifacts/10092321489': { id: 10092321489, name: `server-browser-${SOURCE}-34322039891-1`, size_in_bytes: 640802387, digest: 'sha256:6d4b187bf776d59a4386e19b03993c29f939a31b610444c058ce59b4fca1f53e', expired: false, expires_at: '2026-09-11T07:07:41Z', workflow_run: { id: 34322039891, repository_id: 1311964413, head_repository_id: 1311964413, head_branch: 'main', head_sha: SOURCE } },
  '/actions/artifacts/10092216577': { id: 10092216577, name: 'arthello-v52-verification-34321728779', size_in_bytes: 173108033, digest: 'sha256:33a11d033bdf075304d3a42289974b3802c4a5884b1d11ac7db6f6a886a5eae3', expired: false, expires_at: '2026-10-09T07:04:24Z', workflow_run: { id: 34321728779, repository_id: 1311964413, head_repository_id: 1311964413, head_branch: 'main', head_sha: SOURCE } },
};
  ...
  const dependencies = {
    ...
    docker: args => {
      ...
      if (args[0] === 'ps') {
        ...
        assert.ok([BROWSER, APP].includes(id));
      ...
      const target = args[3];
      assert.ok([BROWSER, APP, APP_TAGS[0]].includes(target), 'No alternate removal target');
      removals.push([...args]);
      ...
      if (target === APP_TAGS[0]) {
        const tagged = [...images.values()].find(x => x.RepoTags.includes(target));
        assert.ok(tagged);
        tagged.RepoTags = tagged.RepoTags.filter(tag => tag !== target);
        if (tagged.RepoTags.length === 0) images.delete(tagged.Id);
      } else images.delete(target);
...
test('browser alone satisfies unchanged budget and app is never inspected or queried', async () => {
  const f = fixture();
  const result = await retireR10Images(f.dependencies);
  assert.deepEqual(result, { kind: 'r10-image-retirement', result: 'ready', browser: 'retired', capacity: { scratchAvailableKiB: 9000000, scratchRequiredKiB: 3348735, dockerAvailableKiB: 9000000, dockerRequiredKiB: 7995084 } });
...
test('invalid size/source/temp inputs reject before any Docker mutation', async () => {
  for (const environment of [{ ARCHIVE_BYTES: '0' }, { ARCHIVE_BYTES: 'PRIVATE' }, { ARCHIVE_BYTES: '2e3' }, { EXPANDED_BYTES: '640' }, { EXPANDED_BYTES: String(101 * 1024 ** 3) }, { CHECKED_SOURCE_SHA: SOURCE }, { CHECKED_SOURCE_SHA: 'bad' }, { RUNNER_TEMP: 'relative' }, { RUNNER_TEMP: '/tmp\nPRIVATE' }]) {
    ...
    assert.equal((await retireR10Images(f.dependencies)).result, 'blocked');
    assert.deepEqual(f.removals, []);
  }
});
...
for (const artifactId of [10092321489]) test(`artifact ${artifactId} must match every immutable recovery field and fresh expiry`, async () => {
  ...
  for (const mutate of mutations) {
    ...
    const result = await retireR10Images(f.dependencies);
    assert.equal(result.result, 'blocked');
    assert.deepEqual(f.removals, artifactId === 10092321489 ? [] : [['image', 'rm', '--no-prune', BROWSER]]);
  }
});
...
test('only unique own-repository self digests are accepted, independently from runtime fingerprint', async () => {
  ...
  assert.equal((await retireR10Images(f.dependencies)).result, 'ready');
});
...
test('recovery archive is freshly rechecked immediately before retirement', async () => {
  ...
  f.dependencies.readGitHub = async path => {
    ...
    if (path === '/actions/artifacts/10092321489' && ++archives > 1) value.expired = true;
    return value;
  };
  const result = await retireR10Images(f.dependencies);
  assert.equal(result.result, 'blocked');
...
test('CLI has no target/force/general mode and suppresses argument/exception details', () => {
  const script = fileURLToPath(new URL('../../deploy/browser/retire-r10-images.mjs', import.meta.url));
