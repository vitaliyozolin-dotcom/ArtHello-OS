...
import { retireD143Browser } from '../../deploy/browser/retire-d143-browser.mjs';

const SOURCE = '349dff08bd24b0dc22ce46dc0f33c48f81ad97e0';
const NEXT = 'a'.repeat(40);
const TREE = 'fd9e545bdf8e30919194959f3f35f9b82ea2650c';
const BROWSER = 'sha256:30bce9433c3dd455bfa89fef9b7bd733aa15eae94c28edd1d0859fe828e486d7';
const APP = 'sha256:ad894fb128d65abd4c97ee818eae075a8d5a922383c67c17b2d8dcc9c6751fed';
const BROWSER_TAG = 'arthello-e2e:' + SOURCE;
const APP_TAGS = ['arthello-direct:' + SOURCE, 'arthello-v52-verify:' + SOURCE];
const FINGERPRINTS = {
  [BROWSER]: '2eff806c5998095dfebeb0d80ecd5955171b8357aee287554a0af86947b24945',
  [APP]: 'f3ed8f81115381db73da791f141594dc4807cd5608c73d335fa9808cbdf43645',
};
const ARCHIVES = {
  '/actions/artifacts/10195789841': { id: 10195789841, name: `server-browser-${SOURCE}-34591320950-1`, size_in_bytes: 640808941, digest: 'sha256:fc3dcad818bdc864529e118d382732cdbbcfe3aeacdcd512dc22392ce481b013', expired: false, expires_at: '2026-09-13T10:54:28Z', workflow_run: { id: 34591320950, repository_id: 1311964413, head_repository_id: 1311964413, head_branch: 'main', head_sha: SOURCE } },
};
...
function fixture(options = {}) {
  ...
  const dependencies = {
    environment: { CHECKED_SOURCE_SHA: NEXT, ARCHIVE_BYTES: '640808799', EXPANDED_BYTES: '1585953792', RUNNER_TEMP: '/owned runner/temp', ...options.environment },
...
test('only verified D143 browser is retired; app, backup and volumes remain outside the command surface', async () => {
  const f = fixture();
  const result = await retireD143Browser(f.dependencies);
  assert.deepEqual(result, { kind: 'd143-browser-retirement', result: 'ready', browser: 'retired', capacity: { scratchAvailableKiB: 9000000, scratchRequiredKiB: 3348732, dockerAvailableKiB: 9000000, dockerRequiredKiB: 7995081 } });
...
test('invalid size/source/temp inputs reject before any Docker mutation', async () => {
  for (const environment of [{ ARCHIVE_BYTES: '0' }, { ARCHIVE_BYTES: 'PRIVATE' }, { ARCHIVE_BYTES: '2e3' }, { EXPANDED_BYTES: '640' }, { EXPANDED_BYTES: String(101 * 1024 ** 3) }, { CHECKED_SOURCE_SHA: SOURCE }, { CHECKED_SOURCE_SHA: 'bad' }, { RUNNER_TEMP: 'relative' }, { RUNNER_TEMP: '/tmp\nPRIVATE' }]) {
    ...
    assert.equal((await retireD143Browser(f.dependencies)).result, 'blocked');
    assert.deepEqual(f.removals, []);
  }
});
...
for (const artifactId of [10195789841]) test(`artifact ${artifactId} must match every immutable recovery field and fresh expiry`, async () => {
...
test('only unique own-repository self digests are accepted, independently from runtime fingerprint', async () => {
  ...
  assert.equal((await retireD143Browser(f.dependencies)).result, 'ready');
});
...
test('recovery archive is freshly rechecked immediately before retirement', async () => {
  ...
  f.dependencies.readGitHub = async path => {
    ...
    if (path === '/actions/artifacts/10195789841' && ++archives > 1) value.expired = true;
    return value;
  };
  const result = await retireD143Browser(f.dependencies);
  assert.equal(result.result, 'blocked');
...
test('CLI has no target/force/general mode and suppresses argument/exception details', () => {
  const script = fileURLToPath(new URL('../../deploy/browser/retire-d143-browser.mjs', import.meta.url));
