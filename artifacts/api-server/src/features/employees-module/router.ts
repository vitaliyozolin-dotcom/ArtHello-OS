...
async function getFullAttribution(
  ...
  try {
    ...
    for (const r of topGroupsRes.rows) {
      if (!topGroupsMap.has(r.teacher_crm_id))
        topGroupsMap.set(r.teacher_crm_id, []);
    ...
    for (const id of teacherCrmIds) {
      ...
      if (SYNTHETIC_TEACHER_IDS.has(id))
        warnings.push("synthetic_or_shared_teacher");
      result.set(id, {
        lessonsCount: 0,
        groupsCount: 0,
        studentsCount: 0,
    ...
    for (const r of aggRes.rows) {
      ...
      if (row.lessonsCount > 0 && row.attendanceCount === 0)
        row.warnings.push("no_attendance_data");
      if (row.topGroups.some((g) => !g.groupName))
        row.warnings.push("historical_group_id");
...
employeesModuleRouter.post("/departments", async (req, res) => {
  try {
    ...
    if (err instanceof z.ZodError)
      return void res.status(400).json({ error: err.issues });
...
employeesModuleRouter.get("/employees", async (req, res) => {
  try {
    ...
    if (employment_type)
      conditions.push(eq(employeesTable.employmentType, employment_type));
    if (department)
      conditions.push(eq(employeesTable.primaryDepartment, department));
    if (branch) conditions.push(eq(employeesTable.branchCrmId, branch));
    if (employee_kind)
      conditions.push(eq(employeesTable.employeeKind, employee_kind));
    if (classification_status)
      conditions.push(
        eq(employeesTable.classificationStatus, classification_status),
      );
    if (employee_type)
      conditions.push(eq(employeesTable.employeeType, employee_type));
    if (search) {
      const s = or(
    ...
    const result = employees.map((e) => {
      ...
      return {
        ...
        attributionStatus:
          attr?.attributionStatus ?? (e.teacherCrmId ? "missing_data" : null),
        accrualsRub: null,
        marginRub: null,
        marginPct: null,
      };
...
const createEmployeeSchema = z.object({
  ...
  employmentType: z
    .enum(["employee", "self_employed", "contractor", "sole_proprietor"])
    .default("employee"),
...
employeesModuleRouter.get("/employees/stats", async (req, res) => {
  ...
  try {
    ...
    const [crm] = (
      await client.query(`SELECT COUNT(*) AS total FROM crm_teachers`)
    ).rows as Record<string, string>[];
    ...
    res.json({
      ...
      teacher_link_coverage_percent:
        empTotal > 0 ? Math.round((withTeacher / empTotal) * 100) : 0,
      person_link_coverage_percent:
        empTotal > 0 ? Math.round((withPerson / empTotal) * 100) : 0,
...
function isSyntheticTeacher(
  name: string,
  teacherCrmId: string | null,
): boolean {
  return (
    teacherCrmId === "321" || name.toLowerCase().includes("приходящий педагог")
  );
}
...
employeesModuleRouter.post("/employees/classify", async (req, res) => {
  ...
  try {
    ...
    if (teacherIds.length > 0) {
      const last12Res = await client.query<{
        teacher_crm_id: string;
        cnt: string;
      }>(
    ...
    if (teacherIds.length > 0) {
      ...
      for (const r of dirRes.rows) {
        if (!directionsMap.has(r.teacher_crm_id))
          directionsMap.set(r.teacher_crm_id, []);
      ...
      try {
        ...
        if (isTechnicalRecord(emp.full_name)) {
          ...
          if (last12Count > 0) {
            ...
            if (
              emp.exclude_from_staff_analytics &&
              emp.classification_status === "excluded_from_staff_analytics"
            ) {
              classStatus = "excluded_from_staff_analytics";
              exclude = true;
              reason =
                "Ранее исключён (bootstrap или ручное исключение); уроки не найдены";
              excluded++;
            } else {
              classStatus = "needs_review";
              exclude = false;
              reason =
                "Привязан к CRM-учителю, но уроки не найдены — нужна проверка";
              needsReview++;
            }
          }
        } else {
          ...
          if (
            ...
            exclude = true;
            reason =
              "Ранее исключён (bootstrap или ручное исключение); нет привязки к CRM";
            excluded++;
          } else {
            classStatus = "needs_review";
            exclude = false;
            reason =
              "Нет привязки к CRM-учителю — требуется ручная классификация";
            needsReview++;
          }
        }
...
employeesModuleRouter.post(
  "/employees/populate-from-crm-teachers",
  async (req, res) => {
    const client = await pool.connect();
    try {
      // 1. All CRM teachers
      ...
      const existingIds = new Set(
        existingRes.rows.map((r) => r.teacher_crm_id),
      );
      ...
      const toCreate = teachersRes.rows.filter(
        (t) => !existingIds.has(String(t.crm_id)),
      );
      ...
      for (const teacher of toCreate) {
        try {
          ...
          req.log.error(
            { rowErr, teacher_crm_id: teacher.crm_id },
            "populate: row error",
          );
          errors++;
        }
      ...
      req.log.error(
        { err },
        "POST /employees/populate-from-crm-teachers failed",
      );
      res.status(500).json({ error: "Internal error" });
    } finally {
      client.release();
...
employeesModuleRouter.get("/employees/attribution", async (req, res) => {
  try {
    ...
    const summary = {
      ...
      total_attendance_attributed: rows.reduce(
        (s, r) => s + r.attendanceCount,
        0,
      ),
      total_unique_students_attributed: rows.reduce(
        (s, r) => s + r.studentsCount,
        0,
      ),
      total_crm_income_attributed: rows.reduce(
        (s, r) => s + r.crmIncomeAttributed,
        0,
      ),
      attribution_status_counts: {
        ...
        missing_data: rows.filter((r) => r.attributionStatus === "missing_data")
          .length,
...
employeesModuleRouter.get("/employees/:id", async (req, res) => {
  try {
    ...
    const roles = await db
      ...
      .orderBy(
        desc(employeeRolesTable.isPrimary),
        asc(employeeRolesTable.validFrom),
      );
    ...
    const rules = await db
      ...
      .where(
        and(
          eq(payrollRulesTable.employeeId, id),
          eq(payrollRulesTable.isActive, true),
        ),
      )
...
employeesModuleRouter.patch(
  "/employees/:id/classification",
  async (req, res) => {
    try {
      const { id } = req.params;
      ...
      if (body.employeeKind !== undefined)
        updates.employeeKind = body.employeeKind;
      if (body.employeeType !== undefined)
        updates.employeeType = body.employeeType;
      if (body.primaryDepartment !== undefined)
        updates.primaryDepartment = body.primaryDepartment;
      ...
      if (body.excludeFromStaffAnalytics !== undefined)
        updates.excludeFromStaffAnalytics = body.excludeFromStaffAnalytics;
      if (body.classificationReason !== undefined)
        updates.classificationReason = body.classificationReason;
...
employeesModuleRouter.get("/employees/:id/payroll-rules", async (req, res) => {
  try {
    ...
    const rows = await db
      ...
      .orderBy(
        desc(payrollRulesTable.isActive),
        asc(payrollRulesTable.ruleType),
      );
    res.json(rows);
  } catch (err) {
...
employeesModuleRouter.post(
  "/employees/map-educational-units",
  async (req, res) => {
    try {
      const BRANCH_ID = "6";
      ...
      for (const row of assocResult.rows) {
        try {
          const isSynthetic =
            SYNTHETIC_TEACHER_IDS.has(row.teacher_crm_id) ||
          ...
          switch (row.educational_unit_type) {
            case "kindergarten_group":
              roleInUnit = "educator";
              break;
            case "school_class":
              roleInUnit = "class_teacher";
              break;
            default:
              roleInUnit = "teacher";
              break;
          ...
          const attrModel =
            row.unit_attribution_model ??
            (row.educational_unit_type === "kindergarten_group" ||
            row.educational_unit_type === "school_class"
              ? "group_based"
              : "lesson_based");
          ...
          if (attrModel === "group_based")
            groupBasedLinks.push(row.employee_id);
          else lessonBasedLinks.push(row.employee_id);
        } catch (rowErr) {
          errors++;
          logger.error(
            { rowErr, employee_id: row.employee_id },
            "map-educational-units: row failed",
          );
        }
      }
      ...
      const individualEmployeeIds = new Set(
        individualResult.rows.map((r) => r.employee_id),
      );
    ...
    `);
      ...
      res.json({
        ...
        audit: {
          ...
          employees_by_model: Object.fromEntries(
            empAttrResult.rows.map((r) => [r.attribution_model, Number(r.cnt)]),
          ),
        },
...
employeesModuleRouter.get(
  "/employees/:id/educational-unit-links",
  async (req, res) => {
    try {
      const { id } = req.params;
      ...
      req.log.error(
        { err },
        "GET /employees/:id/educational-unit-links failed",
      );
      res.status(500).json({ error: "Internal error" });
    }
  },
...
employeesModuleRouter.patch(
  ...
  async (req, res) => {
    try {
      ...
      const attrModel =
        body.attribution_model ?? eu.attribution_model ?? "lesson_based";
      ...
      req.log.info(
        { employee_id: id, educational_unit_id: body.educational_unit_id },
        "manual edu-unit link saved",
      );
      ...
      req.log.error(
        { err },
        "PATCH /employees/:id/educational-unit-links failed",
      );
      res.status(500).json({ error: "Internal error" });
    }
  },
...
employeesModuleRouter.post(
  "/employees/exclude-non-atlas-bootstrap",
  async (req, res) => {
    try {
      const client = await pool.connect();
      try {
        ...
        req.log.info(
          { marked, examples },
          "POST /employees/exclude-non-atlas-bootstrap: marked non-Atlas employees",
        );
      ...
      req.log.error(
        { err },
        "POST /employees/exclude-non-atlas-bootstrap failed",
      );
      res.status(500).json({ error: "Internal error" });
    }
  },
