...
function nameSimilarity(
  ...
  let prefixLen = 0;
  while (prefixLen < Math.min(minLen, 4) && na[prefixLen] === nb[prefixLen])
    prefixLen++;
...
async function findMatchCandidates(opts: {
  ...
  if (normPhone) {
    const rows = await db
      .select({
        personId: personContactsTable.personId,
        isParent: personsTable.isParent,
      })
      .from(personContactsTable)
      .innerJoin(
        personsTable,
        eq(personsTable.id, personContactsTable.personId),
      )
      .where(
        and(
    ...
    if (opts.phone && opts.phone !== normPhone) {
      ...
      for (const r of rawRows)
        merge(r.personId, 0.9, `phone_raw:${opts.phone}`);
    }
  }
...
async function processLeadEvent(leadId: string): Promise<void> {
  const [lead] = await db
    ...
    .where(eq(leadEventsTable.id, leadId));
  if (!lead) return;
  ...
  const existing = await db
    ...
    .where(
      and(
        eq(personLinksTable.entityType, "lead"),
        eq(personLinksTable.entityId, leadId),
      ),
    );
  ...
  if (top && top.confidence >= 0.9) {
    ...
    await db
      .insert(identityMatchQueueTable)
      .values({
        leadEventId: leadId,
...
async function buildFamilies(): Promise<{
  ...
  for (const [normPhone, groupStudents] of phoneGroups) {
    ...
    for (const student of groupStudents) {
      ...
      if (studentPersonId) {
        await db
          .insert(guardianStudentLinksTable)
          .values({
            guardianPersonId,
            studentPersonId,
            studentCrmId: student.crmId,
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      const unlinkedLeads = await db
        ...
        .from(leadEventsTable).where(sql`${leadEventsTable.id} NOT IN (
      ...
      const [{ count: personsTotal }] = await db
        .select({ count: count() })
        .from(personsTable);
      const [{ count: familiesTotal }] = await db
        .select({ count: count() })
        .from(familiesTable);
...
identityRouter.get("/identity/stats", async (req, res): Promise<void> => {
  const [
    ...
    db
      .select({ count: count() })
      .from(personsTable)
      .where(eq(personsTable.isLead, true)),
    db
      .select({ count: count() })
      .from(personsTable)
      .where(eq(personsTable.isStudent, true)),
    db
      .select({ count: count() })
      .from(personsTable)
      .where(eq(personsTable.isParent, true)),
    db
      .select({ count: count() })
      .from(identityMatchQueueTable)
      .where(eq(identityMatchQueueTable.status, "manual_review")),
    db
      .select({ count: count() })
      .from(identityMatchQueueTable)
      .where(eq(identityMatchQueueTable.status, "auto_matched")),
    db
      .select({ count: count() })
      .from(leadEventsTable)
      .where(
...
identityRouter.get("/identity/persons/:id", async (req, res): Promise<void> => {
  ...
  const contacts = await db
    .select()
    .from(personContactsTable)
    .where(eq(personContactsTable.personId, id));
  const links = await db
    .select()
    .from(personLinksTable)
    .where(eq(personLinksTable.personId, id));
    ...
    rows.map(async (item) => {
      ...
      if (item.leadEventId) {
        [lead] = await db
          .select()
          .from(leadEventsTable)
          .where(eq(leadEventsTable.id, item.leadEventId));
      }
      if (item.suggestedPersonId) {
        [suggestedPerson] = await db
          .select()
          .from(personsTable)
...
identityRouter.patch("/identity/queue/:id", async (req, res): Promise<void> => {
  ...
  const { action } = req.body as {
    action: "approve" | "reject" | "new_person";
  };
  ...
  if (action === "approve" && item.suggestedPersonId && item.leadEventId) {
    ...
    await db
      .update(identityMatchQueueTable)
      .set({
        status: "auto_matched",
        resolvedAt: new Date(),
        resolvedBy: "manual",
...
identityRouter.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const rows = await db
      ...
      .from(crmStudentsTable)
      .where(
        sql`${crmStudentsTable.crmId} NOT IN (
...
identityRouter.get("/identity/duplicates", async (req, res): Promise<void> => {
  ...
  const realDuplicates = rows.rows.filter((r) => !r["all_are_guardians"]);
  const familyGroups = rows.rows.filter((r) => r["all_are_guardians"]);
...
identityRouter.get(
  ...
  `);
    ...
    res.json({
      ...
      guardianLinks,
      familyTotalPaid: paymentsRow
        ? Number(paymentsRow["family_total_paid"])
        : 0,
    });
  },
);
  ...
  `);
    ...
    const result = {
      studentsWithoutPhone: {
        count: studentsWithoutPhone.rows.length,
        records: studentsWithoutPhone.rows,
      },
      studentsInvalidPhone: {
        count: studentsInvalidPhone.rows.length,
        records: studentsInvalidPhone.rows,
      },
      familyGroups: {
        count: familyGroups.rows.length,
        groups: familyGroups.rows,
      },
      personsStudentOnly: {
        count: personsStudentOnly.rows.length,
        records: personsStudentOnly.rows,
      },
      personsLeadOnly: {
        count: personsLeadOnly.rows.length,
        records: personsLeadOnly.rows,
      },
      personsBothStudentAndLead: {
        count: personsBothStudentAndLead.rows.length,
        records: personsBothStudentAndLead.rows,
      },
      studentsLinkedToPerson: {
        count: studentsLinkedToPerson.rows.length,
        records: studentsLinkedToPerson.rows,
      },
      possiblePhoneMismatch: {
        count: possiblePhoneMismatch.rows.length,
        records: possiblePhoneMismatch.rows,
      },
    };
    ...
    if (format === "csv" && section && section in result) {
      ...
      if (rows.length === 0) {
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="${section}.csv"`,
        );
        res.send("no data\n");
        return;
      }
