...
function applyMapping(
  ...
  return {
    ...
    counterpartyName: get(mapping.counterpartyName)
      ? String(get(mapping.counterpartyName))
      : null,
    counterpartyInn: get(mapping.counterpartyInn)
      ? String(get(mapping.counterpartyInn))
      : null,
    purpose: get(mapping.purpose) ? String(get(mapping.purpose)) : null,
    amount,
    direction,
    accountName: get(mapping.accountName)
      ? String(get(mapping.accountName))
      : null,
    accountNumber: get(mapping.accountNumber)
      ? String(get(mapping.accountNumber))
      : null,
    raw: row,
  };
}
...
function detectMapping(headers: string[]): ColumnMapping {
  ...
  return {
    ...
    counterpartyName: h(
      /контрагент|получател|плательщик|наименован|partner|counterpart/,
    ),
    ...
    amount:
      !h(/приход|зачислен|credit|поступлени|доход/) &&
      !h(/расход|списани|debit|оплата|выплат/)
        ? h(/сумм|amount/)
        : undefined,
...
async function categoriseRow(
  row: Pick<
    ParsedRow,
    "direction" | "counterpartyName" | "counterpartyInn" | "purpose" | "amount"
  >,
  ...
  for (const rule of rules) {
    ...
    if (row.amount != null) {
      if (rule.amountMin != null && row.amount < Number(rule.amountMin))
        continue;
      if (rule.amountMax != null && row.amount > Number(rule.amountMax))
        continue;
    }
    ...
    const cpMatch =
      ...
      cpPats.some(
        (p) =>
          cpLow.includes(p.toLowerCase()) || innLow.includes(p.toLowerCase()),
      );
    const purMatch =
      purPats.length === 0 ||
      purPats.some((p) => purLow.includes(p.toLowerCase()));
    if (cpMatch && purMatch) {
...
financeRouter.post(
  ...
  async (req, res): Promise<void> => {
    ...
    req.log.info(
      { bankName, accountName, branchCrmId, fileName },
      "Bank import started",
    );
    ...
    try {
      ...
      for (let i = skip; i < rawRows.length; i++) {
        ...
        if (!parsed.amount || !parsed.operationDate) {
          skipped++;
          continue;
        }
      ...
      req.log.info(
        { imported, unclear, skipped, batchId: batch.id },
      ...
      res.json({
        ...
        skipped,
        batchId: batch.id,
        message: `Imported ${imported} transactions (${unclear} unclear, ${skipped} skipped)`,
      });
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
...
financeRouter.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const conditions = [];
    if (branch)
      conditions.push(sql`${bankTransactionsTable.branchCrmId} = ${branch}`);
    if (direction)
      conditions.push(sql`${bankTransactionsTable.direction} = ${direction}`);
    if (unclear === "true")
      conditions.push(sql`${bankTransactionsTable.isUnclear} = true`);
    if (dateFrom)
      conditions.push(
        sql`${bankTransactionsTable.operationDate} >= ${dateFrom}`,
      );
    if (dateTo)
      conditions.push(sql`${bankTransactionsTable.operationDate} <= ${dateTo}`);
  ...
  async (req, res): Promise<void> => {
    ...
    await db
      ...
      .set({
        ...
        isDebtInterest:
          typeof isDebtInterest === "boolean" ? isDebtInterest : undefined,
        isTax: typeof isTax === "boolean" ? isTax : undefined,
        isTransferBetweenOwnAccounts:
          typeof isTransferBetweenOwnAccounts === "boolean"
            ? isTransferBetweenOwnAccounts
            : undefined,
...
financeRouter.post(
  ...
  async (req, res): Promise<void> => {
    ...
    res.json({
      ...
      resolved,
      stillUnclear: unclear.length - resolved,
    });
  },
);
...
financeRouter.get("/finance/stats", async (req, res): Promise<void> => {
  ...
  res.json({
    ...
    currentMonthNet:
      Number(s["income_mtd"] ?? 0) - Number(s["expense_mtd"] ?? 0),
  });
});
