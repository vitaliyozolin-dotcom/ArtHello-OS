...
} from "../../lib/banking/connectors/tochka.js";
...
bankingRouter.post("/banking/connectors", async (req, res) => {
  ...
  try {
    ...
    const config = sealBankConnectorConfig(connectorId, data.config ?? {});
...
bankingRouter.patch("/banking/connectors/:id", async (req, res) => {
  ...
  try {
    ...
    const update: Partial<typeof bankConnectorsTable.$inferInsert> = {
      updatedAt: new Date(),
    };
    if (parsed.data.displayName !== undefined)
      update.displayName = parsed.data.displayName;
    if (parsed.data.connectorStatus !== undefined)
      update.connectorStatus = parsed.data.connectorStatus;
    if (parsed.data.syncFrequencyMinutes !== undefined)
      update.syncFrequencyMinutes = parsed.data.syncFrequencyMinutes;
...
bankingRouter.post("/banking/connectors/:id/sync", async (req, res) => {
  ...
  const periodDays = req.body?.periodDays
    ? parseInt(String(req.body.periodDays))
    : undefined;
  try {
    const result = await syncConnector(
      id,
      runType as "balances" | "transactions" | "full",
      periodDays,
    );
    res.json(result);
  } catch (err) {
...
bankingRouter.get("/banking/connectors/:id/customers", async (req, res) => {
  ...
  try {
    ...
    if (row.bankName !== "tochka") {
      res.status(400).json({ error: "Customers endpoint only for Tochka" });
      return;
    }
    ...
    const resolvedCustomerCode =
      (cfg["customerCode"] as string | null | undefined) ?? null;
    ...
    const debug = {
      ...
      customer_code_saved:
        ...
        resolvedCustomerCode === businessCustomer?.customerCode,
      customer_code_after_reread: resolvedCustomerCode,
      total_customers: customers.length,
      business_count: customers.filter(
        (c) => (c.customerType ?? "").toLowerCase() === "business",
      ).length,
    };
...
bankingRouter.post(
  "/banking/connectors/:id/resolve-customer",
  async (req, res) => {
    const { id } = req.params;
    const { customerCode } = req.body as { customerCode?: string };
    if (!customerCode?.trim()) {
      res.status(400).json({ error: "customerCode is required" });
      return;
    }
    ...
    try {
      ...
      req.log.error(
        { err },
        "POST /banking/connectors/:id/resolve-customer failed",
      );
      res.status(500).json({ error: "Internal server error" });
    }
  },
...
bankingRouter.post("/banking/connectors/:id/oauth/start", async (req, res) => {
  ...
  try {
    ...
    if (row.bankName !== "tochka") {
      res.status(400).json({ error: "OAuth flow only supported for Tochka" });
      return;
    }
...
bankingRouter.post(
  "/banking/connectors/:id/oauth/callback",
  async (req, res) => {
    const { id } = req.params;
    ...
    try {
      ...
      req.log.info(
        { connectorId: id },
        "Tochka OAuth callback: hybrid token obtained, connector active",
      );
      ...
      setImmediate(() => {
        syncConnector(id, "balances")
          .then(() => {
            req.log.info(
              { connectorId: id },
              "Tochka: auto-sync after OAuth complete",
            );
          })
          .catch((err: unknown) => {
            req.log.warn(
              { connectorId: id, err },
              "Tochka: auto-sync after OAuth failed (non-fatal)",
            );
          });
      });
      ...
      res.json({
        ok: true,
        stage: "token_ok",
        message: "Hybrid token получен. Коннектор активен.",
      });
    } catch (err) {
      req.log.error(
        { err },
        "POST /banking/connectors/:id/oauth/callback failed",
      );
      res.status(500).json({ error: "Internal server error" });
    }
  },
...
bankingRouter.get("/banking/oauth/callback", async (req, res) => {
  ...
  const error =
    typeof req.query["error"] === "string" && req.query["error"].length <= 128
      ? req.query["error"]
      : undefined;
  ...
  try {
    ...
    const connector = new TochkaConnector(
      matchingRow.id,
      decryptBankConnectorConfig(matchingRow.id, matchingRow.config),
    );
    ...
    req.log.info(
      { connectorId: matchingRow.id },
      "Tochka OAuth: authorization complete",
    );
    ...
    setImmediate(() => {
      syncConnector(connectorId, "balances")
        .then(() => {
          req.log.info(
            { connectorId },
            "Tochka: auto-sync after browser OAuth complete",
          );
        })
        .catch((err: unknown) => {
          req.log.warn(
            { connectorId, err },
            "Tochka: auto-sync after browser OAuth failed (non-fatal)",
          );
        });
    });

    res.redirect(
      `${frontendBase}/?tochka_oauth=success&connector=${matchingRow.id}`,
    );
  } catch (err) {
...
bankingRouter.get("/banking/accounts", async (req, res) => {
  try {
    const rows = await db
      ...
      .leftJoin(
        bankConnectorsTable,
        eq(bankAccountsTable.bankConnectorId, bankConnectorsTable.id),
      )
...
bankingRouter.get("/banking/cashflow", async (req, res) => {
  try {
    ...
    const todayIncome = toNum(
      todayRows.find((r) => r.direction === "income")?.total,
    );
    const todayExpense = toNum(
      todayRows.find((r) => r.direction === "expense")?.total,
    );
    ...
    const runwayDays =
      avgDailyExpense > 0 ? Math.floor(totalBalance / avgDailyExpense) : null;
...
bankingRouter.get("/banking/alerts", async (req, res) => {
  try {
    ...
    const totalBalance = accounts.reduce(
      (s, a) => s + toNum(a.currentBalance),
      0,
    );
    if (
      accounts.length > 0 &&
      totalBalance === 0 &&
      accounts.every((a) => a.lastBalanceSyncAt)
    ) {
      alerts.push({
          ...
          const [existing] = await db
            ...
            .where(
              and(
                eq(bankTransactionsTable.direction, "expense"),
                gte(
                  sql`${bankTransactionsTable.operationDate}::text`,
                  monthStart,
                ),
...
bankingRouter.get("/banking/forecast", async (req, res) => {
  try {
    ...
    const avgDailyIncome =
      toNum(flowRows.find((r) => r.direction === "income")?.total) / 30;
    const avgDailyExpense =
      toNum(flowRows.find((r) => r.direction === "expense")?.total) / 30;
    ...
    const minimumSafeBalance =
      avgDailyExpense * MIN_SAFE_BALANCE_DAYS +
      dailyObligations * MIN_SAFE_BALANCE_DAYS;
...
bankingRouter.get("/banking/sync-runs", async (req, res) => {
  try {
    ...
    const rows = await db
      ...
      .leftJoin(
        bankConnectorsTable,
        eq(bankSyncRunsTable.bankConnectorId, bankConnectorsTable.id),
      )
...
bankingRouter.get("/banking/transactions", async (req, res) => {
  try {
    ...
    if (accountId)
      conditions.push(eq(bankTransactionsTable.accountId, String(accountId)));
    if (connectorId)
      conditions.push(
        eq(bankTransactionsTable.bankConnectorId, String(connectorId)),
      );
    if (direction)
      conditions.push(eq(bankTransactionsTable.direction, String(direction)));
    if (matchStatus)
      conditions.push(
        eq(bankTransactionsTable.matchStatus, String(matchStatus)),
      );
    if (minAmount)
      conditions.push(
        gte(
          sql`${bankTransactionsTable.amount}::numeric`,
          sql`${parseFloat(String(minAmount))}`,
        ),
      );
    if (maxAmount)
      conditions.push(
        lte(
          sql`${bankTransactionsTable.amount}::numeric`,
          sql`${parseFloat(String(maxAmount))}`,
        ),
      );
    if (counterparty)
      conditions.push(
        ilike(bankTransactionsTable.counterpartyName, `%${counterparty}%`),
      );
    if (inn)
      conditions.push(ilike(bankTransactionsTable.counterpartyInn, `%${inn}%`));
    if (purpose)
      conditions.push(ilike(bankTransactionsTable.purpose, `%${purpose}%`));
...
bankingRouter.get("/banking/transactions/stats", async (req, res) => {
  try {
    ...
    if (accountId)
      base.push(eq(bankTransactionsTable.accountId, String(accountId)));

    const [[inRow], [outRow]] = await Promise.all([
      db
        .select({ total: sum(bankTransactionsTable.amount), cnt: count() })
        .from(bankTransactionsTable)
...
bankingRouter.get("/banking/data-truth", async (req, res) => {
  try {
    const [accounts, balRow, txCountRow, txRangeRow, lastRun] =
      await Promise.all([
        db
          .select({
            id: bankAccountsTable.id,
        ...
        db
          .select({
            total: sql<string>`coalesce(sum(current_balance::numeric), 0)`,
          })
          .from(bankAccountsTable),
    ...
    const lastBalanceSyncAt = accounts.reduce<Date | null>((latest, a) => {
      ...
      return !latest || a.lastBalanceSyncAt > latest
        ? a.lastBalanceSyncAt
        : latest;
    }, null);
    ...
    const lastStatementSyncAt = accounts.reduce<Date | null>((latest, a) => {
      ...
      return !latest || a.lastStatementSyncAt > latest
        ? a.lastStatementSyncAt
        : latest;
    }, null);
    ...
    res.json({
      ...
      debug: {
        ...
        balances_loaded_count: accounts.filter((a) => a.lastBalanceSyncAt)
          .length,
