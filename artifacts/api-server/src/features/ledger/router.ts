...
import { operations, operationHistory, articles } from "@workspace/db";
import {
  eq,
  desc,
  asc,
...
const DEFAULT_ARTICLES: (typeof articles.$inferInsert)[] = [
  ...
  {
    code: "1.4",
    name: "Продажа материалов",
    groupName: "Прочие доходы",
    type: "income",
    affectsDds: true,
    affectsPl: true,
    affectsEbitda: true,
  ...
  {
    code: "2.2",
    name: "ФОТ — административный персонал",
    groupName: "Фонд оплаты труда",
    type: "expense",
    affectsDds: true,
    affectsPl: true,
  ...
  // ── Operations ──────────────────────────────────────────────────────────────
  {
    ...
    groupName: "Операционные расходы",
    type: "expense",
    affectsDds: true,
    affectsPl: true,
    affectsEbitda: true,
  ...
  // ── Materials ───────────────────────────────────────────────────────────────
  {
    ...
    groupName: "Материалы и расходники",
    type: "expense",
    affectsDds: true,
    affectsPl: true,
    affectsEbitda: true,
  ...
  {
    code: "8.1",
    name: "Подрядчики — разовые",
    groupName: "Прочие расходы",
    type: "expense",
    affectsDds: true,
    affectsPl: true,
    affectsEbitda: true,
...
ledgerRouter.post("/ledger/seed-articles", async (req, res) => {
  try {
    ...
    const toInsert = DEFAULT_ARTICLES.filter(
...
ledgerRouter.post("/ledger/operations", async (req, res) => {
  ...
  const accrualDate = data.accrualDate
    ? new Date(data.accrualDate)
    : cashflowDate;
...
ledgerRouter.patch("/ledger/operations/:id", async (req, res) => {
  ...
  try {
    ...
    const updates: Partial<typeof operations.$inferInsert> = {
      ...(data as Partial<typeof operations.$inferInsert>),
      updatedAt: new Date(),
    };
...
ledgerRouter.get("/ledger/stats", async (req, res) => {
  const { month, mode = "dds" } = req.query as {
    month?: string;
    mode?: string;
  };
  ...
  try {
    ...
    const verifiedCount =
      byVerification.find((r) => r.status === "verified")?.cnt ?? 0;
    const totalCount = byVerification.reduce((s, r) => s + Number(r.cnt), 0);
    const trustScore =
      totalCount > 0
        ? Math.round((Number(verifiedCount) / totalCount) * 100)
        : 0;

    // Unverified count
    const unverifiedCount =
      byVerification.find((r) => r.status === "unverified")?.cnt ?? 0;
