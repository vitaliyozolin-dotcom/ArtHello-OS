...
async function projectToLedger(
  ...
  const operationType =
    direction === "in"
      ...
      : direction === "out"
        ? "expense"
        : "transfer";
  const counterpartyType = matchData.familyId
    ? "family"
    : matchData.contractorId
      ? "contractor"
      : matchData.employeeId
        ? "employee"
        : null;
  ...
  if ((existing.rows?.length ?? 0) > 0) {
    ...
    const insertResult = await db.execute(sql`
      ...
      RETURNING id
    `);
    const newId = (insertResult.rows?.[0] as Record<string, unknown>)
      ?.id as string;
  ...
  try {
    ...
    const unmatchedQuery = db
      ...
      .where(
        and(
          ...
          ...(connectorId
            ? [eq(bankTransactionsTable.bankConnectorId, connectorId)]
            : []),
        ),
      )
      .limit(500);
    ...
    if (txns.length === 0) {
      res.json({
        ...
        skipped: 0,
        total: 0,
        message: "Нет несопоставленных поступлений",
      });
      return;
    }
    ...
    if (families.length === 0) {
      res.json({
        ...
        total: txns.length,
        message:
          "Нет семей для сопоставления. Запустите «Построить семьи» сначала.",
      });
      return;
    }
    ...
    for (const txn of txns) {
      ...
      const matchStatus =
        confidence >= 85 ? "matched" : confidence >= 50 ? "suggested" : null;
    ...
    req.log.info(
      { total: txns.length, matched, suggested, skipped },
      "Banking: run-matching complete",
    );
...
bankingMatchRouter.get("/banking/transactions", async (req, res) => {
  try {
    ...
    if (connectorId)
      conditions.push(eq(bankTransactionsTable.bankConnectorId, connectorId));
...
bankingMatchRouter.get("/banking/match-stats", async (req, res) => {
  try {
    ...
    const conditions = [];
    if (from)
      conditions.push(gte(bankTransactionsTable.operationDate, from as string));
    if (to)
      conditions.push(lte(bankTransactionsTable.operationDate, to as string));
    ...
    for (const r of rows) {
      map[r.matchStatus ?? "null"] = Number(r.cnt);
    }
...
bankingMatchRouter.patch(
  "/banking/transactions/:id/match",
  async (req, res) => {
    try {
      const { id } = req.params;
      const { action, familyId, contractorId, employeeId, articleId, reason } =
        req.body as {
          action: string;
          familyId?: string;
          contractorId?: string;
      ...
      switch (action) {
        ...
        case "unmatch": {
          ...
          const linkedOpId = (
            txRow.rows[0] as Record<string, unknown> | undefined
      ...
      if (!["ignore", "unmatch", "internal_transfer"].includes(action)) {
        const projected = await projectToLedger(id, {
          familyId,
          contractorId,
          employeeId,
          articleId,
        ...
        req.log.info(
          {
            txId: id,
            matchAction: action,
            ledgerAction: projected.action,
            operationId: projected.operationId,
          },
          "PATCH match: projected to ledger",
        );
      }
...
bankingMatchRouter.get(
  "/banking/transactions/by-family/:familyId",
  async (req, res) => {
    try {
