...
const dummyPasswordHash = hashPassword(
  `${randomBytes(32).toString("base64url")}7a`,
);
...
function sessionCookieName(): string {
  return isProduction() ? "__Host-arthello_session" : "arthello_session";
}
function csrfCookieName(): string {
  return isProduction() ? "__Host-arthello_csrf" : "arthello_csrf";
}
function cookieBaseOptions() {
  return { secure: isProduction(), sameSite: "strict" as const, path: "/" };
}
...
function loginAttemptKey(login: string, req: Request): string {
  return sha256(`${normalizeLogin(login)}\n${requestAddress(req)}`);
}
...
authRouter.post("/auth/password", async (req, res) => {
  ...
  try {
    ...
    const updated = await store.updateUserPassword(
      user.id,
...
authRouter.get("/auth/users", async (_req, res) => {
  ...
  try {
    ...
    logger.error({ err }, "User list failed");
    res.status(503).json({ error: "Список пользователей недоступен" });
  }
});

authRouter.post("/auth/users", async (req, res) => {
  const auth = requireOwner(res);
  if (!auth) return;
  const parsed = z
    .object({
  ...
  const scope: BusinessScope =
    parsed.data.role === "owner"
      ...
      : {
          unrestricted: false,
          branchIds: normalizeIds(parsed.data.branchIds),
          legalEntityIds: normalizeIds(parsed.data.legalEntityIds),
        };
  if (parsed.data.role !== "owner" && !hasCompleteBusinessScope(scope)) {
    res.status(400).json({
      error: "Для ограниченной роли нужны филиалы и юридические лица",
    });
    return;
  }
  try {
...
authRouter.post("/auth/users/:userId/reset-password", async (req, res) => {
  ...
  if (!auth) return;
  const parsed = z
    .object({ temporaryPassword: z.string().min(1).max(1024) })
    .safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: "Некорректный запрос" });
    return;
...
authRouter.patch("/auth/users/:userId", async (req, res) => {
  ...
  if (req.params.userId === auth.session.userId && !parsed.data.active) {
    res
      .status(409)
      .json({ error: "Нельзя отключить собственную учётную запись" });
    return;
  }
  try {
    ...
    if (
      !parsed.data.active &&
      target.role === "owner" &&
      (await store.countActiveOwners()) <= 1
    ) {
      res.status(409).json({ error: "Нельзя отключить последнего владельца" });
      return;
    }
...
export async function requireRouteAccess(
  ...
  if (shouldAuditAccess(req.method, req.path)) {
    try {
      ...
      if (decision.allowed) {
        res.status(503).json({ error: "Аудит доступа временно недоступен" });
        return;
      }
    }
  }
    ...
    logger.warn(
      {
        ...
        legalEntityScopeCount: auth.session.scope.legalEntityIds.length,
      },
      "Route access denied",
    );
    res.status(403).json({ error: "Недостаточно прав" });
