...
/**
 * Conservative route-level policy:
 * - owner: all authenticated routes;
 * - non-owner account lifecycle routes remain available;
 * - all non-owner business routes fail closed until each handler applies
 *   both branch and legal-entity predicates and is explicitly registered;
 * - merely carrying scope metadata never makes an unscoped handler safe.
 */
export function decideRouteAccess(
  ...
  if (role === "owner") {
    return { allowed: true, policy: "owner-full-access" };
  }
