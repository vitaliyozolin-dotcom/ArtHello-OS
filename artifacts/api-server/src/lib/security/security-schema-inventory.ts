export const REQUIRED_SECURITY_TABLE_COLUMNS: Readonly<
...
export const REQUIRED_SECURITY_MIGRATIONS = [
  {
    ...
    hash: "b6249690fc7e5827aa543a2bfcfbfc1170a0183278492da161907c16c786efab",
  },
  {
    timestamp: "1784858251868",
    hash: "2394a6f7fa2f17c10d8b92560219a05fc1f054a327c133b278a17662087f1843",
  },
] as const;
...
export function validateSecuritySchemaInventory(
  inventory: SecuritySchemaInventory,
): void {
    ...
    for (const columnName of requiredColumns) {
      if (!columns.has(`${tableName}.${columnName}`)) {
        throw new SecuritySchemaNotReadyError(
          `Required security schema column is missing: ${tableName}.${columnName}`,
        );
      }
    }
  }
  ...
  for (const indexName of REQUIRED_SECURITY_INDEXES) {
    if (!indexes.has(indexName)) {
      throw new SecuritySchemaNotReadyError(
        `Required security schema index is missing: ${indexName}`,
      );
    }
  }

  const migrations = new Set(
    inventory.migrations.map(
      ({ timestamp, hash }) => `${timestamp}:${hash}`,
    ),
  );
  for (const migration of REQUIRED_SECURITY_MIGRATIONS) {
    if (
      !migrations.has(`${migration.timestamp}:${migration.hash}`)
    ) {
      throw new SecuritySchemaNotReadyError(
        `Required security migration is not recorded with its expected hash: ${migration.timestamp}`,
      );
    }
  }
}
