...
const CALLBACK_URL = "https://alpha-crm-sync.replit.app/api/integrations/evotor/user-token";
...
async function getConnector(): Promise<ConnectorRow | null> {
  const res = await pool.query("SELECT * FROM evotor_connectors ORDER BY created_at DESC LIMIT 1");
...
evotorRouter.post("/integrations/evotor/publisher-token", async (req, res): Promise<void> => {
  ...
  res.json({ success: true, maskedLast4: `****${last4}`, tokenPresent: true, readiness: "TOKEN_SAVED" });
});

// ── GET /api/integrations/evotor/status ──────────────────────────────────────
evotorRouter.get("/integrations/evotor/status", async (req, res): Promise<void> => {
...
evotorRouter.post("/integrations/evotor/user-token", async (req, res): Promise<void> => {
  const body = req.body as Record<string, unknown>;
  const rawToken =
    body.token ?? body.userToken ?? body.access_token ?? body.jwt ?? body.user_token;
  if (!rawToken) {
    req.log.warn({ bodyKeys: Object.keys(body) }, "Evotor user-token callback: no token field found");
  ...
  res.json({ success: true, message: "User token received and saved", readiness: "USER_TOKEN_RECEIVED" });
});

// ── POST /api/integrations/evotor/discover ───────────────────────────────────
evotorRouter.post("/integrations/evotor/discover", async (req, res): Promise<void> => {
  const row = await getConnector();
  if (!row?.publisher_token_enc) {
    return void res.status(400).json({
      success: false,
      message: "Publisher token not configured. Save it first via the Evotor tab.",
    });
  }
  ...
  try {
    ...
    return void res.status(500).json({ success: false, message: "Failed to decrypt publisher token" });
  }
  ...
  async function fetchEvotor(path: string): Promise<unknown[] | null> {
    try {
      ...
      if (!r.ok) { errors.push(`${path}: HTTP ${r.status}`); return null; }
      const data = await r.json() as unknown;
      return Array.isArray(data) ? data : ((data as Record<string, unknown>).items as unknown[] | undefined) ?? [];
    } catch (e) {
  ...
  if (stores)    { storesCount    = stores.length;    discoveryRaw.stores    = stores; }
  if (devices)   { devicesCount   = devices.length;   discoveryRaw.devices   = devices; }
  if (employees) { employeesCount = employees.length; discoveryRaw.employees = employees; }

  const readiness: EvotorReadiness =
    errors.length >= 3 ? "ERROR"
    : storesCount > 0  ? "DISCOVERY_READY"
    : row.user_token_enc ? "USER_TOKEN_RECEIVED"
    : "TOKEN_SAVED";
  ...
  await pool.query(
    `UPDATE evotor_connectors
       SET stores_count      = $1,
           ...
           updated_at        = NOW()
       WHERE id = $6`,
    [storesCount, devicesCount, employeesCount, JSON.stringify(discoveryRaw), readiness, row.id],
  );
  ...
  if (totalItems > 0) {
    ...
    if (batchId) {
      ...
      for (const { type, item } of allItems) {
        const extId = (item as Record<string, unknown>).id
  ...
  req.log.info({ storesCount, devicesCount, employeesCount, readiness, errors }, "Evotor discover complete");
  res.json({
...
evotorRouter.get("/coverage/evotor-coverage-audit", async (req, res): Promise<void> => {
  const row      = await getConnector();
  const readiness = computeReadiness(row);

  const readinessReason: Record<EvotorReadiness, string> = {
    NOT_CONNECTED:       "Publisher token not saved. Enter it in Technical Mode → Evotor / POS tab.",
    TOKEN_SAVED:         "Publisher token saved. Register callback URL in Evotor Marketplace, or run Discovery.",
    USER_TOKEN_RECEIVED: "User token received via OAuth callback. Run Discovery to fetch stores/devices.",
    ...
    ERROR:               "Discovery failed — check token validity and Evotor API availability.",
  };
