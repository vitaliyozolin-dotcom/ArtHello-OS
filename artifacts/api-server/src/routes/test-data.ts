...
import {
  operations, articles,
  contractorsTable, contractorAccrualsTable, contractorPaymentsTable, contractorDocumentsTable,
  taxObligationsTable, taxReserveTable,
  staffProfilesTable, staffBonusesTable,
} from "@workspace/db";
...
testDataRouter.get("/test-data/status", async (req, res) => {
  try {
    const [{ ops }] = await db.select({ ops: sql<number>`count(*)::int` }).from(operations).where(eq(operations.isTestData, true));
    const [{ ctrs }] = await db.select({ ctrs: sql<number>`count(*)::int` }).from(contractorsTable).where(eq(contractorsTable.isTestData, true));
    const [{ taxes }] = await db.select({ taxes: sql<number>`count(*)::int` }).from(taxObligationsTable).where(eq(taxObligationsTable.isTestData, true));
...
testDataRouter.delete("/test-data", async (req, res) => {
  try {
    const [{ ops }] = await db.delete(operations).where(eq(operations.isTestData, true)).returning({ id: operations.id }).then((r) => [{ ops: r.length }]);
    const [{ ctrs }] = await db.delete(contractorsTable).where(eq(contractorsTable.isTestData, true)).returning({ id: contractorsTable.id }).then((r) => [{ ctrs: r.length }]);
    const [{ taxes }] = await db.delete(taxObligationsTable).where(eq(taxObligationsTable.isTestData, true)).returning({ id: taxObligationsTable.id }).then((r) => [{ taxes: r.length }]);
    await db.delete(taxReserveTable).where(eq(taxReserveTable.isTestData, true));
...
testDataRouter.post("/test-data/seed", async (req, res) => {
  try {
    ...
    const articleList = await db.select({ id: articles.id, type: articles.type, name: articles.name, code: articles.code, groupName: articles.groupName }).from(articles).where(eq(articles.isActive, true));
    ...
    for (let year = 2025; year <= 2026; year++) {
      ...
      for (let month = 1; month <= maxMonth; month++) {
        ...
        for (let i = 0; i < incomeCount; i++) {
          ...
          const plM = isAdvance
            ? monthStr(month === 12 ? year + 1 : year, month === 12 ? 1 : month + 1)
            : cashM;
          ...
          const [op] = await db.insert(operations).values({
            ...
            currency: "RUB",
            description: isAdvance ? `Предоплата за ${plM} (ребёнок ${rand(1, 200)})` : `Оплата обучения ${cashM} (ребёнок ${rand(1, 200)})`,
        ...
        const rentArt = expenseArticles.find((a) => a.name.toLowerCase().includes("аренд")) ?? expenseArticles[0];
        if (rentArt) {
          const [op] = await db.insert(operations).values({
        ...
        const payrollArt = expenseArticles.find((a) => a.name.toLowerCase().includes("зарплат") || a.name.toLowerCase().includes("фот")) ?? expenseArticles[1];
        if (payrollArt) {
          const [op] = await db.insert(operations).values({
        ...
        const mktgArt = expenseArticles.find((a) => a.name.toLowerCase().includes("маркет") || a.name.toLowerCase().includes("реклам")) ?? expenseArticles[2];
        if (mktgArt) {
        ...
        if (month % 3 === 1) {
          const taxArt = expenseArticles.find((a) => a.name.toLowerCase().includes("налог")) ?? expenseArticles[3];
          if (taxArt) {
            const [op] = await db.insert(operations).values({
              operationType: "tax",
              source: "manual",
        ...
        for (const misc of miscExpenses) {
          const art = expenseArticles.find((a) => a.name.toLowerCase().includes(misc.name.split(" ")[0].toLowerCase())) ?? pick(expenseArticles);
          const [op] = await db.insert(operations).values({
            operationType: "expense",
            source: "manual",
            direction: "out",
            ...
            counterpartyType: "contractor",
            paymentStatus: "paid",
            verificationStatus: Math.random() < 0.3 ? "unverified" : "verified",
    ...
    const contractorSeeds = [
      { name: "ООО АртСтудия", inn: "7701234567", type: "ooo", taxStatus: "osno", riskLevel: "low", trustScore: 90 },
      { name: "ИП Петрова Е.А.", inn: "771234567890", type: "ip", taxStatus: "usn", riskLevel: "low", trustScore: 85 },
      { name: "Самозанятый Иванов", inn: "771987654321", type: "self_employed", taxStatus: "npd", riskLevel: "medium", trustScore: 65 },
      { name: "ООО ЦифровойМир", inn: "7709876543", type: "ooo", taxStatus: "usn", riskLevel: "low", trustScore: 80 },
      { name: "ИП Сидоров В.Б.", inn: "772345678901", type: "ip", taxStatus: "usn", riskLevel: "medium", trustScore: 55 },
      { name: "ООО КлинингПро", inn: "7703456789", type: "ooo", taxStatus: "osno", riskLevel: "low", trustScore: 75 },
      { name: "ИП Козлова Н.В.", inn: "773456789012", type: "ip", taxStatus: "usn", riskLevel: "high", trustScore: 30 },
      { name: "ООО МедиаГрупп", inn: "7705678901", type: "ooo", taxStatus: "osno", riskLevel: "medium", trustScore: 70 },
      { name: "Самозанятый Федоров", inn: "774567890123", type: "self_employed", taxStatus: "npd", riskLevel: "medium", trustScore: 60 },
      { name: "ООО ЭкоПитание", inn: "7706789012", type: "ooo", taxStatus: "usn", riskLevel: "low", trustScore: 88 },
    ];
    ...
    for (const c of contractorSeeds) {
      ...
      for (let i = 0; i < accrualCount; i++) {
        ...
        const [accrual] = await db.insert(contractorAccrualsTable).values({
        ...
        if (Math.random() < 0.75) {
          const paidAmt = Math.random() < 0.9 ? amount : rand(Math.floor(amount * 0.4), amount - 1000);
          await db.insert(contractorPaymentsTable).values({
            contractorId: contractor.id,
            accrualId: accrual.id,
            amount: String(paidAmt),
            paymentDate: dateStr(year, month, rand(accrual.accrualDate ? 1 : 1, 28)),
    ...
    const taxTypes: Array<{ taxType: string; taxName: string; rate: number; quarterOnly?: boolean }> = [
    ...
    for (let year = 2025; year <= 2026; year++) {
      ...
      for (let month = 1; month <= maxMonth; month++) {
        ...
        await db
          ...
          .values({ periodMonth, reserveAmount: String(reserveAmt), isTestData: true })
          .onConflictDoUpdate({
            target: taxReserveTable.periodMonth,
            set: { reserveAmount: String(reserveAmt), isTestData: true, updatedAt: new Date() },
          });
      }
    }
    ...
    const positions = ["Педагог", "Старший педагог", "Методист", "Администратор", "Помощник педагога", "Завуч"];
    const departments = ["Art", "Music", "Dance", "Language", "Math", "Science", "Drama"];
    const hireDates = [
      "2020-09-01", "2021-01-15", "2021-03-10", "2021-06-01", "2022-01-09",
      "2022-04-05", "2022-08-15", "2023-01-09", "2023-03-01", "2023-09-01",
    ];

    const staffSeeds = [
      "Иванова Мария Петровна", "Петров Алексей Сергеевич", "Сидорова Елена Викторовна",
      "Козлов Дмитрий Игоревич", "Новикова Анна Александровна", "Михайлов Сергей Юрьевич",
      "Фёдорова Ольга Николаевна", "Соколов Иван Михайлович", "Попова Татьяна Владимировна",
      "Лебедев Андрей Константинович", "Смирнова Наталья Ивановна", "Волков Роман Андреевич",
      "Зайцева Ксения Олеговна", "Морозов Павел Денисович", "Орлова Светлана Евгеньевна",
      "Николаев Антон Викторович", "Семёнова Ирина Сергеевна", "Голубев Кирилл Алексеевич",
      "Виноградова Дарья Петровна", "Богданов Максим Романович", "Кузнецова Юлия Игоревна",
      "Медведев Илья Дмитриевич", "Захарова Людмила Николаевна", "Коновалов Артём Михайлович",
      "Пономарёва Валерия Андреевна", "Тихонов Степан Юрьевич", "Крылова Надежда Олеговна",
      "Беляев Владислав Константинович", "Тарасова Вероника Павловна", "Громов Евгений Сергеевич",
    ];
    ...
    for (let i = 0; i < staffSeeds.length; i++) {
      ...
      const [profile] = await db.insert(staffProfilesTable).values({
        ...
        set: { position, department, hireDate, notes: `Тестовый педагог — ${staffSeeds[i]}`, updatedAt: new Date() },
      ...
      if (Math.random() < 0.7) {
        ...
        for (let b = 0; b < bonusMonths; b++) {
          ...
          await db.insert(staffBonusesTable).values({
            teacherCrmId,
            periodMonth: bPeriod,
            amount: String(rand(2000, 15000)),
            reason: pick(["Выполнение KPI", "Наставничество", "Доп. нагрузка", "Праздничная премия", "За дополнительные занятия"]),
    ...
    const splitExamples = [
      { cashM: "2025-08", plM: "2025-09", desc: "Предоплата за сентябрь (семья Ковалёвых)", amount: 28000 },
      { cashM: "2025-11", plM: "2025-12", desc: "Предоплата за декабрь (семья Романовых)", amount: 35000 },
      { cashM: "2025-12", plM: "2026-01", desc: "Предоплата за январь 2026 (семья Орловых)", amount: 31000 },
      { cashM: "2026-02", plM: "2026-03", desc: "Предоплата за март (семья Лебедевых)", amount: 24000 },
      { cashM: "2026-04", plM: "2026-06", desc: "Аренда оплачена авансом на 2 мес.", amount: 400000 },
      { cashM: "2025-09", plM: "2025-10", desc: "Сервис оплачен за 2 месяца (IT)", amount: 14000 },
    ];
    ...
    for (const ex of splitExamples) {
      ...
      await db.insert(operations).values({
        ...
        plMonth: ex.plM,
        articleId: art?.id ?? null,
        articleName: ex.amount > 100000 ? "Аренда" : (art?.name ?? "Оплата за обучение"),
        articleCode: ex.amount > 100000 ? "2.1" : (art?.code ?? "1.1"),
        counterpartyName: ex.amount > 100000 ? "ООО Арендодатель" : `Семья тест`,
