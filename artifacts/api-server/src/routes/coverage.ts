...
function extractAlphaId(item: unknown): string | null {
  ...
  const candidates = ["id", "alpha_id", "customer_id", "lesson_id", "pay_id", "group_id", "teacher_id"];
...
function computeStatus(httpStatus: number | null, itemCount: number | null, error: string | null): string {
...
function buildCandidates(branchId: string): EndpointDef[] {
  ...
  return [
    { entityKey: "branches",           ...base("branch"),              notes: "School branches/locations" },
    { entityKey: "students",           ...base("customer"),            notes: "Students / customers (is_study=1)" },
    { entityKey: "leads",              ...base("lead"),                notes: "Leads / prospects (is_study=0)" },
    { entityKey: "lessons",            ...base("lesson"),              notes: "Lessons — may contain visits[] attendance" },
    { entityKey: "regular_lessons",    ...base("regular-lesson"),      notes: "Recurring lesson schedule templates" },
    { entityKey: "payments",           ...base("pay"),                 notes: "Payments / transactions from clients" },
    { entityKey: "subjects",           ...base("subject"),             notes: "Subjects / directions" },
    { entityKey: "teachers",           ...base("teacher"),             notes: "Teachers / instructors" },
    { entityKey: "groups",             ...base("group"),               notes: "Study groups" },
    { entityKey: "tariffs",            ...base("tariff"),              notes: "Tariff templates / subscription plans" },
    { entityKey: "customer_tariffs",   ...base("customer-tariff"),     notes: "Customer subscriptions / абонементы" },
    { entityKey: "communications",     ...base("communication"),       notes: "Notes / communications / comments" },
    { entityKey: "tasks",              ...base("task"),                notes: "Tasks / reminders" },
    { entityKey: "discounts",          ...base("discount"),            notes: "Discount records" },
    { entityKey: "absence_reasons",    ...base("absence-reason"),      notes: "Absence / skip reason codes" },
    { entityKey: "bonuses",            ...base("bonus"),               notes: "Bonus / loyalty records" },
    { entityKey: "study_statuses",     ...base("study-status"),        notes: "Study status codes (studying/paused/left)" },
    { entityKey: "cgi",                ...base("cgi"),                 notes: "Customer-group index / enrollments" },
    { entityKey: "invoices",           ...base("invoice"),             notes: "Invoices / bills (if available)" },
    { entityKey: "contracts",          ...base("contract"),            notes: "Contracts / documents" },
    { entityKey: "rooms",              ...base("room"),                notes: "Rooms / classrooms" },
    { entityKey: "statuses",           ...base("status"),              notes: "Lead/customer status codes" },
    { entityKey: "tariff_movements",   ...base("tariff-movement"),     notes: "Subscription movements / burns / freezes" },
    { entityKey: "customer_notes",     ...base("customer-note"),       notes: "Customer notes" },
    { entityKey: "lesson_topics",      ...base("lesson-topic"),        notes: "Lesson topic / homework" },
    { entityKey: "sources",            ...base("lead-source"),         notes: "Lead source codes" },
    { entityKey: "directories",        ...base("directory"),           notes: "General directory entries" },
    { entityKey: "attendance",
      endpoint: `${b}/lesson/index`,
      method: "POST",
      body: { page: 0, count: 5 },
      notes: "Attendance extracted from lesson.visits[] — not a separate endpoint" },
  ];
}
...
coverageRouter.get("/coverage/environment", async (req, res) => {
  ...
  try {
    ...
    const isProduction = process.env.NODE_ENV === "production" || dbName.includes("prod");

    res.json({
      environment: process.env.NODE_ENV ?? "development",
      database: dbName,
      isProduction,
      pgVersion: (dbInfo[0]?.pg_version as string)?.split(" ").slice(0, 2).join(" ") ?? "unknown",
      alfacrmDomain: process.env.ALFACRM_DOMAIN ?? null,
      tableCounts: tableCounts[0] ?? {},
      warning: "Bank data exists (accounts, balances, statements, transactions) but most transactions are NOT yet matched/classified/reconciled. AlphaCRM sync results are source coverage data only — NOT final financial truth.",
    });
  } catch (err) {
...
coverageRouter.get("/coverage/field-inventory", async (req, res) => {
  ...
  try {
    ...
    const limit = Math.min(Number((req.query as Record<string, string>).limit ?? "500"), 2000);
    ...
    const inventory: Record<string, Record<string, { count: number; nullCount: number; samples: unknown[] }>> = {};
    ...
    for (const row of rawRows) {
      ...
      for (const [k, v] of Object.entries(payload)) {
        if (!inventory[et][k]) inventory[et][k] = { count: 0, nullCount: 0, samples: [] };
...
coverageRouter.get("/coverage/raw", async (req, res) => {
  const client = await pool.connect();
  try {
    const { entityType, branchId, alphaId } = req.query as Record<string, string>;
    const limit = Math.min(Number((req.query as Record<string, string>).limit ?? "50"), 200);
    const conditions: string[] = [];
    const params: unknown[] = [];
    let idx = 1;
    if (entityType) { conditions.push(`entity_type = $${idx++}`); params.push(entityType); }
    if (branchId)   { conditions.push(`branch_id = $${idx++}`);   params.push(branchId); }
    if (alphaId)    { conditions.push(`alpha_id = $${idx++}`);    params.push(alphaId); }
...
coverageRouter.post("/coverage/discover", async (req, res) => {
  const { branchId: rawBranchId, fromDate, toDate } = req.body as Record<string, string>;
  const client = await pool.connect();

  // Use active scope branchId if not explicitly provided — default is Atlas (6)
  const branchId = rawBranchId ?? await getActiveScopeBranchId(client as unknown as DbClient);
  ...
  res.json({ batchId, status: "started", branchId, message: `Discovery started for branch ${branchId}` });
    ...
    try {
        ...
        try {
          ...
          if (def.entityKey === "attendance") {
            await bgClient.query(
              ...
              [def.entityKey, def.endpoint, branchId, def.method, JSON.stringify(def.body ?? {}), def.notes ?? null],
            );
            checked++;
            continue;
          }

          const probe = await crmProbe(def.endpoint, def.method, def.body, token);
          ...
          else { failed++; if (probe.error) errors.push({ entityKey: def.entityKey, error: probe.error }); }

          const nextAction =
            status === "NOT_FOUND"   ? "Endpoint does not exist in this AlphaCRM version" :
            status === "FORBIDDEN"   ? "Access denied — check API key permissions" :
            status === "NOT_EXPOSED" ? "Endpoint not exposed via API — may need to request from AlphaCRM support" :
            status === "EMPTY"       ? "Endpoint exists but returned 0 records — may be correct or need date filter" :
            status === "ERROR"       ? "Check error message and retry" :
            status === "OK"          ? "Ready for raw sync" : "Investigate";
          ...
          await bgClient.query(
            `INSERT INTO alpha_endpoint_registry
             ...
             ON CONFLICT (entity_key, branch_id) DO UPDATE SET
               ...
               updated_at = NOW()`,
            [
              def.entityKey, def.endpoint, branchId, def.method,
          ...
          checked++;
          logger.info({ entityKey: def.entityKey, status, httpStatus: probe.status }, "discovery: endpoint checked");
        } catch (err) {
          failed++;
          errors.push({ entityKey: def.entityKey, error: String(err) });
          logger.error({ err, entityKey: def.entityKey }, "discovery: endpoint probe failed");
        }
      }
      ...
      await bgClient.query(
        `UPDATE alpha_sync_batches SET
         ...
         WHERE id = $1`,
        [batchId, succeeded, failed, checked, errors.length, JSON.stringify(errors)],
      );
    } catch (err) {
...
coverageRouter.post("/coverage/sync", async (req, res) => {
  const {
    ...
    toDate,
    entityKeys,
  } = req.body as { branchId?: string; fromDate?: string; toDate?: string; entityKeys?: string[] };
  ...
  if (outOfScope) {
    logger.warn({ branchId, scopeBranchId }, "raw sync: requested branch is outside active scope");
  }
  ...
  const registryParams = entityKeys?.length ? [branchId, entityKeys] : [branchId];
  ...
  if (toSync.length === 0) {
    ...
    res.status(400).json({
      ...
      hint: outOfScope ? `Branch ${branchId} is outside scope (${scopeBranchId}/${scopeBranchName}). Run discovery for the Atlas branch first.` : undefined,
    });
    return;
  }
  ...
  res.json({
    ...
    message: outOfScopeWarning ?? `Raw sync started for Atlas branch (${branchId}/${scopeBranchName})`,
  });
    ...
    try {
        ...
        try {
          ...
          while (page < MAX_PAGES) {
            ...
            if (!probe.status || probe.status >= 400) {
              errors.push({ entityKey, page, error: probe.error ?? `HTTP ${probe.status}` });
              totalErrors++;
              break;
            }
            ...
            const currPageIds = new Set<string>(items.map(extractAlphaId).filter((id): id is string => id !== null));
            ...
            if (prevPageIds !== null && prevPageIds.size === currPageIds.size && [...currPageIds].every((id) => prevPageIds!.has(id))) {
              logger.warn({ entityKey, page, items: items.length }, "raw sync: same alpha_ids as previous page — pagination loop detected, stopping");
              break;
            }
            ...
            for (const item of items) {
              try {
                ...
                const { rows: upsertRows } = await bgClient.query(
                  ...
                  [
                    alphaId, entityKey, endpoint, branchId,
                    JSON.stringify(item), hash, batchId, page,
                    fromDate, effectiveTo,
                  ],
                );

                const isInsert = upsertRows[0]?.is_insert === true;
                if (isInsert) { entitySaved++; totalSaved++; }
                else { totalUpdated++; }
              } catch (rowErr) {
                totalErrors++;
                errors.push({ entityKey, alphaId: extractAlphaId(item), error: String(rowErr) });
              }
            }
          ...
          succeeded++;
          entitySaved > 0 && logger.info({ entityKey, fetched: entityFetched, saved: entitySaved }, "raw sync: entity done");
        } catch (err) {
          failed++;
          totalErrors++;
      ...
      await bgClient.query(
        `UPDATE alpha_sync_batches SET
         ...
         WHERE id = $1`,
        [batchId, succeeded, failed, totalFetched, totalSaved, totalUpdated, totalSkipped, totalErrors, JSON.stringify(errors)],
      );
    } catch (err) {
...
coverageRouter.get("/coverage/verification-report", async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      res.json({ report: null, status: "not_run", reportId: null, startedAt: null, finishedAt: null });
      return;
    }
    const row = rows[0];
    res.json({
...
coverageRouter.get("/coverage/scope", async (req, res) => {
  ...
  try {
    ...
    const excludedBranches = knownBranches
      ...
      .map((b) => ({ branchId: b.id, branchName: b.name, reason: "Not in current Atlas-only scope" }));

    res.json({
      scope: active[0] ?? null,
      allScopes: all,
      excludedBranches,
      scopeWarning: "Current AlphaCRM audit scope is Atlas only. Other branches are discovered but excluded from current raw sync and normalization.",
    });
  } catch (err) {
...
coverageRouter.post("/coverage/scope", async (req, res) => {
  const {
    ...
    reason,
    notes,
  } = req.body as { branchId: string; branchName: string; scopeName?: string; reason?: string; notes?: string };
  ...
  const client = await pool.connect();
  try {
    await client.query(`UPDATE alpha_sync_scope SET is_active = false, updated_at = NOW()`);
    const { rows } = await client.query(
    ...
    req.log.info({ branchId, branchName, scopeName }, "coverage: scope updated");
    res.json({ scope: rows[0], message: `Scope set to "${scopeName}" (branch ${branchId} — ${branchName})` });
  } catch (err) {
...
coverageRouter.get("/coverage/branches", async (req, res) => {
  ...
  try {
    ...
    const branchCountMap = new Map(branchCounts.map((r) => [r.branch_id as string, r]));
    ...
    const SUSPECTED: Record<string, { meaning: string; use: string }> = {
      ...
      "5": { meaning: "Онлайн Школа (дубль/новая) — LOW VOLUME",     use: "EXCLUDE_NOW" },
      "6": { meaning: "Главный физический центр АртХелло (Атлас)",   use: "CURRENT_SCOPE" },
      "7": { meaning: "Кемпинг — сезонный/временный",                use: "EXCLUDE_NOW" },
      "8": { meaning: "Школа 1-11 — отдельная программа, нет оплат", use: "UNKNOWN_REVIEW_REQUIRED" },
    };

    const branches = Array.from(allIds).map((bid) => {
      const known = knownBranches.find((b) => b.id === bid);
      const counts = branchCountMap.get(bid);
      const guess  = SUSPECTED[bid];
      const name   = known?.name ?? guess?.meaning?.split(" — ")[0] ?? `Branch ${bid}`;
      return {
        branchId:         bid,
        branchName:       name,
        isActiveScope:    bid === activeBranchId,
        recommendedUse:   guess?.use ?? "UNKNOWN_REVIEW_REQUIRED",
        suspectedMeaning: guess?.meaning ?? "Неизвестное назначение — требует проверки",
    ...
    const atlasBranch = branches.find((b) => b.recommendedUse === "CURRENT_SCOPE");
...
coverageRouter.get("/coverage/atlas-summary", async (req, res) => {
  ...
  try {
    ...
    const dbHostMasked = dbHostRaw.length > 3 ? dbHostRaw.substring(0, 3) + "***" : "***";
    const isProduction = process.env.NODE_ENV === "production" || dbName.toLowerCase().includes("prod");
    ...
    for (const row of entityCounts) countMap[row.entity_type as string] = Number(row.raw_count);
    ...
    const subjectIdSet      = new Set<string>((subjectIdRows[0]?.ids      as string[] | null) ?? []);
    const lessonSubjectRefs = new Set<string>(((lessonSubjectRefRows[0]?.ids as string[] | null) ?? []).filter(Boolean));
    const resolvedSubjectIds   = [...lessonSubjectRefs].filter((id) => subjectIdSet.has(id));
    const unresolvedSubjectIds = [...lessonSubjectRefs].filter((id) => !subjectIdSet.has(id));
    ...
    const ctFields: string[] = ct?.discovered_fields
      ? (Array.isArray(ct.discovered_fields) ? ct.discovered_fields as string[] : JSON.parse(String(ct.discovered_fields)) as string[])
      : [];
    const ABONEMENT_FIELD_KEYS = [
      "remaining_count", "paid_count", "next_lesson_date", "freeze_date",
      "sold_count", "paid_till", "expiry_date", "lessons_left", "balance",
    ];
    const abonementFieldsFound = ctFields.filter((f) => ABONEMENT_FIELD_KEYS.includes(f));
    ...
    const verLessons = atlasBranchAudit
      ? (atlasBranchAudit as Record<string, unknown>).lessons as Record<string, unknown> | undefined
      : undefined;
    ...
    const discoveryDone = lastDiscRows.length > 0 && lastDiscRows[0].status === "completed";
    const syncDone      = lastSyncRows.length > 0 && lastSyncRows[0].status === "completed";
    ...
    if (!scopeIsAtlas || !scope) blockers.push("Active scope is not Atlas (branchId=6)");
    if (!discoveryDone)          blockers.push("Atlas Discovery has not been run (or not completed)");
    if (!syncDone)               blockers.push("Atlas Raw Sync has not been run (or not completed)");
    if (!hasStudents)            blockers.push("No student records in alpha_raw_records for Atlas");
    if (!hasRegistry)            blockers.push("Endpoint registry empty — run Discovery first");

    if (!hasLessons)  notes.push("No lesson records yet — lesson endpoint may be EMPTY or date filter issue");
    if (!hasPayments) notes.push("No payment records yet");
    if (unresolvedSubjectIds.length > 0)
      notes.push(`${unresolvedSubjectIds.length} subject IDs referenced by lessons but not in subjects table`);
    if (["PERIOD_FILTER_IGNORED", "IGNORED"].includes(dateFilterStatus))
      notes.push("AlphaCRM ignores lesson date filter — full dataset fetched regardless of date range");
    if (!isProduction)
      notes.push(`Running in ${process.env.NODE_ENV ?? "development"} environment (not production DB)`);

    const readinessStatus: "READY" | "PARTIAL" | "NOT_READY" =
      blockers.length === 0 && hasStudents && hasLessons && hasPayments ? "READY" :
      blockers.length === 0 && hasStudents                              ? "PARTIAL" :
      "NOT_READY";
    ...
    res.json({
      ...
      scopeStatus:     scope ? (scope.is_active ? "ACTIVE" : "INACTIVE") : "NOT_CONFIGURED",
      ...
      lessonSummary: {
        ...
        byMonth:            lsnByMonth.map((r) => ({ month: String(r.month), count: Number(r.cnt) })),
        lessonsWithVisits,
        attendanceExtracted: totalVisits,
        avgVisitsPerLesson: lessonsWithVisits > 0
      ...
      paymentSummary: {
        ...
        byMonth:            payByMonth.map((r) => ({ month: String(r.month), count: Number(r.cnt) })),
      ...
      unresolvedEndpoints: registryRows
        .filter((r) => !["OK","EMPTY","EMBEDDED"].includes(r.status as string))
      ...
      lessonDates:  { earliest: lsnDates[0]?.earliest ?? null, latest: lsnDates[0]?.latest ?? null },
      paymentDates: { earliest: payDates[0]?.earliest ?? null, latest: payDates[0]?.latest ?? null },
...
coverageRouter.post("/coverage/verify", async (req, res) => {
  ...
  res.json({ reportId, status: "started", message: "Deep verification audit started in background" });
...
async function runVerification(token: string, fromDate: string, toDate: string): Promise<Record<string, unknown>> {
  const PAGE_SIZE = 100;
  const suspiciousFlags: string[] = [];
  const nextActions: string[] = [];

  // ── 1. Detect branches ────────────────────────────────────────────────────
  const branchProbe = await crmProbe("0/branch/index", "POST", { page: 0, count: 100 }, token);
  ...
  if (branches.length === 0) {
    branches = [{ id: "1", name: "Branch 1 (assumed — branch endpoint returned empty)" }];
    suspiciousFlags.push("BRANCH_LIST_EMPTY — using assumed branchId=1");
    nextActions.push("Verify AlphaCRM branch endpoint access; try different branch URL patterns");
  }
  ...
  for (const branch of branches) {
    ...
    // ── Students / customers ───────────────────────────────────────────────
    const stdAll   = await crmProbe(`${bid}/customer/index`, "POST", { page: 0, count: PAGE_SIZE }, token);
    const stdItems = extractItems(stdAll.parsedJson);
    const stdTotal = extractTotal(stdAll.parsedJson);

    const stdActive   = await crmProbe(`${bid}/customer/index`, "POST", { page: 0, count: PAGE_SIZE, is_study: 1 }, token);
    const stdActItems = extractItems(stdActive.parsedJson);
    const stdActTotal = extractTotal(stdActive.parsedJson);

    const stdLeads   = await crmProbe(`${bid}/customer/index`, "POST", { page: 0, count: PAGE_SIZE, is_study: 0 }, token);
    const stdLdTotal = extractTotal(stdLeads.parsedJson);

    // Date filter check on students
    const stdWithDate    = await crmProbe(`${bid}/customer/index`, "POST", { page: 0, count: PAGE_SIZE, date_from: fromDate, date_to: toDate }, token);
    const stdWithDateTotal = extractTotal(stdWithDate.parsedJson);
    const stdDateFilter  = stdWithDateTotal !== null && stdTotal !== null && stdWithDateTotal < stdTotal
      ? "SUPPORTED" : "PERIOD_FILTER_IGNORED";
    ...
    if (effStudentTotal < 100) {
      suspiciousFlags.push(`[branch ${bid}] STUDENT_ACTIVE_COUNT=${effStudentTotal} SUSPICIOUS_LOW_VOLUME`);
      nextActions.push(`[branch ${bid}] Verify is_study filter — check if students are archived or in a different branch`);
    }

    // ── Lessons ───────────────────────────────────────────────────────────
    const lsnWith     = await crmProbe(`${bid}/lesson/index`, "POST", { page: 0, count: PAGE_SIZE, date_from: fromDate, date_to: toDate }, token);
    const lsnWithItems = extractItems(lsnWith.parsedJson);
    const lsnWithTotal = extractTotal(lsnWith.parsedJson);

    const lsnWithout     = await crmProbe(`${bid}/lesson/index`, "POST", { page: 0, count: PAGE_SIZE }, token);
    const lsnWithoutTotal = extractTotal(lsnWithout.parsedJson);

    const lsnDateFilter = (lsnWithTotal !== null && lsnWithoutTotal !== null && lsnWithTotal < lsnWithoutTotal)
      ? "SUPPORTED"
      : (lsnWithTotal === lsnWithoutTotal ? "PERIOD_FILTER_IGNORED" : "UNKNOWN");
    ...
    // Regular lessons
    const regLsn = await crmProbe(`${bid}/regular-lesson/index`, "POST", { page: 0, count: PAGE_SIZE }, token);
    const regLsnTotal = extractTotal(regLsn.parsedJson);
    const regLsnStatus = computeStatus(regLsn.status, extractItems(regLsn.parsedJson).length, regLsn.error);

    const effLessonTotal = lsnWithTotal ?? lsnWithItems.length;
    if (effLessonTotal < 200) {
      suspiciousFlags.push(`[branch ${bid}] LESSON_COUNT=${effLessonTotal} SUSPICIOUS_LOW_VOLUME (from ${fromDate} to ${toDate})`);
      nextActions.push(`[branch ${bid}] If lesson date filter is IGNORED, try fetching without filter; check lesson statuses (cancelled/draft may be excluded)`);
    }

    // Pagination completeness for lessons
    const lsnPagination = lsnWithTotal !== null
    ...
    // ── Payments ───────────────────────────────────────────────────────────
    const payWith      = await crmProbe(`${bid}/pay/index`, "POST", { page: 0, count: PAGE_SIZE, date_from: fromDate, date_to: toDate }, token);
    const payWithItems = extractItems(payWith.parsedJson);
    const payWithTotal = extractTotal(payWith.parsedJson);

    const payWithout      = await crmProbe(`${bid}/pay/index`, "POST", { page: 0, count: PAGE_SIZE }, token);
    const payWithoutTotal = extractTotal(payWithout.parsedJson);

    const payDateFilter = (payWithTotal !== null && payWithoutTotal !== null && payWithTotal < payWithoutTotal)
      ? "SUPPORTED"
      : (payWithTotal === payWithoutTotal ? "PERIOD_FILTER_IGNORED" : "UNKNOWN");
    ...
    const payHasBalance = payFields.some((f) => f.includes("balance") || f.includes("debt"));
    const payHasInvoice = payFields.some((f) => f.includes("invoice") || f.includes("bill"));

    const effPayTotal = payWithTotal ?? payWithItems.length;
    if (effPayTotal < 50) {
      suspiciousFlags.push(`[branch ${bid}] PAYMENT_COUNT=${effPayTotal} SUSPICIOUS_LOW_VOLUME`);
    }

    // ── Subjects ───────────────────────────────────────────────────────────
    const subj      = await crmProbe(`${bid}/subject/index`, "POST", { page: 0, count: PAGE_SIZE }, token);
    ...
    const subjectIds = subjItems.map((s) => String((s as Record<string, unknown>).id ?? "?"));
    ...
    const unresolvedSubjectIds = Array.from(lsnSubjectRefs).filter((id) => !subjectIds.includes(id));
    if (unresolvedSubjectIds.length > 0) {
      suspiciousFlags.push(`[branch ${bid}] ${unresolvedSubjectIds.length} UNRESOLVED_SUBJECT_IDS in lesson sample`);
    }

    // ── Groups ─────────────────────────────────────────────────────────────
    const grp      = await crmProbe(`${bid}/group/index`, "POST", { page: 0, count: PAGE_SIZE }, token);
    ...
    const ct       = await crmProbe(`${bid}/customer-tariff/index`, "POST", { page: 0, count: PAGE_SIZE }, token);
    ...
    const ctHasRemaining = ctFields.some((f) => f.includes("remain") || f.includes("lesson_count") || f.includes("left") || f.includes("paid_count"));
    const ctHasExpiry    = ctFields.some((f) => f.includes("expire") || f.includes("end_date") || f.includes("to_date") || f.includes("finish"));
    const ctHasFreeze    = ctFields.some((f) => f.includes("freeze") || f.includes("pause") || f.includes("stop"));
    const ctHasBurned    = ctFields.some((f) => f.includes("burned") || f.includes("spent") || f.includes("used"));

    if (ctStatus === "NOT_FOUND" || ctStatus === "FORBIDDEN" || ctStatus === "NOT_EXPOSED") {
      suspiciousFlags.push(`[branch ${bid}] SUBSCRIPTIONS_NOT_ACCESSIBLE (customer-tariff status=${ctStatus})`);
      nextActions.push(`[branch ${bid}] Check AlphaCRM plan — subscriptions (абонементы) may not be exposed via API in current plan`);
    }

    // ── Tariff-movements ───────────────────────────────────────────────────
    const tm      = await crmProbe(`${bid}/tariff-movement/index`, "POST", { page: 0, count: PAGE_SIZE }, token);
    ...
    const custHasBalance = custFields.some((f) => f.includes("balance") || f.includes("debt") || f.includes("paid") || f.includes("balance_contract"));
    const custHasDebt    = custFields.some((f) => f.includes("debt") || f.includes("arrear") || f.includes("overdue"));
  ...
  if (nextActions.length === 0) {
    nextActions.push("Run POST /coverage/sync to fetch all pages and save to alpha_raw_records");
    nextActions.push("Run POST /coverage/discover to re-probe all 28 endpoint candidates after verification");
  }

  const overallStatus = suspiciousFlags.length > 3 ? "suspicious" : suspiciousFlags.length > 0 ? "warning" : "clean";
  ...
  return {
    ...
    environment: {
      ...
      alfacrmDomain: process.env.ALFACRM_DOMAIN ?? null,
      credentialsSource: "environment secrets (ALFACRM_EMAIL + ALFACRM_API_KEY)",
      warningNotProduction: process.env.NODE_ENV !== "production"
        ? "⚠ This audit is NOT running against production data. NODE_ENV=" + (process.env.NODE_ENV ?? "development")
        : null,
    },
    branchCoverage: {
      ...
      multibranchNote: branches.length > 1
...
async function runLinkingDiagnostics(client: DbClient, batchId: string, branchId: string) {
  try {
