...
categorizationRouter.post("/banking/setup-suggestion-rules", async (req, res) => {
  try {
    ...
    for (const row of articleRows.rows as Array<{ id: string; code: string }>) {
    ...
    for (const rule of SEED_RULES) {
      ...
      if (rule.suggestedArticleId && ARTICLE_CODE_MAP[rule.suggestedArticleId]) {
        articleId = codeToId[ARTICLE_CODE_MAP[rule.suggestedArticleId]] ?? null;
      }
    ...
    req.log.info({ seeded, updated, skipped }, "setup-suggestion-rules P3.2 complete");
    res.json({ ok: true, seeded, updated, skipped, totalRules: SEED_RULES.length });
  } catch (err) {
...
categorizationRouter.get("/banking/transactions/:id/suggestion", async (req, res) => {
  try {
    const { id } = req.params;
    ...
    type RuleRow = {
      id: string; rule_name: string | null;
      match_field: string; match_type: string; pattern: string;
      match_direction: string;
      suggested_action: string | null; suggested_article_id: string | null;
    ...
    for (const rule of rules) {
      ...
      switch (rule.match_type) {
        case "contains":    matches = fieldValue.toLowerCase().includes(pattern.toLowerCase()); break;
        case "equals":      matches = fieldValue.toLowerCase() === pattern.toLowerCase();       break;
        case "starts_with": matches = fieldValue.toLowerCase().startsWith(pattern.toLowerCase()); break;
        case "regex":
          try { matches = new RegExp(pattern, "i").test(fieldValue); } catch { matches = false; }
          break;
      }
...
categorizationRouter.get("/banking/suggestion-stats", async (_req, res) => {
  try {
    ...
    type TxRow = { id: string; counterparty_name: string | null; purpose: string | null; bank_name: string | null; direction: string | null };
      ...
      for (const rule of rules) {
        ...
        const fieldValue = (() => {
          switch (rule.match_field) {
            ...
            case "purpose":      return tx.purpose ?? "";
            case "bank_name":    return tx.bank_name ?? "";
            case "direction":    return tx.direction ?? "";
            default:             return "";
          }
        })();
        ...
        let hits = false;
        switch (rule.match_type) {
          case "contains":    hits = fieldValue.toLowerCase().includes(pattern.toLowerCase()); break;
          case "equals":      hits = fieldValue.toLowerCase() === pattern.toLowerCase();       break;
          case "starts_with": hits = fieldValue.toLowerCase().startsWith(pattern.toLowerCase()); break;
          case "regex":
            try { hits = new RegExp(pattern, "i").test(fieldValue); } catch { hits = false; }
            break;
        }
        if (hits) {
    ...
    const topRules = rules
      ...
      .map((r) => ({ ruleId: r.id, ruleName: r.rule_name, hits: ruleHits[r.id] ?? 0 }))
    ...
    res.json({
      ...
      withSuggestion,
      withoutSuggestion,
      coveragePct: txns.length > 0 ? Math.round((withSuggestion / txns.length) * 100) : 0,
      topRules,
