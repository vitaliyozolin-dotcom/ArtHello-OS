...
financeQaRouter.get("/qa/summary", async (req, res) => {
  try {
    ...
    const [[total], monthRows, [noArt], [noDocs], [split], [unver], trustRows, typeRows, dirRows] =
      await Promise.all([
        db.select({ count: sql<number>`count(*)::int` }).from(operations).where(base),

        db.selectDistinct({ month: operations.cashflowMonth })
          .from(operations).where(base).orderBy(operations.cashflowMonth),
        ...
        }).from(operations).where(base)
          .groupBy(operations.cashflowMonth).orderBy(operations.cashflowMonth),

        db.select({
          type: operations.operationType,
          count: sql<number>`count(*)::int`,
          total: sql<number>`sum(${operations.amount}::numeric)::float`,
        }).from(operations).where(base).groupBy(operations.operationType),
    ...
    res.json({
      ...
      cashflowByMonth: dirRows.reduce<Record<string, { in: number; out: number }>>((acc, r) => {
