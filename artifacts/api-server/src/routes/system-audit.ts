...
function parseDbFingerprint(dbUrl: string): {
  host: string;
  name: string;
  schema: string;
  fingerprint: string;
...
async function maxDate(tableName: string, col: string): Promise<string | null> {
  try {
    const r = await db.execute(
      sql.raw(`SELECT MAX(${col})::text AS d FROM ${tableName}`),
    );
...
router.get("/system/db-audit", async (req, res) => {
  ...
  try {
    const vRow = await db.execute(
      sql.raw("SELECT version() AS v, current_database() AS db"),
    );
    pgVersion =
      (vRow.rows[0] as { v: string }).v?.split(" ").slice(0, 2).join(" ") ??
      "unknown";
  ...
  // ── data source map ────────────────────────────────────────────────────────
  const dataSourceMap = {
    "Пульс / cashflow block": {
      endpoint: "/api/banking/cashflow",
    ...
    "Banking / Overview": {
      endpoint: "/api/banking/connectors",
      tables: ["bank_connectors", "bank_accounts"],
      rows: {
    ...
    "Banking / Операции": {
      endpoint: "/api/banking/transactions OR /api/ledger/operations",
      tables: ["bank_transactions", "operations"],
      rows: {
        bank_transactions: cntBankTransactions,
    ...
    "Banking / ДДС Отчёт": {
      endpoint: "/api/ledger/cashflow-report",
      tables: ["operations", "articles"],
      rows: { operations: cntOperations, articles: cntArticles },
    },
    "Banking / Сопоставление": {
      endpoint: "/api/banking/match-stats",
      tables: ["bank_transactions", "operations"],
      rows: {
        bank_transactions: cntBankTransactions,
        operations: cntOperations,
    ...
    "Banking / Счета": {
      ...
      rows: { bank_accounts: cntBankAccounts },
    },
    "Banking / Коннекторы": {
      endpoint: "/api/banking/connectors",
      tables: ["bank_connectors"],
      rows: { bank_connectors: cntBankConnectors },
    },
    "Banking / Статус данных": {
      endpoint: "/api/banking/sync-runs",
      tables: ["bank_sync_runs", "bank_transactions"],
      rows: {
    ...
    "Реестр операций": {
      endpoint: "/api/ledger/operations",
      tables: ["operations", "articles"],
      rows: { operations: cntOperations, articles: cntArticles },
    },
    "Сверка с банком": {
