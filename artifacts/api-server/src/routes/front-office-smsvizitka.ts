...
function phoneHash(value: string): string {
  ...
  if (!secret)
    throw new Error("FRONT_OFFICE_PHONE_MATCH_SECRET is not configured");
...
function suppliedWebhookToken(req: Request): string {
  ...
  return String(
    req.header("x-webhook-key") ?? req.header("x-api-key") ?? "",
  ).trim();
}
...
smsVizitkaShadowRouter.post(
  "/webhooks/smsvizitka",
  async (req, res): Promise<void> => {
    if (!isShadowEnabled()) {
      res
        .status(423)
        .json({ status: "error", error: "SHADOW mode is disabled" });
      return;
    }
    if (!configured()) {
      res
        .status(503)
        .json({ status: "error", error: "Integration is not configured" });
      return;
    }
    ...
    if (!parsed.success) {
      res
        .status(400)
        .json({ status: "error", error: "Invalid webhook payload" });
      return;
    }
    ...
    try {
      ...
      res
        .status(500)
        .json({ status: "error", error: "Webhook processing failed" });
    } finally {
      client.release();
    }
  },
...
smsVizitkaShadowRouter.get(
  "/front-office/channel-inbox",
  async (req, res): Promise<void> => {
    if (!isShadowEnabled()) {
      res.json({
        contract: {
    ...
    try {
      ...
      res.json({
        ...
        conversations: (conversations.rows as Record<string, unknown>[]).map(
          (row) => ({
            ...row,
