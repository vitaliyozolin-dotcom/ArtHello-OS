...
trustScoreRouter.get("/trust-score/monthly", async (req, res) => {
  try {
    ...
    const result = await Promise.all(
      monthRows.map(async ({ month }) => {
        ...
        const [total] = await db
          .select({ cnt: count(operations.id), revenue: sum(operations.amount) })
          .from(operations)
        ...
        const [highAmount] = await db
          .select({ cnt: count(operations.id) })
          .from(operations)
          .where(and(eq(operations.plMonth, month), sql`amount::numeric > 500000`));
        ...
        // Score = 100 - penalties
        const noArticlePenalty = totalOps > 0 ? (noArticleCnt / totalOps) * 40 : 0;
        const unverifiedPenalty = totalOps > 0 ? (unverifiedCnt / totalOps) * 40 : 0;
        const highAmtPenalty = highAmtCnt * 5;
        const score = Math.max(0, Math.round(100 - noArticlePenalty - unverifiedPenalty - Math.min(highAmtPenalty, 20)));
    ...
    res.json(result.filter(Boolean).sort((a, b) => a!.month.localeCompare(b!.month)));
  } catch (err) {
...
trustScoreRouter.get("/trust-score/issues", async (req, res) => {
  try {
    ...
    const baseCondition = month ? and(eq(operations.plMonth, month)) : undefined;
    ...
    const noArticle = await db
      ...
      .from(operations)
      .where(and(baseCondition, sql`article_id IS NULL AND direction IN ('in','out')`))
    ...
    const unverified = await db
      ...
      .from(operations)
      .where(and(baseCondition, eq(operations.verificationStatus, "unverified"), sql`direction IN ('in','out')`))
      .orderBy(desc(operations.amount))
      .limit(50);
    ...
    const highAmount = await db
      ...
      .from(operations)
      .where(and(baseCondition, sql`amount::numeric > 500000 AND direction IN ('in','out')`))
      .orderBy(desc(operations.amount))
      .limit(20);

    const all = [...noArticle, ...unverified, ...highAmount]
