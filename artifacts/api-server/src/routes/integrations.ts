...
integrationsRouter.get(
  ...
  async (req, res): Promise<void> => {
    const connectors = await db
      .select()
      .from(sourceConnectorsTable)
      .orderBy(sourceConnectorsTable.createdAt);
    res.json(connectors);
  },
);
  ...
  async (req, res): Promise<void> => {
    ...
    if (!configured) {
      res.json({
        configured: false,
        error:
          "GOOGLE_SERVICE_ACCOUNT_JSON or GOOGLE_SHEET_ID_PROMOTION not set",
      });
      return;
    }
    try {
...
integrationsRouter.get(
  "/integrations/google-sheets/sheets",
  async (req, res): Promise<void> => {
    try {
      const auth = getGoogleAuth();
      const sheets = google.sheets({ version: "v4", auth });
...
integrationsRouter.get(
  ...
  async (req, res): Promise<void> => {
    ...
    if (!sheetName) {
      res.status(400).json({ error: "sheetName required" });
      return;
    }
    ...
    try {
      ...
      res.json({
        headers,
        rows: dataRows,
        totalRowsInSheet: rows.length - headerIdx - 1,
      });
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
...
integrationsRouter.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (!sheetName || !mapping) {
      res.status(400).json({ error: "sheetName and mapping required" });
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < dataRows.length; i++) {
        ...
        headers.forEach((h, ci) => {
          rawPayload[h] = String(row[ci] ?? "");
        });

        const hash = makeHash(
          `google_sheets__${sheetId}__${sheetName}__${i}__${JSON.stringify(rawPayload)}`,
        );
...
integrationsRouter.patch(
  ...
  async (req, res): Promise<void> => {
    const { id } = req.params;
    const { config, status } = req.body as {
