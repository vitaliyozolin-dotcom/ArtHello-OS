...
import {
  directionsTable, programsTable, classGroupsTable,
  enrollmentsTable, educationalUnitsTable,
  type Direction, type ClassGroup,
...
educationalModuleRouter.get("/educational/directions", async (_req, res) => {
  const rows = await db.select().from(directionsTable).orderBy(directionsTable.sortOrder);
  res.json(rows);
});

educationalModuleRouter.post("/educational/directions", async (req, res) => {
  const schema = z.object({
    name: z.string(), code: z.string().optional(), color: z.string().optional(),
...
educationalModuleRouter.patch("/educational/directions/:id", async (req, res) => {
  const { id } = req.params;
  const schema = z.object({
    name: z.string().optional(), code: z.string().optional(), color: z.string().optional(),
    description: z.string().optional(), sortOrder: z.number().optional(),
    isActive: z.boolean().optional(),
  });
  const data = schema.parse(req.body);
  const [row] = await db.update(directionsTable).set(data)
    .where(eq(directionsTable.id, id as `${string}-${string}-${string}-${string}-${string}`))
    .returning();
  if (!row) { res.status(404).json({ error: "Not found" }); return; }
  res.json(row);
});

educationalModuleRouter.delete("/educational/directions/:id", async (req, res) => {
  await db.delete(directionsTable)
    .where(eq(directionsTable.id, req.params.id as `${string}-${string}-${string}-${string}-${string}`));
  res.json({ ok: true });
});
...
educationalModuleRouter.get("/educational/programs", async (req, res) => {
  ...
  const rows = await db.select().from(programsTable)
    .where(directionId
      ? eq(programsTable.directionId, directionId as `${string}-${string}-${string}-${string}-${string}`)
      : undefined)
...
educationalModuleRouter.post("/educational/programs", async (req, res) => {
  const schema = z.object({
    name: z.string(), directionId: z.string().uuid().optional(),
    description: z.string().optional(), durationMonths: z.number().optional(),
    ageFrom: z.number().optional(), ageTo: z.number().optional(),
    pricePerMonth: z.string().optional(), pricePerLesson: z.string().optional(),
    lessonsPerWeek: z.number().optional(),
  });
  const data = schema.parse(req.body);
  const [row] = await db.insert(programsTable).values(data as typeof programsTable.$inferInsert).returning();
  res.status(201).json(row);
});

educationalModuleRouter.delete("/educational/programs/:id", async (req, res) => {
  await db.delete(programsTable)
    .where(eq(programsTable.id, req.params.id as `${string}-${string}-${string}-${string}-${string}`));
  res.json({ ok: true });
});
...
educationalModuleRouter.get("/educational/groups", async (req, res) => {
  ...
  const where = [];
  if (directionId) where.push(eq(classGroupsTable.directionId, directionId as `${string}-${string}-${string}-${string}-${string}`));
  if (active === "true") where.push(eq(classGroupsTable.isActive, true));

  const groups = await db.select().from(classGroupsTable)
  ...
  const result = groups.map((g: ClassGroup) => ({
    ...
    directionColor: g.directionId ? (dirMap[g.directionId]?.color ?? null) : null,
  }));
...
educationalModuleRouter.post("/educational/groups", async (req, res) => {
  const schema = z.object({
    name: z.string(), directionId: z.string().uuid().optional(),
    programId: z.string().uuid().optional(), branchCrmId: z.string().optional(),
    teacherCrmId: z.string().optional(), level: z.string().optional(),
    maxStudents: z.number().optional(), scheduleInfo: z.string().optional(),
    monthlyRevenue: z.string().optional(), isTestData: z.boolean().optional(),
  });
  const data = schema.parse(req.body);
  const [row] = await db.insert(classGroupsTable).values(data as typeof classGroupsTable.$inferInsert).returning();
  res.status(201).json(row);
});
...
educationalModuleRouter.patch("/educational/groups/:id", async (req, res) => {
  const { id } = req.params;
  const schema = z.object({
    name: z.string().optional(), directionId: z.string().uuid().optional(),
    programId: z.string().uuid().optional(), branchCrmId: z.string().optional(),
    teacherCrmId: z.string().optional(), level: z.string().optional(),
    maxStudents: z.number().optional(), currentStudents: z.number().optional(),
    scheduleInfo: z.string().optional(), monthlyRevenue: z.string().optional(),
    isActive: z.boolean().optional(),
  });
  const data = schema.parse(req.body);
  const [row] = await db.update(classGroupsTable)
    .set({ ...data, updatedAt: new Date() })
    .where(eq(classGroupsTable.id, id as `${string}-${string}-${string}-${string}-${string}`))
    .returning();
  if (!row) { res.status(404).json({ error: "Not found" }); return; }
  res.json(row);
});
...
educationalModuleRouter.delete("/educational/groups/:id", async (req, res) => {
  await db.delete(classGroupsTable)
    .where(eq(classGroupsTable.id, req.params.id as `${string}-${string}-${string}-${string}-${string}`));
  res.json({ ok: true });
});
...
educationalModuleRouter.get("/educational/enrollments", async (req, res) => {
  ...
  const where = [];
  if (groupId)      where.push(eq(enrollmentsTable.classGroupId, groupId as `${string}-${string}-${string}-${string}-${string}`));
...
educationalModuleRouter.post("/educational/enrollments", async (req, res) => {
  const schema = z.object({
    studentCrmId: z.string(), classGroupId: z.string().uuid(),
    enrolledAt: z.string(), contractId: z.string().uuid().optional(),
    notes: z.string().optional(),
  });
  const data = schema.parse(req.body);
  const [row] = await db.insert(enrollmentsTable)
    .values(data as typeof enrollmentsTable.$inferInsert).returning();
...
educationalModuleRouter.get("/educational/stats", async (_req, res) => {
  const [dirs, groups, enrollments] = await Promise.all([
    ...
    db.select().from(enrollmentsTable).where(eq(enrollmentsTable.status, "active")),
  ]);

  const activeGroups = groups.filter((g: ClassGroup) => g.isActive);
  const totalRevenue = activeGroups.reduce((s: number, g: ClassGroup) => s + parseFloat(g.monthlyRevenue ?? "0"), 0);
...
function classifyGroup(name: string): {
  ...
  if (KINDERGARTEN_REGEX.test(trimmed)) {
    return {
      ...
      classificationReason: "Kindergarten group — name matches Atlas country-group pattern",
    };
  }
  ...
  if (lo.includes("лагерь") || (lo.includes("лето") && !KINDERGARTEN_REGEX.test(trimmed))) {
    return {
  ...
  if (serviceKeywords.some(k => lo.includes(k))) {
    return {
      ...
      classificationStatus: "classified",
      classificationReason: "Service/fee group (membership fee, equipment, etc.)",
    };
  }

  // ── 5. Мастер-классы ──────────────────────────────────────────────────────
  if (lo.includes("мастер") || lo.includes("workshop") || lo.includes("разовое")) {
    return {
  ...
  async (req, res): Promise<void> => {
    try {
      ...
      for (const g of groups) {
        try {
          const cls = g.name ? classifyGroup(g.name) : {
          ...
          const r = await pool.query<{ inserted: boolean }>(
            ...
            [
              g.crm_id, g.name,
              cls.educationalUnitType, cls.department, cls.attributionModel,
              cls.revenueModel, isActive, cls.classificationStatus, cls.classificationReason,
            ],
          );

          if (r.rows[0]?.inserted) created++; else updated++;
          counts[cls.educationalUnitType] = (counts[cls.educationalUnitType] ?? 0) + 1;
          if (cls.classificationStatus === "needs_review") needsReview++;
          if (examples.length < 10) {
            examples.push({
              crm_id: g.crm_id, name: g.name,
              educational_unit_type: cls.educationalUnitType,
              department: cls.department, attribution_model: cls.attributionModel,
            });
          }
        } catch (rowErr) {
          errors++;
          logger.error({ rowErr, crm_id: g.crm_id }, "educational-units: classify row failed");
        }
      }
      ...
      logger.error({ err }, "educational-units/classify-from-crm-groups: failed");
  ...
  async (req, res): Promise<void> => {
    try {
      ...
      const result = rows.rows.map(r => ({
        ...
        warnings: [
          r.educational_unit_type === "kindergarten_group" || r.educational_unit_type === "school_class"
  ...
  async (req, res): Promise<void> => {
    try {
      ...
      res.json({
        ...
        individual: {
          total_ungrouped_lessons: indivRows.rows.reduce((s, r) => s + Number(r.lessons_count), 0),
...
educationalModuleRouter.post(
  ...
  async (req, res): Promise<void> => {
    try {
      ...
      const currentTypes = await pool.query<{ id: string; educational_unit_type: string }>(
        `SELECT id, educational_unit_type FROM educational_units`,
      );
      const typeByIdMap = new Map(currentTypes.rows.map(r => [r.id, r.educational_unit_type]));
      ...
      for (const row of existing.rows) {
        ...
        try {
          ...
          await pool.query(
            ...
            [
              cls.educationalUnitType, cls.department, cls.attributionModel,
              cls.revenueModel, cls.classificationStatus, cls.classificationReason,
              row.id,
            ],
          );
      ...
      const byType: Record<string, Array<{ crm_group_id: string; name: string; classification_status: string }>> = {};
      for (const r of auditResult.rows) {
        if (!byType[r.educational_unit_type]) byType[r.educational_unit_type] = [];
      ...
      const verifyNames = ["Китайский язык", "Английский 5-6", "Легкая атлетика 1-2 класс", "Лёгкая атлетика 1-2 класс"];
...
educationalModuleRouter.patch(
  ...
  async (req, res): Promise<void> => {
    try {
      ...
      const setClauses: string[] = ["classification_status = 'manual_override'", "updated_at = NOW()"];
      ...
      const result = await pool.query<{
        id: string; crm_group_id: string; name: string;
        educational_unit_type: string; department: string;
        attribution_model: string; revenue_model: string;
        classification_status: string; classification_reason: string;
      }>(
        `UPDATE educational_units
         SET ${setClauses.join(", ")}
         WHERE id = ${idParam}
      ...
      if (result.rows.length === 0) {
        res.status(404).json({ success: false, message: "Educational unit not found" });
        return;
      }

      const updated = result.rows[0]!;
      logger.info({ id, changes: body }, "educational-unit: manual override applied");
...
educationalModuleRouter.get("/educational-units/responsibility-audit", async (req, res) => {
  try {
    const BRANCH_ID = "6";
    ...
    for (const r of statsResult.rows) {
      if (!byUnit.has(r.educational_unit_id)) byUnit.set(r.educational_unit_id, []);
    ...
    for (const unit of allUnitsResult.rows) {
      ...
      if (!primary) {
        ...
        if (second) {
          ...
          if (gap <= REVIEW_GAP_PCT && second.coverage_pct >= 20) needsReview = true;
        }
      }

      const totalLessons = candidates.reduce((s, c) => s + c.lessons_taught, 0);
    ...
    const summary = {
      ...
      kindergarten:          results.filter(u => u.educational_unit_type === "kindergarten_group").length,
      school:                results.filter(u => u.educational_unit_type === "school_class").length,
      club:                  results.filter(u => u.educational_unit_type.startsWith("club")).length,
      other:                 results.filter(u =>
                               u.educational_unit_type !== "kindergarten_group" &&
                               u.educational_unit_type !== "school_class" &&
                               !u.educational_unit_type.startsWith("club")).length,
      with_high_confidence:   results.filter(u => u.responsibility_confidence === "high").length,
      with_medium_confidence: results.filter(u => u.responsibility_confidence === "medium").length,
      with_low_confidence:    results.filter(u => u.responsibility_confidence === "low").length,
      needs_review:           results.filter(u => u.needs_review).length,
      no_data:                results.filter(u => u.responsibility_confidence === "no_data").length,
    };
    ...
    const empAttrResult = await pool.query<{ id: string; attribution_model: string | null }>(
      `SELECT id, attribution_model FROM employees WHERE exclude_from_staff_analytics = false`,
    );
    const empAttrMap = new Map(empAttrResult.rows.map(r => [r.id, r.attribution_model]));
    ...
    for (const unit of results) {
      ...
      if (unit.primary_responsible_employee_id) {
        ...
        es.primary_units.push({ id: unit.educational_unit_id, name: unit.name, type: unit.educational_unit_type, confidence: unit.responsibility_confidence });
      }
      ...
      for (const cand of unit.all_candidates.slice(1)) {
        ...
        if (unit.primary_responsible_employee_id !== eid) {
          ...
          es.secondary_units.push({ id: unit.educational_unit_id, name: unit.name, type: unit.educational_unit_type });
        }
      }
    }

    const employeeSummary = Array.from(empSummaryMap.values())
      .sort((a, b) => b.primary_units_count - a.primary_units_count);
    ...
    const message = err instanceof Error ? err.message : String(err);
    logger.error({ err }, "GET /educational-units/responsibility-audit failed");
    res.status(500).json({ success: false, message });
  }
});
