...
function parseTinkoff(lines: string[]): ParsedRow[] {
  ...
  const dateIdx = header.findIndex((h) => h.includes("Дата операции") || h.includes("Дата платежа"));
  const amtIdx = header.findIndex((h) => h.includes("Сумма платежа") || h.includes("Сумма операции"));
  const descIdx = header.findIndex((h) => h.includes("Описание") || h.includes("Категория"));
...
function parseSberbank(lines: string[]): ParsedRow[] {
  ...
  const descIdx = header.findIndex((h) => h.includes("Описание") || h.includes("Назначение"));
  const amtIdx = header.findIndex((h) => h.includes("Сумма"));
  const accIdx = header.findIndex((h) => h.includes("Счёт") || h.includes("Счет"));
...
function parseGenericCsv(lines: string[]): ParsedRow[] {
  ...
  const descIdx = find("описание", "назначение", "description", "purpose", "комментарий");
  const counterpartyIdx = find("контрагент", "counterparty", "получатель", "плательщик");
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (dirIdx >= 0) {
      ...
      if (d.includes("расход") || d.includes("out") || d.includes("expense")) direction = "expense";
      if (d.includes("приход") || d.includes("in") || d.includes("income")) direction = "income";
    }
    ...
    rows.push({
      ...
      counterpartyName: counterpartyIdx >= 0 ? cols[counterpartyIdx] : undefined,
    });
  }
  return rows;
}
...
function detectFormat(text: string): "tinkoff" | "sberbank" | "generic" {
  ...
  if (firstLine.includes("кэшбэк") || firstLine.includes("mcc") || firstLine.includes("бонус")) return "tinkoff";
  if (firstLine.includes("счёт") || firstLine.includes("счет")) return "sberbank";
  return "generic";
}

function parseStatement(rawText: string, bankName: string): { rows: ParsedRow[]; format: string } {
  ...
  if (hint.includes("tinkoff") || hint.includes("тинькофф") || hint.includes("тинк")) {
    format = "tinkoff";
  } else if (hint.includes("sberbank") || hint.includes("сбер") || hint.includes("sbr")) {
    format = "sberbank";
  } else {
  ...
  const rows =
    format === "tinkoff" ? parseTinkoff(lines)
    : format === "sberbank" ? parseSberbank(lines)
...
reconciliationRouter.post("/reconciliation/import", async (req, res) => {
  ...
  if (rows.length === 0) { res.status(400).json({ error: "No rows parsed — check format or data" }); return; }
  ...
  try {
    ...
    // Compute date range
    const dates = rows.map((r) => r.operationDate).filter(Boolean).sort();
    if (dates.length > 0) {
    ...
    for (const row of rows) {
      ...
      try {
        // Check existing hash
        const existing = await db.select({ id: bankTransactionsTable.id })
...
reconciliationRouter.get("/reconciliation/batches", async (req, res) => {
  try {
    const batches = await db.select().from(bankImportBatchesTable).orderBy(desc(bankImportBatchesTable.importedAt));
    res.json(batches);
  } catch (err) {
...
reconciliationRouter.delete("/reconciliation/batches/:id", async (req, res) => {
  try {
    const txs = await db.select({ id: bankTransactionsTable.id })
      .from(bankTransactionsTable)
    ...
    if (txIds.length > 0) {
      const result = await db.delete(bankTransactionsTable)
        .where(inArray(bankTransactionsTable.id, txIds))
        .returning({ id: bankTransactionsTable.id });
      deleted = result.length;
    }

    await db.delete(bankImportBatchesTable).where(eq(bankImportBatchesTable.id, req.params.id));
    res.json({ deleted });
  } catch (err) {
...
reconciliationRouter.get("/reconciliation/transactions", async (req, res) => {
  ...
  try {
    ...
    const matchedOps = matchedIds.length > 0
      ? await db.select({
          id: operations.id,
        ...
        }).from(operations).where(inArray(operations.id, matchedIds))
      : [];
    ...
    const enriched = txs.map((t) => ({
      ...t,
      matchedOperation: t.matchedOperationId ? (opMap.get(t.matchedOperationId) ?? null) : null,
    }));
...
reconciliationRouter.post("/reconciliation/auto-match", async (req, res) => {
  ...
  try {
    ...
    if (month) {
      const [y, m] = month.split("-").map(Number);
      conditions.push(gte(bankTransactionsTable.operationDate, `${y}-${String(m).padStart(2, "0")}-01`));
      conditions.push(lte(bankTransactionsTable.operationDate, `${y}-${String(m).padStart(2, "0")}-31`));
    }

    const unmatchedTxs = await db.select().from(bankTransactionsTable).where(and(...conditions));
    ...
    for (const tx of unmatchedTxs) {
      if (tx.matchStatus === "matched") { alreadyMatched++; continue; }
      if (!tx.operationDate || !tx.amount) { noMatch++; continue; }
...
reconciliationRouter.post("/reconciliation/match", async (req, res) => {
  const { bankTxId, operationId } = req.body as { bankTxId: string; operationId: string };
  if (!bankTxId || !operationId) { res.status(400).json({ error: "bankTxId and operationId required" }); return; }
...
reconciliationRouter.delete("/reconciliation/match/:bankTxId", async (req, res) => {
  try {
...
reconciliationRouter.post("/reconciliation/ignore/:bankTxId", async (req, res) => {
  try {
    await db.update(bankTransactionsTable).set({
      isIgnored: true,
      matchStatus: "ignored",
...
reconciliationRouter.post("/reconciliation/create-operation/:bankTxId", async (req, res) => {
  const { articleId, notes } = req.body as { articleId?: string; notes?: string };
  ...
  try {
    ...
    if (articleId) {
      const [art] = await db.select({ name: articles.name, code: articles.code })
        .from(articles).where(eq(articles.id, articleId));
      if (art) { articleName = art.name; articleCode = art.code; }
    }
