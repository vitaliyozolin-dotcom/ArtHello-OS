...
type OwnerAuth = { tokenHash: string; session: { userId: string; role: AuthRole } };
...
function accessFromRow(row: AccessRow | undefined): ExistingEmployeeAccess | null {
  if (!row) return null;
  ...
  return { id: row.id, employeeId: row.employee_id, login: row.login, role: row.role, active: row.is_active, scope };
}
...
const planInputSchema = z.object({
  ...
  legalEntityIds: z.array(z.string().trim().min(1).max(128)).max(100).optional(),
});
...
async function loadPlanRecords(employeeId: string) {
  ...
  return { employee: employeeFromRow(employeeRow), access: accessFromRow(accessResult.rows[0]) };
}
  ...
  try {
    ...
    const employees = result.rows.map((row) => {
      const access = row.access_id
        ? accessFromRow({
            id: row.access_id, employee_id: row.employee_id,
            login: row.login ?? "", role: row.role as AuthRole,
            scope_mode: row.scope_mode ?? "restricted", branch_ids: row.branch_ids,
            legal_entity_ids: row.legal_entity_ids, is_active: row.is_active === true,
          })
        : null;
      return summarizePersonnelReadiness(employeeFromRow(row), access);
    });
    res.json({
      meta: { sourceOfTruth: "employees + auth_users", applyRequiresOwnerConfirmation: true, invitationSendEnabled: false },
      summary: {
        employeesChecked: employees.length,
        activeAccess: employees.filter((item) => item.status === "ACCESS_ACTIVE").length,
        readyForProvisioningInput: employees.filter((item) => item.status === "READY_FOR_PROVISIONING_INPUT").length,
        blocked: employees.filter((item) => item.status === "BLOCKED_PERSONNEL_DATA").length,
      },
      employees,
    });
  } catch (err) {
...
peopleAccessRouter.post("/people-access/plan", async (req, res) => {
  ...
  try {
    ...
    if (!records) { res.status(404).json({ error: "Сотрудник не найден" }); return; }
    const { employeeId: _employeeId, ...input } = parsed.data;
    const plan = buildPeopleAccessPlan(records.employee, records.access, input as PeopleAccessPlanInput);
    res.json(plan);
  } catch (err) {
...
peopleAccessRouter.post("/people-access/apply", async (req, res) => {
  const auth = ownerAuth(res); if (!auth) return;
  ...
  try {
    ...
    const employeeRow = employeeResult.rows[0];
    if (!employeeRow) { await client.query("ROLLBACK"); res.status(404).json({ error: "Сотрудник не найден" }); return; }
    ...
    const { employeeId: _employeeId, planHash: _planHash, confirmation: _confirmation, temporaryPassword, ...planInput } = parsed.data;
    const employee = employeeFromRow(employeeRow);
    const plan = buildPeopleAccessPlan(employee, existing, planInput as PeopleAccessPlanInput);
    if (!plan.ready) { await client.query("ROLLBACK"); res.status(409).json({ error: "План доступа заблокирован", blockers: plan.blockers }); return; }
    if (plan.planHash !== parsed.data.planHash) { await client.query("ROLLBACK"); res.status(409).json({ error: "План изменился. Постройте его заново.", code: "PLAN_HASH_MISMATCH" }); return; }

    let accessId: string;
    if (plan.action === "PROVISION") {
      if (!temporaryPassword) { await client.query("ROLLBACK"); res.status(400).json({ error: "Нужен временный пароль" }); return; }
      const passwordError = validatePasswordPolicy(temporaryPassword);
      if (passwordError) { await client.query("ROLLBACK"); res.status(400).json({ error: passwordError }); return; }
      const inserted = await client.query<{ id: string }>(
        ...
        [employee.id, plan.target.login, normalizeAccessLogin(plan.target.login ?? undefined), await hashPassword(temporaryPassword),
         plan.target.role, plan.target.displayName, JSON.stringify(plan.target.scope.branchIds),
         JSON.stringify(plan.target.scope.legalEntityIds), auth.session.userId],
      );
      ...
      await client.query(
        `UPDATE auth_users SET login=$2, login_normalized=$3, role=$4,
         ...
         WHERE id=$1`,
        [accessId, plan.target.login, normalizeAccessLogin(plan.target.login ?? undefined), plan.target.role,
         plan.target.displayName, JSON.stringify(plan.target.scope.branchIds), JSON.stringify(plan.target.scope.legalEntityIds)],
      );
      await client.query(`UPDATE auth_sessions SET revoked_at=NOW() WHERE user_id=$1 AND revoked_at IS NULL`, [accessId]);
    } else {
      accessId = existing!.id;
      await client.query(`UPDATE auth_users SET is_active=FALSE, updated_at=NOW() WHERE id=$1`, [accessId]);
      await client.query(`UPDATE auth_sessions SET revoked_at=NOW() WHERE user_id=$1 AND revoked_at IS NULL`, [accessId]);
    }
    ...
    await client.query(
      ...
      [employee.id, accessId, plan.action, plan.planHash, auth.session.userId,
       JSON.stringify({ effects: plan.effects, targetRole: plan.target.role, branchCount: plan.target.scope.branchIds.length, legalEntityCount: plan.target.scope.legalEntityIds.length })],
    );
    await client.query("COMMIT");
    res.json({ ok: true, action: plan.action, employeeId: employee.id, accessId, sessionsRevoked: plan.action !== "PROVISION", invitationSent: false });
  } catch (err) {
    await client.query("ROLLBACK").catch(() => {});
    if ((err as { code?: string }).code === "23505") { res.status(409).json({ error: "Логин или связь с сотрудником уже существуют" }); return; }
