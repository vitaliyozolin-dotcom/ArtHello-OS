...
timelineRouter.post("/identity/build-timeline", async (_req, res): Promise<void> => {
  ...
  const dbg = {
    payments: { total_in_crm: 0, linked_primary: 0, linked_via_phone: 0, no_customer_id: 0, orphan_not_synced: 0, skipped_no_date: 0 },
    attendance: { total_in_crm: 0, linked_to_family: 0, note: null as string | null },
  };
  ...
  for (const row of payments.rows) {
    ...
    // Pick best linkage
    const familyId         = (row["family_id_primary"] ?? row["family_id_phone"]) as string | null;
    const guardianPersonId = (row["guardian_id_primary"] ?? row["guardian_id_phone"]) as string | null;
    const studentPersonId  = (row["student_person_id_primary"] ?? row["student_person_id_phone"]) as string | null;
    const studentName      = (row["student_name_primary"] ?? row["student_name_phone"]) as string | null;
    const linkedViaPhone   = !row["family_id_primary"] && !!row["family_id_phone"];
    ...
    await db.insert(timelineEventsTable).values({
      ...
      eventType:       isAdjustment ? "payment_adjustment"
                     : isNoAmount   ? "payment_overdue"
                     :                "payment_received",
      ...
      raw:             { student_crm_id: studentCrmId, student_name: studentName, amount, link_method: linkedViaPhone ? "phone" : familyId ? "profile" : "unlinked" },
    }).onConflictDoNothing();
    paymentsAdded++;
  }
  ...
  for (const row of students.rows) {
    ...
    if (dob && dob !== "") {
      ...
      const dobDate = parts ? new Date(`${parts[0]}-${parts[1]}-${parts[2]}`) : new Date(dob);
      if (!isNaN(dobDate.getTime())) {
        const ageYears = Math.floor((Date.now() - dobDate.getTime()) / (365.25 * 86_400_000));
        if (ageYears > 0 && ageYears < 30) ageStr = `, ${ageYears} лет`;
      }
    }

    const statusLabel = studyStatusLabel[studyStatusId ?? ""] ?? (isStudy === "1" ? "активный" : "неактивный");
    ...
    if (balanceNum !== 0) descParts.push(`Баланс: ${balanceNum.toLocaleString("ru-RU")} ₽`);
    if (balanceBaseNum !== 0 && balanceBaseNum !== balanceNum) descParts.push(`Базовый баланс: ${balanceBaseNum.toLocaleString("ru-RU")} ₽`);
    if (paidLessonsNum > 0) descParts.push(`Уроков оплачено: ${paidLessonsNum}`);
    if (paidTill) descParts.push(`Оплачено до: ${paidTill}`);
    if (lastAttend) descParts.push(`Последнее занятие: ${lastAttend}`);
    if (nextLesson) descParts.push(`Следующее занятие: ${nextLesson.slice(0, 10)}`);
    ...
    await db.insert(timelineEventsTable).values({
      ...
      severity: "info",
      raw: {
        student_crm_id: crmId, full_name: fullName, dob, balance, balance_base: balanceBase,
        paid_lesson_count: paidLessons, paid_till: paidTill, study_status_id: studyStatusId,
        last_attend_date: lastAttend, next_lesson_date: nextLesson, is_study: isStudy,
      },
    }).onConflictDoNothing();
    subscriptionsAdded++;
  }
  ...
  try {
    if (subjectMapSetting.rows[0]?.["value"]) {
      subjectMap = JSON.parse(subjectMapSetting.rows[0]["value"] as string) as Record<string, string>;
    }
  ...
  if (attendance.rows.length === 0) {
    dbg.attendance.note = "Посещения не извлечены. Нажмите «Извлечь посещения из уроков» в разделе CRM Sync, затем пересоберите историю.";
  }
  ...
  for (const row of attendance.rows) {
    ...
    await db.insert(timelineEventsTable).values({
      ...
      severity,
      raw: {
        status, student_crm_id: studentCrmId, student_name: studentName,
        subject_id: subjectId, subject_name: subjectName,
        time_from: timeFromRaw, time_to: timeToRaw, reason_name: reasonName,
      },
    }).onConflictDoNothing();
    attendanceAdded++;
  }
  ...
  for (const row of leads.rows) {
    ...
    await db.insert(timelineEventsTable).values({
      ...
      description: [channel ? `Канал: ${channel}` : null, source ? `Источник: ${source}` : null, status ? `Статус: ${status}` : null]
  ...
  const totalEventsRow = await db.execute(sql`SELECT COUNT(*) AS cnt FROM timeline_events`);
  const totalEvents = parseInt(totalEventsRow.rows[0]?.["cnt"] as string ?? "0", 10);
...
export async function calcAllFamilyHealth(): Promise<{ familiesCalculated: number; alertsCreated: number }> {
  ...
  for (const family of families) {
    ...
    const paymentCount = parseInt(payRow?.["payment_count"] as string ?? "0", 10);
    ...
    `);
    const lastAttendanceStr = lastAttendanceRow.rows[0]?.["last_attendance"] as string | null;
    const lastAttendanceAt = lastAttendanceStr ? new Date(lastAttendanceStr) : null;

    // ── Score calculation ─────────────────────────────────────────────────
    let paymentScore = 0;
    if (paymentCount > 0) {
      if (daysSincePayment !== null && daysSincePayment <= 30) paymentScore = 40;
      else if (daysSincePayment !== null && daysSincePayment <= 60) paymentScore = 30;
      else if (daysSincePayment !== null && daysSincePayment <= 90) paymentScore = 20;
    ...
    const engagementScore = activeStudents > 0 ? 20 : (activeStudents + inactiveStudents > 0 ? 5 : 0);
    const activeBonus = activeStudents > 0 && inactiveStudents === 0 ? 10
      : activeStudents > 0 ? 6
      : 0;

    const healthScore = Math.min(100, paymentScore + attendanceScore + engagementScore + activeBonus);
    const churnRiskScore = parseFloat((1 - healthScore / 100).toFixed(3));

    await db.insert(familyHealthTable).values({
      familyId: family.id,
    ...
    if (paymentCount === 0 && activeStudents > 0) {
      alerts.push({ alertType: "overdue_payment", severity: "critical", message: "Нет ни одного платежа для активных учеников" });
    } else if (daysSincePayment !== null && daysSincePayment > 60 && activeStudents > 0) {
      alerts.push({ alertType: "overdue_payment", severity: "warning", message: `Последний платёж ${daysSincePayment} дней назад` });
    }

    if (activeStudents === 0 && inactiveStudents > 0) {
      alerts.push({ alertType: "inactive_family", severity: "warning", message: "Все ученики неактивны" });
    }

    if (total30 > 0 && missed30 / total30 > 0.4) {
      alerts.push({ alertType: "attendance_drop", severity: "warning", message: `Пропущено ${missed30} из ${total30} уроков за 30 дней` });
    }

    if (healthScore < 40) {
      alerts.push({ alertType: "churn_risk", severity: "critical", message: `Высокий риск оттока (score: ${healthScore})` });
    } else if (healthScore < 60) {
      alerts.push({ alertType: "churn_risk", severity: "warning", message: `Риск оттока (score: ${healthScore})` });
    }
...
timelineRouter.post("/identity/calc-health", async (_req, res): Promise<void> => {
...
function buildAiSummary(p: {
  ...
  if (p.paymentCount > 0) {
    ...
    if (p.lastPaymentAt) {
      const days = Math.floor((Date.now() - p.lastPaymentAt.getTime()) / 86_400_000);
  ...
  if (p.healthScore !== null) {
    ...
    else if (p.healthScore >= 25) parts.push("Рекомендуется связаться с семьёй.");
...
timelineRouter.get("/identity/families/:id/stats", async (req, res): Promise<void> => {
  const { id } = req.params;
  ...
  const lastPaymentAt = pay["last_payment_at"] ? new Date(pay["last_payment_at"] as string) : null;
  const lastPaymentAmount = lastPayRow.rows[0]?.["amount"] as string | null ?? null;

  const totalStudents = parseInt(stu["total_students"] as string ?? "0", 10);
  const activeStudents = parseInt(stu["active_students"] as string ?? "0", 10);
  const firstSeenAt = stu["first_seen_at"] ? new Date(stu["first_seen_at"] as string) : null;
  const daysWithUs = firstSeenAt
    ? Math.floor((Date.now() - firstSeenAt.getTime()) / 86_400_000)
    : null;

  const attendedTotal = parseInt(att["attended_total"] as string ?? "0", 10);
  const missedTotal = parseInt(att["missed_total"] as string ?? "0", 10);
  const lastAttendanceAt = att["last_attendance_at"] as string | null ?? null;

  const totalEvents = parseInt(evtRow.rows[0]?.["cnt"] as string ?? "0", 10);
  ...
  res.json({
    ...
    health_score: healthScore !== null ? String(Math.round(healthScore)) : null,
...
timelineRouter.get("/identity/families/:id/timeline", async (req, res): Promise<void> => {
  const { id } = req.params;
...
timelineRouter.get("/identity/families/:id/health", async (req, res): Promise<void> => {
  const { id } = req.params;
  ...
  if (healthRows.rows.length === 0) {
    res.status(404).json({ error: "Health data not found — run calc-health first" });
    return;
  }
...
timelineRouter.get("/identity/timeline-stats", async (_req, res): Promise<void> => {
