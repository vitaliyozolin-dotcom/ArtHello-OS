...
bankingAnalyticsRouter.get("/banking/analytics", async (req, res) => {
  try {
    ...
    const expenseRows = await db
      ...
      .where(
        and(
          ...
          gte(
            sql<string>`to_char(${bankTransactionsTable.operationDate}, 'YYYY-MM')`,
            threeMonthsAgo,
          ),
        ),
      )
      ...
      .orderBy(
        desc(sql`to_char(${bankTransactionsTable.operationDate}, 'YYYY-MM')`),
      )
      .limit(6);

    const recentExpenseMonths = expenseRows.slice(0, 3);
    const avgBurnRate =
      recentExpenseMonths.length > 0
        ? recentExpenseMonths.reduce(
            (s, r) => s + parseFloat(String(r.expenses ?? "0")),
            0,
          ) / recentExpenseMonths.length
        : null;
    ...
    const avgIncome =
      incomeRows.length > 0
        ? incomeRows.reduce(
            (s, r) => s + parseFloat(String(r.income ?? "0")),
            0,
          ) / incomeRows.length
        : null;
    ...
    const runway =
      avgBurnRate != null && avgBurnRate > 0 ? bankBalance / avgBurnRate : null;
    const safeBalance = avgBurnRate != null ? avgBurnRate * 2 : null;
    const isBelowSafe = safeBalance != null ? bankBalance < safeBalance : null;
    const safeBalanceGap =
      safeBalance != null ? Math.max(0, safeBalance - bankBalance) : null;
    ...
    for (const r of expenseRows) {
      if (!r.month) continue;
      if (!trendMap[r.month])
        trendMap[r.month] = { month: r.month, income: 0, expense: 0 };
