...
import {
  ...
  sectionForPath,
  type OwnerSection,
} from "@/features/navigation/section-routes";
...
const PulsePageComponent = lazy(() =>
  import("@/pages/pulse").then((m) => ({ default: m.PulsePage })),
);
const BankingPageComponent = lazy(() =>
  import("@/pages/banking").then((m) => ({ default: m.BankingPage })),
);
const BankIntegrationsPageComponent = lazy(() =>
  import("@/pages/banking").then((m) => ({ default: m.BankIntegrationsPage })),
);
const IdentityFamilies = lazy(() =>
  import("@/pages/identity-families").then((m) => ({ default: m.FamiliesTab })),
);
const LedgerPageComponent = lazy(() => import("@/pages/ledger"));
const ReconciliationPageComponent = lazy(
...
const EmployeesPageComponent = lazy(() =>
  import("@/pages/employees").then((m) => ({ default: m.EmployeesPage })),
);
const FrontOfficePageComponent = lazy(() =>
...
const NAV_GROUPS: { label: string; items: NavItem[] }[] = [
  {
    label: "",
    items: [{ key: "pulse", label: "Пульс", Icon: Zap }],
  },
  {
    label: "Финансы",
    items: [
...
function BankingAnalyticsPanel() {
  ...
  const fmt = (n: number) => formatRubles(n);
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
      <div className="bg-white rounded-[18px] border border-black/[0.06] shadow-[0_1px_4px_rgba(0,0,0,0.05)] p-4">
        ...
        <p
          className={`text-lg font-bold ${data.isBelowSafe ? "text-red-500" : "text-blue-600"}`}
        >
      ...
      <div className="bg-white rounded-[18px] border border-black/[0.06] shadow-[0_1px_4px_rgba(0,0,0,0.05)] p-4">
        ...
        <p
          className={`text-lg font-bold ${runwayOk ? "text-emerald-600" : runwayWarn ? "text-amber-600" : "text-red-500"}`}
        >
      ...
      <div
        className={`rounded-[18px] border shadow-[0_1px_4px_rgba(0,0,0,0.05)] p-4 ${data.isBelowSafe ? "bg-red-50 border-red-200" : "bg-white border-black/[0.06]"}`}
      >
        <div className="flex items-center gap-2 mb-1">
          <ShieldAlert
            className={`w-3.5 h-3.5 ${data.isBelowSafe ? "text-red-500" : "text-emerald-500"}`}
          />
          <p className="text-[10px] text-gray-400 font-medium">
            Безопасный остаток
          </p>
        ...
        <p
          className={`text-lg font-bold ${data.isBelowSafe ? "text-red-600" : "text-emerald-600"}`}
        >
          {fmt(data.safeBalance)}
        </p>
  ...
  return (
    <div className="flex flex-col h-full bg-white border-r border-gray-100">
      ...
      <nav className="flex-1 overflow-y-auto px-2.5 py-3 space-y-3">
        {NAV_GROUPS.filter(
          (group) =>
            group.label !== "Продажи и сервис" ||
            canViewFrontOfficePreview(user?.role),
        ).map((group) => (
      ...
      <div className="px-2.5 py-3 border-t border-gray-50 space-y-0.5">
        ...
        {user?.role === "owner" && (
          <a
            href="https://pay-188-225-38-55.sslip.io"
            className="w-full flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg text-[13px] font-medium text-gray-500 hover:bg-violet-50 hover:text-violet-700 transition-colors"
          >
        ...
        >
          <MonitorCog
            className={`w-4 h-4 shrink-0 ${isTechnical ? "text-amber-600" : "text-gray-400"}`}
          />
          <span className="flex-1 text-left truncate">Техн. режим</span>
          <div
            className={`w-7 h-4 rounded-full transition-colors relative shrink-0 ${isTechnical ? "bg-amber-400" : "bg-gray-200"}`}
          >
            <div
              className={`w-3 h-3 bg-white rounded-full absolute top-0.5 transition-transform shadow-sm ${isTechnical ? "translate-x-3.5" : "translate-x-0.5"}`}
            />
          </div>
        </button>
      </div>
...
function MobileBottomNav({
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex items-center lg:hidden z-30"
...
function OwnerContent({ section }: { section: OwnerSection }) {
  switch (section) {
    case "pulse":
      return <PulsePageComponent />;
    case "front-office":
      return <FrontOfficePageComponent />;
    case "ledger":
      return <LedgerPageComponent />;
    case "reconciliation":
      return <ReconciliationPageComponent />;
    case "employees":
      return <EmployeesPageComponent />;
    case "staff":
      return <StaffPageComponent />;
    case "pnl":
      return <PnlPageComponent />;
    case "contractors":
      return <ContractorsPageComponent />;
    case "taxes":
      return <TaxesPageComponent />;
    case "test-data":
      return <TestDataPageComponent />;
    case "cfo":
      return <CfoPageComponent />;
    case "trust-score":
      return <TrustScorePageComponent />;
    case "month-closing":
      return <MonthClosingPageComponent />;
    case "finance-qa":
      return <FinanceQaPageComponent />;
    case "articles":
    ...
    case "educational":
      return <EducationalPageComponent />;
    case "schedule":
        ...
        <div className="px-6 lg:px-8 py-6 max-w-[1440px] mx-auto">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-400 mt-0.5">
              Группировка учеников по родителям
            </p>
          </div>
          <IdentityFamilies />
        </div>
    ...
    default:
      return (
        <ComingSoon
          label={NAV.find((n) => n.key === section)?.label ?? section}
        />
      );
  }
}
...
export function AppShell() {
  ...
  const setSection = (nextSection: OwnerSection) =>
  ...
  return (
    <div
      ...
      style={{ height: "100dvh", background: "#F7F8FB" }}
    >
      {/* Desktop sidebar */}
            ...
            {isTechnical ? (
              <div className="px-4 py-4 max-w-[1440px] mx-auto">
                <div className="mb-4 flex items-center gap-2 px-3 py-2 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-800">
                  ...
                  <span>
                    <strong>Технический режим</strong> — служебные данные,
                    интеграции, sync-логи.
                  </span>
                </div>
                <TechnicalDashboard />
              </div>
