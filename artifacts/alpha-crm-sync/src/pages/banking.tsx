...
import {
  Banknote, RefreshCw, Loader2, AlertTriangle, CheckCircle2,
  XCircle, Settings, Zap, TrendingUp, TrendingDown, Minus,
  ChevronRight, ChevronDown, Clock, Plug, Wifi, WifiOff, BarChart3,
  CircleDollarSign, Calendar, ArrowUpRight, ArrowDownRight, ArrowDownLeft,
  ShieldCheck, ShieldX, Activity, Key, Globe, Info,
  Copy, CopyCheck, ExternalLink, ArrowRight, CheckCheck,
  Lock, Unlock, Timer, AlertCircle, Terminal, Database, List,
  Search, SlidersHorizontal,
  Bell, Building2, UserPlus, Ban, Link2, Briefcase, FileText, Users, Server,
  Sparkles, ArrowLeftRight, ClipboardList, Plus, ThumbsUp, OctagonX, Wallet,
} from 'lucide-react';
...
function applyRules(tx: TxItem, rules: CategorizationRuleItem[]): SuggestionResult {
  ...
  for (const rule of rules) {
    ...
    const rawField = (() => {
      switch (rule.matchField) {
        case 'counterparty': return tx.counterpartyName ?? '';
    ...
    switch (rule.matchType) {
      case 'contains':    matches = field.includes(pattern);             break;
      case 'equals':      matches = field === pattern;                   break;
      case 'starts_with': matches = field.startsWith(pattern);           break;
      case 'regex':
        try { matches = new RegExp(rule.pattern, 'i').test(rawField); } catch { matches = false; }
        break;
    }
    if (matches) {
      return {
...
const fmtDate = (v: string | null | undefined) => {
  if (!v) return '—';
  return new Date(v).toLocaleString('ru', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' });
};

const BANK_META: Record<string, { label: string; color: string; bgColor: string; borderColor: string; authType: string; configFields: string[] }> = {
  tochka: {
    label: 'Точка',
...
function StatusBadge({ status }: { status: string | null | undefined }) {
  ...
  const cfg =
    status === 'active'            ? { cls: 'bg-green-100 text-green-800 border-green-200',  icon: <CheckCircle2 className="w-3 h-3" />, label: 'Активен' } :
    status === 'connected_partial' ? { cls: 'bg-amber-100 text-amber-800 border-amber-200',  icon: <Activity className="w-3 h-3" />,     label: 'Частично' } :
    status === 'error'             ? { cls: 'bg-red-100 text-red-800 border-red-200',        icon: <XCircle className="w-3 h-3" />,      label: 'Ошибка' } :
    status === 'not_configured'    ? { cls: 'bg-gray-100 text-gray-700 border-gray-200',     icon: <Settings className="w-3 h-3" />,     label: 'Не настроен' } :
                                     { cls: 'bg-gray-100 text-gray-600 border-gray-200',     icon: <Minus className="w-3 h-3" />,        label: 'Неактивен' };
  return (
    <Badge className={`${cfg.cls} flex items-center gap-1 px-2 py-0.5 text-[11px] border font-medium`}>
      {cfg.icon}{cfg.label}
    </Badge>
  );
}
...
const FIELD_HINTS: Record<string, Record<string, { placeholder: string; description?: string }>> = {
  tochka: {
    clientId:     { placeholder: 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx', description: 'OAuth 2.0 Client ID из личного кабинета Точки → Интеграции' },
    clientSecret: { placeholder: 'ваш client_secret', description: 'Client Secret. Хранится в зашифрованном виде.' },
  },
  tinkoff: {
    apiKey: { placeholder: 'ваш API ключ', description: 'API ключ из личного кабинета Т-Банк Бизнес' },
  },
  vtb: {
    clientId:     { placeholder: 'client_id', description: 'OAuth Client ID из ЛК ВТБ' },
    clientSecret: { placeholder: 'client_secret', description: 'OAuth Client Secret' },
  },
};
...
function ConfigDialog({ connector, bankName, open, onClose, onSaved }: ConfigDialogProps) {
  ...
  const handleSave = () => {
    ...
    if (connector) {
      update.mutate({
        id: connector.id,
        data: { config, connectorStatus: Object.keys(config).length > 0 ? 'inactive' : 'inactive' },
      });
    } else if (bankName) {
      create.mutate({
...
function DebugBlock({ debug, diagnosticCode, diagnosticMessage }: {
  ...
  const handleCopy = () => {
    const text = JSON.stringify({ diagnosticCode, diagnosticMessage, debug }, null, 2);
      ...
      {open && (
        <div className="bg-gray-900 text-gray-200 p-3 space-y-2">
          <div className="flex justify-end">
            <button className="text-gray-400 hover:text-white flex items-center gap-1" onClick={handleCopy}>
              {copied ? <CopyCheck className="w-3 h-3 text-green-400" /> : <Copy className="w-3 h-3" />}
              {copied ? 'Скопировано' : 'Копировать'}
            </button>
          </div>
          {diagnosticCode && (
            <div><span className="text-gray-500">code:</span> <span className="text-yellow-300">{diagnosticCode}</span></div>
          )}
          {debug?.tokenUrl && (
            <div><span className="text-gray-500">token_url:</span> <span className="text-blue-300 break-all">{debug.tokenUrl}</span></div>
          )}
          {debug?.clientId && (
            <div><span className="text-gray-500">client_id:</span> <span className="text-green-300">{debug.clientId}</span></div>
          )}
          {debug?.customerCode !== undefined && (
            <div>
              <span className="text-gray-500">customer_code: </span>
              <span className={debug.customerCode && debug.customerCode !== '(не указан — возможно нужен)' && debug.customerCode !== '(не получен)' ? 'text-green-300' : 'text-red-300'}>
                {debug.customerCode ?? '(не указан)'}
              </span>
            </div>
          )}
          {debug?.resolvedCustomerCode !== undefined && debug.resolvedCustomerCode !== debug.customerCode && (
            <div>
              <span className="text-gray-500">resolved_customer_code: </span>
              <span className={debug.resolvedCustomerCode && debug.resolvedCustomerCode !== '(не получен)' ? 'text-green-300' : 'text-red-400'}>
          ...
          {debug?.introspectSub !== undefined && (
            <div>
              ...
              <span className={debug.introspectSub && debug.introspectSub !== '(не получен)' ? 'text-cyan-300' : 'text-amber-400'}>
                {debug.introspectSub}
              </span>
            </div>
          )}
          {debug?.customerCodeSource !== undefined && (
            <div>
              ...
              <span className={debug.customerCodeSource === 'introspect' ? 'text-cyan-300' : debug.customerCodeSource === 'customers_api' ? 'text-yellow-300' : 'text-red-400'}>
          ...
          {debug?.sentCustomerHeader !== undefined && (
            <div>
              ...
              <span className={debug.sentCustomerHeader ? 'text-green-300' : 'text-red-400 font-bold'}>
                {String(debug.sentCustomerHeader)}
              </span>
              {!debug.sentCustomerHeader && <span className="text-red-400 ml-2">⚠ CustomerCode header не отправлен!</span>}
            </div>
          )}
          {debug?.customersResponse && (
            <div className="border-t border-gray-700 pt-2 mt-1">
              <div className="text-gray-500 text-[10px] uppercase tracking-wide mb-1">CUSTOMERS RESPONSE</div>
          ...
          {debug?.tokenExists !== undefined && (
            <div className="border-t border-gray-700 pt-2 mt-1 space-y-1">
              <div className="text-gray-500 text-[10px] uppercase tracking-wide mb-1">TOKEN VALIDATION</div>
              <div>
                <span className="text-gray-500">token_exists: </span>
                <span className={debug.tokenExists ? 'text-green-300' : 'text-red-400 font-bold'}>
                  {String(debug.tokenExists)}
                </span>
              </div>
              {debug?.tokenLength !== undefined && debug.tokenLength !== null && (
                <div>
                  <span className="text-gray-500">token_length: </span>
                  <span className={debug.tokenLength > 0 ? 'text-green-300' : 'text-red-400'}>
                    {debug.tokenLength}
                  </span>
                  {debug.tokenLength > 0 && debug.tokenLength < 20 && (
                    <span className="text-red-400 ml-2">⚠ слишком короткий</span>
                  )}
                </div>
              )}
              {debug?.tokenTypeOf && (
                <div>
                  ...
                  <span className={debug.tokenTypeOf === 'string' ? 'text-green-300' : 'text-red-400 font-bold'}>
                    {debug.tokenTypeOf}
                  </span>
                  {debug.tokenTypeOf !== 'string' && (
                    <span className="text-red-400 ml-2">⚠ должен быть string!</span>
                  )}
                </div>
              )}
              {debug?.tokenMasked && (
                <div>
                  ...
                  <span className="text-yellow-300 font-mono">{debug.tokenMasked}</span>
                </div>
              )}
            </div>
          )}
          ...
          {(debug?.requestUrl || debug?.accountsUrl) && (
            <div className="border-t border-gray-700 pt-2 mt-1 space-y-1">
              <div className="text-gray-500 text-[10px] uppercase tracking-wide mb-1">FINAL REQUEST</div>
              {debug?.accountsUrl && (
                <div><span className="text-gray-500">accounts_url:</span> <span className="text-blue-300 break-all">{debug.accountsUrl}</span></div>
              )}
              {debug?.requestUrl && debug.requestUrl !== debug.accountsUrl && (
                <div><span className="text-gray-500">request_url:</span> <span className="text-blue-300 break-all">{debug.requestUrl}</span></div>
              )}
              {debug?.sentHeaders && (
                <div>
                  <div className="text-gray-500 mb-1">sent_headers:</div>
                  <pre className="text-cyan-300 text-[10px] whitespace-pre-wrap">{debug.sentHeaders}</pre>
                </div>
              )}
            </div>
          )}
          {debug?.responseStatus !== undefined && debug?.responseStatus !== null && (
            <div className="border-t border-gray-700 pt-2 mt-1 space-y-1">
              <div className="text-gray-500 text-[10px] uppercase tracking-wide mb-1">RESPONSE</div>
              <div>
                <span className="text-gray-500">status: </span>
                <span className={debug.responseStatus >= 400 ? 'text-red-300 font-bold' : 'text-green-300'}>
...
function ConnectorHealthPanel({ connectorId, bankName }: { connectorId: string; bankName: string }) {
  const meta = BANK_META[bankName] ?? BANK_META['tochka'];
  const { data: health, isFetching, refetch } = useGetBankConnectorHealth(connectorId, {
    query: {
  ...
  const statusIcon = !health ? null :
    health.status === 'active' ? <ShieldCheck className="w-5 h-5 text-green-600" /> :
    health.status === 'not_configured' ? <Settings className="w-5 h-5 text-gray-400" /> :
      ...
      {health && !isFetching && (
        <div className="space-y-3">
          ...
          <div className={`flex items-start gap-3 p-3 rounded-lg border ${
            ...
            <div className="flex-1 min-w-0">
              <div className={`text-sm font-semibold ${
                health.status === 'active' ? 'text-green-800' :
                health.status === 'not_configured' ? 'text-gray-700' : 'text-red-800'
              }`}>
                {health.message}
              </div>
              {health.diagnosticMessage && health.diagnosticMessage !== health.message && (
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              { label: 'Авторизация (OAuth)', ok: health.authOk, icon: <Key className="w-3 h-3" /> },
              { label: 'Доступ к счетам', ok: health.accountsAccessOk, icon: <CircleDollarSign className="w-3 h-3" /> },
              { label: 'Scope: accounts', ok: health.hasAccountsScope, icon: <Globe className="w-3 h-3" /> },
              { label: 'Scope: transactions', ok: health.hasTransactionsScope, icon: <Globe className="w-3 h-3" /> },
            ].map(({ label, ok, icon }) => (
              <div key={label} className={`flex items-center gap-2 p-2 rounded-md border text-xs ${
          ...
          {health.tokenExpiresAt && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <span className="font-medium text-foreground">
                {new Date(health.tokenExpiresAt).toLocaleString('ru', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}
              </span>
            </div>
          )}
          ...
          {health.scopesGranted && health.scopesGranted.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {health.scopesGranted.map((s) => (
                <span key={s} className="text-[10px] px-1.5 py-0.5 rounded bg-blue-100 text-blue-700 font-mono">
                  {s}
                </span>
              ))}
            </div>
          ...
          {/* Account count */}
          {health.accountCount !== null && health.accountCount !== undefined && (
            <div className="text-xs text-muted-foreground flex items-center gap-1.5">
              <CircleDollarSign className="w-3 h-3" />
              Найдено счетов: <strong className="text-foreground">{health.accountCount}</strong>
            </div>
          )}
...
const OAUTH_STAGE_LABELS: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  not_configured:    { label: 'Не настроено',          color: 'text-gray-500',   icon: <Settings className="w-3.5 h-3.5" /> },
  service_token_ok:  { label: 'Токен сервиса получен', color: 'text-blue-600',   icon: <Key className="w-3.5 h-3.5" /> },
  consent_pending:   { label: 'Consent создан',         color: 'text-amber-600',  icon: <Timer className="w-3.5 h-3.5" /> },
  awaiting_callback: { label: 'Ожидаем подтверждения', color: 'text-amber-600',  icon: <Timer className="w-3.5 h-3.5 animate-pulse" /> },
  token_ok:          { label: 'Авторизован',            color: 'text-green-700',  icon: <CheckCheck className="w-3.5 h-3.5" /> },
};

const OAUTH_STEPS = ['service_token_ok', 'consent_pending', 'awaiting_callback', 'token_ok'];
...
function OAuthDebugBlock({ snap, show, onToggle }: {
  ...
  const lines: { key: string; value: string; ok?: boolean | null }[] = [
    ...
    },
    {
      ...
      value: snap.isServerUrlEnterTochka === undefined ? '—' : String(snap.isServerUrlEnterTochka),
      ok: snap.isServerUrlEnterTochka === true ? true : snap.isServerUrlEnterTochka === false ? false : null,
    },
    {
    ...
    },
    {
      ...
      value: snap.callbackReceived === undefined ? '—' : String(snap.callbackReceived),
      ok: snap.callbackReceived === true ? true : snap.callbackReceived === false ? false : null,
    },
    {
      key: 'code_received',
      value: snap.codeReceived === undefined ? '—' : snap.codeReceived ? `true (${snap.callbackCodePreview ?? '…'})` : 'false',
      ok: snap.codeReceived === true ? true : snap.codeReceived === false ? false : null,
    },
    {
      key: 'state_matches',
      value: snap.stateMatches === undefined ? '—' : `${String(snap.stateMatches)}${snap.callbackStateValue ? ` (${snap.callbackStateValue.slice(0, 12)}…)` : ''}`,
      ok: snap.stateMatches === true ? true : snap.stateMatches === false ? false : null,
    },
    {
      key: 'exchange_status',
      value: snap.exchangeStatus ?? '—',
      ok: snap.exchangeStatus === 'ok' ? true : snap.exchangeStatus === 'error' ? false : null,
    },
    {
    ...
    },
    {
      ...
      value: snap.hybridTokenSaved === undefined ? '—' : String(snap.hybridTokenSaved),
      ok: snap.hybridTokenSaved === true ? true : snap.hybridTokenSaved === false ? false : null,
    },
    {
      key: 'next_step',
      value: snap.nextStep ?? '—',
        ...
        <div className="mt-1.5 bg-gray-900 rounded-lg p-3 text-[10px] font-mono space-y-1 relative">
          ...
          {lines.map(({ key, value, ok }) => (
            <div key={key} className="flex gap-2 leading-relaxed">
              ...
              <span className={`break-all ${ok === true ? 'text-green-400' : ok === false ? 'text-red-400' : 'text-gray-300'}`}>
                {value}
              </span>
            </div>
          ))}
...
function OAuthFlowPanel({ connectorId, onDone }: { connectorId: string; onDone: () => void }) {
  ...
  const { data: status, refetch: refetchStatus } = useGetTochkaOAuthStatus(connectorId, {
    query: {
      queryKey: getGetTochkaOAuthStatusQueryKey(connectorId),
      refetchInterval: (query) => {
        const stage = query.state.data?.stage;
        return stage === 'awaiting_callback' || stage === 'consent_pending' ? 3000 : false;
      },
    },
  });
  ...
  const stageInfo = OAUTH_STAGE_LABELS[stage] ?? OAUTH_STAGE_LABELS['not_configured']!;
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const isEnterTochka = url.startsWith('https://enter.tochka.com/connect/authorize');
      setDebugSnap(prev => ({
        ...prev,
  ...
  const handleManualSubmit = async () => {
    ...
    setLocalError(null);
    setDebugSnap(prev => ({ ...prev, exchangeStatus: 'pending', exchangeResponseBody: undefined }));
    try {
      ...
      setDebugSnap(prev => ({
        ...
        nextStep: 'Токен получен и сохранён. CustomerCode определяется автоматически.',
      }));
      setLocalSuccess(true);
      await queryClient.invalidateQueries({ queryKey: getGetBankConnectorsQueryKey() });
      await refetchStatus();
  ...
  useEffect(() => {
    ...
    if (oauthResult || code) {
      setDebugSnap(prev => ({
        ...
        stateMatches: state ? (state === prev.serverGeneratedAuthorizeUrl?.match(/state=([^&]+)/)?.[1]) : undefined,
    ...
    if (oauthResult === 'success' && isOurConnector) {
      ...
      queryClient.invalidateQueries({ queryKey: getGetBankConnectorsQueryKey() });
      refetchStatus();
      setTimeout(onDone, 1500);
    }
    ...
    return (
      <div className="border border-green-200 rounded-xl p-4 mt-3 space-y-2.5 bg-green-50/50">
        <div className="flex items-center gap-2">
          ...
          <div className="text-sm font-semibold text-green-800">Авторизация завершена</div>
        </div>
        <div className="text-xs text-green-700 leading-relaxed">
          CustomerCode определён автоматически через introspect API и сохранён.
          Синхронизация счетов запущена в фоне — обновите страницу через несколько секунд.
        </div>
        <Button
          size="sm"
          variant="outline"
          className="gap-1.5 text-xs h-7 border-green-300 text-green-800 hover:bg-green-100"
          onClick={() => { refetchStatus(); onDone(); }}
        >
  ...
  return (
    <div className="border border-amber-200 rounded-xl p-4 mt-3 space-y-4 bg-amber-50/40">
          ...
          return (
            <div key={s} className="flex items-center flex-1 min-w-0">
              ...
              {i < OAUTH_STEPS.length - 1 && (
                <div className={`h-0.5 flex-1 mx-0.5 ${passed ? 'bg-green-400' : 'bg-gray-200'}`} />
              )}
            </div>
          );
        })}
      ...
      <div className={`flex items-center gap-2 text-xs font-medium ${stageInfo.color}`}>
        {stageInfo.icon}
        {stageInfo.label}
        {status?.consentId && (
          <span className="text-gray-400 font-mono ml-1">consent: {status.consentId.slice(0, 8)}…</span>
        )}
      </div>
      ...
      {(stage === 'not_configured' || stage === 'service_token_ok') && (
        <p className="text-xs text-muted-foreground leading-relaxed">
          Нажмите «Начать авторизацию» — система создаст consent в Точке и откроет страницу подтверждения в новой вкладке.
          После подтверждения вы будете перенаправлены обратно автоматически.
        </p>
      )}
      {(stage === 'consent_pending' || stage === 'awaiting_callback') && (
        <div className="flex items-start gap-2 text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-lg p-3">
          ...
          <div>
            Страница авторизации открыта в новой вкладке. Войдите в Точку и подтвердите доступ.
            После подтверждения страница обновится автоматически.
          </div>
        </div>
      )}
      ...
      {showManual && (
        <div className="space-y-2">
          <div className="text-xs text-muted-foreground">
            Если автоматический редирект не сработал, скопируйте code= из адресной строки:
          </div>
          <div className="flex gap-2">
            <Input
            ...
            >
              {submitMutation.isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : <ArrowRight className="w-3 h-3" />}
            </Button>
          </div>
        </div>
      )}
      ...
      <OAuthDebugBlock
        snap={{ ...debugSnap, hybridTokenSaved: stage === 'token_ok' || debugSnap.hybridTokenSaved }}
        show={showDebug}
...
function ConnectorCard({ bankName, connector, onConfigure, onSync, isSyncing, onDelete }: ConnectorCardProps) {
  ...
  return (
    <Card className={`border ${connector ? meta.borderColor : 'border-dashed border-gray-200'}`}>
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-md flex items-center justify-center text-sm font-bold ${meta.bgColor} ${meta.color}`}>
              {meta.label.slice(0, 1)}
            </div>
            <div>
              <div className="font-semibold text-sm">{meta.label}</div>
              <div className="text-xs text-muted-foreground">{meta.authType}</div>
            </div>
          </div>
        ...
        {connector ? (
          <div className="space-y-2 text-sm">
            ...
            {connector.totalBalance !== null && connector.totalBalance !== undefined && (
              <div className="flex justify-between font-medium">
                <span className="text-xs text-muted-foreground">Остаток</span>
                <span className={`text-sm ${(isActive || isPartial) ? 'text-green-700' : 'text-muted-foreground'}`}>
            ...
            {isPartial && (
              <div className="flex items-start gap-2 text-xs text-amber-700 bg-amber-50 rounded p-1.5 border border-amber-200">
                ...
                <span>Счета и остатки загружены. Операции пока не загружены: нужен endpoint выписок Tochka Statements.</span>
              </div>
            )}
            ...
            {showHealth && (
              <ConnectorHealthPanel connectorId={connector.id} bankName={bankName} />
            )}
...
function LiveCashPanel() {
  const { data: cashflow, isLoading } = useGetBankingCashflow({
    query: { queryKey: getGetBankingCashflowQueryKey(), refetchInterval: 60000 },
  });
    ...
    return (
      <Card className="border-dashed">
        <CardContent className="py-8 text-center">
          ...
          <p className="text-sm font-medium text-muted-foreground">Нет live данных</p>
          <p className="text-xs text-muted-foreground mt-1">
            Подключите хотя бы один банк и выполните sync чтобы увидеть live балансы
          </p>
        </CardContent>
      </Card>
    );
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="border-green-200 bg-green-50/50">
          <CardContent className="pt-4 pb-3">
            ...
            <div className="text-xl font-bold text-green-800">{fmt(cashflow.totalBalance)}</div>
            {cashflow.runwayDays !== null && cashflow.runwayDays !== undefined && (
        ...
        <Card>
          <CardContent className="pt-4 pb-3">
            <div className="text-xs text-muted-foreground mb-1 flex items-center gap-1">
              <ArrowUpRight className="w-3 h-3 text-green-600" /> Сегодня получено
            </div>
            <div className="text-xl font-bold text-green-700">{fmt(cashflow.todayIncome)}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-4 pb-3">
            <div className="text-xs text-muted-foreground mb-1 flex items-center gap-1">
              <ArrowDownRight className="w-3 h-3 text-red-600" /> Сегодня потрачено
            </div>
            <div className="text-xl font-bold text-red-700">{fmt(cashflow.todayExpense)}</div>
          </CardContent>
        </Card>
        ...
        <Card className={netPositive ? 'border-green-200' : 'border-red-200'}>
          <CardContent className="pt-4 pb-3">
            <div className="text-xs text-muted-foreground mb-1 flex items-center gap-1">
              {netPositive ? <TrendingUp className="w-3 h-3 text-green-600" /> : <TrendingDown className="w-3 h-3 text-red-600" />}
              Net за день
            </div>
            <div className={`text-xl font-bold ${netPositive ? 'text-green-700' : 'text-red-700'}`}>
              {netPositive ? '+' : ''}{fmt(cashflow.netToday)}
            </div>
            <div className="text-xs text-muted-foreground mt-1">
      ...
      {cashflow.byBank.length > 0 && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-muted-foreground">Остатки по банкам</CardTitle>
          </CardHeader>
          <CardContent className="pt-0">
            <div className="space-y-2">
              {cashflow.byBank.map((b, i) => {
                const meta = BANK_META[b.bankName ?? ''] ?? { label: b.displayName ?? b.bankName, color: 'text-gray-700', bgColor: 'bg-gray-100' };
                return (
                  <div key={i} className="flex items-center gap-3 py-1.5 border-b last:border-0">
                    <div className={`w-7 h-7 rounded text-xs font-bold flex items-center justify-center ${meta.bgColor} ${meta.color}`}>
                      {meta.label.slice(0, 1)}
                    </div>
                    <div className="flex-1">
                      <div className="text-sm font-medium">{b.displayName ?? b.bankName}</div>
                      <div className="text-xs text-muted-foreground">{b.accountCount} счёт(а)</div>
                    </div>
                    <div className="text-right">
                      {(b.connectorStatus === 'active' || b.connectorStatus === 'connected_partial') ? (
                        <div className="font-semibold text-sm">{fmt(b.totalBalance)}</div>
                      ) : (
                        <StatusBadge status={b.connectorStatus} />
                      )}
                      {b.lastSyncAt && (
                        <div className="text-[10px] text-muted-foreground">{fmtDate(b.lastSyncAt)}</div>
                      )}
                    </div>
                  </div>
                );
    ...
    <Card className="border-yellow-200">
      <CardHeader className="pb-2">
        <CardTitle className="text-sm flex items-center gap-2">
          ...
          Алерты
          <Badge className="bg-yellow-100 text-yellow-800 border-yellow-200 text-[10px]">{data.total}</Badge>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-2 pt-0">
        {data.alerts.map((a) => (
          ...
          >
            <AlertTriangle className={`w-3.5 h-3.5 mt-0.5 shrink-0 ${a.severity === 'critical' ? 'text-red-600' : 'text-yellow-600'}`} />
...
function ForecastPanel() {
  const { data } = useGetBankingForecast({
    query: { queryKey: getGetBankingForecastQueryKey(), refetchInterval: 300000 },
  });
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {/* Trend chip */}
        <div className={`flex items-center gap-2 text-sm font-medium ${data.burnRateTrend === 'positive' ? 'text-green-700' : 'text-red-700'}`}>
          {data.burnRateTrend === 'positive'
            ? <TrendingUp className="w-4 h-4" />
            : <TrendingDown className="w-4 h-4" />
          }
          Среднедневной net: {data.avgDailyNet >= 0 ? '+' : ''}{fmt(data.avgDailyNet)}/день
        </div>

        {/* Projected balances */}
        <div className="grid grid-cols-3 gap-3">
          {points.map(({ label, value }) => (
            <div key={label} className={`rounded-md p-3 border text-center ${isPositive(value) ? 'border-green-200 bg-green-50/50' : 'border-red-200 bg-red-50/50'}`}>
              <div className="text-[10px] text-muted-foreground mb-1">{label}</div>
              <div className={`text-sm font-bold ${isPositive(value) ? 'text-green-800' : 'text-red-800'}`}>
                {fmt(value)}
              </div>
            </div>
          ))}
        ...
        {data.cashGapDate && (
          <div className="flex items-center gap-2 p-2.5 rounded-md bg-red-50 border border-red-200 text-sm text-red-800">
            ...
            <span>
              Возможное обнуление баланса: <strong>{new Date(data.cashGapDate).toLocaleDateString('ru', { day: '2-digit', month: 'long' })}</strong>
            </span>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3 text-xs text-muted-foreground">
          <div>
            ...
            <span className="text-sm font-medium text-foreground">{fmt(data.monthlyObligations)}</span>
          </div>
          <div>
            <span className="block">Мин. безопасный остаток</span>
            <span className="text-sm font-medium text-foreground">{fmt(data.minimumSafeBalance)}</span>
          </div>
        </div>
      </CardContent>
    </Card>
...
function SyncHistory() {
  const { data: runs, isLoading } = useGetBankSyncRuns(
    { limit: 20 },
    { query: { queryKey: getGetBankSyncRunsQueryKey({ limit: 20 }), refetchInterval: 30000 } },
  );
  ...
  if (isLoading) return <div className="py-4 text-center text-sm text-muted-foreground"><Loader2 className="w-4 h-4 animate-spin inline mr-2" />Загрузка...</div>;
  if (!runs?.length) return <div className="py-4 text-center text-sm text-muted-foreground">Sync-запусков ещё не было</div>;
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="text-right w-16" title="Выписок запрошено / сохранено">Выписки</TableHead>
            <TableHead className="text-right w-16" title="Транзакций получено">Опер.</TableHead>
            <TableHead className="text-right w-16" title="Новых транзакций">Новых</TableHead>
            <TableHead className="text-right w-16" title="Дублей (пропущено)">Дубли</TableHead>
            <TableHead className="text-right w-16" title="Обновлено балансов">Балансы</TableHead>
            <TableHead className="text-right w-16" title="Длительность">Время</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {(runs ?? []).map((r) => (
            <TableRow key={r.id}>
              <TableCell className="text-sm font-medium">{r.displayName ?? r.bankName ?? '—'}</TableCell>
              <TableCell className="text-xs text-muted-foreground">{r.runType}</TableCell>
              <TableCell className="text-xs text-muted-foreground">{fmtDate(r.startedAt)}</TableCell>
              <TableCell>
                <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${statusCls[r.status ?? ''] ?? 'text-gray-600 bg-gray-100'}`}>
                  {r.status}
                </span>
                {r.errorMessage && (
                  <p className="text-[10px] text-red-600 mt-0.5 max-w-[160px] truncate" title={r.errorMessage}>{r.errorMessage}</p>
                )}
              </TableCell>
              <TableCell className="text-right text-xs text-muted-foreground">
                {r.statementsRequested != null && r.statementsRequested > 0
                  ? <span title={`Сохранено: ${r.statementsSaved ?? '?'}`}>{r.statementsRequested}</span>
                  : '—'}
              </TableCell>
              <TableCell className="text-right text-xs text-muted-foreground">{r.transactionsReceived ?? '—'}</TableCell>
              <TableCell className="text-right text-xs">
                {r.transactionsNew != null
                  ? <span className="text-green-700 font-medium">+{r.transactionsNew}</span>
                  : '—'}
              </TableCell>
              <TableCell className="text-right text-xs">
                {r.transactionsDuplicates != null
                  ? <span className="text-gray-500">{r.transactionsDuplicates}</span>
                  : '—'}
              </TableCell>
              <TableCell className="text-right text-xs">{r.balancesUpdated ?? '—'}</TableCell>
              <TableCell className="text-right text-xs text-muted-foreground font-mono">{fmtDur(r.durationMs)}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
...
function AccountsList() {
  ...
  if (isLoading) return (
    <div className="py-10 text-center text-[14px] text-gray-400">
      <Loader2 className="w-5 h-5 animate-spin inline mr-2" />Загрузка счетов…
    </div>
  );
  if (!accounts?.length) return (
    <div className="py-14 text-center">
      ...
      <p className="text-[13px] text-gray-400 mt-1">Подключите банк и запустите sync.</p>
    </div>
  );

  const total = accounts.reduce((s, a) => s + (a.currentBalance != null ? parseFloat(a.currentBalance) : 0), 0);

  return (
    <div className="space-y-3">
      {/* Summary row */}
      <div className="flex items-center justify-between px-1">
        <span className="text-[13px] text-gray-500 font-medium">{accounts.length} счёт{accounts.length !== 1 ? 'а' : ''}</span>
        <span className="text-[15px] font-bold text-gray-900">Итого: {fmt(total)}</span>
      </div>

      {/* Compact list */}
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        {accounts.map((a, i) => {
          const bal   = a.currentBalance   != null ? parseFloat(a.currentBalance)   : null;
          ...
          const meta = BANK_META[a.bankName ?? ''] ?? { label: a.displayName ?? a.bankName ?? '?', bgColor: 'bg-gray-100', color: 'text-gray-700', borderColor: 'border-gray-200' };
          const isEnabled   = accountStatus?.toLowerCase() === 'enabled';
          const balPositive = bal === null || bal >= 0;
          const displayMask = masked ?? (a.accountNumber ? `****${a.accountNumber.slice(-4)}` : '—');
          const syncTime = a.lastBalanceSyncAt
            ? new Date(a.lastBalanceSyncAt).toLocaleTimeString('ru', { hour: '2-digit', minute: '2-digit' })
            : null;
          ...
          return (
            <div
              ...
              style={{ minHeight: 78 }}
            >
              <div className={`w-9 h-9 rounded-xl text-[13px] font-bold flex items-center justify-center shrink-0 ${meta.bgColor} ${meta.color}`}>
                {meta.label.slice(0, 1)}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[14px] font-bold text-gray-900 font-mono leading-tight">{displayMask}</div>
                <div className="text-[12px] text-gray-400 leading-tight mt-0.5">
                  {meta.label} · {a.currency ?? 'RUB'}{syncTime ? ` · ${syncTime}` : ''}
                </div>
              </div>
              <div className="text-right shrink-0">
                <div className={`text-[18px] font-bold leading-tight tabular-nums ${balPositive ? 'text-gray-900' : 'text-red-600'}`}>
                  {bal !== null ? fmt(bal) : '—'}
                </div>
                {avail !== null && avail !== bal && (
                  <div className="text-[11px] text-gray-400 leading-tight mt-0.5">доступно: {fmt(avail)}</div>
                )}
...
type TxSheetAction = 'match_family' | 'match_contractor' | 'ignore' | 'internal_transfer';
const NO_ARTICLE_SHEET_ACTIONS: TxSheetAction[] = ['ignore', 'internal_transfer'];

function TxDetailSheet({
  tx, open, onClose, onActionComplete,
}: {
  tx: TxItem | null;
  open: boolean;
  onClose: () => void;
  ...
  const qc = useQueryClient();
  const [pendingAction, setPendingAction] = useState<TxSheetAction | null>(null);
  const [actingAction, setActingAction] = useState<TxSheetAction | null>(null);

  const matchMutation = useMatchBankingTransaction({
    mutation: {
      onSuccess: () => {
        void qc.invalidateQueries({ queryKey: getGetBankingMatchStatsQueryKey() });
        void qc.invalidateQueries({ queryKey: getGetBankingTransactionsQueryKey() });
      },
    },
  });
  ...
  const maskedDisplay = account.length >= 4 ? `****${account.slice(-4)}` : account;
  ...
  const executeAction = async (action: TxSheetAction, articleId?: string) => {
    ...
    try {
      await matchMutation.mutateAsync({ id: tx.id, data: { action, ...(articleId ? { articleId } : {}) } });
          ...
          {tx && (
            <>
              <div className="px-6 pt-1 pb-5 border-b border-gray-100 text-center">
                <div className={`text-[34px] font-bold leading-none mt-2 ${isIn ? 'text-green-600' : 'text-red-600'}`}>
                  {isIn ? '+' : '−'}{amount !== null ? fmt(amount) : '—'}
                </div>
                <div className="text-[13px] text-gray-400 mt-2">
                  {tx.operationDate ?? '—'} · {isIn ? 'Поступление' : 'Расход'}
                </div>
                {tx.matchStatus && (
                  <span className={`inline-block mt-2 px-3 py-0.5 rounded-full text-[12px] font-medium ${
                    tx.matchStatus === 'unmatched' ? 'bg-yellow-50 text-yellow-700' : 'bg-green-50 text-green-700'
                  }`}>
                    {tx.matchStatus === 'unmatched' ? 'Не сопоставлено' : tx.matchStatus}
                  </span>
                )}
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-4 pt-4 pb-2 space-y-4">
                  ...
                  {fields.map(([label, value]) =>
                    value ? (
                      <div key={label} className="flex items-start gap-3">
                        <span className="text-[12px] text-gray-400 w-[120px] shrink-0 pt-0.5">{label}</span>
                        <span className="text-[13px] text-gray-900 font-medium flex-1 break-all">{value}</span>
                      </div>
                    ) : null
                  )}
                </div>
                ...
                {/* Actions */}
                <div>
                  <p className="text-[11px] text-gray-400 uppercase tracking-wide font-semibold mb-2 px-1">Действия</p>
                  <div className="space-y-1.5">
                    ...
                    <button
                      ...
                      onClick={() => { onClose(); navigate('finance-matching'); }}
                    >
                      ...
                      onClick={() => {
                        if (tx.counterpartyName) {
                          toast.info(`Контрагент: ${tx.counterpartyName}`, {
                            description: tx.counterpartyInn ? `ИНН ${tx.counterpartyInn}` : undefined,
                          });
                        } else {
                    ...
                    <button
                      type="button"
                      className={btnCls('bg-gray-50 text-gray-700')}
                      onClick={() => toast.info('Создание контрагентов — в следующей версии', {
                        description: tx.counterpartyName ? `Будет предзаполнено: ${tx.counterpartyName}` : undefined,
                      })}
                    >
...
const EMPTY_FILTERS: FilterValues = { counterparty: '', inn: '', purpose: '', minAmount: '', maxAmount: '' };
      ...
      <DrawerContent>
        <DrawerHeader className="pb-0">
          <div className="flex items-center justify-between">
            ...
            {hasActive && (
              <button className="text-[13px] text-violet-600 font-semibold" onClick={() => setLocal(EMPTY_FILTERS)}>
                Сбросить
              </button>
            )}
          </div>
        ...
        <div className="px-4 pb-8 pt-4 space-y-3">
          ...
          ] as Array<[keyof FilterValues, string, string]>).map(([key, label, placeholder]) => (
            <div key={key}>
              <label className="text-[12px] text-gray-500 font-semibold mb-1.5 block">{label}</label>
              <input
          ...
          <div>
            <label className="text-[12px] text-gray-500 font-semibold mb-1.5 block">Диапазон суммы</label>
            <div className="flex items-center gap-2">
              <input
                ...
                placeholder="От ₽" type="number" value={local.minAmount}
                onChange={(e) => setLocal((prev) => ({ ...prev, minAmount: e.target.value }))}
              />
              <span className="text-gray-400 shrink-0">—</span>
              <input
                className="flex-1 h-11 px-4 rounded-xl border border-gray-200 bg-white text-[14px] outline-none focus:ring-1 focus:ring-violet-300 transition-colors"
                placeholder="До ₽" type="number" value={local.maxAmount}
                onChange={(e) => setLocal((prev) => ({ ...prev, maxAmount: e.target.value }))}
              />
            </div>
          </div>
          <button
...
function CounterStrip({ total, countIn, countOut }: { total: number; countIn: number; countOut: number }) {
  const now = new Date();
  const updatedStr = now.toLocaleDateString('ru', { day: 'numeric', month: 'short' }) + ', ' +
  ...
  const cols = [
    ...
    { label: 'Обновлено',      value: updatedStr,       color: 'text-gray-600', small: true },
  ];
  return (
    <div className="flex items-stretch rounded-xl overflow-hidden" style={{ background: '#F7F4FF', minHeight: 44 }}>
      {cols.map((col, i) => (
        <div key={i} className="flex-1 flex flex-col items-center justify-center py-1.5 relative">
          {i > 0 && <div className="absolute left-0 top-2 bottom-2 w-px bg-violet-200" />}
          <div className={`font-bold leading-tight ${col.small ? 'text-[10px]' : 'text-[13px]'} ${col.color}`}>
            {col.value}
          </div>
          <div className="text-[9px] text-gray-400 mt-0.5 text-center px-0.5 leading-tight">{col.label}</div>
        </div>
      ))}
    </div>
  );
...
function SummaryCards({ inAmount, outAmount, netAmount, countIn, countOut }: {
  ...
  return (
    <div className="grid grid-cols-3 gap-1.5">
      <div className="rounded-xl p-2.5 flex flex-col gap-0.5" style={{ background: '#F0FFF4' }}>
        <div className="text-[10px] font-semibold text-green-600">Поступления</div>
        <div className="font-bold text-green-700 text-[12px] sm:text-[13px] tabular-nums truncate">
          {inAmount !== null ? fmt(inAmount) : '—'}
        </div>
        <div className="text-[10px] text-green-500">{fmtNum(countIn)} опер.</div>
      </div>
      <div className="rounded-xl p-2.5 flex flex-col gap-0.5" style={{ background: '#FFF5F5' }}>
      ...
      <div className="rounded-xl p-2.5 flex flex-col gap-0.5" style={{ background: net >= 0 ? '#F0FFF4' : '#FFF5F5' }}>
        <div className="text-[10px] font-semibold text-gray-500">Net</div>
        <div className={`font-bold text-[12px] sm:text-[13px] tabular-nums truncate ${net >= 0 ? 'text-green-700' : 'text-red-700'}`}>
...
function TxBankRow({ tx, onOpen }: { tx: TxItem; onOpen: () => void }) {
  ...
  const dateDay = d ? d.toLocaleDateString('ru', { day: 'numeric', month: 'short' }) : '—';
  ...
  return (
    <div
      ...
      onClick={onOpen}
    >
      <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${isIn ? 'bg-green-100' : 'bg-red-100'}`}>
        {isIn
      ...
      <div className="w-[40px] shrink-0">
        <div className="text-[11px] font-medium text-gray-700 leading-tight">{dateDay}</div>
        <div className="text-[10px] text-gray-400 leading-tight">{dateYear}</div>
      </div>
      <div className="w-[84px] sm:w-[100px] shrink-0 text-right">
        <div className={`text-[13px] font-bold leading-tight tabular-nums ${isIn ? 'text-green-600' : 'text-red-600'}`}>
          {isIn ? '+' : '−'}{amount !== null ? fmt(amount) : '—'}
        </div>
        {maskedDisplay && (
          <div className="text-[10px] text-gray-400 font-mono leading-tight">{maskedDisplay}</div>
        )}
      </div>
      <div className="flex-1 min-w-0">
        <div className="text-[12px] font-semibold text-gray-900 truncate leading-tight">{tx.counterpartyName || 'Без контрагента'}</div>
        <div className="text-[10px] text-gray-400 truncate leading-tight mt-0.5">
          {[tx.counterpartyInn, tx.purpose].filter(Boolean).join(' · ') || 'Назначение не указано'}
        </div>
      </div>
...
function TxDesktopRow({ tx, onOpen }: { tx: TxItem; onOpen: () => void }) {
  ...
  return (
    <TableRow className="cursor-pointer hover:bg-violet-50/30 transition-colors" onClick={onOpen}>
      <TableCell className="text-[11px] text-gray-500 whitespace-nowrap py-2">{tx.operationDate ?? '—'}</TableCell>
      <TableCell className="py-2">
        <span className={`text-[13px] font-bold whitespace-nowrap tabular-nums ${isIn ? 'text-green-600' : 'text-red-600'}`}>
          {isIn ? '+' : '−'}{amount !== null ? fmt(amount) : '—'}
        </span>
      </TableCell>
      <TableCell className="py-2">
        <div className="text-[10px] font-mono text-gray-500">{maskedDisplay || '—'}</div>
      </TableCell>
      <TableCell className="py-2">
        <div className="text-[12px] font-semibold text-gray-900 max-w-[200px] truncate">{tx.counterpartyName || 'Без контрагента'}</div>
        {tx.counterpartyInn && <div className="text-[10px] font-mono text-gray-400">{tx.counterpartyInn}</div>}
      </TableCell>
      <TableCell className="max-w-[220px] py-2">
        <div className="text-[11px] text-gray-500 truncate">{tx.purpose || 'Назначение не указано'}</div>
      </TableCell>
      <TableCell className="py-2">
...
function getPeriodDates(period: Period): { from: string; to: string } {
  ...
  switch (period) {
    ...
    case '7d':        return { from: d(7), to: today };
    case '30d':       return { from: d(30), to: today };
    case '90d':       return { from: d(90), to: today };
    case 'month': {
    ...
    case 'lastmonth': {
      ...
      return { from: d1.toISOString().slice(0, 10), to: d2.toISOString().slice(0, 10) };
    }
    default: return { from: d(30), to: today };
  }
}
...
function PeriodChips({ period, onChange, customFrom, customTo, onCustomChange }: PeriodSelectorProps) {
  return (
    <div className="space-y-3">
          ...
          <button
            ...
            onClick={() => {
              if (key !== 'custom') {
                ...
                onChange('custom', customFrom ?? getPeriodDates('30d').from, customTo ?? getPeriodDates('30d').to);
              }
            }}
      ...
      {period === 'custom' && (
        <div className="flex items-center gap-2">
          <Input type="date" className="h-9 flex-1 text-sm" value={customFrom ?? ''}
            onChange={(e) => onCustomChange?.(e.target.value, customTo ?? '')} />
          <span className="text-gray-400 text-sm shrink-0">—</span>
          <Input type="date" className="h-9 flex-1 text-sm" value={customTo ?? ''}
...
function TransactionsTab({ period, customFrom, customTo, onPeriodChange, onCustomChange }: TransactionsTabProps) {
  ...
  const { data: stats } = useGetBankingTransactionStats(
    statsParams,
    { query: { queryKey: getGetBankingTransactionStatsQueryKey(statsParams), refetchInterval: 60000 } },
  );

  const today = new Date().toISOString().slice(0, 10);
  const allTimeParams = { from: '2020-01-01', to: today };
  const { data: allTimeStats } = useGetBankingTransactionStats(
    allTimeParams,
    { query: { queryKey: [...getGetBankingTransactionStatsQueryKey(allTimeParams), 'alltime'], refetchInterval: 120000 } },
  );
  ...
  const { data, isLoading } = useGetBankingTransactions(
    txParams,
    { query: { queryKey: getGetBankingTransactionsQueryKey(txParams), refetchInterval: 60000 } },
  );
  ...
  useEffect(() => {
    setOffset(0);
    setAccTxns([]);
  }, [period, dirFilter, debouncedCounterparty, debouncedInn, debouncedPurpose, customFrom, customTo]);
  ...
  const inAmount  = stats?.totalIn  != null ? parseFloat(String(stats.totalIn))  : null;
  const outAmount = stats?.totalOut != null ? parseFloat(String(stats.totalOut)) : null;
  ...
  return (
    <div className="px-3 pb-5 pt-2 space-y-2">
          ...
          {(['', 'income', 'expense'] as const).map((d) => (
            <button
              ...
              className={`px-2 sm:px-2.5 py-1 rounded-[8px] text-[11px] font-medium transition-all whitespace-nowrap ${
                dirFilter === d ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'
              }`}
            >
      ...
      {/* Counter strip */}
      {stats?.hasData && (
        <CounterStrip total={total} countIn={stats.countIn ?? 0} countOut={stats.countOut ?? 0} />
      )}
      ...
      {isLoading && offset === 0 ? (
        ...
        <div className="py-14 text-center">
          ...
          {hasAnyTransactions ? (
            <>
              <p className="text-[15px] font-semibold text-gray-700">Операций за этот период нет</p>
              <p className="text-[13px] text-gray-400 mt-1.5">Выберите другой период.</p>
            </>
          ) : (
            <>
              <p className="text-[15px] font-semibold text-gray-700">Выписки ещё не загружены</p>
              <p className="text-[13px] text-gray-400 mt-1.5">Перейдите в «Коннекторы» и запустите Sync.</p>
            </>
          )}
        </div>
      ) : (
        <>
          ...
          <div className="md:hidden bg-white rounded-2xl overflow-hidden border border-gray-100">
            {visibleTxns.length === 0 ? (
              <div className="py-10 text-center text-[13px] text-gray-400">Нет операций по фильтру</div>
            ) : (
              visibleTxns.map((tx) => {
                const key = tx.id ?? tx.externalId ?? String(Math.random());
                return <TxBankRow key={key} tx={tx} onOpen={() => handleOpenTx(tx)} />;
              })
            )}
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50/80 hover:bg-gray-50/80">
                  ...
                  <TableHead className="text-xs max-w-[220px]">Назначение</TableHead>
              ...
              <TableBody>
                {visibleTxns.map((tx) => {
                  const key = tx.id ?? tx.externalId ?? String(Math.random());
                  return <TxDesktopRow key={key} tx={tx} onOpen={() => handleOpenTx(tx)} />;
                })}
              </TableBody>
            </Table>
          </div>
          ...
          {hasMore && (
            <div className="text-center pt-2 pb-1">
              ...
              <div className="text-[12px] text-gray-400 mt-2">{fmtNum(accTxns.length)} из {fmtNum(total)}</div>
            </div>
          )}
        </>
      )}
      ...
      <FilterSheet
        ...
        filters={filterValues}
        onApply={(f) => { setFilterValues(f); setOffset(0); setAccTxns([]); }}
      />
    </div>
  );
}
...
function ArticlePickerDialog({
  ...
  const groups = useMemo(() => {
    ...
    for (const a of articles) {
      const key = (a as unknown as Record<string, unknown>).groupName as string ?? 'Прочее';
    ...
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="max-w-sm max-h-[78dvh] flex flex-col p-0 gap-0">
        <DialogHeader className="px-5 pt-5 pb-3 shrink-0">
          <DialogTitle className="text-[15px] font-bold">Выберите статью ДДС</DialogTitle>
        </DialogHeader>
        <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-3 min-h-0">
          {Array.from(groups.entries()).map(([group, arts]) => (
            <div key={group}>
              <div className="text-[10px] text-gray-400 uppercase tracking-wide font-semibold mb-1 px-2">{group}</div>
              <div className="space-y-0.5">
                {arts.map((a) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-[11px] font-mono text-gray-400 w-[30px] shrink-0">{art.code as string}</span>
                      <span className="text-[13px] font-medium text-gray-800">{art.name as string}</span>
                    </button>
                  );
                })}
              </div>
          ...
          {articles.length === 0 && (
            <p className="text-[13px] text-gray-400 text-center py-6">Нет статей. Сначала добавьте справочник статей.</p>
          )}
        </div>
...
const MATCH_QUICK_ACTIONS = [
  { action: 'match_family',        label: 'Семья',            Icon: Users,      cls: 'bg-blue-50 text-blue-700 border-blue-100' },
  { action: 'match_contractor',    label: 'Подрядчик',        Icon: Briefcase,  cls: 'bg-violet-50 text-violet-700 border-violet-100' },
  { action: 'match_employee',      label: 'Сотрудник',        Icon: Building2,  cls: 'bg-indigo-50 text-indigo-700 border-indigo-100' },
  { action: 'internal_transfer',   label: 'Внутр. перевод',   Icon: ArrowRight, cls: 'bg-orange-50 text-orange-700 border-orange-100' },
  { action: 'ignore',              label: 'Игнорировать',     Icon: Ban,        cls: 'bg-red-50 text-red-600 border-red-100' },
] as const;
...
function MatchStatsRow({ stats }: { stats: BankingMatchStats | undefined }) {
  ...
  const items = [
    ...
    { label: 'Не сопост.',      value: fmtNum(stats.unmatched),                  color: 'text-amber-600', bg: '#FFF8E7' },
    { label: 'Сопост.',         value: fmtNum(stats.matched + stats.suggested),  color: 'text-green-700', bg: '#F0FFF4' },
    { label: 'Игнор.',          value: fmtNum(stats.ignored),                    color: 'text-gray-500' },
    { label: 'Внутр.',          value: fmtNum(stats.internalTransfer),           color: 'text-blue-600' },
  ];
  return (
    <div className="grid grid-cols-5 gap-1.5">
      {items.map((item) => (
        <div key={item.label} className="rounded-xl p-2.5 text-center" style={{ background: item.bg ?? '#F7F8FB' }}>
          <div className={`text-[15px] font-bold leading-none ${item.color}`}>{item.value}</div>
          <div className="text-[10px] text-gray-400 mt-1 leading-tight">{item.label}</div>
        </div>
      ))}
    </div>
  );
...
function MatchingTab() {
  ...
  const [offset, setOffset] = useState(0);
  const [accTxns, setAccTxns] = useState<TxItem[]>([]);
  const [pendingAction, setPendingAction] = useState<{ txId: string; action: QuickMatchAction } | null>(null);
  const PAGE_SIZE = 50;

  // Load suggestion rules once (cached, staleTime 5 min)
  const { data: rulesData } = useGetCategorizationRules({
    query: { queryKey: getGetCategorizationRulesQueryKey(), staleTime: 5 * 60 * 1000 },
  });
  const rules = rulesData?.rules ?? [];

  const { data: matchStats } = useGetBankingMatchStats(
    {},
    { query: { queryKey: getGetBankingMatchStatsQueryKey(), refetchInterval: 30000 } },
  );

  const txParams = { matchStatus: 'unmatched', limit: PAGE_SIZE, offset };
  const { data, isLoading } = useGetBankingTransactions(
    txParams,
    { query: { queryKey: getGetBankingTransactionsQueryKey(txParams), refetchInterval: 30000 } },
  );

  const matchMutation = useMatchBankingTransaction({
    mutation: { onSuccess: () => { qc.invalidateQueries({ queryKey: getGetBankingMatchStatsQueryKey() }); qc.invalidateQueries({ queryKey: getGetBankingTransactionsQueryKey() }); } },
  });
  ...
  const NO_ARTICLE_ACTIONS: QuickMatchAction[] = ['ignore', 'internal_transfer'];
  ...
  const handleApplySuggestion = useCallback((txId: string, s: NonNullable<SuggestionResult>) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between gap-2">
        <div>
          <p className="text-[15px] font-bold text-gray-900">Очередь сопоставления</p>
          <p className="text-[11px] text-gray-400 mt-0.5">Только несопоставленные операции · {fmtNum(total)} шт.</p>
        </div>
        <ReconciliationBtn />
      </div>
      ...
      {isLoading && offset === 0 ? (
        ...
        <div className="py-14 text-center">
          ...
          <p className="text-[16px] font-bold text-gray-700">Все операции сопоставлены!</p>
          <p className="text-[13px] text-gray-400 mt-1">Нет операций, требующих сопоставления.</p>
        </div>
      ) : (
        <>
          <div className="space-y-2">
            {accTxns.map((tx) => {
              ...
              const d = tx.operationDate
                ...
                : null;
              const dateStr = d ? d.toLocaleDateString('ru', { day: 'numeric', month: 'short' }) : '—';
              const suggestion = applyRules(tx, rules);

              return (
                <div key={tx.id ?? tx.externalId} className="bg-white rounded-2xl border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] overflow-hidden">
                  ...
                  >
                    ...
                    <div className="w-[42px] shrink-0">
                      <div className="text-[11px] font-medium text-gray-600 leading-tight">{dateStr}</div>
                    </div>
                    <div className="w-[88px] shrink-0 text-right">
                      <div className={`text-[15px] font-bold tabular-nums ${isIn ? 'text-green-600' : 'text-red-600'}`}>
                        {isIn ? '+' : '−'}{amount !== null ? fmt(amount) : '—'}
                      </div>
                      {tx.maskedAccount && (
                        <div className="text-[10px] text-gray-400 font-mono">{tx.maskedAccount.length >= 4 ? `****${tx.maskedAccount.slice(-4)}` : tx.maskedAccount}</div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-[13px] font-semibold text-gray-900 truncate">{tx.counterpartyName || '—'}</div>
                  ...
                  {suggestion && suggestion.confidence >= 80 && (
                    <div className="flex items-center gap-1.5 px-4 pb-2">
                      ...
                      <span className="text-[11px] text-violet-600 font-medium">
                        {suggestion.suggestedAction === 'internal_transfer' && 'Внутренний перевод'}
                        {suggestion.suggestedAction === 'ignore'            && 'Игнорировать'}
                        {suggestion.suggestedAction === 'family'            && 'Оплата от семьи'}
                        {suggestion.suggestedAction === 'contractor'        && (suggestion.suggestedArticleCode
                          ? `${suggestion.suggestedArticleCode} ${suggestion.suggestedArticleName ?? ''}`
                          : 'Подрядчик'
                        )}
                        {!['internal_transfer','ignore','family','contractor'].includes(suggestion.suggestedAction ?? '') && (suggestion.suggestedArticleName ?? suggestion.suggestedAction ?? 'Предложение')}
                      </span>
                      <span className="text-[10px] text-gray-400 font-mono">{suggestion.confidence}%</span>
                      {SUGGESTION_TO_ACTION[suggestion.suggestedAction ?? ''] && (
                        <button
                          ...
                          onClick={() => tx.id && handleApplySuggestion(tx.id, suggestion)}
                  ...
                  <div className="flex items-center gap-1.5 px-3 pb-3 flex-wrap">
                    {MATCH_QUICK_ACTIONS.map(({ action, label, Icon, cls }) => (
                      <button
                        ...
                        onClick={() => tx.id && handleQuickAction(tx.id, action)}
                        disabled={matchMutation.isPending}
                      >
          ...
          {hasMore && (
            <div className="text-center pt-1">
              <Button variant="outline" size="sm" className="rounded-xl px-6 h-10"
                onClick={() => setOffset((p) => p + PAGE_SIZE)} disabled={isLoading}>
                {isLoading
                  ? <><Loader2 className="w-4 h-4 animate-spin mr-2" />Загрузка…</>
                  : `Показать ещё ${Math.min(PAGE_SIZE, total - accTxns.length)}`
                }
...
interface DbAuditResponse {
  ...
  connectors: Array<{ bank_name: string; status: string; last_sync: string | null }>;
}
  ...
  if (loading) {
    return (
      <div className="py-4 text-center text-xs text-muted-foreground">
        <Loader2 className="w-3.5 h-3.5 animate-spin inline mr-1.5" />Диагностика источника…
      </div>
    );
  }
  if (fetchError || !audit) {
    return (
      <div className="text-xs text-red-600 px-1">Ошибка /api/system/db-audit: {fetchError}</div>
    );
  }

  const isProd = audit.isProductionDataSource;
  const lastBankSync = audit.maxDates.latestSyncRunStartedAt
    ? new Date(audit.maxDates.latestSyncRunStartedAt).toLocaleString('ru-RU', { day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit' })
    : '—';
  const lastLedger = audit.maxDates.latestOperationDate
    ? new Date(audit.maxDates.latestOperationDate).toLocaleString('ru-RU', { day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit' })
    : '—';
  const lastBankTx = audit.maxDates.latestBankTransactionDate
    ? new Date(audit.maxDates.latestBankTransactionDate).toLocaleDateString('ru-RU', { day:'2-digit', month:'2-digit', year:'numeric' })
    : '—';

  return (
    <div className="space-y-3 mt-4">
      <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
        <Server className="w-3.5 h-3.5" />Источник данных
      </h4>
        ...
        {[
          {
            ...
            badge: isProd
              ? <span className="px-1.5 py-0.5 rounded-full bg-green-100 text-green-700 font-semibold text-[10px]">production</span>
              : <span className="px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700 font-semibold text-[10px]">development</span>,
          },
          {
            label: 'База',
          ...
          },
          {
            ...
            badge: audit.counts.bank_accounts > 0
          ...
          },
          {
            ...
            badge: audit.counts.bank_transactions > 0
              ? <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />
              : <XCircle className="w-3.5 h-3.5 text-amber-400" />,
          },
          {
            ...
            badge: audit.counts.operations > 0
              ? <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />
              : <XCircle className="w-3.5 h-3.5 text-amber-400" />,
          },
          {
          ...
          >
            ...
            <span className="font-mono text-[11px] text-gray-700 truncate text-right">{value}</span>
      ...
      {audit.connectors.length > 0 && (
        <div className="rounded-xl border bg-white overflow-hidden text-xs">
          <div className="px-3 py-1.5 border-b bg-gray-50/60 text-[11px] font-semibold text-muted-foreground uppercase tracking-wide">Коннекторы</div>
          {audit.connectors.map((c, i) => (
            <div key={i} className={`flex items-center gap-3 px-3 py-2 ${i < audit.connectors.length - 1 ? 'border-b border-gray-100' : ''}`}>
              <div className={`w-2 h-2 rounded-full shrink-0 ${c.status === 'active' ? 'bg-green-500' : 'bg-red-400'}`} />
              <span className="font-medium">{c.bank_name}</span>
              <span className={`ml-auto font-mono text-[10px] ${c.status === 'active' ? 'text-green-600' : 'text-red-500'}`}>{c.status}</span>
              {c.last_sync && (
                <span className="text-muted-foreground text-[10px] shrink-0">
                  {new Date(c.last_sync).toLocaleString('ru-RU', { day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit' })}
                </span>
              )}
            </div>
          ))}
...
function DataTruthPanel() {
  const { data: truth, isLoading, refetch } = useGetBankDataTruth({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Счетов загружено',    value: String(truth.accounts.count),          ok: hasAccounts },
          { label: 'Остаток на счетах',   value: fmt(truth.balances.totalBalance ?? 0), ok: hasBalances },
          { label: 'Операций в базе',     value: fmtNum(truth.transactions.count),      ok: hasTx },
          { label: 'Источник',            value: truth.source ?? 'Tochka API',          ok: null as boolean | null },
        ].map(({ label, value, ok }) => (
          <Card key={label} className={ok === true ? 'border-green-200 bg-green-50/30' : ok === false ? 'border-amber-200 bg-amber-50/30' : ''}>
            <CardContent className="pt-4 pb-3">
              <div className="text-xs text-muted-foreground mb-1">{label}</div>
              <div className="flex items-center gap-1.5">
                <span className="text-base font-bold leading-tight">{value}</span>
                {ok === true  && <CheckCircle2 className="w-4 h-4 text-green-600 shrink-0" />}
                {ok === false && <XCircle className="w-4 h-4 text-amber-500 shrink-0" />}
              </div>
            </CardContent>
          </Card>
        ))}
          ...
          {[
            {
              ...
              ok: hasAccounts,
              sub: truth.accounts.lastSyncAt ? `Синхронизировано ${fmtDate(truth.accounts.lastSyncAt)}` : 'Не синхронизировано',
            },
            {
              label: 'Остатки',
              value: fmt(truth.balances.totalBalance ?? 0),
              ok: hasBalances,
              sub: truth.balances.lastSyncAt ? `Синхронизировано ${fmtDate(truth.balances.lastSyncAt)}` : 'Не синхронизировано',
            },
            {
              label: 'Транзакции',
              value: hasTx ? `${fmtNum(truth.transactions.count)} операций` : 'Нет данных',
              ok: hasTx,
              sub: hasTx && truth.transactions.from && truth.transactions.to
            ...
            <div key={label} className="flex items-start justify-between py-2.5">
              ...
              <div className="flex items-center gap-2 shrink-0 ml-4">
                ...
                {ok === true  && <CheckCircle2 className="w-4 h-4 text-green-600" />}
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm flex items-center justify-between">
              ...
              <span className={`text-xs font-medium px-2 py-0.5 rounded ${syncOk ? 'bg-green-100 text-green-800' : syncPartial ? 'bg-amber-100 text-amber-800' : 'bg-red-100 text-red-800'}`}>
          ...
          <CardContent>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
              {[
                { label: 'Выписок запрошено',  value: String(truth.lastSync.statementsRequested ?? '—') },
                { label: 'Выписок сохранено',  value: String(truth.lastSync.statementsSaved ?? '—') },
                { label: 'Новых операций',      value: `+${truth.lastSync.transactionsNew ?? 0}`, cls: 'text-green-700' },
                { label: 'Дублей пропущено',   value: String(truth.lastSync.transactionsDuplicates ?? 0) },
              ].map(({ label, value, cls }) => (
                <div key={label}>
            ...
            {syncPartial && (
              <div className="mt-3 text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded p-2 flex items-start gap-2">
                ...
                <span>
                  <strong>Выписка заказана.</strong> Банк готовит операции — это займёт 1–2 минуты.
                  Повторите Sync через минуту, чтобы забрать готовые данные.
                </span>
              </div>
            )}
            ...
            {truth.lastSync.startedAt && (
              <div className="mt-2 text-xs text-muted-foreground flex gap-4">
                ...
                {truth.lastSync.durationMs != null && (
                  <span>Длительность: {(truth.lastSync.durationMs / 1000).toFixed(1)}с</span>
                )}
              </div>
            )}
          </CardContent>
      ...
      <div className="flex justify-end">
        <Button size="sm" variant="outline" className="gap-1.5 text-xs h-7" onClick={() => refetch()}>
          <RefreshCw className="w-3 h-3" />
          Обновить
        </Button>
      </div>
...
interface DdsSectionRow { direction?: string; articleCode?: string | null; articleName?: string | null; total?: string; }

function DdsSection({ title, total, rows, color, sign }: {
  title: string; total: number; rows: DdsSectionRow[]; color: 'emerald' | 'red'; sign: string;
}) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <span className={`text-[15px] font-bold tabular-nums ${color === 'emerald' ? 'text-emerald-600' : 'text-red-600'}`}>
            {sign}{fmtAmt(total)}
          </span>
      ...
      {expanded && (
        <div>
          {sorted.length === 0
            ? <p className="px-4 py-3 text-[12px] text-gray-400">Нет данных за период</p>
            : sorted.map((row, i) => {
                const amt = parseFloat(row.total ?? '0');
                const pct = total > 0 ? Math.round((amt / total) * 100) : 0;
                return (
                  <div key={i} className="flex items-center px-4 py-2 border-b border-gray-50/80 last:border-0">
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] text-gray-800 leading-tight truncate">
                        {row.articleCode && <span className="text-gray-300 font-mono text-[11px] mr-1.5">{row.articleCode}</span>}
                        {row.articleName ?? <span className="text-gray-400 italic">Без категории</span>}
                      </p>
                    </div>
                    <div className="flex items-center gap-3 ml-2 shrink-0">
                      <span className="text-[11px] text-gray-400 tabular-nums w-8 text-right">{pct}%</span>
                      <span className="text-[13px] font-semibold text-gray-900 tabular-nums w-24 text-right">{fmtAmt(amt)}</span>
                    </div>
                  </div>
                );
              })
...
function getDdsPeriodRange(p: DdsPeriod): { from: string; to: string } {
  ...
  d.setMonth(now.getMonth() - (p === 'q' ? 2 : p === 'half' ? 5 : now.getMonth()));
...
function LedgerDdsTab() {
  ...
  const { data: report, isLoading } = useGetLedgerCashflowReport(
    { from, to },
    { query: { queryKey: getGetLedgerCashflowReportQueryKey({ from, to }), staleTime: 30_000 } }
  );
  const { data: stats } = useGetLedgerFinancialStats(
    { month: period === 'month' ? from : undefined },
    { query: { queryKey: getGetLedgerFinancialStatsQueryKey({ month: period === 'month' ? from : undefined }), staleTime: 30_000 } }
  );

  const income  = useMemo(() => (report?.byArticle ?? []).filter(r => r.direction === 'in'),  [report]);
  const expense = useMemo(() => (report?.byArticle ?? []).filter(r => r.direction === 'out'), [report]);
  ...
  if (isLoading) return (
    <div className="py-12 flex flex-col items-center gap-2 text-gray-400">
      <Loader2 className="w-6 h-6 animate-spin" /><p className="text-sm">Загрузка реестра...</p>
    </div>
  );

  const isEmpty = (stats?.totalCount ?? 0) === 0 && income.length === 0 && expense.length === 0;
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Period selector */}
      <div className="flex items-center gap-1.5 flex-wrap">
        {([['month','Этот месяц'],['q','3 мес'],['half','6 мес'],['year','Год']] as const).map(([k, l]) => (
        ...
        <span className="ml-auto text-[11px] text-gray-400 self-center font-mono">{from}{from !== to ? ` — ${to}` : ''}</span>
      </div>
      ...
      {isEmpty ? (
        <div className="bg-white rounded-2xl border border-gray-100 py-10 text-center">
          ...
          <p className="text-sm font-medium text-gray-700">Реестр операций пуст</p>
          <p className="text-[12px] text-gray-400 mt-1 max-w-64 mx-auto leading-relaxed">
            Сопоставьте банковские транзакции — они автоматически появятся в реестре
          </p>
          <Button variant="outline" size="sm" className="mt-4 text-xs" onClick={() => navigate('finance-matching')}>
            Перейти к сопоставлению <ChevronRight className="w-3.5 h-3.5 ml-1" />
          </Button>
        </div>
      ) : (
        <>
          <DdsSection title="ПОСТУПЛЕНИЯ" total={totalIn}  rows={income}  color="emerald" sign="+" />
          <DdsSection title="РАСХОДЫ"     total={totalOut} rows={expense} color="red"     sign="−" />
          <div className={`rounded-2xl px-5 py-4 flex items-center justify-between ${net >= 0 ? 'bg-emerald-50 border border-emerald-100' : 'bg-red-50 border border-red-100'}`}>
            <div>
              <p className="text-[11px] uppercase tracking-wide font-semibold text-gray-500">Чистый денежный поток</p>
              <p className="text-[11px] text-gray-400 mt-0.5 font-mono">{from}{from !== to ? ` — ${to}` : ''}</p>
            </div>
            <span className={`text-[20px] font-bold tabular-nums ${net >= 0 ? 'text-emerald-700' : 'text-red-700'}`}>
              {net >= 0 ? '+' : ''}{fmt(net)}
            </span>
          </div>
          {stats && (
            <p className="text-[11px] text-gray-400 text-right">
              {stats.totalCount} операций · {stats.verifiedCount} проверено{(stats.unmatchedCount ?? 0) > 0 ? ` · ${stats.unmatchedCount} ожидают` : ''}
            </p>
          )}
        </>
      )}
...
const PAYABLE_STATUS_META: Record<string, { label: string; color: string }> = {
  ...
  pending_review: { label: 'На проверке',    color: 'bg-amber-50 text-amber-700 border-amber-200' },
  approved:       { label: 'Подтверждено',   color: 'bg-blue-50 text-blue-700 border-blue-200' },
  scheduled:      { label: 'Запланировано',  color: 'bg-sky-50 text-sky-700 border-sky-200' },
  paid:           { label: 'Оплачено',       color: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
  cancelled:      { label: 'Отменено',       color: 'bg-red-50 text-red-600 border-red-200' },
  overdue:        { label: 'Просрочено',     color: 'bg-red-50 text-red-700 border-red-200' },
};

const PAYABLE_SOURCE_LABELS: Record<string, string> = {
  manual: 'Ручное', email: 'Email', upload: 'Загрузка',
  recurring: 'Регулярное', api: 'API',
};
...
function PayableCard({ ob, onApprove, onCancel, onMarkPaid, isActing }: PayableCardProps) {
  const es = ((ob as unknown) as Record<string, unknown>).effectiveStatus as string || ob.status;
  const meta = PAYABLE_STATUS_META[es] ?? PAYABLE_STATUS_META[ob.status] ?? { label: ob.status, color: 'bg-gray-100 text-gray-600' };
  const dueDate = ob.dueDate ?? null;
  const isOverdue = es === 'overdue';

  return (
    <div className={`flex items-start gap-3 px-4 py-3 border-b border-gray-50 last:border-0 ${isOverdue ? 'bg-red-50/30' : ''}`}>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
          <span className="text-[13px] font-semibold text-gray-900 truncate max-w-[200px]">{ob.counterpartyName}</span>
          <span className={`inline-flex items-center px-1.5 py-0.5 rounded-md text-[10px] font-semibold border ${meta.color}`}>{meta.label}</span>
          {ob.sourceType && ob.sourceType !== 'manual' && (
            <span className="text-[10px] text-gray-400">{PAYABLE_SOURCE_LABELS[ob.sourceType] ?? ob.sourceType}</span>
          )}
        </div>
        <div className="flex items-center gap-3 text-[11px] text-gray-400">
          ...
          {ob.description && <span className="truncate max-w-[140px]">{ob.description}</span>}
        </div>
      </div>
      <div className="flex flex-col items-end gap-1.5 shrink-0">
        <span className="text-[15px] font-bold text-gray-900 tabular-nums">{fmtRub(ob.amountTotal)}</span>
        {(onApprove || onCancel || onMarkPaid) && (
          <div className="flex items-center gap-1">
            ...
            {onMarkPaid && (
              ...
              >
                {isActing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Wallet className="w-3.5 h-3.5" />}
              </button>
            )}
            {onCancel && (
              <button
                ...
                title="Отменить"
              >
                {isActing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <OctagonX className="w-3.5 h-3.5" />}
              </button>
            )}
          </div>
        )}
...
function CreatePayableModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  const [form, setForm] = useState<CreatePayableFormState>({
    counterpartyName: '', amountTotal: '', dueDate: '',
    documentNumber: '', description: '', notes: '',
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMut.mutate(
      ...
      {
        onSuccess: () => { toast.success('Обязательство создано'); onCreated(); onClose(); },
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-[20px] w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-gray-100">
          <h3 className="text-[15px] font-bold text-gray-900">Новое обязательство</h3>
        ...
        <form onSubmit={handleSubmit} className="px-5 pt-4 pb-5 space-y-3">
          <div>
            <label className="block text-[11px] font-semibold text-gray-500 mb-1">Контрагент *</label>
            <input
              ...
              onChange={(e) => setForm((s) => ({ ...s, counterpartyName: e.target.value }))}
              required
            />
          </div>
          <div className="flex gap-3">
            <div className="flex-1">
              <label className="block text-[11px] font-semibold text-gray-500 mb-1">Сумма, ₽ *</label>
              <input
                type="number"
                min="0"
                step="0.01"
                ...
                onChange={(e) => setForm((s) => ({ ...s, amountTotal: e.target.value }))}
                required
              />
            </div>
            <div className="flex-1">
              <label className="block text-[11px] font-semibold text-gray-500 mb-1">Срок оплаты</label>
              <input
                type="date"
                className="w-full h-9 px-3 rounded-xl border border-gray-200 text-[13px] focus:outline-none focus:border-violet-400"
                value={form.dueDate}
          ...
          <div>
            <label className="block text-[11px] font-semibold text-gray-500 mb-1">Номер документа</label>
            <input
              className="w-full h-9 px-3 rounded-xl border border-gray-200 text-[13px] focus:outline-none focus:border-violet-400"
          ...
          <div>
            <label className="block text-[11px] font-semibold text-gray-500 mb-1">Назначение</label>
            <input
              className="w-full h-9 px-3 rounded-xl border border-gray-200 text-[13px] focus:outline-none focus:border-violet-400"
...
function PayablesTab() {
  ...
  const qKey = getListPayableObligationsQueryKey(filter === 'all' ? {} : { status: filter });
  ...
  const obligations = (listData?.obligations ?? []) as (PayableObligation & { effectiveStatus?: string })[];
  ...
  const handleCancel = (id: string) => {
    ...
    cancelMut.mutate(
      { id },
      {
        onSuccess: () => { toast.success('Отменено'); invalidate(); },
        onError: () => toast.error('Ошибка'),
        onSettled: () => setActingId(null),
      },
    );
  ...
  const handleMarkPaid = (id: string) => {
    ...
    markPaidMut.mutate(
      { id },
      {
        onSuccess: () => { toast.success('Отмечено как оплачено'); invalidate(); },
        onError: () => toast.error('Ошибка'),
        onSettled: () => setActingId(null),
      },
    );
  ...
  const FILTERS: { key: PayableFilter; label: string; count?: number }[] = [
    ...
    { key: 'pending_review', label: 'На проверке',   count: summary?.pendingReviewCount ?? 0 },
    { key: 'approved',       label: 'Подтверждено',  count: summary?.approvedCount ?? 0 },
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        <div className="bg-white rounded-[14px] border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] px-3 py-2.5">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400 mb-1">К оплате (30 дн.)</p>
          {loadingSummary ? (
            <div className="h-5 w-20 bg-gray-100 rounded animate-pulse" />
          ) : (
            <p className="text-[17px] font-bold text-gray-900 tabular-nums">{fmtRub(summary?.totalDueNext30Days)}</p>
          )}
        </div>
        <div className={`bg-white rounded-[14px] border shadow-[0_1px_4px_rgba(0,0,0,0.04)] px-3 py-2.5 ${(summary?.overdueCount ?? 0) > 0 ? 'border-red-200 bg-red-50/30' : 'border-gray-100'}`}>
          <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400 mb-1">Просрочено</p>
          {loadingSummary ? (
            <div className="h-5 w-20 bg-gray-100 rounded animate-pulse" />
          ) : (
            <p className={`text-[17px] font-bold tabular-nums ${(summary?.overdueCount ?? 0) > 0 ? 'text-red-600' : 'text-gray-900'}`}>
              {fmtRub(summary?.overdueTotal)}
              {(summary?.overdueCount ?? 0) > 0 && (
                <span className="text-[11px] font-normal text-red-400 ml-1">({summary?.overdueCount})</span>
              )}
            </p>
          )}
        </div>
        <div className="bg-white rounded-[14px] border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] px-3 py-2.5">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400 mb-1">На неделе</p>
          {upcoming ? (
            <p className="text-[17px] font-bold text-gray-900 tabular-nums">{fmtRub(upcoming.dueThisWeek)}</p>
          ) : (
            <div className="h-5 w-20 bg-gray-100 rounded animate-pulse" />
          )}
        </div>
        <div className="bg-white rounded-[14px] border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] px-3 py-2.5">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400 mb-1">На проверке</p>
          {loadingSummary ? (
            <div className="h-5 w-16 bg-gray-100 rounded animate-pulse" />
          ) : (
            <p className={`text-[17px] font-bold tabular-nums ${(summary?.pendingReviewCount ?? 0) > 0 ? 'text-amber-600' : 'text-gray-900'}`}>
          ...
          {FILTERS.map(({ key, label, count }) => (
            ...
            >
              ...
              {count !== undefined && count > 0 && (
                <span className={`ml-1 ${filter === key ? 'text-violet-200' : 'text-gray-400'}`}>
                  {count}
                </span>
              )}
            </button>
      ...
      <div className="bg-white rounded-[16px] border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.05)] overflow-hidden">
        {loadingList ? (
          ...
          <div className="flex flex-col items-center justify-center py-14 text-center">
            ...
            <p className="text-[14px] font-semibold text-gray-400">Нет обязательств</p>
          ...
          obligations.map((ob) => {
            ...
            const canApprove  = !['paid', 'cancelled', 'approved', 'scheduled'].includes(es);
            ...
            return (
              <PayableCard
                ...
                onMarkPaid={canMarkPaid  ? () => handleMarkPaid(ob.id) : undefined}
...
const RECURRING_TYPE_LABELS: Record<string, string> = {
  rent: 'Аренда', payroll: 'Зарплата', taxes: 'Налоги',
  utilities: 'Коммунальные', telecom: 'Телефония', software: 'ПО/SaaS',
  marketing: 'Маркетинг', leasing: 'Лизинг', food: 'Питание',
  security: 'Охрана', infrastructure: 'Инфраструктура', custom: 'Прочее',
};
...
const RECURRING_TYPE_ICONS: Record<string, any> = {
  rent: Building2, payroll: Users, taxes: Banknote, utilities: Zap,
  telecom: Wifi, software: Server, marketing: Bell, leasing: Key,
  food: CircleDollarSign, security: ShieldCheck, infrastructure: Database,
...
function fmtShortDate(d: string | null | undefined): string {
  if (!d) return '—';
  try {
    return new Date(d + 'T12:00:00Z').toLocaleDateString('ru-RU', { day: 'numeric', month: 'short' });
  } catch { return d; }
}
...
function ConfidencePill({ score }: { score: number | null | undefined }) {
  ...
  const color = score >= 90 ? 'text-emerald-700 bg-emerald-50' : score >= 75 ? 'text-yellow-700 bg-yellow-50' : 'text-gray-600 bg-gray-100';
  return <span className={`text-[11px] font-semibold px-1.5 py-0.5 rounded ${color}`}>{score}%</span>;
}

function ObligationCard({ ob, onApprove, onReject, isPatching }: {
  ...
  const typeColor = RECURRING_TYPE_COLORS[ob.type] ?? RECURRING_TYPE_COLORS.custom;
    ...
    <div className="flex items-start gap-3 px-4 py-3 border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <span className={`text-[11px] font-medium px-1.5 py-0.5 rounded border ${typeColor}`}>{typeLabel}</span>
          <ConfidencePill score={ob.confidenceScore} />
          {ob.relatedParty && <span className="text-[10px] text-gray-400 italic">связанное</span>}
        </div>
        <p className="text-[13px] font-semibold text-gray-900 truncate">{ob.title || ob.counterpartyName || '—'}</p>
        <p className="text-[11px] text-gray-400 mt-0.5">
          {ob.frequency === 'monthly' ? 'ежемесячно' : ob.frequency === 'quarterly' ? 'ежеквартально' : ob.frequency}
          {ob.counterpartyName && ob.title !== ob.counterpartyName && <span className="ml-1.5 text-gray-300">· {ob.counterpartyName.slice(0, 40)}</span>}
        </p>
      </div>

      {/* Center: amounts + dates */}
      <div className="text-right shrink-0 min-w-[110px]">
        <p className="text-[14px] font-bold text-gray-900">{fmtRubles(ob.expectedAmount)}</p>
        {ob.minAmount !== ob.maxAmount && ob.minAmount && ob.maxAmount && (
          <p className="text-[10px] text-gray-400">{fmtRubles(ob.minAmount)} – {fmtRubles(ob.maxAmount)}</p>
        )}
        <div className="flex items-center justify-end gap-1 mt-1">
          <Clock className="w-3 h-3 text-gray-300 shrink-0" />
          <span className={`text-[11px] ${days !== null && days <= 7 ? 'text-orange-600 font-semibold' : 'text-gray-400'}`}>
            {days !== null ? (days === 0 ? 'сегодня' : days < 0 ? `${Math.abs(days)} дн назад` : `через ${days} дн`) : '—'}
          </span>
        </div>
        <p className="text-[10px] text-gray-300">прошлая: {fmtShortDate(ob.lastPaidAt)}</p>
      </div>
      ...
      {(onApprove || onReject) && (
        <div className="flex flex-col gap-1 shrink-0">
          ...
          >
            <CheckCircle2 className="w-3 h-3" />Да
          </button>
          <button
            disabled={isPatching}
            onClick={onReject}
...
function RecurringTab() {
  ...
  const suggested   = obligations.filter(o => o.status === 'suggested').sort((a, b) => (b.confidenceScore ?? 0) - (a.confidenceScore ?? 0));
  const approved    = obligations.filter(o => o.status === 'approved').sort((a, b) => (a.nextExpectedDate ?? '').localeCompare(b.nextExpectedDate ?? ''));
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="bg-white rounded-[16px] border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.05)] flex items-center gap-3 px-4 py-2.5">
        ...
        >
          {detect.isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
          Обнаружить
        </button>
      </div>
      ...
      {detect.isSuccess && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-50 text-emerald-700 text-[12px] font-medium">
          ...
          Обнаружение завершено: +{(detect.data as any)?.inserted ?? 0} новых, обновлено {(detect.data as any)?.updated ?? 0}
        </div>
      )}
      ...
      {(upcoming.length > 0 || loadingUp) && (
        <div>
          ...
          <div className="flex items-center justify-between py-2">
            <span className="text-[13px] font-bold text-gray-900">Ближайшие 30 дней</span>
            {upData?.totalExpected && (
              <span className="text-[13px] font-bold text-gray-700">{fmtRubles(upData.totalExpected)}</span>
            )}
          </div>
          {loadingUp ? (
            <div className="flex justify-center py-5"><Loader2 className="w-4 h-4 animate-spin text-gray-300" /></div>
          ) : (
            <div className="bg-white rounded-[16px] border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.05)] overflow-hidden">
              {upcoming.map(ob => {
                const days = daysUntil(ob.nextExpectedDate);
                const typeLabel = RECURRING_TYPE_LABELS[ob.type] ?? ob.type;
                const typeColor = RECURRING_TYPE_COLORS[ob.type] ?? RECURRING_TYPE_COLORS.custom;
                const TypeIcon = RECURRING_TYPE_ICONS[ob.type] ?? CircleDollarSign;
                return (
                  <div key={ob.id} className="flex items-center gap-2.5 px-4 py-2.5 border-b border-gray-50 last:border-0">
                    <span className={`flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded border shrink-0 ${typeColor}`}>
                      <TypeIcon className="w-2.5 h-2.5 shrink-0" />
                      {typeLabel}
                    </span>
                    <span className="text-[12px] font-medium text-gray-800 truncate flex-1 uppercase tracking-wide">{ob.title || ob.counterpartyName}</span>
                    <span className="text-[12px] font-bold text-gray-900 shrink-0">{fmtRubles(ob.expectedAmount)}</span>
                    <span className={`text-[11px] font-semibold shrink-0 w-12 text-right ${days !== null && days <= 5 ? 'text-orange-600' : 'text-gray-400'}`}>
      ...
      {(suggested.length > 0 || loadingAll) && (
        <div className="bg-white rounded-[16px] border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.05)] overflow-hidden">
          <div className="flex items-center gap-2 px-4 pt-3 pb-2 border-b border-gray-50">
            ...
            <span className="text-[13px] font-bold text-gray-900">На проверке</span>
            {suggested.length > 0 && <span className="ml-auto text-[11px] text-gray-400">{suggested.length} — подтвердите или отклоните</span>}
          </div>
          {loadingAll ? (
            <div className="flex justify-center py-5"><Loader2 className="w-4 h-4 animate-spin text-gray-300" /></div>
          ) : (
      ...
      {approved.length > 0 && (
        <div className="bg-white rounded-[16px] border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.05)] overflow-hidden">
          <div className="flex items-center gap-2 px-4 pt-3 pb-2 border-b border-gray-50">
            ...
            <span className="text-[13px] font-bold text-gray-900">Подтверждённые</span>
            <span className="ml-auto text-[11px] text-gray-400">{approved.length}</span>
          </div>
          <div>
            {approved.map(ob => (
      ...
      {!loadingAll && obligations.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-[14px] font-semibold text-gray-400">Нет обязательств</p>
          <p className="text-[12px] text-gray-300 mt-1">Нажмите «Обнаружить» — система проанализирует операции</p>
        </div>
      )}
    </div>
  );
...
export type BankTab = 'transactions' | 'matching' | 'ledger-dds' | 'pnl' | 'recurring' | 'payables';

const SECTION_META: Record<BankTab, { title: string; subtitle: string }> = {
  transactions: { title: 'Операции',        subtitle: 'Финансовые операции из банка и ручных платежей' },
  matching:     { title: 'Сопоставление',   subtitle: 'Классификация и привязка финансовых операций' },
  'ledger-dds': { title: 'ДДС',             subtitle: 'Движение денежных средств по операциям' },
  pnl:          { title: 'ОПиУ',            subtitle: 'Отчёт о прибылях и убытках по месяцу начисления' },
  recurring:    { title: 'План платежей',   subtitle: 'Регулярные и ожидаемые обязательства' },
  payables:     { title: 'Обязательства',   subtitle: 'Входящие счета, акты и исходящие платежи' },
};

export function BankingPage({ activeSection }: { activeSection?: BankTab } = {}) {
  const qc = useQueryClient();
        ...
        <div className="bg-white rounded-[18px] border border-gray-100 shadow-[0_1px_8px_rgba(0,0,0,0.04)] overflow-hidden">
          <div className="flex items-center justify-between px-3 pt-2.5 pb-2 border-b border-gray-50">
            <div className="flex items-center gap-1.5">
              ...
              <h3 className="text-[14px] font-bold text-gray-900">Реестр операций</h3>
            </div>
            <div className="flex items-center gap-1 px-2.5 h-7 rounded-xl border border-gray-200 bg-gray-50 text-[12px] text-gray-500 select-none cursor-default">
              ...
              <span>{PERIODS.find((p) => p.key === txPeriod)?.label ?? '30 дней'}</span>
            </div>
          </div>
          <TransactionsTab
            period={txPeriod}
            ...
            onPeriodChange={(p, from, to) => {
              setTxPeriod(p);
              if (p !== 'custom') { setTxCustomFrom(from); setTxCustomTo(to); }
            }}
            onCustomChange={(from, to) => { setTxCustomFrom(from); setTxCustomTo(to); }}
          />
        </div>
      )}
...
export function BankIntegrationsPage() {
  ...
  const [configTarget, setConfigTarget] = useState<{ connector: BankConnectorRow | null; bank: string } | null>(null);
  ...
  useEffect(() => {
    ...
    if (oauthResult === 'success') {
      setOauthBanner({ type: 'success', message: 'Авторизация в Точка Банке успешно завершена. Коннектор активен.' });
      qc.invalidateQueries({ queryKey: getGetBankConnectorsQueryKey() });
      window.history.replaceState({}, '', window.location.pathname);
    } else if (oauthError) {
      setOauthBanner({ type: 'error', message: `Ошибка авторизации: ${decodeURIComponent(oauthError)}` });
      window.history.replaceState({}, '', window.location.pathname);
    }
  }, [qc]);

  const { data: connectors, isLoading: connectorsLoading } = useGetBankConnectors({
    query: { queryKey: getGetBankConnectorsQueryKey(), refetchInterval: 30000 },
  });

  const deleteConnector = useDeleteBankConnector({
    mutation: {
      onSuccess: () => qc.invalidateQueries({ queryKey: getGetBankConnectorsQueryKey() }),
    },
  });
  ...
  const activeCount  = (connectors ?? []).filter((c) => c.connectorStatus === 'active').length;
  ...
  const handleDelete = (id: string) => {
    if (confirm('Удалить коннектор? Аккаунты и история sync будут сохранены.')) {
    ...
    <div className="space-y-6">
      ...
      {oauthBanner && (
        <div className={`flex items-start gap-3 rounded-xl px-4 py-3 border text-sm ${
          ...
          <button className="text-xs opacity-60 hover:opacity-100" onClick={() => setOauthBanner(null)}>✕</button>
        </div>
      )}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-[28px] font-bold text-gray-900 leading-tight">Интеграции · Банки</h2>
          </div>
          <p className="text-[15px] text-gray-500 mt-0.5 leading-snug">
            Технический раздел: подключение банков, счета, синхронизация и диагностика.
          </p>
          {activeCount > 0 ? (
            <div className="inline-flex items-center gap-1.5 mt-2.5 px-3 rounded-full bg-green-100 text-green-700 text-[13px] font-semibold" style={{ height: 28 }}>
              <div className="w-1.5 h-1.5 rounded-full bg-green-500" />
              {activeCount} активных коннектора
            </div>
          ) : !connectorsLoading && (
            <div className="inline-flex items-center gap-1.5 mt-2.5 px-3 rounded-full bg-gray-100 text-gray-500 text-[13px] font-semibold" style={{ height: 28 }}>
      ...
      {!connectorsLoading && activeCount === 0 && (
        <Card className="border-dashed border-2 border-blue-200 bg-blue-50/30">
          <CardContent className="py-4 text-center">
            ...
            <p className="text-sm font-medium text-blue-900">Требуется настройка коннектора</p>
            <p className="text-xs text-muted-foreground mt-1">
              Добавьте учётные данные для банка в разделе «Коннекторы» ниже. Данные не подменяются.
            </p>
            <Button size="sm" variant="outline" className="mt-3" onClick={() => setTab('connectors')}>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {['tochka', 'tinkoff', 'vtb'].map((bank) => (
              <ConnectorCard
                ...
                onConfigure={(c, b) => setConfigTarget({ connector: c, bank: b })}
                onSync={handleSync}
          ...
          <Card className="bg-muted/30">
            <CardContent className="py-3 px-4 text-xs text-muted-foreground">
              <strong className="text-foreground">Webhook endpoints</strong> (для банков с push-уведомлениями):
        ...
        <div>
          <div className="mb-3">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Точные счётчики из базы данных. Если счётчик 0 — данных нет, значения не подставляются.
            </p>
          </div>
          <DataTruthPanel />
          <DbSourcePanel />
      ...
      <ConfigDialog
        ...
        onSaved={() => qc.invalidateQueries({ queryKey: getGetBankConnectorsQueryKey() })}
      />
    </div>
  );
}
