...
const ATTR_STATUS_LABEL: Record<string, { label: string; color: string }> = {
  ready: {
    label: "Атрибуция: готово",
    color: "bg-emerald-50 border-emerald-200 text-emerald-700",
  },
  partial: {
    label: "Атрибуция: частично",
...
const WARNING_LABELS: Record<string, string> = {
  synthetic_or_shared_teacher:
    "⚠ Синтетический/общий педагог — данные не персонифицированы",
...
const EMPLOYEE_KIND_META: Record<
  string,
  { label: string; color: string; short: string }
> = {
  real_employee: {
    label: "Реальный сотрудник",
    color: "bg-emerald-50 border-emerald-200 text-emerald-700",
    short: "Реальный",
  },
  former_employee: {
...
const CLASSIFICATION_STATUS_META: Record<
  string,
...
function fmtCompact(n: number): string {
  ...
  const sign = n < 0 ? "−" : "";
  if (abs >= 1_000_000)
    return `${sign}${(abs / 1_000_000).toFixed(2).replace(/\.?0+$/, "")} млн ₽`;
  if (abs >= 100_000) return `${sign}${Math.round(abs / 1000)} тыс. ₽`;
  if (abs >= 1_000)
    return `${sign}${(abs / 1000).toFixed(1).replace(".0", "")} тыс. ₽`;
...
function KindBadge({
  ...
  return (
    <span
      className={`inline-flex items-center text-[10px] px-2 py-0.5 rounded-full border font-medium whitespace-nowrap ${meta.color}`}
    >
...
function StatCard({
  ...
  return (
    <div className="bg-white rounded-[22px] border border-black/[0.06] shadow-[0_1px_6px_rgba(0,0,0,0.06)] p-5 flex items-start gap-4">
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${accent ?? "bg-[#F7F8FB]"}`}
      >
        {icon}
      </div>
      <div>
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-3.5 sm:hidden">
        ...
        <div className="shrink-0 flex flex-col items-end gap-1">
          ...
          {e.excludeFromStaffAnalytics ? (
            ...
            <span
              className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border font-medium ${sm.color}`}
            >
      ...
      <div className="hidden sm:grid grid-cols-[2fr_1fr_1fr_auto_auto_auto_auto_auto_auto] gap-3 items-center px-4 py-3">
        ...
        <p className="text-sm text-gray-500 truncate hidden lg:block">
          {e.employeeType
            ? (EMPLOYEE_TYPE_LABELS[e.employeeType] ?? e.employeeType)
            : (EMPLOYMENT_LABELS[e.employmentType] ?? e.employmentType)}
        </p>
        {/* CRM */}
        <div>
        ...
        {/* Person */}
        <div>
          <LinkBadge linked={!!e.personId} yesLabel="ID ✓" noLabel="нет ID" />
        </div>
        {/* Занятий */}
        ...
        <div className="flex flex-col items-end gap-1">
          {e.excludeFromStaffAnalytics ? (
            ...
            <span
              className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border font-medium whitespace-nowrap ${sm.color}`}
            >
              <SmIcon className="w-2.5 h-2.5" />
              {sm.label}
            </span>
...
function ClassificationEditor({
  ...
  const sel =
    "w-full text-xs bg-white border border-gray-200 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-violet-300";
    ...
    <div className="mt-3 border-t border-gray-100 pt-3">
      ...
      {open && (
        <div className="mt-2.5 bg-gray-50 rounded-xl p-3 border border-gray-100 space-y-2.5">
          ...
          <div>
            <label className="text-[10px] font-medium text-gray-400 block mb-1.5">
              Участие в аналитике
            </label>
...
function EducationalUnitsBlock({
  ...
  const lessonBased = links.filter(
    (l) => l.attribution_model === "lesson_based",
  );
  const other = links.filter(
    (l) =>
      l.attribution_model !== "group_based" &&
      l.attribution_model !== "lesson_based",
  );

  const revGroupIds = new Set(
    revenue?.topGroups.map((g) => g.groupCrmId) ?? [],
  );
    ...
    <div>
      ...
      {groupBased.length > 0 && (
        <div className="mb-3">
          ...
          <div className="space-y-1.5">
            {groupBased.map((link) => {
              const hasRevenue = link.crm_group_id
                ? revGroupIds.has(link.crm_group_id)
                : false;
              const topGroup = revenue?.topGroups.find(
                (g) => g.groupCrmId === link.crm_group_id,
              );
              return (
                <div
                  key={link.link_id}
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <span
                        className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium ${CONFIDENCE_COLORS[link.confidence ?? "low"] ?? "bg-gray-100 text-gray-500"}`}
                      >
                        {link.confidence ?? "?"}
                      </span>
                      <span
                        className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium ${SOURCE_COLORS[link.source ?? "inferred"] ?? "bg-gray-100 text-gray-400"}`}
                      >
                        {link.source ?? "—"}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-[10px] text-gray-500">
                      {EDU_TYPE_LABELS[link.educational_unit_type] ??
                        link.educational_unit_type}
                    </span>
                    <span className="text-[10px] text-gray-400">·</span>
                    <span className="text-[10px] text-gray-500">
                      {ROLE_LABELS[link.role_in_unit ?? ""] ??
                        link.role_in_unit ??
                        "—"}
                    </span>
                    {link.department && (
                  ...
                  {hasRevenue && topGroup && (
                    <div className="flex items-center gap-3 pt-0.5">
                      <span className="text-[10px] text-emerald-700">
                        {topGroup.lessonsCount} урок
                        {topGroup.lessonsCount === 1
                          ? ""
                          : topGroup.lessonsCount < 5
                            ? "а"
                            : "ов"}{" "}
                        · {month}
                      </span>
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[9px] text-gray-400 px-1.5 py-0.5 rounded-full bg-gray-100">
                    {ATTR_MODEL_LABELS[link.attribution_model ?? ""] ??
                      link.attribution_model ??
                      "—"}
                  </span>
                </div>
              </div>
      ...
      {links.length === 0 && (
        <p className="text-xs text-gray-400 italic">
          Нет привязанных образовательных единиц
        </p>
      )}
    </div>
  );
...
function EmployeeCard({
  ...
  const { data, isLoading } = useQuery<EmployeeDetail>({
    ...
    queryFn: () =>
      apiFetch(`/api/employees/${employeeId}?month=${month}`).then((r) =>
        r.json(),
      ),
  });
      ...
      <div className="flex-1 px-5 py-4 space-y-5 pb-8">
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl border border-black/[0.06] divide-y divide-gray-50">
            <InfoRow
              icon={
                <DisplayIcon
                  className={`w-3.5 h-3.5 ${displaySm.color.split(" ")[0]}`}
                />
              }
              label="Статус"
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl border border-black/[0.06] p-4 space-y-3">
            ...
            {/* Manual editor */}
            <ClassificationEditor employee={employee} onSaved={() => {}} />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <FinanceCell
              label="Выручка"
              value={
                finance.revenueRub != null
                  ? fmtCompact(finance.revenueRub)
                  : null
              }
            ...
            <FinanceCell
              label="Начислено"
              value={
                finance.accrualsRub != null
                  ? fmtCompact(finance.accrualsRub)
                  : null
              }
              color="text-blue-600"
            ...
            <FinanceCell
              label="Маржа"
              value={
                finance.marginRub != null ? fmtCompact(finance.marginRub) : null
              }
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            {revenue?.attributionStatus && (
              <span
                className={`text-[10px] px-2 py-0.5 rounded-full border font-medium ${ATTR_STATUS_LABEL[revenue.attributionStatus]?.color ?? "bg-gray-50 border-gray-200 text-gray-400"}`}
              >
                {ATTR_STATUS_LABEL[revenue.attributionStatus]?.label ??
                  revenue.attributionStatus}
              </span>
            )}
          </div>
          ...
          {revenue?.warnings && revenue.warnings.length > 0 && (
            <div className="mt-3 space-y-1.5">
              {revenue.warnings.map((w) => (
                <p
                  key={w}
                  className="text-[10px] text-amber-700 bg-amber-50 border border-amber-100 rounded-xl px-3 py-1.5"
                >
                  {WARNING_LABELS[w] ?? w}
                </p>
              ))}
                ...
                {revenue.topGroups.map((g) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0 text-[10px] text-gray-500">
                      ...
                      {g.uniqueStudentsCount > 0 && (
                        <span>{g.uniqueStudentsCount} дет.</span>
                      )}
                    </div>
                  </div>
                ))}
...
function InfoRow({
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2.5">
      ...
      <p className="text-sm text-gray-800 font-medium flex-1 min-w-0 truncate">
        {value}
      </p>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6">
        ...
        {error && (
          <p className="text-sm text-red-500 mt-3 bg-red-50 rounded-xl px-3 py-2">
            {error}
          </p>
        )}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          <p className="text-sm font-bold text-emerald-800 mb-2">
            ✅ Загрузка из AlphaCRM завершена
          </p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
            ...
            <div className="bg-white rounded-xl px-3 py-2 border border-emerald-100">
              ...
              <p
                className={`text-base font-bold ${result.errors > 0 ? "text-red-600" : "text-gray-400"}`}
              >
                {result.errors}
              </p>
            </div>
        ...
        <button
          onClick={onClose}
          className="w-6 h-6 rounded-full bg-white border border-emerald-200 flex items-center justify-center hover:bg-emerald-100 transition-colors shrink-0 mt-0.5"
        >
  ...
  return (
    <div className="mx-4 sm:mx-6 lg:mx-8 mb-4 bg-violet-50 border border-violet-200 rounded-2xl px-4 py-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs mb-2">
            ...
            <div className="bg-white rounded-xl px-3 py-2 border border-violet-100">
              ...
              <p
                className={`text-base font-bold ${result.needs_review > 0 ? "text-amber-600" : "text-gray-400"}`}
              >
                {result.needs_review}
              </p>
            </div>
        ...
        <button
          onClick={onClose}
          className="w-6 h-6 rounded-full bg-white border border-violet-200 flex items-center justify-center hover:bg-violet-100 transition-colors shrink-0 mt-0.5"
        >
          <X className="w-3 h-3 text-gray-400" />
        </button>
      </div>
...
function FilterDrawer({
  ...
  if (!open) return null;
  const selectCls =
    "w-full text-sm bg-white border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-violet-200";
  return (
    <div
        ...
        <label className="flex items-center gap-3 cursor-pointer p-3 bg-gray-50 rounded-2xl">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400">
              Включая технические записи и бывших сотрудников
            </p>
          </div>
        </label>
...
function ConfidenceBadge({
  ...
  return (
    <span
      className={`inline-flex items-center text-[10px] font-semibold px-2 py-0.5 rounded-full ${map[confidence] ?? "bg-gray-100 text-gray-400"}`}
    >
...
function EduTypePill({ type }: { type: string }) {
  ...
  return (
    <span
      className={`text-[10px] font-medium px-1.5 py-0.5 rounded-md border ${m.cls}`}
    >
      {m.label}
    </span>
  );
      ...
      >
        <td className="px-4 py-2.5">
          <div className="flex items-center gap-2">
            <ChevronRight
              className={`w-3 h-3 text-gray-300 shrink-0 transition-transform ${expanded ? "rotate-90" : ""}`}
            />
            <p className="text-xs font-medium text-gray-800 leading-tight">
              {unit.name}
            </p>
        ...
        <td className="px-3 py-2.5 text-right hidden sm:table-cell">
          <span className="text-xs text-gray-500">
            {unit.primary_lessons_count > 0 ? unit.primary_lessons_count : "—"}
          </span>
        </td>
        <td className="px-3 py-2.5 text-right hidden md:table-cell">
          <span className="text-xs text-gray-500">
            {unit.primary_coverage_pct > 0
              ? `${unit.primary_coverage_pct}%`
              : "—"}
          </span>
        </td>
        <td className="px-3 py-2.5">
            ...
            <div className="space-y-1">
              {unit.all_candidates.map((c, i) => (
                <div key={c.employee_id} className="flex items-center gap-3">
                  <span
                    className={`text-[10px] font-bold w-4 ${i === 0 ? "text-violet-500" : "text-gray-300"}`}
                  >
                    #{i + 1}
                  </span>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-50 bg-gray-50/50">
              <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide">
                Группа
              </th>
              <th className="px-3 py-2.5 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide hidden sm:table-cell">
                Тип
              </th>
              <th className="px-3 py-2.5 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide">
                Ответственный
              </th>
              <th className="px-3 py-2.5 text-right text-[11px] font-semibold text-gray-400 uppercase tracking-wide hidden sm:table-cell">
                Уроков
              </th>
              <th className="px-3 py-2.5 text-right text-[11px] font-semibold text-gray-400 uppercase tracking-wide hidden md:table-cell">
                Охват
              </th>
              <th className="px-3 py-2.5 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide">
                Уверенность
          ...
          <tbody>
            {units.map((u) => (
              <UnitResponsibilityRow key={u.educational_unit_id} unit={u} />
            ))}
          </tbody>
        </table>
      </div>
...
type ResponsibilitySection =
  "all" | "kindergarten" | "school" | "club" | "employees";
...
function ResponsibilityTab() {
  ...
  const { data, isLoading, error, refetch, isFetching } =
    useQuery<ResponsibilityAudit>({
      queryKey: ["responsibility-audit"],
      queryFn: () =>
        apiFetch("/api/educational-units/responsibility-audit").then((r) =>
          r.json(),
        ),
      staleTime: 5 * 60 * 1000,
    });
  ...
  const SECTIONS: {
    ...
    { key: "club", label: "Кружки", count: s.club },
    {
      key: "employees",
      label: "Сотрудники",
      count: data.employee_summary.length,
    },
  ];

  const filteredEmployees = empSearch
    ? data.employee_summary.filter((e) =>
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 pb-8 space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">
            Итог · {s.total_units} единиц · данные за {data.lookback_months}{" "}
            мес.
          </p>
          <button
            onClick={() => refetch()}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-2">
          {[
            ...
            },
            {
              label: "— NO DATA",
              value: s.no_data,
              cls: "bg-gray-50 border-gray-100 text-gray-500",
            },
          ].map(({ label, value, cls }) => (
      ...
      <div className="flex gap-1 flex-wrap">
        {SECTIONS.map((sec) => (
          ...
          >
            {sec.label}
            <span
              className={`text-[10px] font-bold px-1 rounded-full ${section === sec.key ? "bg-violet-500 text-white" : "bg-gray-100 text-gray-400"}`}
            >
              {sec.count}
            </span>
          </button>
      ...
      {section === "all" && (
        <div className="space-y-6">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-50 bg-gray-50/50">
                      ...
                      <th className="px-3 py-2.5 text-[11px] font-semibold text-gray-400 uppercase tracking-wide">
                        Уверенность
                      </th>
                    </tr>
                  </thead>
          ...
          <UnitTable
            ...
            title={`Все единицы (${data.all_units.length})`}
          />
        </div>
      )}
      ...
      {section === "kindergarten" && (
        <UnitTable
          units={data.kindergarten}
          title={`Детский сад (${data.kindergarten.length} групп)`}
        />
      )}
      ...
      {section === "school" && (
        <UnitTable
          units={data.school}
          title={`Школа (${data.school.length} классов)`}
        />
      )}
      ...
      {section === "club" && (
        <div className="space-y-4">
          <UnitTable
            units={data.club_top30}
            title={`Кружки — топ-30 по урокам`}
          />
          {data.all_units.filter((u) =>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            {filteredEmployees.slice(0, 80).map((emp) => (
              <div
                key={emp.employee_id}
                className="bg-white rounded-2xl border border-black/[0.06] shadow-[0_1px_6px_rgba(0,0,0,0.04)] px-4 py-3"
              >
            ...
            {filteredEmployees.length > 80 && (
              <p className="text-xs text-gray-400 text-center py-2">
                Показано 80 из {filteredEmployees.length}
              </p>
            )}
          </div>
        </div>
...
export function EmployeesPage() {
  ...
  const [activeTab, setActiveTab] = useState<"staff" | "responsibility">(
    "staff",
  );
  const qc = useQueryClient();
  ...
  const populateMutation = useMutation({
    mutationFn: () =>
      apiFetch("/api/employees/populate-from-crm-teachers", {
        method: "POST",
      }).then((r) => r.json()),
  ...
  const classifyMutation = useMutation({
    mutationFn: () =>
      apiFetch("/api/employees/classify", { method: "POST" }).then((r) =>
        r.json(),
      ),
  ...
  const activeFilterCount =
    [department, employmentType].filter(Boolean).length +
    (status !== "active" ? 1 : 0) +
    (showExcluded ? 1 : 0);
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* Left panel */}
      <div
        className={`flex flex-col flex-1 min-w-0 overflow-hidden ${selectedId ? "hidden lg:flex" : "flex"}`}
      >
        {/* Header */}
        <div className="px-4 sm:px-6 lg:px-8 pt-5 sm:pt-6 pb-4 shrink-0">
          ...
          {activeTab === "staff" && (
            <>
              <div className="hidden sm:grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
                <StatCard
                ...
                <StatCard
                  ...
                  sub={
                    stats
                      ? `реальных · ${stats.needs_review ?? 0} на проверке`
                      : "загрузка..."
                  }
                ...
                <StatCard
                  label="CRM coverage"
                  value={
                    stats ? `${stats.teacher_link_coverage_percent}%` : "—"
                  }
                  sub={
                    stats
                      ? `${stats.employees_with_teacher_crm_id} из ${stats.employees_total}`
                      : "загрузка..."
                  }
              ...
              <div className="flex sm:hidden gap-2 overflow-x-auto pb-2 mb-3 scrollbar-none">
                {[
                  {
                    label: `${stats?.real_employees ?? "—"} реальных`,
                    color: "bg-emerald-50 text-emerald-700 border-emerald-200",
                  },
                  {
                    label: `${stats?.needs_review ?? "—"} проверить`,
                    color: "bg-amber-50 text-amber-700 border-amber-200",
                  },
                  {
                    label: `${stats?.technical_records ?? "—"} техн.`,
                    color: "bg-red-50 text-red-600 border-red-200",
                  },
                  {
                    label: `${stats?.former_employees ?? "—"} бывших`,
                    color: "bg-gray-50 text-gray-500 border-gray-200",
                  },
                  {
                    label: `${stats?.teacher_link_coverage_percent ?? "—"}% CRM`,
                    color: "bg-blue-50 text-blue-700 border-blue-200",
                  },
                ].map(({ label, color }) => (
                  <span
                    key={label}
                    className={`shrink-0 text-[11px] font-medium px-3 py-1.5 rounded-full border whitespace-nowrap ${color}`}
                  >
                    {label}
                  </span>
                ))}
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="hidden sm:flex items-center gap-2 flex-wrap">
                  ...
                  >
                    ...
                    <span>
                      {showExcluded ? "Скрыть архив" : "Показать архив"}
                    </span>
                  </button>
                </div>
              </div>
        ...
        {activeTab === "responsibility" ? (
          ...
          <>
            {/* Result banners */}
            {classifyResult && (
              <ClassifyResultBanner
                result={classifyResult}
                onClose={() => setClassifyResult(null)}
              />
            )}
            {populateResult && (
              <PopulateResultBanner
                result={populateResult}
                onClose={() => setPopulateResult(null)}
              />
            )}
              ...
              {isLoading ? (
                ...
                <div className="bg-white rounded-2xl border border-black/[0.06] shadow-[0_1px_6px_rgba(0,0,0,0.04)] overflow-hidden">
                  ...
                  <div className="hidden sm:grid grid-cols-[2fr_1fr_1fr_auto_auto_auto_auto_auto_auto] gap-3 px-4 py-2.5 border-b border-gray-50 bg-gray-50/50">
                    ...
                    <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide hidden lg:block">
                      Должность
                    </p>
                    <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide">
                      CRM
                    ...
                    <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide text-right">
                      Занятий
                    </p>
                    <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide text-right">
                      Групп
                  ...
                  <div className="px-4 py-2.5 border-t border-gray-50 bg-gray-50/30">
                    <p className="text-[11px] text-gray-400">
                      ...
                      {totalRevenue > 0 &&
                        ` · Выручка по фильтру: ${fmtCompact(totalRevenue)}`}
                      {!showExcluded &&
                        stats &&
                        stats.employees_excluded_from_analytics > 0 && (
                          <span
                            className="ml-2 text-violet-500 cursor-pointer hover:underline"
                            onClick={() => setShowExcluded(true)}
                          >
                            +{stats.employees_excluded_from_analytics} скрыто
                            (архив)
                          </span>
                        )}
                    </p>
