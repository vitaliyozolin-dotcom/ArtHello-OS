...
function useRetiredLegacyAction() {
  return {
    ...
    mutate: (..._args: unknown[]) =>
      toast.error(
        "Legacy-действие удалено. Используйте утверждённый sandbox или scoped workflow.",
      ),
  };
}
...
function EnvironmentPanel() {
  ...
  if (isLoading)
    return <div className="h-16 bg-gray-100 rounded-[12px] animate-pulse" />;
  if (!data) return null;
    ...
    <div className="space-y-2">
      ...
      <div className="bg-gray-950 text-gray-300 rounded-[12px] p-3 font-mono text-[11px]">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span
            className={`ml-auto px-1.5 py-0.5 rounded text-[9px] font-bold ${isProduction ? "bg-red-900 text-red-300" : "bg-amber-900 text-amber-300"}`}
          >
...
function BatchHistory() {
  ...
  if (isLoading)
    return <div className="h-24 bg-gray-100 rounded-[12px] animate-pulse" />;
      ...
      <table className="w-full text-[11px]">
        ...
        <tbody>
          {batches.map((b) => (
            <tr
              key={b.id}
              className="border-b border-gray-100 last:border-0 font-mono"
            >
              <td className="px-3 py-1.5 text-[10px] text-violet-600 font-semibold">
                {b.mode}
              </td>
              <td className="px-3 py-1.5">
                <span
                  className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${BATCH_STATUS_COLORS[b.status ?? ""] ?? "bg-gray-100 text-gray-600"}`}
                >
                  {b.status}
                </span>
              </td>
              ...
              <td className="px-3 py-1.5 text-right tabular-nums">
                {b.endpoints_checked ?? b.entities_requested ?? "—"}
              </td>
              <td className="px-3 py-1.5 text-right tabular-nums">
                {b.total_fetched ?? "—"}
              </td>
              <td className="px-3 py-1.5 text-right tabular-nums text-emerald-600 font-semibold">
                {b.total_saved ?? "—"}
              </td>
...
function FieldInventory({ entityType }: { entityType: string }) {
  const { data, isLoading } = useGetCoverageFieldInventory(
    entityType ? { entityType } : {},
  );
  const inventory =
    (data?.fieldInventory as Record<
      string,
      {
        fieldName: string;
        appearedInRecordsCount: number;
        emptyCount: number;
  ...
  if (entities.length === 0) {
    return (
      <div className="text-center py-6 text-gray-400 text-[12px]">
        Нет данных — сначала запустите Raw Sync
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      <p className="text-[11px] text-gray-500 font-mono">
        entity: <strong className="text-violet-600">{target}</strong> ·{" "}
        {fields.length} fields · {data?.totalRecordsAnalyzed} records analyzed
      </p>
        ...
        <table className="w-full text-[11px]">
          ...
          <tbody>
            {fields.map((f) => (
              ...
              >
                <td className="px-3 py-1.5 font-mono text-[10px] text-blue-700">
                  {f.fieldName}
                </td>
                <td className="px-3 py-1.5 text-right tabular-nums text-gray-700">
                  {f.appearedInRecordsCount}
                </td>
                <td className="px-3 py-1.5 text-right tabular-nums text-gray-400">
                  {f.emptyCount > 0 ? f.emptyCount : "—"}
                </td>
        ...
        <tbody>
          {issues.map((issue) => (
            <tr
              ...
              className="border-b border-gray-100 last:border-0 hover:bg-gray-50/60"
            >
              <td className="px-3 py-1.5 font-mono text-[10px] text-violet-600">
                {issue.entity_type ?? "—"}
              </td>
              <td className="px-3 py-1.5 font-mono text-[10px] text-gray-500">
                {issue.alpha_id ?? "—"}
              </td>
              <td className="px-3 py-1.5 font-mono text-[10px] text-red-600">
                {issue.issue_type ?? "—"}
              </td>
              <td className="px-3 py-1.5">
                <span
                  className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${issue.severity === "critical" ? "bg-red-100 text-red-700" : issue.severity === "warning" ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-500"}`}
                >
                  {issue.severity ?? "info"}
                </span>
              </td>
              ...
              <td className="px-3 py-1.5 text-gray-400 max-w-[160px] truncate text-[10px]">
...
function RawRecordsBrowser() {
  ...
  const { data, isLoading } = useGetCoverageRawRecords(
    applied ? { entityType: applied, limit: 30 } : { limit: 30 },
  );
...
function MetricRow({
  ...
  return (
    <div className="flex items-baseline justify-between py-1 border-b border-gray-100 last:border-0">
      ...
      <span
        className={`text-[11px] font-semibold font-mono ${suspicious ? "text-red-600" : "text-gray-800"}`}
      >
...
function BranchAuditCard({
  ...
  const sections: {
    ...
    {
      key: "students",
      label: "Студенты / Клиенты",
      data: (audit.students as Record<string, unknown>) ?? {},
    },
    {
      key: "lessons",
      label: "Занятия",
      data: (audit.lessons as Record<string, unknown>) ?? {},
    },
    {
      key: "payments",
      label: "Платежи",
      data: (audit.payments as Record<string, unknown>) ?? {},
    },
    {
      key: "subjects",
      label: "Предметы / Направления",
      data: (audit.subjects as Record<string, unknown>) ?? {},
    },
    {
      key: "groups",
      label: "Группы",
      data: (audit.groups as Record<string, unknown>) ?? {},
    },
    {
      key: "subscriptions",
      label: "Абонементы",
      data: (audit.subscriptions as Record<string, unknown>) ?? {},
    },
  ];
          ...
          return (
            <div key={key}>
              ...
              >
                ...
                <span
                  className={`text-[12px] font-semibold flex-1 ${isSuspicious ? "text-red-700" : "text-gray-700"}`}
                >
                  {label}
                </span>
                {isOpen ? (
                ...
                <div className="px-4 pb-3 bg-gray-50/50">
                  {Object.entries(data)
                    ...
                    .map(([k, v]) => (
                      <MetricRow
                        ...
                        suspicious={
                          k.toLowerCase().includes("suspicious") && !!v
                        }
                      />
                    ))}
                    ...
                    <div className="mt-2">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {Object.entries(
                          ...
                          <span
                            key={s}
                            className="px-1.5 py-0.5 bg-gray-200 text-gray-700 rounded text-[9px] font-mono"
                          >
                            {s}: {cnt}
                          </span>
                        ))}
                  ...
                  {key === "subscriptions" && (
                    <div className="space-y-2 mt-1">
                      {(["customerTariffs", "tariffMovements"] as const).map(
                        (sub) => {
                          const subData = data[sub] as
                            Record<string, unknown> | undefined;
                          if (!subData) return null;
                          return (
                            <div
                              key={sub}
                              className="border border-gray-200 rounded-[8px] p-2"
                            >
                              <p className="text-[10px] font-bold text-gray-600 mb-1 font-mono">
                                {sub}
                              </p>
                              {Object.entries(subData)
                                .filter(
                                  ([k]) => !["discoveredFields"].includes(k),
                                )
                                .map(([k, v]) => (
                                  <MetricRow
                                    key={k}
                  ...
                  {key === "subjects" &&
                    !!data.subjectNames &&
                    Object.keys(data.subjectNames as object).length > 0 && (
                      <div className="mt-2">
                        <p className="text-[10px] font-semibold text-gray-500 mb-1">
                          Subjects found:
                        </p>
                        <div className="flex flex-wrap gap-1">
                          {Object.entries(
                            data.subjectNames as Record<string, string>,
                          ).map(([id, name]) => (
                            <span
                              key={id}
                              className="px-1.5 py-0.5 bg-violet-100 text-violet-700 rounded text-[9px] font-mono"
                            >
                              {id}: {name}
                            </span>
                          ))}
...
function VerificationReport({
  ...
  const handleVerify = () => {
    verifyMut.mutate(
      { data: { fromDate, toDate } },
      {
        onSuccess: (d) => {
          toast.success(
            `Верификация запущена (${d.reportId?.slice(0, 8)}…) — займёт 1–2 минуты`,
          );
          setTimeout(
            () =>
              qc.invalidateQueries({
                queryKey: getGetCoverageVerificationReportQueryKey(),
              }),
            15000,
          );
          setTimeout(
  ...
  const branchAudits =
    (reportData?.branchAudits as Record<string, Record<string, unknown>>) ?? {};
  const branchCoverage = reportData?.branchCoverage as
    Record<string, unknown> | undefined;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 p-3 bg-violet-50 rounded-[12px] border border-violet-200">
        ...
        <div className="flex-1">
          ...
          <p className="text-[11px] text-violet-600">
            Проверяет AlphaCRM live: ветки, студентов, занятия, платежи,
            предметы, абонементы. Выявляет подозрительные объёмы и незаполненные
            поля. Занимает 1–2 минуты.
          </p>
        </div>
        <button
          type="button"
        ...
        <div className="flex items-center gap-3 p-4 bg-blue-50 rounded-[12px] border border-blue-200">
          ...
          <div>
            <p className="text-[13px] font-bold text-blue-800">
              Верификация выполняется…
            </p>
            <p className="text-[11px] text-blue-600">
              Опрашиваем AlphaCRM endpoints. Обновится автоматически через ~60
              секунд.
            </p>
          </div>
        </div>
      ...
      {reportData && reportStatus !== "running" && (
        <>
          {/* Overall status */}
          <div
            className={`flex items-center gap-3 p-3 rounded-[12px] border ${overallColor}`}
          >
            {overallStatus === "clean" ? (
            ...
            <div>
              <p className="text-[13px] font-bold">
                ...
                {suspiciousFlags.length > 0 &&
                  ` — ${suspiciousFlags.length} suspicious flags`}
              </p>
              <p className="text-[11px]">
                Аудит: {String(reportData.requestedFromDate ?? "?")} →{" "}
                {String(reportData.requestedToDate ?? "?")} · Сгенерирован:{" "}
                {fmtTs(reportData.generatedAt as string)}
              </p>
            </div>
            <button
              type="button"
          ...
          {env && (
            <div className="bg-gray-950 text-gray-300 rounded-[12px] p-3 font-mono text-[10px]">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-1">
                <span>
                  <span className="text-gray-500">nodeEnv:</span>{" "}
                  {String(env.nodeEnv ?? "—")}
                </span>
                <span>
                  <span className="text-gray-500">isProduction:</span>{" "}
                  <span
                    className={
                      env.isProduction ? "text-red-400" : "text-amber-400"
                    }
                  >
                    {String(env.isProduction)}
                  </span>
                </span>
                <span>
                  <span className="text-gray-500">domain:</span>{" "}
                  {String(env.alfacrmDomain ?? "—")}
                </span>
                <span>
                  <span className="text-gray-500">creds:</span>{" "}
                  {String(env.credentialsSource ?? "—")}
                </span>
              </div>
              {!!env.warningNotProduction && (
                <p className="mt-2 text-amber-400 text-[10px]">
                  {String(env.warningNotProduction)}
                </p>
              )}
            </div>
          )}
          ...
          {branchCoverage && (
            <div className="bg-blue-50 rounded-[12px] border border-blue-200 p-3">
              <p className="text-[12px] font-bold text-blue-900 mb-1">
                Branch Coverage —{" "}
                {String(branchCoverage.branchesDetected ?? "?")} ветка(и)
                обнаружено, {String(branchCoverage.branchesAudited ?? "?")}{" "}
                проверено
              </p>
              <p className="text-[11px] text-blue-700">
                {String(branchCoverage.multibranchNote ?? "")}
              </p>
              {Array.isArray(branchCoverage.branches) && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {(
                    branchCoverage.branches as { id: string; name: string }[]
                  ).map((b) => (
                    <span
                      key={b.id}
                      className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded-lg text-[10px] font-mono border border-blue-200"
                    >
                      [{b.id}] {b.name}
                    </span>
                  ))}
          ...
          {nextActions.length > 0 && (
            <div className="border border-gray-200 rounded-[12px] p-4 bg-gray-50">
              <p className="text-[12px] font-bold text-gray-700 mb-2 flex items-center gap-1.5">
                <Activity className="w-4 h-4 text-gray-500" /> Рекомендуемые
                следующие шаги
              </p>
...
function BranchRow({ branch }: { branch: AlphaBranchDetail }) {
  const cls =
    RECOMMENDED_USE_COLORS[branch.recommendedUse ?? "EXCLUDE_NOW"] ??
    RECOMMENDED_USE_COLORS.EXCLUDE_NOW;
  return (
    <div
      className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-[12px] ${branch.isActiveScope ? "bg-emerald-50 border-emerald-200" : "bg-gray-50 border-gray-150"}`}
    >
        ...
        {branch.rawCounts && !branch.notYetSynced && (
          <div className="flex gap-3 mt-1 text-[10px] text-gray-500">
            ...
            <span className="text-gray-400">
              total: {String(branch.rawCounts.total ?? 0)}
            </span>
          </div>
        )}
      </div>
    ...
    setScopeMut.mutate(
      {
        data: {
          ...
          reason: "Main physical ArtHello location — P6.2 scope lock",
        },
      },
      {
        onSuccess: () => {
          ...
          qc.invalidateQueries({
            queryKey: getGetCoverageAtlasSummaryQueryKey(),
          });
        },
  ...
  if (scopeQ.isLoading || branchesQ.isLoading) {
    return (
      <div className="flex items-center gap-2 text-gray-400 text-[12px]">
        <Loader2 className="w-3.5 h-3.5 animate-spin" /> Загрузка scope...
      </div>
    );
  }
    ...
    <div className="space-y-3">
            ...
            <div>
              <p className="text-[13px] font-bold text-emerald-800">
                ...
                <span className="font-black">
                  {String(scope?.branch_name ?? scope?.branchName ?? "Атлас")}
                </span>
                <span className="ml-1.5 text-[11px] font-normal text-emerald-600">
                  (branchId={String(scope?.branch_id ?? scope?.branchId ?? "6")}
                  )
                </span>
              </p>
              <p className="text-[11px] text-emerald-700 mt-0.5">
                {String(scope?.scope_name ?? scope?.scopeName ?? "Atlas only")}{" "}
                — только этот branch участвует в raw sync и нормализации
              </p>
              {!!(scope?.reason ?? scope?.notes) && (
                <p className="text-[10px] text-emerald-600 mt-1 italic">
                  {String(scope?.reason ?? scope?.notes ?? "")}
                </p>
              )}
            </div>
          </div>
      ...
      {atlasId && (
        <div className="bg-blue-50 border border-blue-200 rounded-[14px] p-3">
          ...
          <p className="text-[11px] text-blue-700">
            <strong>branchId={String(atlasId.atlasBranchId ?? "?")}</strong> ·{" "}
            {String(atlasId.atlasBranchName ?? "?")}
          </p>
          <p className="text-[10px] text-blue-600 mt-0.5">
            {String(atlasId.reason ?? "")}
          </p>
        </div>
      )}
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-[14px] p-3 flex items-start gap-2">
        ...
        <div>
          <p className="text-[12px] font-semibold text-amber-800">
            Scope Lock: Atlas Only
          </p>
          <p className="text-[11px] text-amber-700 mt-0.5">
            Current AlphaCRM audit scope is <strong>Atlas only</strong>. Other
            branches are discovered but excluded from current raw sync and
            normalization. All-branch sync is blocked.
          </p>
        </div>
      </div>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-[14px] p-3">
          <div className="flex items-center gap-1.5 mb-1.5">
            ...
            <span className="text-[11px] font-bold text-gray-600">
              Исключены из текущего scope ({excluded.length} веток)
            </span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {excluded.map((b) => (
              <span
                key={b.branchId}
                className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-gray-200 text-gray-600 text-[10px] font-medium"
              >
          ...
          <p className="text-[10px] text-gray-500 mt-1.5 italic">
            Эти ветки видны в Discovery, но не синхронизируются в основные
            таблицы (families, students, payments).
          </p>
        </div>
      )}
    </div>
...
function ReadinessBadge({ status }: { status: string }) {
  ...
  return (
    <div
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-[12px] font-bold border ${cfg.cls}`}
    >
...
function MiniStat({
  ...
  return (
    <div
      className={`rounded-[10px] p-2.5 border ${highlight && value > 0 ? "bg-emerald-50 border-emerald-200" : "bg-gray-50 border-gray-150"}`}
    >
      <p className="text-[10px] text-gray-500 leading-tight">{label}</p>
      <p
        className={`text-[19px] font-black mt-0.5 leading-tight ${value === 0 ? "text-gray-300" : highlight ? "text-emerald-700" : "text-gray-800"}`}
      >
        {value}
      </p>
    </div>
    ...
    <div className="bg-gray-50 border border-gray-200 rounded-[10px] p-2.5">
      ...
      <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[11px]">
        ...
        <span>
          ...
          {String(batch.total_saved ?? "—")}
        </span>
        <span>
          <span className="text-gray-400">fetched: </span>
          {String(batch.total_fetched ?? "—")}
        </span>
        <span>
          <span className="text-gray-400">at: </span>
          {fmtTs(batch.finished_at as string)}
        </span>
      </div>
    </div>
  );
...
function AtlasSummaryPanel() {
  ...
  const runDiscover = () => {
    discoverMut.mutate(
      ...
      {
        onSuccess: () => {
          toast.success("Atlas Discovery запущен — обновится через ~30 сек");
  ...
  const runSync = () => {
    syncMut.mutate(
      ...
      },
      {
        onSuccess: () => {
          toast.success(
            "Atlas Raw Sync запущен — может занять несколько минут",
          );
          setTimeout(invalidate, 60000);
          setTimeout(invalidate, 180000);
        },
        onError: (e: unknown) => {
  ...
  if (q.isLoading) {
    return (
      <div className="flex items-center gap-2 text-gray-400 text-[12px]">
        <Loader2 className="w-3.5 h-3.5 animate-spin" /> Загрузка Atlas
        summary...
      </div>
    );
  }
  ...
  const epStatus = d.endpointStatusSummary as
    Record<string, number> | undefined;
  const readiness = d.bigAlphaAuditReadiness as
    Record<string, unknown> | undefined;
  ...
  const lastDisc = d.lastDiscoveryBatch as
    Record<string, unknown> | null | undefined;
  const lastSync = d.lastRawSyncBatch as
    Record<string, unknown> | null | undefined;
  ...
  const lsnByMonth =
    (lsnS?.byMonth as { month: string; count: number }[]) ?? [];
  const payByMonth =
    (payS?.byMonth as { month: string; count: number }[]) ?? [];
  ...
  return (
    <div className="space-y-4">
        ...
        <div>
          <p className="text-[14px] font-bold text-gray-900">
            ...
            <span className="ml-1.5 text-[11px] font-normal text-gray-500">
              branchId={String(d.atlasBranchId ?? "6")}
            </span>
          </p>
          <p className="text-[11px] text-gray-500 mt-0.5">
            Scope:{" "}
            <span
              className={`font-semibold ${d.scopeStatus === "ACTIVE" ? "text-emerald-600" : "text-red-500"}`}
            >
              {String(d.scopeStatus ?? "—")}
            </span>
            {d.scopeName ? (
      ...
      {!d.isProduction && (
        <div className="flex items-center gap-2 px-3 py-2 bg-amber-50 border border-amber-200 rounded-[12px]">
          ...
          <p className="text-[11px] text-amber-800">
            ...
            Host:{" "}
            <span className="font-mono">{String(d.dbHostMasked ?? "—")}</span> ·
            AlphaCRM:{" "}
            <span className="font-mono text-[10px]">
              {String(d.alphaCrmBaseUrl ?? "—")}
            </span>
          </p>
        </div>
      )}
      ...
      {(blockers.length > 0 ||
        ...
        >
          <p className="text-[11px] font-bold mb-2 text-gray-700 uppercase tracking-wide">
            Big Alpha Audit Readiness Checklist
          </p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-1 mb-2">
            {Object.entries(checklist).map(([k, v]) => (
              <div key={k} className="flex items-center gap-1 text-[10px]">
                ...
                <span
                  className={v ? "text-gray-600" : "text-red-600 font-semibold"}
                >
                  {k}
                </span>
              </div>
          ...
          {blockers.length > 0 && (
            <div className="space-y-1 mt-2">
              {blockers.map((b, i) => (
                <div
                  key={i}
                  className="flex items-start gap-1 text-[11px] text-red-700"
                >
      ...
      <div>
        <p className="text-[11px] font-bold text-gray-500 uppercase tracking-wide mb-2">
          Raw Records по типу (Atlas branchId=6)
        </p>
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-1.5">
          ...
          <div className="bg-violet-50 border border-violet-200 rounded-[10px] p-2.5">
            ...
            <p className="text-[19px] font-black text-violet-700 mt-0.5">
              {counts?.total ?? 0}
            </p>
          </div>
        </div>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-[14px] p-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-1 text-[11px]">
          <span>
            ...
            <strong>{Number(lsnS?.count ?? 0)}</strong>
          </span>
          <span>
            <span className="text-gray-400">earliest: </span>
            {String(lsnS?.earliest ?? "—")}
          </span>
          <span>
          ...
          <span>
            <span className="text-gray-400">with attendance: </span>
            {Number(lsnS?.lessonsWithVisits ?? 0)}
          </span>
          <span>
            <span className="text-gray-400">visits extracted: </span>
            <strong>{Number(lsnS?.attendanceExtracted ?? 0)}</strong>
          </span>
          <span>
            <span className="text-gray-400">avg visits/lesson: </span>
            {String(lsnS?.avgVisitsPerLesson ?? 0)}
          </span>
          <span>
            <span className="text-gray-400">with subjectId: </span>
            {Number(lsnS?.withSubjectId ?? 0)}
          </span>
          <span>
            <span className="text-gray-400">with groupId: </span>
            {Number(lsnS?.withGroupId ?? 0)}
          </span>
          <span>
            <span className="text-gray-400">with teacherId: </span>
            {Number(lsnS?.withTeacherId ?? 0)}
          </span>
          <span>
            <span className="text-gray-400">cancelled: </span>
            {Number(lsnS?.cancelledCount ?? 0)}
          </span>
          <span className="col-span-2">
            <span className="text-gray-400">dateFilter: </span>
            <span
              className={`font-mono font-bold text-[10px] ${
                String(lsnS?.dateFilterStatus) === "ACCEPTED"
                  ? "text-emerald-600"
                  : String(lsnS?.dateFilterStatus).includes("IGNORED")
                    ? "text-red-600"
                    : "text-gray-500"
              }`}
            >
              {String(lsnS?.dateFilterStatus ?? "UNKNOWN")}
            </span>
          </span>
        </div>
        {lsnByMonth.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {lsnByMonth.map((m) => (
              <span
                key={m.month}
                className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-blue-100 border border-blue-200 rounded text-[10px] text-blue-700 font-mono"
              >
      ...
      <div className="bg-emerald-50 border border-emerald-100 rounded-[14px] p-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-1 text-[11px]">
          <span>
            <span className="text-gray-400">count: </span>
            <strong>{Number(payS?.count ?? 0)}</strong>
          </span>
          <span>
            <span className="text-gray-400">earliest: </span>
            {String(payS?.earliest ?? "—")}
          </span>
          <span>
            <span className="text-gray-400">latest: </span>
            {String(payS?.latest ?? "—")}
          ...
          <span>
            <span className="text-gray-400">with customerId: </span>
            {Number(payS?.withCustomerId ?? 0)}
          </span>
          <span>
            <span className="text-gray-400">without customerId: </span>
            {Number(payS?.withoutCustomerId ?? 0)}
          </span>
          <span>
            <span className="text-gray-400">has note field: </span>
            <span
              className={
                payS?.hasNoteField ? "text-emerald-600" : "text-gray-400"
              }
            >
              {payS?.hasNoteField ? "✓" : "✗"}
            </span>
          ...
          <span>
            <span className="text-gray-400">has balance field: </span>
            <span
              className={
                payS?.hasBalanceField ? "text-emerald-600" : "text-gray-400"
              }
            >
              {payS?.hasBalanceField ? "✓" : "✗"}
            </span>
          </span>
          <span>
            <span className="text-gray-400">has debt field: </span>
            <span
              className={
                payS?.hasDebtField ? "text-emerald-600" : "text-gray-400"
              }
            >
              {payS?.hasDebtField ? "✓" : "✗"}
            </span>
        ...
        {payByMonth.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {payByMonth.map((m) => (
              <span
                key={m.month}
                className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-emerald-100 border border-emerald-200 rounded text-[10px] text-emerald-700 font-mono"
              >
                {m.month}: <strong>{m.count}</strong>
              </span>
            ))}
        ...
        <div className="bg-purple-50 border border-purple-100 rounded-[14px] p-3">
          ...
          <div className="space-y-1 text-[11px]">
            <div className="flex justify-between">
              <span className="text-gray-500">total in DB:</span>{" "}
              <strong>{Number(subS?.count ?? 0)}</strong>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">refs from lessons:</span>{" "}
              <strong>{Number(subS?.referencedByLessons ?? 0)}</strong>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">resolved:</span>{" "}
              <span className="text-emerald-600 font-bold">
                {Number(subS?.resolved ?? 0)}
              </span>
            </div>
            <div className="flex justify-between">
        ...
        <div className="bg-orange-50 border border-orange-100 rounded-[14px] p-3">
          ...
          <div className="space-y-1 text-[11px]">
            <div className="flex justify-between">
              ...
              <StatusBadge
                status={String(ctS?.endpointStatus ?? "NOT_DISCOVERED")}
              />
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">records:</span>{" "}
              <strong>{Number(ctS?.count ?? 0)}</strong>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">abonement data:</span>
              <span
                className={
                  ctS?.hasAbonementData
          ...
          {(ctS?.abonementFieldsFound as string[] | undefined)?.length ? (
            <div className="flex flex-wrap gap-1 mt-1.5">
              {(ctS?.abonementFieldsFound as string[]).map((f) => (
                <span
                  key={f}
                  className="px-1 py-0.5 bg-orange-100 rounded text-[9px] font-mono text-orange-700"
                >
                  {f}
                </span>
              ))}
            ...
            <p className="text-[10px] text-gray-400 italic mt-1">
              {String(ctS?.endpointStatus ?? "") === "NOT_DISCOVERED"
                ? "Run Discovery to probe endpoint"
                : "No abonement fields found"}
            </p>
          )}
        </div>
      </div>
      ...
      {(registry.length > 0 || epStatus) && (
        <div>
          ...
          {epStatus && (
            <div className="flex flex-wrap gap-1.5 mb-2">
              {[
                ...
                .filter(({ k }) => (epStatus[k] ?? 0) > 0)
                .map(({ k, label, cls }) => (
                  <span
                    key={k}
                    className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg border text-[10px] font-bold ${cls}`}
                  >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-1">
            {registry.map((r) => (
              <div
                key={String(r.entityKey)}
                className="flex items-center justify-between px-2 py-1 rounded-lg bg-gray-50 border border-gray-150"
              >
      ...
      {unresolved.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-[12px] p-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {unresolved.map((u) => (
              <span
                key={String(u.entityKey)}
                className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-amber-100 text-amber-700 text-[10px] border border-amber-200"
              >
                {String(u.entityKey)}{" "}
...
function AuditStatCard({
  ...
  return (
    <div className="bg-white rounded-[14px] border border-black/[0.06] p-3 flex flex-col gap-1">
      ...
      <div
        className={`text-[22px] font-bold tabular-nums ${color ?? "text-[#0F172A]"}`}
      >
  ...
  const color =
    rate >= 80
      ...
      : rate >= 30
        ...
        : "text-red-500";
  const bg =
    rate >= 80 ? "bg-emerald-500" : rate >= 30 ? "bg-amber-400" : "bg-red-500";
  return (
    <tr className="border-t border-gray-100 hover:bg-gray-50">
      ...
      <td className="px-3 py-2 text-right tabular-nums text-[12px]">
        {(row.rawCount ?? 0).toLocaleString("ru")}
      </td>
      <td className="px-3 py-2 text-right tabular-nums text-[12px]">
        {row.normalizedCount != null
          ? row.normalizedCount.toLocaleString("ru")
          : "—"}
      </td>
      <td className="px-3 py-2 text-right tabular-nums text-[12px] text-red-500 font-semibold">
        {row.notNormalized != null && row.notNormalized > 0
          ? row.notNormalized.toLocaleString("ru")
          : "—"}
      </td>
...
function NormalizationAuditPanel({ branchId }: { branchId: string }) {
  const { data, isLoading, error, refetch } = useGetCoverageNormalizationAudit(
    ...
    {
      query: {
        queryKey: getGetCoverageNormalizationAuditQueryKey({ branchId }),
        staleTime: 60_000,
      },
    },
  );
  ...
  if (isLoading)
    return (
      <div className="flex items-center gap-2 text-[12px] text-gray-400 py-6">
        ...
        Загрузка аудита нормализации…
      </div>
    );
  if (error || !data)
    return (
      <div className="text-[12px] text-red-500 py-4">
        Ошибка загрузки.{" "}
  ...
  const verdict = (d.verdict as { critical: string[]; warning: string[] }) ?? {
    critical: [],
    warning: [],
  };
    ...
    <div className="space-y-5">
      {verdict.critical.length > 0 && (
        <div className="rounded-[14px] border border-red-200 bg-red-50 p-4 space-y-1">
          <div className="text-[12px] font-semibold text-red-700 flex items-center gap-1.5">
            <AlertTriangle className="w-3.5 h-3.5" /> Критические проблемы
            нормализации
          </div>
          {verdict.critical.map((msg, i) => (
            <div key={i} className="text-[11px] text-red-600 pl-5">
              {msg}
            </div>
          ))}
        </div>
      ...
      {verdict.warning.length > 0 && (
        <div className="rounded-[14px] border border-amber-200 bg-amber-50 p-4 space-y-1">
          <div className="text-[12px] font-semibold text-amber-700 flex items-center gap-1.5">
            <AlertCircle className="w-3.5 h-3.5" /> Предупреждения
          </div>
          {verdict.warning.map((msg, i) => (
            <div key={i} className="text-[11px] text-amber-600 pl-5">
              {msg}
            </div>
          ))}
        </div>
      ...
      <div>
        ...
        <div className="bg-white rounded-[14px] border border-black/[0.06] overflow-hidden">
          <table className="w-full text-[11px]">
            ...
            <tbody>
              {gap.map((row) => (
                <NormGapRow key={row.entityType} row={row} />
              ))}
            </tbody>
          </table>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <AuditStatCard
          ...
          value={identity.studentProfiles ?? 0}
          sub="identity layer"
        />
        <AuditStatCard
          label="Families"
          value={identity.families ?? 0}
          sub="identity layer"
        />
        <AuditStatCard
          label="Persons"
          value={identity.persons ?? 0}
          sub="identity layer"
        />
        <AuditStatCard
          label="Guardian Links"
          value={identity.guardianStudentLinks ?? 0}
          sub="identity layer"
        />
      </div>
      ...
      <div>
        <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-2">
          Статус студентов в сырых данных
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <AuditStatCard
            label="is_study = 1"
            value={fromRaw.is_study_yes ?? "—"}
            sub="активно учатся"
            color="text-emerald-600"
          />
          <AuditStatCard
            label="is_study = 0"
            value={fromRaw.is_study_no ?? "—"}
            sub="не учатся"
            color="text-amber-600"
          />
          <AuditStatCard
            label="is_archived = 1"
            value={fromRaw.is_archived ?? "—"}
            sub="в архиве"
            color="text-red-500"
          />
          <AuditStatCard
            label="Не архивированы"
            value={fromRaw.not_archived ?? "—"}
            sub="активные"
          />
        </div>
      </div>
      ...
      <div>
        <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-2">
          Отсутствующие критические ссылки
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[
            {
              key: "rawStudentsNotNormalized",
              label: "Raw students → не нормализованы",
            },
            {
              key: "rawLessonsNotNormalized",
              label: "Raw lessons → не нормализованы",
            },
            {
              key: "rawPaymentsNotNormalized",
              label: "Raw payments → не нормализованы",
            },
            {
              key: "studentsWithoutFamily",
              label: "Студенты без семьи (profiles)",
            },
            {
              key: "studentsWithoutPerson",
              label: "Студенты без Person записи",
            },
            ...
            return (
              <AuditStatCard
                ...
                label={label}
                value={val}
                color={val > 0 ? "text-red-500" : "text-emerald-600"}
              />
            );
          })}
        </div>
      ...
      {(missing.rawStudentsNotNormalizedList as unknown[])?.length > 0 && (
        <div>
          <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-2">
            Студенты в raw, но не нормализованы
          </div>
          <div className="bg-white rounded-[14px] border border-black/[0.06] overflow-hidden">
            <table className="w-full text-[11px]">
              <thead>
              ...
              <tbody>
                {(
                  missing.rawStudentsNotNormalizedList as Array<
      ...
      <div className="text-[10px] text-gray-400 italic">
        Сгенерировано: {String(d.generatedAt ?? "—")}
      </div>
    </div>
  );
}
...
function EntityReconciliationPanel({ branchId }: { branchId: string }) {
  const { data, isLoading, error, refetch } =
    useGetCoverageEntityReconciliation(
      ...
      {
        query: {
          queryKey: getGetCoverageEntityReconciliationQueryKey({ branchId }),
          staleTime: 60_000,
        },
      },
    );
  ...
  return (
    <div className="space-y-5">
      <Section title="Студенты (Children)">
        <AuditStatCard
          label="Raw (AlphaCRM)"
          value={Number(students.raw ?? 0)}
          sub="alpha_raw_records"
        />
        <AuditStatCard
          label="Normalized"
          value={Number(students.normalized ?? 0)}
          sub="crm_students"
          color={
            Number(students.normalized ?? 0) < Number(students.raw ?? 1)
              ? "text-amber-600"
              : "text-emerald-600"
          }
        />
        <AuditStatCard
          label="Student Profiles"
          value={Number(students.studentProfiles ?? 0)}
          sub="identity layer"
        />
        <AuditStatCard
          label="В семьях"
          value={Number(students.inFamilies ?? 0)}
          sub="family_id IS NOT NULL"
        />
        <AuditStatCard
          label="Не нормализовано"
          value={Number(students.notNormalized ?? 0)}
          color={
            Number(students.notNormalized ?? 0) > 0
              ? "text-red-500"
              : "text-emerald-600"
          }
        />
        <AuditStatCard
          label="Уник. в уроках"
          value={Number(students.uniqueStudentIdsInLessons ?? 0)}
          sub="по customer_id"
        />
        <AuditStatCard
          label="Уник. в платежах"
          value={Number(students.uniqueStudentIdsInPayments ?? 0)}
          sub="по customer_id"
        />
      </Section>
      ...
      <Section title="Семьи">
        <AuditStatCard
          label="Всего семей"
          value={Number(families.total ?? 0)}
        />
        <AuditStatCard
          label="С детьми"
          value={Number(families.withChildren ?? 0)}
          color="text-emerald-600"
        />
        <AuditStatCard
          label="Без детей"
          value={Number(families.withoutChildren ?? 0)}
          color={
            Number(families.withoutChildren ?? 0) > 0
              ? "text-amber-600"
              : "text-emerald-600"
          }
        />
        <AuditStatCard
          label="Только неактивные дети"
          value={Number(families.withOnlyInactiveChildren ?? 0)}
          color={
            Number(families.withOnlyInactiveChildren ?? 0) > 0
              ? "text-amber-600"
              : "text-emerald-600"
          }
        />
      ...
      <Section title="Уроки">
        <AuditStatCard
          label="Raw"
          value={Number(lessons.raw ?? 0)}
          sub="alpha_raw_records"
        />
        <AuditStatCard
          label="Normalized"
          value={Number(lessons.normalized ?? 0)}
          sub="crm_lessons"
          color={
            Number(lessons.normalizationRate ?? 0) < 10
              ? "text-red-500"
              : "text-amber-600"
          }
        />
        <AuditStatCard
          label="Не нормализовано"
          value={Number(lessons.notNormalized ?? 0)}
          color="text-red-500"
        />
        <AuditStatCard
          label="Rate"
          value={`${lessons.normalizationRate ?? 0}%`}
          color={
            Number(lessons.normalizationRate ?? 0) < 10
              ? "text-red-500"
              : "text-amber-600"
        ...
        <AuditStatCard
          label="Без группы"
          value={Number(lessons.withoutGroup ?? 0)}
          color={
            Number(lessons.withoutGroup ?? 0) > 0
              ? "text-amber-600"
              : "text-emerald-600"
          }
        />
        <AuditStatCard
          label="Без предмета"
          value={Number(lessons.withoutSubject ?? 0)}
          color={
            Number(lessons.withoutSubject ?? 0) > 0
              ? "text-amber-600"
              : "text-emerald-600"
          }
        />
        <AuditStatCard
          label="Без учителя"
          value={Number(lessons.withoutTeacher ?? 0)}
          color={
            Number(lessons.withoutTeacher ?? 0) > 0
              ? "text-amber-600"
              : "text-emerald-600"
          }
        />
        <AuditStatCard
          label="Attendance записи"
          value={Number(lessons.attendanceRecords ?? 0)}
        />
      </Section>
      ...
      <Section title="Платежи">
        <AuditStatCard
          label="Raw"
          value={Number(payments.raw ?? 0)}
          sub="alpha_raw_records"
        />
        <AuditStatCard
          label="Normalized"
          value={Number(payments.normalized ?? 0)}
          sub="crm_payments"
          color={
            Number(payments.normalizationRate ?? 0) < 10
              ? "text-red-500"
              : "text-amber-600"
          }
        />
        <AuditStatCard
          label="Не нормализовано"
          value={Number(payments.notNormalized ?? 0)}
          color="text-red-500"
        />
        <AuditStatCard
          label="Rate"
          value={`${payments.normalizationRate ?? 0}%`}
          color={
            Number(payments.normalizationRate ?? 0) < 10
              ? "text-red-500"
              : "text-amber-600"
        ...
        />
        <AuditStatCard
          label="Привязаны к студенту"
          value={Number(payments.linkedToStudent ?? 0)}
          color="text-emerald-600"
        />
        <AuditStatCard
          label="Без customer_id"
          value={Number(payments.unlinkedFromCustomer ?? 0)}
          color={
            Number(payments.unlinkedFromCustomer ?? 0) > 0
              ? "text-red-500"
              : "text-emerald-600"
          }
        />
        <AuditStatCard
          label="Сумма income (raw)"
          value={Math.round(
            Number(payments.totalIncomeSum ?? 0),
          ).toLocaleString("ru")}
          sub="₽ AlphaCRM"
        />
      </Section>
      <div className="rounded-[10px] bg-amber-50 border border-amber-200 px-4 py-2 text-[11px] text-amber-700">
        ⚠️ AlphaCRM payments = операционные записи.{" "}
        <strong>Не являются банковской истиной.</strong> Требуется сверка с
        банком.
      </div>
      ...
      <Section title="Группы / Предметы / Учителя">
        ...
        <AuditStatCard
          label="Группы с предметом"
          value={Number(groups.withSubject ?? 0)}
          color={
            Number(groups.withSubject ?? 0) > 0
              ? "text-emerald-600"
              : "text-red-500"
          }
        />
        <AuditStatCard
          label="Группы с учителем"
          value={Number(groups.withTeacher ?? 0)}
          color={
            Number(groups.withTeacher ?? 0) > 0
              ? "text-emerald-600"
              : "text-red-500"
          }
        />
        ...
        <AuditStatCard
          label="Предметы в уроках"
          value={Number(subjects.referencedByLessons ?? 0)}
        />
        <AuditStatCard
          label="Неразрешённые subj."
          value={Number(subjects.unresolvedInLessons ?? 0)}
          color={
            Number(subjects.unresolvedInLessons ?? 0) > 0
              ? "text-red-500"
              : "text-emerald-600"
          }
        />
        ...
        <AuditStatCard
          label="Учителя normalized"
          value={Number(teachers.normalized ?? 0)}
        />
      </Section>
        ...
        <div>
          ...
          <div className="bg-white rounded-[14px] border border-black/[0.06] overflow-x-auto">
            <table className="w-full text-[11px]">
              ...
              <tbody>
                {(lessons.byMonth as Array<{ month: string; cnt: string }>).map(
                  (row, i) => (
                    <tr key={i} className="border-t border-gray-100">
                      <td className="px-3 py-1.5 font-mono">{row.month}</td>
                      <td className="px-3 py-1.5 text-right tabular-nums font-semibold">
                        {Number(row.cnt).toLocaleString("ru")}
                      </td>
                    </tr>
                  ),
                )}
              ...
              <tbody>
                {(
                  ...
                  <tr key={i} className="border-t border-gray-100">
                    ...
                    <td className="px-3 py-1.5 text-right tabular-nums font-semibold">
                      {Math.round(Number(row.income_sum)).toLocaleString("ru")}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {(payments.suspiciousHigh as unknown[])?.length > 0 && (
        <div>
          <div className="text-[11px] font-semibold text-red-500 uppercase tracking-wide mb-2">
            ⚠️ Подозрительно высокие платежи (&gt;100к)
          </div>
          <div className="bg-white rounded-[14px] border border-red-100 overflow-x-auto">
            <table className="w-full text-[11px]">
              <thead>
                <tr className="bg-red-50 border-b border-red-100">
                  <th className="px-3 py-2 text-left font-medium text-red-400">
                    Alpha ID
                  </th>
              ...
              <tbody>
                {(payments.suspiciousHigh as Array<Record<string, string>>).map(
                  (r, i) => (
                    <tr key={i} className="border-t border-red-50">
                      <td className="px-3 py-1.5 font-mono">{r.alpha_id}</td>
                      <td className="px-3 py-1.5 text-right tabular-nums text-red-600 font-bold">
                        {Number(r.income).toLocaleString("ru")}
                      </td>
...
function PaymentAuditPanel({ branchId }: { branchId: string }) {
  ...
  const { data, isLoading, error, refetch } = useGetCoveragePaymentTruthAudit(
    ...
    {
      query: {
        queryKey: getGetCoveragePaymentTruthAuditQueryKey({ branchId }),
        staleTime: 60_000,
      },
    },
  );
  ...
  const types =
    (audit?.typeBreakdown as Array<Record<string, unknown>> | undefined) ?? [];
  const monthly =
    (audit?.monthly as Array<Record<string, unknown>> | undefined) ?? [];
  const suspicious = audit?.suspicious as Record<string, unknown> | undefined;
  const topUnlinked =
    (audit?.topUnlinkedCustomerIds as
  ...
  if (isLoading)
    return (
      <div className="py-6 text-center text-gray-400 text-sm">
        <Loader2 className="inline animate-spin mr-1 w-4 h-4" />
        Загрузка payment audit…
      </div>
    );
  if (error)
    return (
      <div className="py-4 text-red-500 text-sm">Ошибка: {String(error)}</div>
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-[12px] text-amber-800 flex gap-2 items-start">
        ...
        <span>
          <strong>Внимание:</strong> Это операционные платежи из AlphaCRM — НЕ
          банковская истина. Банковская сверка выполняется отдельно в модуле
          «Деньги».
        </span>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        ...
        <div className="grid grid-cols-3 gap-3 mb-3">
          ...
          <div className="bg-indigo-50 rounded-lg p-3 text-center">
            <div className="text-[20px] font-bold text-indigo-700">
              {fmt(normCount)}
            </div>
            <div className="text-[11px] text-gray-500">Нормализовано</div>
          </div>
          <div
            className={`rounded-lg p-3 text-center ${normRate >= 99 ? "bg-green-50" : "bg-amber-50"}`}
          >
            <div
              className={`text-[20px] font-bold ${normRate >= 99 ? "text-green-700" : "text-amber-700"}`}
            >
              {normRate}%
            </div>
        ...
        <div className="grid grid-cols-3 gap-2 text-[11px]">
          <div className="text-gray-500">
            ...
            <span className="font-semibold text-red-600">
              {Number(norm?.parseErrors ?? 0)}
            </span>
          </div>
          <div className="text-gray-500">
            Не подтверждено:{" "}
            <span className="font-semibold text-orange-600">
              {fmt(Number(norm?.unconfirmed ?? 0))}
            </span>
          </div>
          <div className="text-gray-500">
            Пробел raw-norm:{" "}
            <span className="font-semibold">{fmt(rawCount - normCount)}</span>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-blue-50 rounded-lg p-3 text-center">
              <div className="text-[16px] font-bold text-blue-700">
                {fmtRub(Number(totals.correctionSum ?? 0))}
              </div>
              <div className="text-[11px] text-gray-500">
                Корректировки (pay_type=6)
              </div>
            </div>
            <div className="bg-red-50 rounded-lg p-3 text-center">
              <div className="text-[16px] font-bold text-red-700">
                {fmtRub(Number(totals.outcomeSum ?? 0))}
              </div>
              <div className="text-[11px] text-gray-500">
                Инкассация (pay_type=12)
              </div>
            </div>
          </div>
          <div className="mt-3 grid grid-cols-3 gap-2 text-[11px] text-gray-500">
            <div>
              Отрицательных:{" "}
              <span className="font-semibold text-red-600">
                {fmt(Number(totals.negativeCount ?? 0))}
              </span>
            </div>
            <div>
              Подозрительных (&gt;100K):{" "}
              <span className="font-semibold text-orange-600">
                {Number(totals.suspiciousHigh ?? 0)}
              </span>
            </div>
            <div>
              Период: {String(totals.earliestDate ?? "?")} →{" "}
              {String(totals.latestDate ?? "?")}
            </div>
          </div>
        </div>
      )}
          ...
          <table className="w-full text-[11px]">
            <thead>
              <tr className="border-b border-gray-100 text-gray-500">
                <th className="text-left pb-1">Тип</th>
                <th className="text-right pb-1">ID</th>
                <th className="text-right pb-1">Кол-во</th>
                <th className="text-right pb-1">Сумма дохода</th>
                <th className="text-right pb-1">Сумма расхода</th>
              </tr>
            </thead>
            <tbody>
              {types.map((t, i) => (
                <tr key={i} className="border-b border-gray-50">
                  <td className="py-1 font-medium">
                    {String(t.type_normalized ?? "?")}
                  </td>
                  <td className="py-1 text-right text-gray-500">
                    {String(t.type_id_raw ?? "?")} /{" "}
                    {String(t.type_name_raw ?? "?")}
                  </td>
                  <td className="py-1 text-right font-semibold">
                    {fmt(Number(t.cnt ?? 0))}
                  </td>
                  <td className="py-1 text-right text-green-700">
                    {fmtRub(Number(t.income_sum ?? 0))}
                  </td>
                  <td className="py-1 text-right text-red-700">
                    {fmtRub(Number(t.outcome_sum ?? 0))}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {links && (
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <div className="text-[13px] font-semibold text-gray-800 mb-3">
            Привязка к студентам / семьям
          </div>
          <div className="grid grid-cols-3 gap-2">
            {[
              {
                label: "Активный студент",
                val: Number(links.activeStudent ?? 0),
                color: "bg-green-50 text-green-700",
              },
              {
                label: "Неактивный студент",
                val: Number(links.inactiveStudent ?? 0),
                color: "bg-yellow-50 text-yellow-700",
              },
              {
                label: "Историческая идентичность",
                val: Number(links.historicalOnly ?? 0),
                color: "bg-blue-50 text-blue-700",
              },
              {
                label: "С семьёй",
                val: Number(links.withFamily ?? 0),
                color: "bg-teal-50 text-teal-700",
              },
              {
                label: "Без привязки",
                val: Number(links.unlinked ?? 0),
                color: "bg-red-50 text-red-700",
              },
              {
                label: "Без customer_id",
                val: Number(links.noCustomerId ?? 0),
                color: "bg-gray-50 text-gray-600",
              },
            ].map(({ label, val, color }) => (
              <div
                key={label}
                className={`${color} rounded-lg p-2.5 text-center`}
              >
          ...
          {topUnlinked.length > 0 && (
            <div className="mt-3">
              <div className="text-[11px] font-semibold text-gray-600 mb-1">
                Топ customer_id без привязки:
              </div>
              <div className="flex flex-wrap gap-1">
                {topUnlinked.map((r, i) => (
                  <span
                    key={i}
                    className="bg-gray-100 text-gray-700 text-[10px] px-2 py-0.5 rounded"
                  >
                    {String(r.student_crm_id ?? r.customer_id ?? "?")} (
                    {String(r.cnt ?? "?")} пл.)
                  </span>
                ))}
              </div>
      ...
      {Number(suspicious?.count ?? 0) > 0 && (
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
          <div className="text-[13px] font-semibold text-orange-800 mb-2">
            ⚠️ Подозрительные суммы ({String(suspicious?.count)} записей
            &gt;100,000 ₽)
          </div>
          <div className="space-y-1">
            {((suspicious?.examples as Array<Record<string, unknown>>) ?? [])
              .slice(0, 5)
              .map((p, i) => (
                <div key={i} className="flex gap-2 text-[11px] text-orange-900">
                  <span className="font-mono text-orange-600">
                    ID:{String(p.crm_id ?? p.alpha_id ?? "?")}
                  </span>
                  <span>{String(p.date ?? p.document_date ?? "?")}</span>
                  <span className="font-semibold">
                    {fmtRub(Number(p.income ?? p.outcome ?? 0))}
                  </span>
                  ...
                  <span className="text-orange-400">
                    customer:
                    {String(p.customer_id ?? p.student_crm_id ?? "нет")}
                  </span>
                </div>
              ))}
          </div>
                  ...
                  {monthly.map((m, i) => (
                    <tr key={i} className="border-b border-gray-50">
                      ...
                      <td className="py-0.5 text-right">
                        {fmt(Number(m.total_payments ?? 0))}
                      </td>
                      <td className="py-0.5 text-right text-green-700">
                        {fmt(Number(m.income_cnt ?? 0))}
                      </td>
                      <td className="py-0.5 text-right text-blue-700">
                        {fmt(Number(m.correction_cnt ?? 0))}
                      </td>
                      <td className="py-0.5 text-right text-red-600">
                        {fmt(Number(m.outcome_cnt ?? 0))}
                      </td>
                      <td className="py-0.5 text-right font-medium">
                        {fmtRub(Number(m.income_sum ?? 0))}
                      </td>
                      <td className="py-0.5 text-right text-gray-500">
                        {fmt(Number(m.linked_student ?? 0))} /{" "}
                        {fmt(Number(m.linked_identity ?? 0))}
                      </td>
                    </tr>
                  ))}
                </tbody>
...
function PaymentCleanupPanel({ branchId }: { branchId: string }) {
  ...
  const { data, isLoading, error, refetch } = useGetCoveragePaymentCleanupAudit(
    ...
    {
      query: {
        queryKey: getGetCoveragePaymentCleanupAuditQueryKey({ branchId }),
        staleTime: 60_000,
      },
    },
  );
  ...
  const crf =
    (d?.correctionsRefundsOutcomes as
      Array<Record<string, unknown>> | undefined) ?? [];
  const unlinkedClass =
    (unl?.classification as Array<Record<string, unknown>> | undefined) ?? [];
  const byType =
    (susp?.byType as Array<Record<string, unknown>> | undefined) ?? [];
  const examples =
    (susp?.examples as Array<Record<string, unknown>> | undefined) ?? [];
  ...
  const readinessIcon =
    readiness === "READY" ? "✅" : readiness === "PARTIAL" ? "⚠️" : "🔴";

  if (isLoading)
    return (
      <div className="py-6 text-center text-gray-400 text-sm">
        <Loader2 className="inline animate-spin mr-1 w-4 h-4" />
        Загрузка cleanup audit…
      </div>
    );
  if (error)
    return (
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {cleanupMut.isPending
            ? "Очистка…"
            : "Запустить P7.6 cleanup (идемпотентно)"}
        </button>
        <button
          onClick={() => void refetch()}
          className="text-[12px] text-gray-500 hover:text-gray-700 flex items-center gap-1"
      ...
      {!cleanupRan && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-[12px] text-gray-600">
          Cleanup ещё не запускался. Нажмите кнопку выше, чтобы проставить
          risk-флаги, перепривязать платежи и создать issues.
        </div>
      )}
      ...
      {cleanupRan && (
        <div className={`border rounded-lg p-3 text-[12px] ${readinessColor}`}>
          <div className="font-semibold mb-1">
            {readinessIcon} Готовность к банковской сверке:{" "}
            <span className="uppercase">{readiness}</span>
          </div>
          {reasons.map((r, i) => (
          ...
          {[
            {
              ...
              val: Number(cnt.unknownCnt ?? 0),
              color:
                Number(cnt.unknownCnt ?? 0) === 0
                  ? "bg-green-50 text-green-700"
                  : "bg-red-50 text-red-700",
            },
            {
              label: "Непривязанных",
              val: Number(cnt.unlinkedCnt ?? 0),
              color: "bg-amber-50 text-amber-700",
            },
            {
              label: "Risk: HIGH",
              val: Number(cnt.riskHigh ?? 0),
              color: "bg-red-50 text-red-700",
            },
            {
              label: "Risk: LOW",
              val: Number(cnt.riskLow ?? 0),
              color: "bg-green-50 text-green-700",
            },
          ].map(({ label, val, color }) => (
      ...
      {unlinkedClass.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <div className="text-[13px] font-semibold text-gray-800 mb-3">
            🔗 Непривязанные платежи — классификация (
            {fmt(Number(cnt?.unlinkedCnt ?? 0))})
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-[11px]">
              <thead>
              ...
              <tbody>
                {unlinkedClass.map((r, i) => (
                  <tr key={i} className="border-b border-gray-50">
                    <td className="py-1 font-mono text-[10px] text-gray-700">
                      {String(r.class ?? "")}
                    </td>
                    ...
                    <td className="py-1 text-right text-green-600">
                      {Number(r.sum_income ?? 0) > 0
                        ? fmtRub(Number(r.sum_income))
                        : "—"}
                    </td>
                    <td className="py-1 text-right text-red-600">
                      {Number(r.sum_outcome ?? 0) > 0
                        ? fmtRub(Number(r.sum_outcome))
                        : "—"}
                    </td>
                  </tr>
                ))}
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            <div className="text-[13px] font-semibold text-red-800">
              🏦 Инкассация / коллекция — HIGH RISK (
              {fmt(Number(col.totalCount ?? 0))} записей,{" "}
              {fmtRub(Number(col.totalAmount ?? 0))})
            </div>
            <button
          ...
          <div className="flex gap-3 text-[11px]">
            <span className="bg-red-100 text-red-800 rounded px-2 py-0.5">
              Риск: {String(col.riskLevel ?? "")}
            </span>
            <span className="bg-red-100 text-red-800 rounded px-2 py-0.5">
              Трактовка: {String(col.financeTreatment ?? "")}
            </span>
            <span className="bg-amber-100 text-amber-800 rounded px-2 py-0.5">
              {String(col.recommendation ?? "")}
            </span>
          </div>
          {showCollection && (
                ...
                <tbody>
                  {(
                    ...
                    <tr key={i} className="border-b border-red-100">
                      ...
                      <td className="py-0.5 text-right">
                        {fmtRub(Number(r.total_outcome ?? 0))}
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {crf.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <div className="text-[13px] font-semibold text-gray-800 mb-3">
            📋 Корректировки / возвраты / инкассация
          </div>
          <table className="w-full text-[11px]">
            <thead>
              <tr className="border-b border-gray-100">
              ...
              {crf.map((r, i) => (
                <tr key={i} className="border-b border-gray-50">
                  <td className="py-1 font-semibold text-gray-700">
                    {String(r.payment_type_normalized ?? "")}
                  </td>
                  ...
                  <td className="py-1 text-right text-gray-500">
                    {fmt(
                      Number(r.linked_student ?? 0) +
                        Number(r.linked_identity ?? 0),
                    )}
                  </td>
                  <td className="py-1 text-right text-amber-600">
                    {fmt(Number(r.unlinked ?? 0))}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {byType.map((r, i) => (
              <div
                key={i}
                className="text-[10px] bg-gray-100 rounded px-2 py-1"
              >
          ...
          {showIssues && examples.length > 0 && (
            <div className="space-y-1">
              {examples.map((e, i) => (
                <div
                  key={i}
                  className="text-[10px] bg-amber-50 border border-amber-100 rounded p-2"
                >
                  <span className="font-mono text-amber-700">
                    [{String(e.issue_type ?? "")}]
                  </span>
                  <span className="text-gray-600 ml-1">
                    {String(e.issue_message ?? "").slice(0, 100)}
                  </span>
                </div>
              ))}
            </div>
...
function AttendanceIdentityPanel({ branchId }: { branchId: string }) {
  ...
  const { data, isLoading, error, refetch } =
    useGetCoverageAttendanceStudentIdentity(
      ...
      {
        query: {
          queryKey: getGetCoverageAttendanceStudentIdentityQueryKey({
            branchId,
          }),
          staleTime: 60_000,
        },
  ...
  const {
    data: branchCov,
    isLoading: branchCovLoading,
    refetch: refetchBranchCov,
  } = useGetCoverageCustomerRawBranchCoverage({
    query: {
      queryKey: getGetCoverageCustomerRawBranchCoverageQueryKey(),
      staleTime: 60_000,
    },
  });
  ...
  const resolveGhostMut = usePostCoverageResolveGhostCustomers({
    mutation: {
      onSuccess: (res) => {
        ...
        toast.success(
          `Ghost resolution завершён: ${saved} записей сохранено из ${ghosts} ghost IDs. Перезагружаю аудит…`,
        );
        void refetch();
      },
      onError: (err: unknown) => {
        toast.error(`Ошибка resolve-ghost-customers: ${String(err)}`);
      },
    },
  });

  const ghostResult = resolveGhostMut.data as unknown as
    Record<string, unknown> | undefined;

  const normalizeHistMut = useRetiredLegacyAction();
  const normHistResult = normalizeHistMut.data as unknown as
    Record<string, unknown> | undefined;
  ...
  if (isLoading)
    return (
      <div className="flex items-center gap-2 text-[12px] text-gray-400 py-4">
        <Loader2 className="w-4 h-4 animate-spin" />
        Загрузка покрытия идентичности…
      </div>
    );
  if (error || !data)
    return (
  ...
  const classSummary =
    (d.classificationSummary as Record<
      string,
  ...
  return (
    <div className="space-y-4">
      ...
      {(perBranch.length > 0 || branchCovLoading) && (
        <div>
          <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-2">
            Raw студенты по веткам (alpha_raw_records)
          </div>
          {branchCovLoading ? (
            <div className="flex items-center gap-1 text-[11px] text-gray-400 py-2">
              <Loader2 className="w-3 h-3 animate-spin" />
              Загрузка…
            </div>
          ) : (
                    ...
                    return (
                      <tr key={i} className="border-t border-gray-100">
                        ...
                        <td className="px-3 py-1.5 font-medium">
                          {String(b.branchName)}
                          {Number(b.branchId) === 6 && (
                            <span className="ml-1 text-violet-600 text-[9px] font-bold">
                              ATLAS
                            </span>
                          )}
                        </td>
                        <td
                          className={`px-3 py-1.5 text-right tabular-nums font-semibold ${ok ? "text-emerald-600" : "text-gray-300"}`}
                        >
                          {ok
                            ? Number(b.rawStudentCount).toLocaleString("ru")
                            : "—"}
                        </td>
                        <td
                          className={`px-3 py-1.5 text-right tabular-nums ${ok ? "text-gray-600" : "text-gray-300"}`}
                        >
                          {ok
                            ? Number(b.rawRecordCount).toLocaleString("ru")
                            : "—"}
                        </td>
                        <td className="px-3 py-1.5">
                          {ok ? (
                        ...
                        <td className="px-3 py-1.5 text-gray-400 text-[10px]">
                          {ok ? fmtTs(String(b.latestSynced ?? "")) : "—"}
                        </td>
                      </tr>
                    );
                  })}
                ...
                {perBranch.length > 0 && (
                  <tfoot>
                    <tr className="bg-gray-50 border-t-2 border-gray-200">
                      <td
                        colSpan={2}
                        className="px-3 py-1.5 text-[10px] font-semibold text-gray-500"
                      >
                        Итого
                      </td>
                      <td className="px-3 py-1.5 text-right tabular-nums font-bold text-[11px]">
                        {Number(bcGlobal.uniqueAlphaIds ?? 0).toLocaleString(
                          "ru",
                        )}
                      </td>
                      <td className="px-3 py-1.5 text-right tabular-nums text-[11px]">
                        {Number(bcGlobal.totalRawRecords ?? 0).toLocaleString(
                          "ru",
                        )}
                      </td>
                      <td className="px-3 py-1.5 text-[10px] text-gray-500">
                        {Number(bcGlobal.branchesWithData ?? 0)} / 8 веток
                      </td>
                      <td className="px-3 py-1.5 text-[10px] text-gray-400">
                        {Number(bcGlobal.multiBranchCustomers ?? 0)}{" "}
                        multi-branch
                      </td>
                    </tr>
                  </tfoot>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <AuditStatCard
          label="Уник. customer_ids"
          value={Number(d.uniqueCustomerIds ?? 0)}
          sub="в attendance"
        />
        <AuditStatCard
          label="Привязано (ids)"
          value={Number(d.linkedCustomerIds ?? 0)}
          color="text-emerald-600"
          sub={`${String(d.linkedPctByCustomer ?? 0)}%`}
        />
        <AuditStatCard
          label="Не привязано (ids)"
          value={Number(d.unlinkedCustomerIds ?? 0)}
          color={
            Number(d.unlinkedCustomerIds ?? 0) > 0
              ? "text-amber-600"
              : "text-emerald-600"
          }
        />
        <AuditStatCard
          label="Привязано (записи)"
          value={Number(d.linkedAttendanceRecords ?? 0)}
          sub={`из ${Number(d.totalAttendanceRecords ?? 0).toLocaleString("ru")} записей`}
          color="text-emerald-600"
        />
      </div>
      ...
      <div>
        <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-2">
          ...
          {branchesAvailable.length > 1 && (
            <span className="ml-2 normal-case font-normal text-emerald-600">
              ({branchesAvailable.length} веток доступно)
            </span>
          )}
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <AuditStatCard
            label="В Atlas raw"
            value={Number(raw.foundInAtlasRaw ?? 0)}
            sub="branchId=6"
            color={
              Number(raw.foundInAtlasRaw ?? 0) > 0
                ? "text-amber-600"
                : "text-gray-400"
            }
          />
          <AuditStatCard
            label="В др. ветвях raw"
            value={Number(raw.foundInOtherBranch ?? 0)}
            sub="other branches"
            color={
              Number(raw.foundInOtherBranch ?? 0) > 0
                ? "text-amber-600"
                : "text-gray-400"
            }
          />
          <AuditStatCard
            label="Multi-branch"
            value={Number(raw.foundInMultiBranch ?? 0)}
            color={
              Number(raw.foundInMultiBranch ?? 0) > 0
                ? "text-blue-600"
                : "text-gray-400"
            }
          />
          <AuditStatCard
            label="Не найдено в raw"
            value={Number(raw.notFoundInAnyRaw ?? 0)}
            color={
              Number(raw.notFoundInAnyRaw ?? 0) > 0
                ? "text-red-500"
                : "text-emerald-600"
            }
            sub="ни в одной ветви"
          />
        </div>
        ...
        {Object.keys(rawBranchCounts).length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {Object.entries(rawBranchCounts).map(([bid, cnt]) => (
              <span
                key={bid}
                className={`inline-flex items-center gap-0.5 text-[10px] px-2 py-0.5 rounded-full font-mono border ${bid === "6" ? "bg-violet-50 border-violet-200 text-violet-700" : "bg-gray-50 border-gray-200 text-gray-600"}`}
              >
                b{bid}: {cnt}
              </span>
            ))}
      ...
      {(() => {
        const ghostData =
          (d.ghostResolution as Record<string, unknown> | undefined) ?? {};
        ...
        if (notFoundInAnyRaw === 0 && !resolutionRan && !ghostResult)
          return null;
        return (
          <div className="rounded-[14px] border border-indigo-200 bg-indigo-50/40 p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-[12px] font-semibold text-indigo-800">
                  Ghost Customer Resolution (P7.4.3b)
                </div>
              ...
              >
                {resolveGhostMut.isPending ? (
                  ...
                  <>
                    ...
                    {resolutionRan ? "Перезапустить" : "Запустить"} Resolution
                  </>
                )}
              </button>
            </div>
            ...
            {ghostResult && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <AuditStatCard
                  label="Ghost IDs"
                  value={Number(ghostResult.ghostCount ?? 0)}
                  sub="входило"
                />
                <AuditStatCard
                  label="Записей сохранено"
                  value={Number(ghostResult.rawRecordsSaved ?? 0)}
            ...
            {ghostResult &&
              Array.isArray(ghostResult.variants) &&
              (ghostResult.variants as Array<Record<string, unknown>>).length >
                0 && (
                <div>
                  <div className="text-[10px] font-semibold text-indigo-600 uppercase tracking-wide mb-1.5">
                    Варианты запросов
                  </div>
                    ...
                    <table className="w-full text-[10px]">
                      <thead>
                        <tr className="bg-indigo-50 border-b border-indigo-100">
                          <th className="px-2 py-1.5 text-left font-medium text-indigo-700">
                            Вариант
                          </th>
                          <th className="px-2 py-1.5 text-right font-medium text-indigo-700">
                            Ghost IDs
                          </th>
                          <th className="px-2 py-1.5 text-right font-medium text-indigo-700">
                            Записей
                      ...
                      <tbody>
                        {(
                          ghostResult.variants as Array<Record<string, unknown>>
                        ).map((v, i) => (
                          <tr key={i} className="border-t border-indigo-50">
                            <td className="px-2 py-1 font-mono text-indigo-700">
                              {String(v.name ?? `V${i}`)}
                            </td>
                            <td className="px-2 py-1 text-right tabular-nums font-semibold">
                              {Number(v.ghostIdsFound ?? 0)}
                            </td>
                            <td className="px-2 py-1 text-right tabular-nums">
                              {Number(v.itemsFetched ?? 0)}
                            </td>
                            <td className="px-2 py-1 text-gray-500">
                              {String(v.paginationStatus ?? "—")}
                            </td>
                          </tr>
                        ))}
                      </tbody>
      ...
      {(() => {
        const ib =
          (d.identityBreakdown as Record<string, unknown> | undefined) ?? {};
        ...
        return (
          <div className="rounded-[14px] border border-teal-200 bg-teal-50/30 p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-[12px] font-semibold text-teal-800">
                  Нормализация исторических студентов (P7.4.3c)
                </div>
            ...
            {(activeLinked > 0 ||
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <AuditStatCard
                  ...
                  value={activeLinked}
                  color="text-emerald-600"
                  sub={`${Number(ib.activePct ?? 0)}%`}
                />
                <AuditStatCard
                  label="Inactive нормализовано"
                  value={inactiveLinked}
                  color="text-orange-600"
                  sub={`${Number(ib.inactivePct ?? 0)}%`}
                />
                <AuditStatCard
                  label="Исторические ID"
                  value={histCreated}
                  color="text-teal-600"
                  sub={`${Number(ib.historicalPct ?? 0)}% · ${attIdentOnly.toLocaleString("ru")} посещ.`}
                />
                <AuditStatCard
                  label="Не разрешено"
                  value={stillUnresolved}
                  color={
                    stillUnresolved > 0 ? "text-red-500" : "text-emerald-600"
                  }
                  sub={
                    attUnresolved > 0
                      ? `${attUnresolved.toLocaleString("ru")} посещ.`
                      : ""
                  }
                />
              </div>
            ...
            {normHistResult &&
              (() => {
                const cA = normHistResult.caseA as
                  Record<string, unknown> | undefined;
                const cB = normHistResult.caseB as
                  Record<string, unknown> | undefined;
                return (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <AuditStatCard
                      label="Case A: найдено"
                      value={Number(cA?.rawFound ?? 0)}
                      sub="archived raw"
                    />
                    <AuditStatCard
                      label="Case A: привязано"
                      value={Number(cA?.relinked ?? 0)}
                      color="text-orange-600"
                      sub="attendance relinked"
                    />
                    <AuditStatCard
                      label="Case B: плейсхолдеров"
                      value={Number(cB?.created ?? 0)}
                      color="text-teal-600"
                      sub={`+${Number(cB?.updated ?? 0)} обн.`}
                    />
                    <AuditStatCard
                      label="Case B: привязано"
                      value={Number(cB?.relinked ?? 0)}
                      color="text-teal-700"
                      sub="identity-linked"
                    />
                  </div>
            ...
            {histCreated > 0 && (
              <div className="rounded-[8px] bg-amber-50 border border-amber-200 px-3 py-2 text-[11px] text-amber-800">
                ⚠️ Исторические плейсхолдеры — это{" "}
                <strong>не полные профили студентов</strong>. Они сохраняют
                исторические данные посещаемости для {histCreated} клиентов,
                которые были <strong>безвозвратно удалены</strong> из AlphaCRM.
                CRM-данные недоступны.
              </div>
            )}
          </div>
        );
      ...
      {Object.keys(classSummary).length > 0 && (
        <div>
          <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-2">
            Классификация непривязанных customer_ids
          </div>
          <div className="bg-white rounded-[14px] border border-black/[0.06] overflow-x-auto">
            <table className="w-full text-[11px]">
              <thead>
              ...
              <tbody>
                {Object.entries(classSummary).map(([cls, v], i) => (
                  <tr key={i} className="border-t border-gray-100">
                    <td
                      className={`px-3 py-1.5 font-mono text-[10px] font-semibold ${clsColor(cls)}`}
                    >
                      {cls}
                    </td>
                    <td className="px-3 py-1.5 text-right tabular-nums font-semibold">
                    ...
                    <td className="px-3 py-1.5 text-right tabular-nums">
                      {v.attendance_count.toLocaleString("ru")}
                    </td>
      ...
      {/* Recommendation */}
      <div
        className={`rounded-[12px] border px-4 py-3 text-[12px] ${recColor}`}
      >
      ...
      {top20.length > 0 && (
        <div>
          <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-2">
            Топ-20 непривязанных customer_ids (по числу посещений)
          </div>
          <div className="bg-white rounded-[14px] border border-black/[0.06] overflow-x-auto">
            <table className="w-full text-[11px]">
              <thead>
              ...
              <tbody>
                {top20.map((r, i) => {
                  const foundBranches =
                    (r.raw_found_in_branches as string[]) ?? [];
                  return (
                    <tr key={i} className="border-t border-gray-100">
                      ...
                      <td className="px-3 py-1.5 text-right tabular-nums font-semibold">
                        {Number(r.attendance_count).toLocaleString("ru")}
                      </td>
                      <td className="px-3 py-1.5 text-right tabular-nums">
                        {Number(r.lesson_count)}
                      </td>
                      <td className="px-3 py-1.5 text-gray-500">
                        {String(r.first_lesson_date ?? "—")}
                      </td>
                      <td className="px-3 py-1.5 text-gray-500">
                        {String(r.last_lesson_date ?? "—")}
                      </td>
                      <td className="px-3 py-1.5 font-mono text-[10px] text-gray-500">
                        {foundBranches.length > 0
                          ? foundBranches.map((b) => `b${b}`).join(", ")
                          : "—"}
                      </td>
                      <td
                        className={`px-3 py-1.5 font-mono text-[10px] font-semibold ${clsColor(String(r.classification))}`}
                      >
      ...
      <div className="text-[10px] text-gray-400 italic">
        Проблем в linking_issues: {Number(d.linkingIssuesInserted ?? 0)} ·
        Сформировано: {fmtTs(String(d.generatedAt ?? ""))}
      </div>
    </div>
  );
}
...
function DuplicatesPanel({ branchId }: { branchId: string }) {
  const { data, isLoading, error, refetch } = useGetCoverageDuplicates(
    ...
    {
      query: {
        queryKey: getGetCoverageDuplicatesQueryKey({ branchId }),
        staleTime: 30_000,
      },
    },
  );
    ...
    detectMut.mutate(
      { data: { branchId } },
      {
        onSuccess: (r) => {
          ...
          toast.success(
            `Детекция завершена: ${res.inserted ?? 0} новых кандидатов`,
          );
          refetch();
        },
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 flex-1">
          <AuditStatCard
            ...
            value={Number(d.totalCandidates ?? 0)}
          />
          <AuditStatCard
            label="Открытых (open)"
            value={Number(d.openCandidates ?? 0)}
            color={
              Number(d.openCandidates ?? 0) > 0
                ? "text-amber-600"
                : "text-emerald-600"
            }
          />
      ...
      <div className="rounded-[10px] bg-amber-50 border border-amber-200 px-4 py-2 text-[11px] text-amber-700">
        ⚠️ Детекция только выявляет кандидатов. Дубликаты{" "}
        <strong>не объединяются автоматически</strong>. Только отчёт.
      </div>

      {openByType.length > 0 && (
        <div>
          <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-2">
            Открытые кандидаты по типу
          </div>
          <div className="bg-white rounded-[14px] border border-black/[0.06] overflow-hidden">
            <table className="w-full text-[11px]">
              <thead>
              ...
              <tbody>
                {openByType.map((row, i) => (
                  <tr key={i} className="border-t border-gray-100">
                    ...
                    <td className="px-3 py-2">
                      <span
                        className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${CAND_COLOR[row.candidate_type] ?? "bg-gray-100 text-gray-600"}`}
                      >
                        {row.candidate_type}
                      </span>
                    </td>
                ...
                {candidates.map((c, i) => (
                  ...
                  >
                    <td className="px-3 py-1.5 font-mono">
                      {String(c.entity_type)}
                    </td>
                    <td className="px-3 py-1.5">
                      <span
                        className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${CAND_COLOR[String(c.candidate_type)] ?? "bg-gray-100 text-gray-600"}`}
                      >
                        {String(c.candidate_type)}
                      </span>
                    </td>
                    <td className="px-3 py-1.5 font-mono text-gray-600">
                      {String(c.entity_a_id).slice(0, 12)}…
                    </td>
                    <td className="px-3 py-1.5 font-mono text-gray-600">
                      {String(c.entity_b_id).slice(0, 12)}…
                    </td>
                    <td className="px-3 py-1.5 text-right tabular-nums font-semibold">
                    ...
                    <td className="px-3 py-1.5">
                      <span
                        className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${c.status === "open" ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-500"}`}
                      >
                        {String(c.status)}
                      </span>
                    </td>
...
function FinalAuditReportPanel({ branchId }: { branchId: string }) {
  const { data, isLoading, error, refetch } =
    useGetCoverageFinalAlphaAuditReport(
      ...
      {
        query: {
          queryKey: getGetCoverageFinalAlphaAuditReportQueryKey({ branchId }),
          staleTime: 120_000,
        },
      },
    );
  ...
  const next = d?.nextStageRecommendation as
    Record<string, unknown> | undefined;
  const ready = d?.readinessForNextStage as Record<string, unknown> | undefined;
  const lims =
    (d?.apiLimitations as Array<Record<string, unknown>> | undefined) ?? [];
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-[11px] text-gray-400 font-mono">
            Сформирован: {String(d.generatedAt ?? "")}
          </div>
          <div className="text-[11px] text-gray-500">
            {String(d.scope ?? "")}
          </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          {
            label: "Big AlphaCRM Audit",
            val: String(exec?.bigAlphaCRMAudit ?? ""),
          },
          {
            label: "Alpha Operational Layer",
            val: String(exec?.alphaOperationalLayer ?? ""),
          },
          {
            label: "Bank Reconciliation",
            val: String(exec?.bankReconciliationReadiness ?? ""),
          },
          {
            label: "Final Financial Truth",
            val: String(exec?.finalFinancialTruth ?? ""),
          },
        ].map(({ label, val }) => (
          <div
            key={label}
            className={`rounded-xl p-3 text-center border ${verdictColor(val)}`}
          >
      ...
      {warns.length > 0 && (
        <div className="space-y-1">
          {warns.map((w, i) => {
            const color = w.startsWith("CRITICAL")
            ...
            return (
              <div
                key={i}
                className={`text-[11px] border rounded-lg px-3 py-2 ${color}`}
              >
                {w}
              </div>
            );
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <div className="grid grid-cols-3 gap-3 sm:grid-cols-6">
            {[
              {
                label: "Студенты",
                val: (norm.students as Record<string, unknown>)?.active,
              },
              {
                label: "Учителя",
                val: (norm.teachers as Record<string, unknown>)?.normalized,
              },
              {
                label: "Группы",
                val: (norm.groups as Record<string, unknown>)?.normalized,
              },
              {
                label: "Уроки",
                val: (norm.lessons as Record<string, unknown>)?.normalized,
              },
              {
                label: "Посещения",
                val: (norm.attendance as Record<string, unknown>)?.extracted,
              },
              {
                label: "Платежи",
                val: (norm.payments as Record<string, unknown>)?.normalized,
              },
            ].map(({ label, val }) => (
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-gray-600 sm:grid-cols-4">
            <div>
              ...
              <span className="font-medium text-gray-800">
                {fmt((norm.students as Record<string, unknown>)?.active)} active
              </span>
              , {fmt((norm.students as Record<string, unknown>)?.inactive)}{" "}
              inactive,{" "}
              {fmt((norm.students as Record<string, unknown>)?.legacyPreP7)}{" "}
              legacy
            </div>
            <div>
              Группы:{" "}
              <span className="font-medium text-gray-800">
                {fmt((norm.groups as Record<string, unknown>)?.withTeacher)}
              </span>{" "}
              с учителем,{" "}
              {fmt(
                (norm.groups as Record<string, unknown>)?.withInferredSubject,
              )}{" "}
              с предметом
            </div>
            <div>
              ...
              <span className="font-medium text-gray-800">
                {String(
                  (norm.lessons as Record<string, unknown>)?.minDate ?? "",
                ).slice(0, 10)}
              </span>{" "}
              →{" "}
              {String(
                (norm.lessons as Record<string, unknown>)?.maxDate ?? "",
              ).slice(0, 10)}
            </div>
            <div>
              Платежи:{" "}
              <span className="font-medium text-green-700">
                {fmtRub((norm.payments as Record<string, unknown>)?.incomeSum)}
              </span>{" "}
              доход
            </div>
          </div>
      ...
      {ident && (
        <div className="bg-white border border-gray-200 rounded-xl p-4">
            ...
            {[
              {
                ...
                val: ident.totalAttendanceRecords,
                color: "text-gray-800",
              },
              {
                label: "Активные студенты",
                val: ident.activeAttendance,
                color: "text-green-700",
              },
              {
                label: "Исторические",
                val: ident.historicalOnlyAttendance,
                color: "text-amber-700",
              },
              {
                label: "Нерешённые",
                val: ident.unresolvedAttendance,
                color:
                  Number(ident.unresolvedAttendance) === 0
                    ? "text-green-600"
                    : "text-red-600",
              },
            ].map(({ label, val, color }) => (
              <div key={label}>
                <div className={`text-[18px] font-bold ${color}`}>
                  {fmt(val)}
                </div>
                <div className="text-[10px] text-gray-500 mt-0.5">{label}</div>
              </div>
          ...
          <div className="mt-2 text-[11px] text-gray-500 bg-amber-50 border border-amber-100 rounded p-2">
            ⚠️ 158 исторических плейсхолдеров — это НЕ активные студенты. Данные
            CRM для них недоступны (hard-deleted из AlphaCRM).
          </div>
        </div>
      )}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <div className="grid grid-cols-5 gap-3 text-center mb-3">
            {[
              ...
              {
                label: "Доходы",
                val: (pay.typeBreakdown as Record<string, unknown>)?.income,
                color: "text-green-700",
              },
              {
                label: "Корректировки",
                val: (pay.typeBreakdown as Record<string, unknown>)?.correction,
                color: "text-blue-700",
              },
              {
                label: "Инкассация",
                val: (pay.typeBreakdown as Record<string, unknown>)?.outcome,
                color: "text-red-700",
              },
              {
                label: "Непривяз.",
                val: (pay.linking as Record<string, unknown>)?.unlinked,
                color: "text-amber-700",
              },
            ].map(({ label, val, color }) => (
              <div key={label}>
          ...
          <div className="flex flex-wrap gap-2 text-[10px]">
            <span className="bg-red-100 text-red-800 rounded px-2 py-0.5">
              HIGH: {fmt((pay.risk as Record<string, unknown>)?.high)}
            </span>
            <span className="bg-amber-100 text-amber-800 rounded px-2 py-0.5">
              MED: {fmt((pay.risk as Record<string, unknown>)?.medium)}
            </span>
            <span className="bg-green-100 text-green-800 rounded px-2 py-0.5">
              LOW: {fmt((pay.risk as Record<string, unknown>)?.low)}
            </span>
            <span className="bg-red-50 text-red-700 rounded px-2 py-0.5 border border-red-200">
              Инкассация:{" "}
              {fmt((pay.collectionRisk as Record<string, unknown>)?.count)}{" "}
              записей,{" "}
              {fmtRub(
                (pay.collectionRisk as Record<string, unknown>)?.amountRub,
              )}
            </span>
            <span className="bg-orange-50 text-orange-700 rounded px-2 py-0.5 border border-orange-200">
              cleanup:{" "}
              {String(
                (pay as Record<string, unknown>)?.cleanupReadiness ?? "PARTIAL",
              )}
            </span>
          </div>
          <div className="mt-2 text-[10px] text-red-700 bg-red-50 border border-red-100 rounded p-2">
            {String(
              (pay.financialTotals as Record<string, unknown>)?.warning ?? "",
            )}
          </div>
        </div>
      )}
      ...
      {la && (
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <div className="mt-2 text-[11px] text-gray-500">
            {String(la.lessonMinDate ?? "").slice(0, 10)} →{" "}
            {String(la.lessonMaxDate ?? "").slice(0, 10)} (29 мес)
          </div>
        </div>
      )}
      ...
      {rem && (
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="text-[13px] font-semibold text-gray-800">
              ⚠️ Оставшиеся issues ({fmt(rem.total)})
            </div>
            <button
              onClick={() => setShowIssues(!showIssues)}
              className="text-[11px] text-gray-500 hover:text-gray-700"
            >
              {showIssues ? "Скрыть" : "Показать все"}
            </button>
          </div>
          {showIssues && (
            <div className="space-y-1.5">
              {((rem.byType as Array<Record<string, unknown>>) ?? []).map(
                (r, i) => {
                  ...
                  return (
                    <div key={i} className="text-[11px] bg-gray-50 rounded p-2">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`font-semibold ${sevColor}`}>
                            {sev}
                          </span>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <div className="space-y-2">
            {(
              ...
              const section = (ready as Record<string, unknown>)[key] as
                Record<string, unknown> | undefined;
              const status = String(section?.status ?? section?.reason ?? "");
              return (
                <div key={key} className="flex items-start gap-3">
                  <div
                    className={`text-[10px] font-bold px-2 py-0.5 rounded whitespace-nowrap mt-0.5 ${verdictColor(status)}`}
                  >
                    {status.replace(/_/g, " ")}
                  </div>
                  <div>
                    ...
                    <div className="text-[10px] text-gray-500">
                      {String(section?.details ?? section?.reason ?? "")}
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="bg-violet-50 border border-violet-200 rounded-xl p-4">
          <div className="flex items-center justify-between mb-2">
            <div className="text-[13px] font-semibold text-violet-800">
              🚀 Следующий этап: {String(next.stage ?? "")}
            </div>
            <button
              onClick={() => setShowNextStage(!showNextStage)}
              className="text-[11px] text-violet-600 hover:text-violet-800"
            >
              {showNextStage ? "Свернуть" : "Развернуть"}
            </button>
          </div>
          {showNextStage && (
            <>
              <div className="text-[11px] text-violet-700 mb-3">
              ...
              <div className="space-y-1">
                {((next.tasks as string[] | undefined) ?? []).map((t, i) => (
                  <div
                    key={i}
                    className="text-[11px] text-violet-800 flex items-start gap-1.5"
                  >
            ...
            <div className="space-y-2">
              <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">
                {(
                  ...
                  <div
                    key={i}
                    className="text-[11px] bg-gray-50 rounded px-2 py-1 flex justify-between"
                  >
                    <span className="font-mono text-gray-700">
              ...
              <div className="mt-3">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-[11px] text-gray-500 hover:text-gray-700"
                  >
                    {showLimitations ? "Скрыть" : "Ограничения API"}
                  </button>
                </div>
                {(
                  (raw.unavailableEndpoints as
                    Array<Record<string, unknown>> | undefined) ?? []
                ).map((r, i) => (
                  ...
                  <div className="mt-2 space-y-1">
                    {lims.map((l, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="text-[10px] bg-amber-50 border border-amber-100 rounded p-1.5"
                        >
                          <span
                            className={`font-mono font-semibold ${sevColor}`}
                          >
                            [{String(l.severity ?? "")}] {String(l.key ?? "")}:
                          </span>
                          <span className="text-gray-600 ml-1">
          ...
          <div className="grid grid-cols-3 gap-4 text-[11px]">
            <div>
              ...
              <div>
                ...
                <span className="font-bold">
                  {fmt((gst.teachers as Record<string, unknown>)?.normalized)}
                </span>
              </div>
              <div>
                Активных:{" "}
                <span className="font-bold text-green-700">
                  {fmt((gst.teachers as Record<string, unknown>)?.active)}
                </span>
              </div>
            </div>
            <div>
              ...
              <div>
                Всего:{" "}
                <span className="font-bold">
                  {fmt((gst.groups as Record<string, unknown>)?.normalized)}
                </span>
              </div>
              <div>
                С учителем:{" "}
                <span className="font-bold">
                  {fmt((gst.groups as Record<string, unknown>)?.withTeacher)}
                </span>
              </div>
              <div>
                С предметом:{" "}
                <span className="font-bold">
                  {fmt(
                    (gst.groups as Record<string, unknown>)
                      ?.withInferredSubject,
                  )}
                </span>
              </div>
            ...
            <div>
              ...
              <div>
                ...
                <span className="font-bold">
                  {fmt(
                    (gst.subjects as Record<string, unknown>)?.totalResolved,
                  )}
                </span>
              </div>
              <div>
                ...
                <span className="font-bold">
                  {fmt(
                    (gst.subjects as Record<string, unknown>)
                      ?.referencedByLessons,
                  )}
                </span>
              </div>
...
function BankReadinessBadge({ value }: { value?: string }) {
  ...
  const icon =
    value === "READY"
      ...
      : value === "PARTIAL"
        ? "⚠️"
        : value === "NOT_APPLICABLE_DEV"
          ? "ℹ️"
          : "🔴";
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold ${map[value] ?? "bg-gray-100 text-gray-600"}`}
    >
      {icon} {value}
    </span>
  );
...
function IssueSeverityBadge({ severity }: { severity?: string }) {
  ...
  return (
    <span
      className={`inline-flex px-1.5 py-0.5 rounded text-[10px] font-semibold ${map[severity ?? ""] ?? "bg-gray-100 text-gray-500"}`}
    >
      {severity}
    </span>
  );
...
function ConnectorStatusBadge({ status }: { status?: string }) {
  ...
  return (
    <span
      className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-semibold ${map[status ?? ""] ?? "bg-gray-100 text-gray-500"}`}
    >
      {status ?? "unknown"}
    </span>
  );
...
function BankAccountsPanel() {
  const { data, isLoading, isError, refetch, isFetching } =
    useGetCoverageBankAccountsAudit({
      query: {
        queryKey: getGetCoverageBankAccountsAuditQueryKey(),
        staleTime: 60_000,
      },
    });
  ...
  if (isLoading)
    return (
      <div className="flex items-center gap-2 text-[13px] text-gray-500 py-8">
        <Loader2 className="w-4 h-4 animate-spin" /> Загружаем банковский аудит…
      </div>
    );
  if (isError || !data)
    return (
      <div className="text-red-500 text-[13px] py-4">
        Ошибка загрузки /coverage/bank-accounts-audit
      </div>
    );
  ...
  const d = data as {
    ...
    environment?: {
      ...
      dbKind?: string;
      bankIntegrationExpected?: boolean;
    ...
    summary?: {
      ...
      totalNormalizedTransactions?: number;
    ...
    otherFinanceData?: {
      ...
      bankTransactionsRaw?: { count?: number; note?: string };
    ...
    <div className="space-y-5 text-[13px]">
      ...
      <div className="flex items-center justify-between">
        <div className="text-[11px] text-gray-400">
          Сгенерировано:{" "}
          {d.generatedAt
            ? new Date(d.generatedAt).toLocaleString("ru-RU")
            : "—"}
        </div>
        <button
          onClick={() => refetch()}
      ...
      {env.auditValidity === "NOT_VALID_DEV" ? (
        <div className="border border-red-200 bg-red-50 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="font-bold text-red-800 text-[14px]">
                🚫 BANK AUDIT NOT VALID IN DEV
              </div>
              <div className="text-red-700 mt-1">
                Банковская интеграция (Точка, счета, транзакции) существует{" "}
                <strong>только в продакшн</strong>. Текущая база данных —
                dev/staging.
              </div>
              <div className="mt-2 space-y-1 text-[12px] text-red-600">
                <div>
                  • Отсутствие банковских счетов в dev — это{" "}
                  <strong>ожидаемо</strong>, не ошибка
                </div>
                <div>
                  • Ошибка коннектора Точки в dev —{" "}
                  <strong>не указывает</strong> на проблему в продакшн
                </div>
                <div>
                  • <strong>Не</strong> запускай OAuth flow и не меняй токены в
                  dev
                </div>
              </div>
              <div className="mt-3 bg-white border border-red-200 rounded px-3 py-2 text-[12px] font-semibold text-red-700">
                → Для реального аудита запусти P8.1 против production базы
                данных / окружения
              </div>
              <div className="text-[11px] text-red-500 mt-2">
                NODE_ENV: <span className="font-mono">{env.nodeEnv}</span> · DB:{" "}
                <span className="font-mono">{env.dbName}</span> · dbKind:{" "}
                <span className="font-mono">{env.dbKind}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg p-3">
          ...
          <div>
            ...
            <div className="text-amber-700 mt-0.5">
              {env.environmentWarning ?? "Текущая база данных — dev/staging."}
            </div>
            <div className="text-[11px] text-amber-600 mt-1">
              NODE_ENV: <span className="font-mono">{env.nodeEnv}</span> · DB:{" "}
              <span className="font-mono">{env.dbName}</span>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {/* Readiness verdict */}
      <div
        className={`border rounded-lg p-4 ${d.bankAccountsReadiness === "NOT_APPLICABLE_DEV" ? "border-gray-200 bg-gray-50" : "border-gray-200"}`}
      >
        ...
        {d.nextRecommendedStep && (
          <div
            className={`text-[12px] rounded px-2 py-1 ${d.bankAccountsReadiness === "NOT_APPLICABLE_DEV" ? "bg-gray-100 text-gray-600" : "bg-blue-50 text-blue-700"}`}
          >
            {d.bankAccountsReadiness === "NOT_APPLICABLE_DEV"
              ? "→"
              : "Следующий шаг:"}{" "}
            {d.nextRecommendedStep}
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          {
            label: "Коннекторов",
            value: summary.totalConnectors ?? 0,
            sub: `${summary.activeConnectors ?? 0} active / ${summary.errorConnectors ?? 0} error`,
          },
          {
            label: "Банк. счетов",
          ...
          {
            label: "Транзакций (raw)",
            value: summary.totalRawTransactions ?? 0,
            sub: "bank_transactions_raw",
          },
          {
            label: "Операций",
            value: summary.totalOperations ?? 0,
            sub: "operations (ручные)",
          },
        ].map((s) => (
          <div
            key={s.label}
          ...
          (d.connectors ?? []).map((c) => (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-[11px] text-gray-500 mt-1">
                <div>
                  Счетов импортировано: <b>{c.accountsImported ?? 0}</b>
                </div>
                <div>
                  Последняя синх.:{" "}
                  {c.lastSyncAt
                ...
                <div>
                  Последний успех:{" "}
                  {c.lastSuccessAt ? (
                    new Date(c.lastSuccessAt).toLocaleString("ru-RU")
                  ) : (
      ...
      <div>
        <div className="text-[12px] font-semibold text-gray-700 mb-2">
          Банковские счета ({(d.accounts ?? []).length})
        </div>
        {(d.accounts ?? []).length === 0 ? (
          <div className="bg-gray-50 border border-gray-100 rounded-lg p-4 text-center text-gray-500">
            ...
            <div className="text-[12px] mt-1 text-gray-400">
              Коннектор Точки не завершил OAuth flow. После авторизации счета
              появятся здесь.
            </div>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-[12px]">
              ...
              <tbody>
                {(d.accounts ?? []).map((acc) => (
                  <tr
                    key={acc.internalAccountId}
                    className="border-b border-gray-50 hover:bg-gray-50"
                  >
                    <td className="py-1.5 pr-3 font-mono">
                      {acc.maskedAccountNumber ?? acc.externalAccountId ?? "—"}
                    </td>
                    ...
                    <td className="py-1.5 pr-3">
                      {acc.transactionsCount ?? 0}
                    </td>
                    <td className="py-1.5 pr-3">
                      {acc.legalEntityName ?? (
                        <span className="text-gray-300 italic">неизвестен</span>
                      )}
                    </td>
                    <td className="py-1.5">
                      <span
                        className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${
                          acc.auditStatus === "OK"
                            ...
                            : acc.auditStatus === "WARNING"
                              ? "bg-yellow-100 text-yellow-700"
                              : acc.auditStatus === "ERROR"
      ...
      {(d.duplicateCandidates ?? []).length > 0 && (
        <div>
          <div className="text-[12px] font-semibold text-gray-700 mb-2">
            Дубликаты ({d.duplicateCandidates?.length})
          </div>
          {d.duplicateCandidates?.map((dup, i) => (
            <div
              key={i}
              className="border border-orange-100 bg-orange-50 rounded p-2.5 mb-1.5"
            >
              <div className="flex items-center gap-2">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          {[
            {
              title: "bank_transactions_raw",
              count: d.otherFinanceData?.bankTransactionsRaw?.count ?? 0,
              note: d.otherFinanceData?.bankTransactionsRaw?.note,
            },
            {
              title: "bank_transactions (norm)",
              count: d.otherFinanceData?.bankTransactionsNormalized?.count ?? 0,
              note: `${d.otherFinanceData?.bankTransactionsNormalized?.minDate ?? "?"} → ${d.otherFinanceData?.bankTransactionsNormalized?.maxDate ?? "?"}`,
            },
            {
              title: "operations",
              count: d.otherFinanceData?.operations?.count ?? 0,
              note: d.otherFinanceData?.operations?.note,
            },
          ].map((s) => (
            <div
              key={s.title}
            ...
            >
              <div className="font-mono text-[11px] text-gray-500 mb-0.5">
                {s.title}
              </div>
              <div className="text-[20px] font-bold text-gray-800">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {(d.sourceConnectors ?? []).map((sc) => (
            <div
              key={sc.source_name}
              className="flex items-center gap-1.5 border border-gray-100 rounded-lg px-2.5 py-1.5 bg-gray-50"
            >
      ...
      <div>
        <div className="text-[12px] font-semibold text-gray-700 mb-2">
          Проблемы
          {d.issueSummary?.critical ? (
            <span className="ml-2 text-red-700 font-bold">
              ● {d.issueSummary.critical} CRITICAL
            </span>
          ) : null}
          {(d.issueSummary?.high ?? 0) > 0 ? (
            <span className="ml-2 text-orange-600">
              ● {d.issueSummary?.high} HIGH
            </span>
          ) : null}
        </div>
        {issues.length === 0 ? (
      ...
      {(d.warnings ?? []).length > 0 && (
        <div className="bg-amber-50 border border-amber-100 rounded-lg p-3">
          ...
          {d.warnings?.map((w, i) => (
            <div key={i} className="text-[12px] text-amber-700 mb-0.5">
              {w}
            </div>
          ))}
        </div>
...
function GapBadge({ verdict }: { verdict?: string }) {
  ...
  return (
    <span
      className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold ${map[verdict] ?? "bg-gray-100 text-gray-500"}`}
    >
      {verdict}
    </span>
  );
...
function BankTransactionsPanel() {
  const { data, isLoading, isError, refetch, isFetching } =
    useGetCoverageBankTransactionsAudit({
      query: {
        queryKey: getGetCoverageBankTransactionsAuditQueryKey(),
        staleTime: 60_000,
      },
    });
  ...
  if (isLoading)
    return (
      <div className="flex items-center gap-2 text-[13px] text-gray-500 py-8">
        <Loader2 className="w-4 h-4 animate-spin" /> Загружаем аудит транзакций…
      </div>
    );
  if (isError || !data)
    return (
      <div className="text-red-500 text-[13px] py-4">
        Ошибка загрузки /coverage/bank-transactions-audit
      </div>
    );
  ...
  const d = data as {
    ...
    rawTransactions?: {
      ...
      uniqueExtIds?: number;
      hasExtId?: number;
      hasOperationDate?: number;
      missingOperationDate?: number;
      hasRawJson?: number;
      hasAmount?: number;
      hasDirection?: number;
      hasCounterpartyName?: number;
      hasCounterpartyInn?: number;
      hasPurpose?: number;
      ...
      normBookkeepingGap?: boolean;
      normBookkeepingGapNote?: string | null;
    };
    dateFieldInventory?: {
      documentProcessDate?: {
        present?: number;
      ...
      normalizedTableDateStatus?: string;
      rootCause?: string;
    ...
    normalizedTransactions?: {
      ...
      hasPurpose?: number;
      hasExtId?: number;
      hasCounterpartyAccount?: number;
      hasAccountId?: number;
      matchStatus?: { unmatched?: number; matched?: number };
    };
    rawNormalizedGap?: {
      rawTotal?: number;
      ...
      verdict?: string;
      trueDuplicatesNote?: string;
      crossAccountNote?: string;
    };
    perAccountCoverage?: Array<{
      accountId?: string;
      maskedAccount?: string;
      rawCount?: number;
      normalizedCount?: number;
      rawNotNormalized?: number;
      rawDateFrom?: string | null;
      rawDateTo?: string | null;
      rawMissingOpDate?: number;
    ...
    duplicates?: {
      ...
      normalizedDuplicates?: { count?: number; note?: string };
    };
    counterpartyReadiness?: {
      uniqueCounterpartyNames?: number;
      uniqueCounterpartyInns?: number;
      uniqueCounterpartyAccounts?: number;
      incomeTransactions?: number;
      expenseTransactions?: number;
      missingCounterparty?: number;
      topIncomeCounterparties?: Array<{
        name?: string;
        inn?: string;
        count?: number;
        totalSum?: number;
      }>;
      topExpenseCounterparties?: Array<{
        name?: string;
        inn?: string;
        count?: number;
        totalSum?: number;
      }>;
      counterpartyLayerReadiness?: string;
      caveats?: string[];
    };
    operations?: {
      total?: number;
      ...
      dateRange?: { min?: string | null; max?: string | null };
      note?: string;
      devProductionDifference?: string;
    };
    statementsPerAccount?: Array<{
  ...
  if (
    env.auditValidity === "BANK_TRANSACTION_AUDIT_NOT_VALID_OUTSIDE_PRODUCTION"
  ) {
    return (
      <div className="space-y-4 text-[13px]">
        <div className="border border-red-200 bg-red-50 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="font-bold text-red-800 text-[14px]">
                🚫 P8.2 NOT VALID IN DEV
              </div>
              <div className="text-red-700 mt-1">
                Bank transaction data exists only in production. Current
                environment is dev/staging.
              </div>
              <div className="text-[11px] text-red-500 mt-2 font-mono">
                NODE_ENV: {env.nodeEnv} · DB: {env.dbName}
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="space-y-5 text-[13px]">
      ...
      <div className="border rounded-lg p-4">
        <div className="flex items-center gap-3 mb-2">
          ...
          {d.p83CanStart && (
            <span className="text-[11px] bg-green-100 text-green-800 px-2 py-0.5 rounded-full font-semibold">
              → P8.3 можно начинать
            </span>
          )}
        </div>
        <div className="text-gray-600 mb-2">{d.readinessReason}</div>
        {d.nextRecommendedStep && (
          <div className="text-[12px] bg-blue-50 text-blue-700 rounded px-2 py-1">
            Следующий шаг: {d.nextRecommendedStep}
          </div>
        )}
        {(d.p83Blockers ?? []).length > 0 && (
          <div className="mt-2 space-y-1">
            {d.p83Blockers!.map((b, i) => (
              <div
                key={i}
                className="text-[12px] text-red-700 bg-red-50 rounded px-2 py-1"
              >
                🔴 {b}
              </div>
            ))}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          },
          {
            label: "Нормализовано",
            value: norm.total ?? 0,
            sub: `${norm.dateRange?.min ?? "?"} → ${norm.dateRange?.max ?? "?"}`,
          },
          {
            label: "Разрыв (gap)",
            value: gap.gap ?? 0,
            sub: gap.verdict ? `${gap.verdict}` : "—",
            warn: (gap.classification?.unexplained ?? 0) > 0,
          },
          {
            label: "Контрагентов",
            value: cp.uniqueCounterpartyNames ?? 0,
            sub: `${cp.uniqueCounterpartyInns ?? 0} ИНН · ${cp.uniqueCounterpartyAccounts ?? 0} счетов`,
          },
        ].map((s) => (
          <div
            key={s.label}
            className={`rounded-lg p-3 border ${s.warn ? "bg-red-50 border-red-200" : "bg-gray-50 border-gray-100"}`}
          >
            <div
              className={`text-[22px] font-bold ${s.warn ? "text-red-700" : "text-gray-900"}`}
            >
              {s.value.toLocaleString("ru-RU")}
            </div>
            <div className="text-[12px] font-medium text-gray-700">
      ...
      <div className="border border-gray-200 rounded-lg p-4">
        ...
        <div className="grid grid-cols-3 gap-3 mb-3">
          {[
            ...
            <div
              key={s.label}
              className={`rounded p-2 text-center ${s.warn && s.value > 0 ? "bg-orange-50 border border-orange-200" : "bg-gray-50 border border-gray-100"}`}
            >
              <div
                className={`text-[18px] font-bold ${s.warn && s.value > 0 ? "text-orange-700" : "text-gray-800"}`}
              >
                {s.value}
              </div>
        ...
        <div className="space-y-1.5">
          <div className="flex items-start gap-2 text-[12px]">
            ...
            <span className="text-gray-700">
              {gap.classification?.trueDuplicates ?? 0} записей —{" "}
              {gap.trueDuplicatesNote}
            </span>
          </div>
          <div className="flex items-start gap-2 text-[12px]">
            ...
            <span className="text-gray-700">
              {gap.classification?.crossAccountTransfers ?? 0} записей —{" "}
              {gap.crossAccountNote}
            </span>
          </div>
          {(gap.classification?.unexplained ?? 0) > 0 && (
            <div className="flex items-start gap-2 text-[12px]">
              ...
              <span className="text-gray-700">
                {gap.classification?.unexplained} записей — требуется
                расследование
              </span>
            </div>
          )}
      ...
      <div className="border border-gray-200 rounded-lg p-4">
        <div className="font-semibold text-gray-800 text-[13px] mb-3">
          📅 Инвентарь полей дат (bank_transactions_raw)
        </div>
        <div className="space-y-2 mb-3">
          {[
            {
              label: "raw_json→documentProcessDate",
              present: d.dateFieldInventory?.documentProcessDate?.present ?? 0,
              total: raw.total ?? 0,
              note: `${d.dateFieldInventory?.documentProcessDate?.minDate ?? "?"} → ${d.dateFieldInventory?.documentProcessDate?.maxDate ?? "?"}`,
              reliable: d.dateFieldInventory?.documentProcessDate?.isReliable,
            },
            {
              label: "operation_date (колонка)",
              present: d.dateFieldInventory?.operationDateColumn?.present ?? 0,
              total: raw.total ?? 0,
              note: `${d.dateFieldInventory?.operationDateColumn?.missing ?? 0} пустых`,
            },
          ].map((f) => (
            <div key={f.label} className="flex items-center gap-3">
              <div className="font-mono text-[11px] text-gray-600 w-64 shrink-0">
                {f.label}
              </div>
              <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
                <div
                  className={`h-2 rounded-full ${f.reliable ? "bg-green-400" : "bg-yellow-400"}`}
                  style={{
                    width: `${f.total > 0 ? Math.round((f.present / f.total) * 100) : 0}%`,
                  }}
                />
              </div>
              <CoveragePct value={f.present} total={f.total} />
              <div className="text-[10px] text-gray-400 w-32 shrink-0">
                {f.note}
              </div>
            </div>
          ))}
        ...
        <div
          className={`rounded p-3 text-[12px] ${d.dateFieldInventory?.rootCause === "documentProcessDate_in_raw_json" ? "bg-green-50 border border-green-200" : "bg-yellow-50 border border-yellow-200"}`}
        >
          <div className="font-semibold mb-1">
            Корневая причина:{" "}
            <code className="font-mono">{d.dateFieldInventory?.rootCause}</code>
          </div>
          <div className="text-gray-700">
            {d.dateFieldInventory?.rootCauseDetail}
          </div>
          <div className="mt-1 text-[11px] text-gray-500">
            Normalized table:{" "}
            <span
              className={`font-semibold ${d.dateFieldInventory?.normalizedTableDateStatus === "complete" ? "text-green-700" : "text-yellow-700"}`}
            >
              {d.dateFieldInventory?.normalizedTableDateStatus?.toUpperCase()}
            </span>{" "}
            — все {norm.total} нормализованных имеют operation_date
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="border border-gray-200 rounded-lg p-3">
          <div className="font-semibold text-[12px] text-gray-700 mb-2">
            Качество полей: bank_transactions_raw ({raw.total ?? 0})
          </div>
          <table className="w-full text-[11px]">
            <tbody>
              {(
                ...
                <tr key={field} className="border-b border-gray-50">
                  <td className="py-0.5 font-mono text-gray-500 pr-2">
                    {field}
                  </td>
                  ...
                  <td className="py-0.5 text-right">
                    <CoveragePct value={val ?? 0} total={raw.total ?? 0} />
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          {raw.normBookkeepingGap && (
            <div className="mt-2 bg-yellow-50 border border-yellow-200 rounded p-2 text-[11px] text-yellow-800">
              ⚠️ normalized_status = 'pending' для всех записей.{" "}
              {raw.normBookkeepingGapNote}
            </div>
          )}
        </div>
        ...
        <div className="border border-gray-200 rounded-lg p-3">
          <div className="font-semibold text-[12px] text-gray-700 mb-2">
            Качество полей: bank_transactions ({norm.total ?? 0})
          </div>
          <table className="w-full text-[11px]">
            <tbody>
              {(
                ...
                <tr key={field} className="border-b border-gray-50">
                  ...
                  <td className="py-0.5 text-right">
                    <CoveragePct value={val ?? 0} total={norm.total ?? 0} />
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div className="mt-2 text-[11px] text-gray-500">
            ...
            <span className="font-semibold text-orange-700">
              {norm.matchStatus?.unmatched ?? 0}
            </span>{" "}
            unmatched ·{" "}
            <span className="font-semibold text-green-700">
              {norm.matchStatus?.matched ?? 0}
            </span>{" "}
            matched
          </div>
        </div>
              ...
              {(d.perAccountCoverage ?? []).map((acc) => (
                ...
                >
                  <td className="py-1.5 pr-3 font-mono font-semibold">
                    {acc.maskedAccount ?? acc.accountId}
                  </td>
                  <td className="py-1.5 pr-3 text-right">
                    {acc.rawCount?.toLocaleString("ru-RU")}
                  </td>
                  <td className="py-1.5 pr-3 text-right">
                    {acc.normalizedCount?.toLocaleString("ru-RU")}
                  </td>
                  <td
                    className={`py-1.5 pr-3 text-right font-semibold ${(acc.rawNotNormalized ?? 0) > 0 ? "text-orange-600" : "text-green-600"}`}
                  >
                    {acc.rawNotNormalized ?? 0}
                  </td>
                  <td className="py-1.5 pr-3 text-gray-500">
                    {acc.rawDateFrom ?? "?"} → {acc.rawDateTo ?? "?"}
                  </td>
                  <td className="py-1.5 pr-3 text-gray-500">
                    {acc.normalizedDateFrom ?? "?"} →{" "}
                    {acc.normalizedDateTo ?? "?"}
                  </td>
                  <td className="py-1.5 pr-3 text-right text-green-700">
                    {acc.incomeSum != null
                      ? acc.incomeSum.toLocaleString("ru-RU", {
                  ...
                  <td className="py-1.5 text-right text-red-700">
                    {acc.expenseSum != null
                      ? acc.expenseSum.toLocaleString("ru-RU", {
                          maximumFractionDigits: 0,
                        })
                      : "—"}
                  </td>
      ...
      <div className="border border-gray-200 rounded-lg p-4">
        <div className="flex items-center gap-2 mb-3">
          <span className="font-semibold text-gray-800 text-[13px]">
            Готовность контрагентного слоя (P8.3)
          </span>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          {[
            {
              label: "Уникальных имён",
              value: cp.uniqueCounterpartyNames ?? 0,
            },
            { label: "Уникальных ИНН", value: cp.uniqueCounterpartyInns ?? 0 },
            {
              label: "Уникальных счетов",
              value: cp.uniqueCounterpartyAccounts ?? 0,
            },
          ].map((s) => (
            <div
              key={s.label}
              className="bg-gray-50 rounded p-2 text-center border border-gray-100"
            >
              <div className="text-[16px] font-bold text-gray-800">
                {s.value}
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <div className="text-[11px] font-semibold text-gray-600 mb-1.5">
              Топ входящих контрагентов
            </div>
            {(cp.topIncomeCounterparties ?? []).slice(0, 5).map((c, i) => (
              <div
                key={i}
                className="flex items-center justify-between text-[11px] mb-0.5 py-0.5 border-b border-gray-50"
              >
                <div
                  className="truncate text-gray-700 mr-2"
                  title={c.name ?? ""}
                >
                  {c.name ?? "—"}
                </div>
                <div className="shrink-0 text-green-700 font-semibold">
                  {c.count} ·{" "}
                  {(c.totalSum ?? 0).toLocaleString("ru-RU", {
                    maximumFractionDigits: 0,
                  })}{" "}
                  ₽
                </div>
          ...
          <div>
            ...
            {(cp.topExpenseCounterparties ?? []).slice(0, 5).map((c, i) => (
              ...
              >
                ...
                <div className="shrink-0 text-red-700 font-semibold">
                  {c.count} ·{" "}
                  {(c.totalSum ?? 0).toLocaleString("ru-RU", {
                    maximumFractionDigits: 0,
                  })}{" "}
      ...
      <div className="border border-gray-200 rounded-lg p-4">
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div className="bg-gray-50 rounded p-3 border border-gray-100">
            <div className="text-[16px] font-bold text-orange-700">
              {d.duplicates?.rawDuplicates?.count ?? 0}
            </div>
            <div className="text-[11px] text-gray-600">
              Истинных дубликатов в raw
            </div>
            <div className="text-[10px] text-gray-400 mt-0.5">
              Одинаковый ext_id + счёт
            </div>
          </div>
          <div className="bg-gray-50 rounded p-3 border border-gray-100">
            <div className="text-[16px] font-bold text-green-700">
              {d.duplicates?.normalizedDuplicates?.count ?? 0}
            </div>
            <div className="text-[11px] text-gray-600">
              Дубликатов в normalized
            </div>
            <div className="text-[10px] text-gray-400 mt-0.5">
              {d.duplicates?.normalizedDuplicates?.note}
            </div>
          </div>
        </div>
        {(d.duplicates?.rawDuplicates?.examples ?? []).length > 0 && (
          <div>
            <div className="text-[11px] font-semibold text-gray-600 mb-1">
              Примеры дубликатов raw:
            </div>
            {d.duplicates!.rawDuplicates!.examples!.slice(0, 3).map((ex, i) => (
              <div
                key={i}
                className="text-[11px] font-mono text-gray-500 mb-0.5"
              >
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
        <div className="font-semibold text-amber-800 text-[12px] mb-1">
          ⚠️ Операции (operations table)
        </div>
        <div className="text-[12px] text-amber-700">
          Продакшн: <b>{d.operations?.total ?? 0}</b> записей · source=bank_api
          · linked_bank_tx: <b>{d.operations?.linkedBankTx ?? 0}</b> · период:{" "}
          {d.operations?.dateRange?.min?.slice(0, 10) ?? "?"} →{" "}
          {d.operations?.dateRange?.max?.slice(0, 10) ?? "?"}
        </div>
      ...
      <div>
        <div className="text-[12px] font-semibold text-gray-700 mb-2">
          ...
          {(d.issueSummary?.medium ?? 0) > 0 ? (
            <span className="ml-2 text-yellow-600">
              ● {d.issueSummary?.medium} MEDIUM
            </span>
          ) : null}
          {(d.issueSummary?.info ?? 0) > 0 ? (
            <span className="ml-2 text-blue-500">
              ● {d.issueSummary?.info} INFO
            </span>
          ) : null}
        </div>
        {issues.length === 0 ? (
...
function CounterpartiesPanel() {
  ...
  if (isLoading)
    return (
      <div className="flex items-center gap-2 text-[13px] text-gray-500 py-8 px-4">
        <Loader2 className="animate-spin w-4 h-4" /> Загрузка аудита
        контрагентов…
      </div>
    );
  if (isError || !data)
    return (
      <div className="p-4">
        <div className="text-red-600 text-[13px] mb-3">
          Ошибка загрузки. Возможно, контрагенты ещё не построены.
        </div>
        <button
  ...
  const srcTx =
    typeof env.sourceTransactionCount === "number"
      ? env.sourceTransactionCount
      : null;
  const p82Exp =
    typeof env.p82ExpectedCount === "number" ? env.p82ExpectedCount : 854;
  const srcMatches = env.sourceMatchesP82 === true;
  const srcBad = env.sourceMismatch === true;

  const readinessColor =
    d.counterpartyFoundationReadiness === "READY_WITH_REVIEW"
      ? "text-green-700 bg-green-50"
      : d.counterpartyFoundationReadiness === "PARTIAL"
        ? "text-yellow-700 bg-yellow-50"
        : "text-red-700 bg-red-50";

  const byType = (d.byType ?? {}) as Record<
    string,
    { count: number; totalAmount?: string }
  >;
  const byConf = (d.byConfidence ?? {}) as Record<string, number>;
  const topIncome = (d.topIncomeCounterparties ?? []) as Array<
    Record<string, unknown>
  >;
  const topExpense = (d.topExpenseCounterparties ?? []) as Array<
    Record<string, unknown>
  >;
  const dupCandidates = (d.duplicateCandidates ?? []) as Array<
    Record<string, unknown>
  >;
    ...
    <div className="space-y-4 pb-8">
      ...
      <div className="mx-4 mt-2 grid grid-cols-2 md:grid-cols-4 gap-2 text-[11px]">
        <div className="bg-gray-50 border border-gray-200 rounded px-2.5 py-1.5">
          ...
          <span
            className={env.isProduction ? "text-green-600" : "text-red-600"}
          >
            {env.isProduction ? "✅" : "❌"}
          </span>
        </div>
        <div className="bg-gray-50 border border-gray-200 rounded px-2.5 py-1.5">
          ...
          <span
            className={`font-semibold ${srcMatches ? "text-green-700" : srcTx === 0 ? "text-red-700" : "text-yellow-700"}`}
          >
        ...
        <div className="bg-gray-50 border border-gray-200 rounded px-2.5 py-1.5">
          ...
          <span className="font-semibold text-gray-700">
            {p82Exp.toLocaleString("ru-RU")}
          </span>
        </div>
        <div
          className={`border rounded px-2.5 py-1.5 ${srcMatches ? "bg-green-50 border-green-200" : srcBad ? "bg-red-50 border-red-200" : "bg-gray-50 border-gray-200"}`}
        >
          <span className="text-gray-400">Источник P8.2 </span>
          <span
            className={`font-semibold ${srcMatches ? "text-green-700" : "text-red-700"}`}
          >
            {srcMatches
              ? "✅ совпадает"
              : srcTx === 0
      ...
      {srcTx === 0 && (
        <div className="mx-4 bg-red-50 border border-red-300 rounded px-3 py-2.5">
          <div className="text-[12px] font-semibold text-red-700 mb-1">
            🚫 bank_transactions пустой в текущей среде
          </div>
          <div className="text-[11px] text-red-700 space-y-0.5">
            <div>
              P8.2 доказал: 854 нормализованных транзакций существуют в
              production-деплое.
            </div>
            <div>
              Текущая среда: dev DATABASE_URL ≠ production DATABASE_URL.
            </div>
            <div className="font-semibold mt-1">
              Решение: опубликовать деплой → нажать «Построить контрагентов» в
              этом интерфейсе.
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3 px-4">
        <span
          className={`text-[12px] font-bold px-3 py-1 rounded-full ${readinessColor}`}
        >
      ...
      {d.nextRecommendedStep && (
        <div className="mx-4 text-[11px] text-blue-700 bg-blue-50 border border-blue-200 rounded px-3 py-1.5">
          Следующий шаг:{" "}
          <span className="font-mono">{d.nextRecommendedStep}</span>
        </div>
      )}
            ...
            <tbody>
              {Object.entries(byType).map(([type, row]) => (
                <tr key={type} className="border-b border-gray-50">
                  ...
                  <td className="py-0.5 text-right pr-2 font-mono">
          ...
          <table className="w-full text-[11px]">
            ...
            <tbody>
              {topIncome.slice(0, 10).map((cp, i) => (
                <tr key={i} className="border-b border-gray-50">
                  <td className="py-0.5 pr-2 max-w-[180px] truncate">
                    {String(cp.canonicalName ?? cp.canonical_name ?? "—")}
                  </td>
                  <td className="py-0.5 text-right pr-2 font-mono text-green-700">
                    {String(cp.totalIncome ?? cp.total_income ?? "—")}
                  </td>
                  <td className="py-0.5 text-right text-gray-500">
                    {String(cp.type ?? "—")}
                  </td>
            ...
            <tbody>
              {topExpense.slice(0, 10).map((cp, i) => (
                <tr key={i} className="border-b border-gray-50">
                  ...
                  <td className="py-0.5 text-right pr-2 font-mono text-red-700">
                    {String(cp.totalExpense ?? cp.total_expense ?? "—")}
                  </td>
                  <td className="py-0.5 text-right text-gray-500">
                    {String(cp.type ?? "—")}
                  </td>
      ...
      {dupCandidates.length > 0 && (
        <div className="mx-4 bg-white border border-amber-200 rounded p-3">
          <div className="text-[12px] font-semibold text-amber-700 mb-2">
            ⚠️ Кандидаты на дубли ({dupCandidates.length})
          </div>
          <table className="w-full text-[11px]">
            <thead>
              <tr className="text-gray-500 border-b border-gray-100">
            ...
            <tbody>
              {dupCandidates.slice(0, 15).map((dup, i) => (
                <tr key={i} className="border-b border-gray-50">
                  <td className="py-0.5 pr-2 font-mono max-w-[180px] truncate">
                    {String(dup.canonicalNameA ?? dup.canonical_name_a ?? "—")}
                  </td>
                  <td className="py-0.5 pr-2 font-mono max-w-[180px] truncate">
                    {String(dup.canonicalNameB ?? dup.canonical_name_b ?? "—")}
                  </td>
          ...
          <div className="space-y-0.5">
            {issues.slice(0, 20).map((iss, i) => (
              <div key={i} className="text-[11px] text-red-700 font-mono">
                [{String(iss.code ?? "")}] {String(iss.message ?? "")}
              </div>
            ))}
          </div>
        </div>
      ...
      {(d.warnings ?? []).length > 0 && (
        <div className="mx-4 bg-amber-50 border border-amber-200 rounded px-3 py-2">
          ...
          {(d.warnings ?? []).map((w, i) => (
            <div key={i} className="text-[11px] text-amber-700 mb-0.5">
              {w}
            </div>
          ))}
        </div>
...
function P84aReclassificationPanel() {
  ...
  const summary = (d.summary ?? {}) as Record<string, unknown>;
  const byCurrentType = (d.byCurrentType ?? {}) as Record<
    string,
    { count: number; totalIncome: number; totalExpense: number }
  >;
  const issues = (d.issues ?? []) as Array<Record<string, unknown>>;
  const internalCompanies = (d.internalCompanies ?? []) as Array<
    Record<string, unknown>
  >;
  const bankOrFee = (d.bankOrFee ?? []) as Array<Record<string, unknown>>;
  const parentClientCandidates = (d.parentClientCandidates ?? []) as Array<
    Record<string, unknown>
  >;
  const needsReviewList = (d.needsManualReviewList ?? []) as Array<
    Record<string, unknown>
  >;
  const dupSample = (d.duplicateCandidatesSample ?? []) as Array<
    Record<string, unknown>
  >;
  const readiness =
    (d.counterpartyReclassificationReadiness as string) ?? "NOT_READY";
  const readinessReason = (d.readinessReason as string) ?? "";
  const byVersion = (summary.byClassificationVersion ?? {}) as Record<
    string,
    number
  >;
  ...
  if (!isProd && !isLoading && !isError) {
    return (
      <div className="p-4 bg-yellow-50 border border-yellow-200 rounded text-[13px] text-yellow-800">
        ...
        <div>
          P8.4a Reclassification Audit доступен только в production (реальные
          bank_transactions). Переключитесь на production URL.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div
          className={`border rounded-lg px-4 py-2.5 flex-1 min-w-0 ${readinessColor}`}
        >
          ...
          <div className="text-[12px]">
            {readinessReason || "Аудит ещё не получен."}
          </div>
        </div>
        <div className="flex gap-2 shrink-0">
          ...
          <button
            onClick={() => reclassMut.mutate()}
            disabled={reclassMut.isPending}
        ...
        {[
          {
            ...
            val: String(summary.totalCounterparties ?? "—"),
            color: "bg-gray-50 border-gray-200",
          },
          {
            label: "Реклассифицировано (p84a_v1)",
            val: String(summary.reclassifiedP84a ?? "—"),
            color: "bg-green-50 border-green-200",
          },
          {
            label: "Исходная P8.3 версия",
            val: String(summary.atP83Original ?? "—"),
            color:
              Number(summary.atP83Original ?? 0) > 0
                ? "bg-red-50 border-red-300"
                : "bg-gray-50 border-gray-200",
          },
          {
            label: "Нужен ручной разбор",
            val: String(summary.needsManualReview ?? "—"),
            color:
              Number(summary.needsManualReview ?? 0) > 0
          ...
          <div
            key={label}
            className={`border rounded-lg px-3 py-2.5 text-center ${color}`}
          >
            <div className="text-[11px] text-gray-500 mb-0.5">{label}</div>
        ...
        <div className="bg-white border border-gray-100 rounded-xl px-4 py-3">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(byVersion).map(([ver, cnt]) => (
              <span
                key={ver}
                className={`px-2.5 py-1 rounded-full text-[11px] font-medium border ${ver === "p84a_v1" ? "bg-green-50 border-green-200 text-green-700" : "bg-red-50 border-red-200 text-red-700"}`}
              >
                {ver}: {cnt}
              </span>
            ))}
      ...
      {Object.keys(byCurrentType).length > 0 && (
        <div className="bg-white border border-gray-100 rounded-xl px-4 py-3">
          <div className="text-[12px] font-semibold text-gray-700 mb-2">
            По типу контрагента (после реклассификации)
          </div>
          <div className="space-y-1.5">
            {Object.entries(byCurrentType)
      ...
      {internalCompanies.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3">
          ...
          <div className="space-y-1">
            {internalCompanies.map((cp) => (
              ...
              >
                ...
                {!!cp.inn && (
                  <span className="text-red-400">ИНН {cp.inn as string}</span>
                )}
                <span className="ml-auto text-red-400">
                  {fmt(Number(cp.totalIncome ?? 0))} /{" "}
                  {fmt(Number(cp.totalExpense ?? 0))} ₽
                </span>
                {!!cp.excludeFromRevExp && (
                  <span className="px-1.5 py-0.5 bg-red-100 border border-red-300 rounded text-[10px] text-red-700">
                    excluded ✓
                  </span>
                )}
              </div>
          ...
          <div className="mt-2 text-[10px] text-red-600 font-medium">
            ⚠️ Исключить из всех расчётов P&L до банковской сверки
          </div>
        </div>
      )}
      ...
      {bankOrFee.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
          <div className="text-[12px] font-semibold text-amber-800 mb-2">
            🏦 Банк / комиссия (требует операционной детализации)
          </div>
          <div className="space-y-1">
            {bankOrFee.map((cp) => (
              <div
              ...
              >
                ...
                <span className="text-amber-500">
                  {cp.operationsCount as number} операций
                </span>
                {!!(cp.riskFlags as Record<string, unknown>)?.[
                  "needs_operation_level_classification"
                ] && (
                  <span className="px-1.5 py-0.5 bg-amber-100 border border-amber-300 rounded text-[10px] text-amber-700">
                    needs op-level split ⚠️
                  </span>
                )}
              </div>
            ))}
          ...
          <div className="mt-1 text-[10px] text-amber-700">
            Не считать все операции Банка Точки расходами — часть может быть
            комиссия эквайринга. Разбор в P8.4b.
          </div>
        </div>
      )}
      ...
      {parentClientCandidates.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
          <div className="text-[12px] font-semibold text-blue-800 mb-1">
            👨‍👩‍👧 Parent-client кандидаты (нужна привязка к AlphaCRM)
          </div>
          <div className="space-y-1">
            {parentClientCandidates.map((cp) => (
              <div key={cp.id as string} className="text-[11px] text-blue-700">
                {cp.displayName as string} — {fmt(Number(cp.totalIncome ?? 0))}{" "}
                ₽ приход
              </div>
            ))}
          </div>
      ...
      {needsReviewList.length > 0 && (
        <div className="bg-white border border-gray-100 rounded-xl px-4 py-3">
          <div className="text-[12px] font-semibold text-gray-700 mb-2">
            🔍 Нужен ручной разбор ({needsReviewList.length})
          </div>
          <div className="space-y-1 max-h-48 overflow-y-auto">
            {needsReviewList.map((cp) => (
              <div
                key={cp.id as string}
                className="flex items-center gap-2 text-[11px] border-b border-gray-50 pb-1"
              >
                <span className="text-gray-700 font-medium min-w-0 truncate">
                  {cp.displayName as string}
                </span>
                <span className="text-gray-400 shrink-0">
                  {cp.counterpartyType as string}
                </span>
                <span className="text-gray-400 shrink-0 ml-auto text-[10px]">
                  {cp.reclassificationReason as string}
                </span>
              </div>
            ))}
          </div>
          <div className="mt-1 text-[10px] text-gray-500">
            Не блокирует P8.4b. Разобрать в ходе Bank ↔ AlphaCRM сверки.
          </div>
        </div>
      )}
      ...
      {dupSample.length > 0 && (
        <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
          <div className="text-[12px] font-semibold text-gray-700 mb-1">
            🪞 Дубли-кандидаты ({Number(summary.openDuplicateCandidates ?? 0)}{" "}
            открытых) — только ручной разбор
          </div>
          <div className="space-y-1 mb-2">
            {dupSample.map((dc, i) => (
              <div key={i} className="text-[11px] text-gray-600">
                {dc.aName as string} ↔ {dc.bName as string}{" "}
                <span className="text-gray-400">({dc.reason as string})</span>
              </div>
            ))}
          </div>
          <div className="text-[10px] text-red-700 font-medium">
            ⚠️ НЕ авто-мержить! same_inn_diff_key = вероятно общий ИНН СБП.
            Разбор вручную.
          </div>
        </div>
      )}
          ...
          <div className="space-y-2">
            {issues.map((issue, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`border rounded px-3 py-2 text-[11px] ${sevColor}`}
                >
                  <div className="font-semibold mb-0.5">
                    [{sev}] {issue.issueType as string} ({issue.count as number}
                    )
                  </div>
                  <div>{issue.description as string}</div>
                  <div className="italic mt-0.5">
                    {issue.recommendedAction as string}
                  </div>
                </div>
              );
            })}
...
function BankAlphaReconciliationPanel() {
  ...
  if (isLoading)
    return (
      <div className="flex items-center gap-2 text-[13px] text-gray-500 py-8 px-4">
        <Loader2 className="animate-spin w-4 h-4" /> Загрузка аудита сверки Bank
        ↔ CRM…
      </div>
    );
  if (isError)
    return (
      <div className="text-red-500 text-[13px] py-4">
        Ошибка загрузки /coverage/bank-alpha-reconciliation-audit
      </div>
    );
  ...
  const byStatus = (matchBreakdown.byStatus ?? {}) as Record<
    string,
    { count: number; totalBankAmount: number }
  >;
  const byConf = (matchBreakdown.byConfidence ?? {}) as Record<string, number>;
  const unmatchedSample = (d.unmatchedBankSample ?? []) as Array<
    Record<string, unknown>
  >;
  const needsReviewSample = (d.needsReviewSample ?? []) as Array<
    Record<string, unknown>
  >;
  const matchedSample = (d.matchedSample ?? []) as Array<
    Record<string, unknown>
  >;
  const issues = (d.issues ?? []) as Array<Record<string, unknown>>;
  ...
  if (!isProd && !isLoading) {
    return (
      <div className="p-4 bg-yellow-50 border border-yellow-200 rounded text-[13px] text-yellow-800">
        <div className="font-semibold mb-1">⚠️ Dev-среда</div>
        <div>
          P8.4b Bank ↔ AlphaCRM Reconciliation доступен только в production
          (реальные bank_transactions). Переключитесь на production URL.
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      {readiness === "NOT_RUN" && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-[12px] text-blue-800">
          ...
          <div>
            Нажми кнопку «legacy action retired» для первого прохода сверки Bank
            ↔ AlphaCRM.
          </div>
          <div className="mt-1 text-[11px] text-blue-600">
            Предусловия: P8.4a COMPLETE ✓ | 854 bank transactions ✓ | 100%
            counterparty links ✓
          </div>
        </div>
      )}
      ...
      {latestRun && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {[
            {
              label: "Bank транзакций",
              val: fmt(Number(latestRun.bankTransactionsChecked ?? 0)),
              color: "bg-gray-50 border-gray-200",
            },
            {
              label: "CRM платежей",
              val: fmt(Number(latestRun.crmPaymentsChecked ?? 0)),
              color: "bg-gray-50 border-gray-200",
            },
            {
              label: "Сопоставлено",
              val: fmt(Number(latestRun.matchedCount ?? 0)),
              color: "bg-green-50 border-green-200",
            },
            {
              label: "Нет совпадений (Bank)",
              val: fmt(Number(latestRun.unmatchedBankCount ?? 0)),
              color:
                Number(latestRun.unmatchedBankCount ?? 0) > 50
                  ? "bg-red-50 border-red-300"
                  : "bg-amber-50 border-amber-200",
            },
          ].map(({ label, val, color }) => (
      ...
      {latestRun && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {[
            {
              label: "Возможные совпадения",
              val: fmt(Number(latestRun.possibleCount ?? 0)),
              color: "bg-yellow-50 border-yellow-200",
            },
            {
              label: "Требуют разбора",
              val: fmt(Number(latestRun.needsReviewCount ?? 0)),
              color:
                Number(latestRun.needsReviewCount ?? 0) > 0
                  ? "bg-amber-50 border-amber-300"
                  : "bg-gray-50 border-gray-200",
            },
            {
              label: "Внутренние (искл.)",
              val: fmt(Number(latestRun.excludedInternalCount ?? 0)),
              color: "bg-gray-50 border-gray-200",
            },
            {
              label: "Инкассация (искл.)",
              val: fmt(Number(latestRun.excludedCollectionCount ?? 0)),
              color: "bg-gray-50 border-gray-200",
            },
          ].map(({ label, val, color }) => (
            <div
      ...
      {latestRun && Number(summary.matchRatePct ?? 0) > 0 && (
        <div className="bg-white border border-gray-100 rounded-xl px-4 py-3">
          <div className="flex items-center justify-between mb-1.5">
            <div className="text-[12px] font-semibold text-gray-700">
              Match rate (income bank → CRM income)
            </div>
            <div className="text-[14px] font-bold text-gray-800">
              {String(summary.matchRatePct ?? 0)}%
            </div>
          </div>
          <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.min(100, Number(summary.matchRatePct ?? 0))}%`,
              }}
            />
          </div>
          <div className="mt-1 text-[10px] text-gray-400">
            {fmt(Number(summary.eligibleBankIncomeCount ?? 0))} eligible bank
            income tx | {fmt(Number(latestRun.matchedCount ?? 0))} confirmed +{" "}
            {fmt(Number(latestRun.possibleCount ?? 0))} possible
          </div>
        </div>
      )}
      ...
      {matchedSample.length > 0 && (
        <div className="bg-white border border-gray-100 rounded-xl px-4 py-3">
          <div className="text-[12px] font-semibold text-gray-700 mb-2">
            ✅ Сопоставленные записи (топ {matchedSample.length} по сумме)
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-[11px]">
              <thead>
              ...
              <tbody>
                {matchedSample.map((m, i) => (
                  <tr key={i} className="border-b border-gray-50">
                    <td className="py-1 pr-2 font-mono font-medium text-green-700">
                      {fmtAmt(Number(m.bankAmount ?? 0))}
                    </td>
                    <td className="py-1 pr-2 font-mono text-gray-600">
                      {(m.bankDate as string) ?? "—"}
                    </td>
                    <td className="py-1 pr-2 font-mono text-gray-600">
                      {(m.crmDate as string) ?? "—"}
                    </td>
                    <td className="py-1 pr-2 text-center font-mono">
                      {m.dateDeltaDays !== null && m.dateDeltaDays !== undefined
                        ? String(m.dateDeltaDays)
                        : "—"}
                    </td>
                    <td className="py-1 pr-2">
                      ...
                      >
                        {CONF_LABELS[m.matchConfidence as string] ??
                          (m.matchConfidence as string)}
                      </span>
                    </td>
                    <td className="py-1 text-gray-500 truncate max-w-[140px]">
                      {(m.bankCounterpartyName as string) ?? "—"}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {needsReviewSample.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
          <div className="text-[12px] font-semibold text-amber-800 mb-2">
            🔍 Требуют разбора — несколько CRM-кандидатов (топ{" "}
            {needsReviewSample.length})
          </div>
          <div className="space-y-1.5 max-h-64 overflow-y-auto">
            {needsReviewSample.map((r, i) => {
              ...
              return (
                <div
                  key={i}
                  className="flex items-start gap-2 text-[11px] border-b border-amber-100 pb-1.5"
                >
                  ...
                  <span className="text-amber-600 shrink-0">
                    {(r.bankDate as string) ?? "—"}
                  </span>
                  <span className="text-amber-700 shrink-0">
                    {(r.bankCounterpartyName as string) ?? "—"}
                  </span>
                  {candidateCount > 0 && (
                    <span className="ml-auto text-amber-500 shrink-0">
                      {candidateCount} CRM-кандидатов
                    </span>
                  )}
                </div>
              );
          ...
          <div className="mt-2 text-[10px] text-amber-700">
            Разбор вручную: несколько CRM платежей с той же суммой. Посмотреть
            контекст студента/цели.
          </div>
        </div>
      )}
      ...
      {unmatchedSample.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3">
          <div className="text-[12px] font-semibold text-red-800 mb-2">
            ❌ Нет совпадения в CRM (топ {unmatchedSample.length} по сумме)
          </div>
          <div className="space-y-1.5 max-h-64 overflow-y-auto">
            {unmatchedSample.map((r, i) => {
              ...
              return (
                <div
                  key={i}
                  className="flex items-start gap-2 text-[11px] border-b border-red-100 pb-1.5"
                >
                  ...
                  <span className="text-red-700 shrink-0 truncate max-w-[160px]">
                    {(r.bankCounterpartyName as string) ?? "—"}
                  </span>
                  {reasons.reason ? (
                    <span className="ml-auto text-red-400 shrink-0 text-[10px]">
          ...
          <div className="mt-2 text-[10px] text-red-700">
            Возможные причины: CRM не зафиксировал платёж, другой период,
            bank-only поступление.
          </div>
        </div>
      )}
...
function EvotorPanel() {
  ...
  const invalidate = () => {
    ...
    qc.invalidateQueries({
      queryKey: getGetCoverageEvotorCoverageAuditQueryKey(),
    });
  };
  ...
  const handleDiscover = () => {
    discoverMut.mutate(undefined, {
      onSuccess: (d) => {
        const r = d as EvotorCoverageAuditResponse & {
          storesCount?: number;
          devicesCount?: number;
        ...
        if (r.storesCount !== undefined) {
          toast.success(
            `Discovery: stores=${r.storesCount}, devices=${r.devicesCount}, employees=${r.employeesCount}`,
          );
        } else {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
        <span
          className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold tracking-wide ${READINESS_COLOR[readiness] ?? "bg-gray-100 text-gray-600"}`}
        >
          {readiness}
        </span>
        <span className="text-[12px] text-gray-500">
          {audit?.readinessReason ?? "…"}
        </span>
        <button
          type="button"
          onClick={invalidate}
        ...
        >
          <RefreshCw
            className={`w-3.5 h-3.5 text-gray-400 ${statusQ.isFetching || auditQ.isFetching ? "animate-spin" : ""}`}
          />
        </button>
      </div>
      ...
      <div className="p-4 bg-white rounded-xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
        ...
        <div className="text-[11px] text-gray-400 mb-3">
          Из Evotor Marketplace → Мои приложения → Ваше приложение → Publisher
          Token. Токен шифруется AES-256-GCM и никогда не возвращается через
          API.
        </div>
        {status?.publisherTokenPresent ? (
          <div className="flex items-center gap-2 mb-3">
            ...
            {status.publisherTokenSavedAt && (
              <span className="text-[11px] text-gray-400 ml-1">
                ·{" "}
                {new Date(String(status.publisherTokenSavedAt)).toLocaleString(
                  "ru-RU",
                )}
              </span>
            )}
      ...
      <div className="p-4 bg-white rounded-xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
        <div className="text-[13px] font-semibold text-gray-800 mb-1">
          Callback URL для Эвотора
        </div>
        <div className="text-[11px] text-gray-400 mb-3">
          Укажите этот URL в настройках приложения в Evotor Marketplace → OAuth
          Redirect / Webhook URL.
        </div>
        <div className="flex items-center gap-2 p-2.5 bg-gray-50 rounded-xl border border-gray-100">
          <code className="flex-1 text-[11px] text-violet-700 font-mono break-all">
            {callbackUrl}
          </code>
          <button
            type="button"
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-3 bg-white rounded-xl border border-gray-100">
          ...
          <div
            className={`text-[12px] font-semibold ${status?.publisherTokenPresent ? "text-emerald-600" : "text-gray-400"}`}
          >
            {status?.publisherTokenPresent
              ? `✓ Сохранён  · ****${status.publisherTokenLast4 ?? ""}`
              : "✗ Не сохранён"}
          </div>
        </div>
        <div className="p-3 bg-white rounded-xl border border-gray-100">
          <div className="text-[11px] text-gray-400 mb-1">
            User Token (OAuth callback)
          </div>
          <div
            className={`text-[12px] font-semibold ${status?.userTokenPresent ? "text-emerald-600" : "text-gray-400"}`}
          >
            {status?.userTokenPresent
              ? `✓ Получен · ****${status.userTokenLast4 ?? ""}`
              : "✗ Не получен"}
          </div>
        </div>
      </div>
      ...
      <div className="p-4 bg-white rounded-xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
        ...
        {status?.lastDiscoveryAt ? (
          <div className="text-[11px] text-gray-400 mb-3">
            Последний запуск:{" "}
            {new Date(String(status.lastDiscoveryAt)).toLocaleString("ru-RU")}
          </div>
        ) : (
          <div className="text-[11px] text-gray-400 mb-3">
            Discovery ещё не запускался
          </div>
        )}
        ...
        <details className="p-3 bg-gray-50 rounded-xl border border-gray-100">
          ...
          <pre className="mt-2 text-[10px] text-gray-600 overflow-x-auto whitespace-pre-wrap break-all">
            {JSON.stringify(
              {
                ...audit,
                discoveryRaw: audit.discoveryRaw ? "(hidden)" : null,
              },
              null,
              2,
            )}
...
export default function CoveragePage() {
  ...
  const syncMut = useRunCoverageSync();
  const scopeQ = useGetCoverageScope();
  const scopeBranchId = String(
    (scopeQ.data?.scope as Record<string, unknown> | null)?.branch_id ?? "6",
  );
  ...
  const invalidateAll = () => {
    ...
    qc.invalidateQueries({ queryKey: getGetCoverageAtlasSummaryQueryKey() });
    qc.invalidateQueries({
      queryKey: getGetCoverageNormalizationAuditQueryKey(),
    });
    qc.invalidateQueries({
      queryKey: getGetCoverageEntityReconciliationQueryKey(),
    });
  ...
  const handleDiscover = () => {
    discoverMut.mutate(
      { data: { branchId, fromDate, toDate } },
      {
        onSuccess: (d) => {
          const bid = String(
            (d as unknown as Record<string, unknown>).branchId ?? branchId,
          );
          toast.success(
            `Discovery запущен для branch ${bid} (batch ${d.batchId?.slice(0, 8)}…) — обновите через 30–60 сек`,
          );
  ...
  const handleSync = () => {
    ...
    syncMut.mutate(
      { data: { branchId, fromDate, toDate } },
      {
        onSuccess: (d) => {
          const warn = String(
            (d as unknown as Record<string, unknown>).outOfScopeWarning ?? "",
          );
          if (warn) toast.warning(warn.slice(0, 100));
          else
            toast.success(
              `Raw sync запущен (batch ${d.batchId?.slice(0, 8)}…) — может занять несколько минут`,
            );
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-[11px] text-gray-400 mt-0.5">
            Технический аудит источника данных — сырой слой, без финальной
            сверки
          </p>
        </div>
        <button
      ...
      <div className="bg-white rounded-[14px] border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-3">
        ...
        {isOutOfScope && (
          <div className="flex items-center gap-1.5 mb-2 px-2 py-1.5 bg-amber-50 border border-amber-200 rounded-xl">
            <AlertTriangle className="w-3.5 h-3.5 text-amber-600 shrink-0" />
            <p className="text-[11px] text-amber-800">
              <strong>⚠ Branch {branchId} вне активного scope</strong> (scope =
              Atlas, branchId={scopeBranchId}). Raw Sync потребует
              подтверждение.
            </p>
          </div>
        )}
      ...
      <div className="bg-white rounded-[14px] border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-4">
        {tab === "bank-p81" && (
          <>
            <SectionHeader
              icon={HardDrive}
              title="P8.1 — Bank Accounts Verification"
              subtitle="Аудит банковской инфраструктуры: коннекторы, счета, балансы, транзакции, владельцы, дубликаты. Вердикт готовности к P8.2."
            />
            <BankAccountsPanel />
          </>
        )}
        {tab === "bank-p82" && (
          <>
            <SectionHeader
              icon={Activity}
              title="P8.2 — Bank Transactions Truth Audit"
              subtitle="Raw→normalized gap, инвентарь полей дат, покрытие по счетам, дубликаты, готовность контрагентного слоя. Production only."
            />
        ...
        {tab === "cp-p83" && (
          <>
            <SectionHeader
              icon={Users}
              title="P8.3 — Counterparty Foundation"
              subtitle="Классификация контрагентов из bank_transactions: тип, уверенность, топ по приходу/расходу, дубли, покрытие. Production only."
            />
            <CounterpartiesPanel />
          </>
        )}
        {tab === "cp-p84a" && (
          <>
            <SectionHeader
              icon={RefreshCw}
              title="P8.4a — Counterparty Reclassification Cleanup"
              subtitle="Исправление ошибок P8.3 классификации: внутренние компании, коммунальщики, лизинг, подрядчики. Исключение внутренних из P&L. Production only."
            />
        ...
        {tab === "bank-p84b" && (
          <>
            <SectionHeader
              icon={GitBranch}
              title="P8.4b — Bank ↔ AlphaCRM Reconciliation"
              subtitle="Сопоставление банковских транзакций с платежами AlphaCRM. Метод: amount + date window. Исключения: internal_company, collection_internal, bank_fee. Production only."
            />
        ...
        {tab === "evotor-p85a" && (
          <>
            <SectionHeader
              icon={Zap}
              title="P8.5a — Evotor Auth Layer"
              subtitle="Подключение Эвотор POS: хранение Publisher Token (AES-256-GCM), OAuth callback, discovery магазинов/устройств/сотрудников. Финансовые отчёты не строятся."
            />
            <EvotorPanel />
          </>
        )}
        {tab === "final-report" && (
          <>
            <SectionHeader
              icon={ClipboardList}
              title="P7.7 — Финальный отчёт AlphaCRM"
              subtitle="Итоговый технический и бизнес-читаемый отчёт Big AlphaCRM Audit. Статус нормализации, идентификации, платежей и готовность к Bank Reconciliation."
            />
        ...
        {tab === "scope" && (
          <>
            <SectionHeader
              icon={Target}
              title="Scope Lock — Atlas Only"
              subtitle="Управление активным scope синхронизации. Только Атлас участвует в raw sync и нормализации данных."
            />
            <ScopePanel />
          </>
        )}
        {tab === "atlas" && (
          <>
            <SectionHeader
              icon={MapPin}
              title="Atlas Branch Summary"
              subtitle="Сводка по сырым данным ветки Атлас — счётчики, даты, статус endpoints"
            />
            <AtlasSummaryPanel />
          </>
        )}
        {tab === "verify" && (
          <>
            <SectionHeader
              ...
              subtitle="Глубокий аудит AlphaCRM: ветки, объёмы, дата-фильтры, пагинация, подозрительные числа"
            />
        ...
        {tab === "registry" && (
          <>
            <SectionHeader
              ...
              subtitle="Результаты последнего discovery — статус каждого AlphaCRM endpoint"
            />
        ...
        {tab === "summary" && (
          <>
            <SectionHeader
              ...
              subtitle="Сводка по сохранённым сырым записям per entity type"
            />
            <SummaryTable />
          </>
        )}
        {tab === "batches" && (
          <>
            <SectionHeader
              icon={Clock}
              title="Sync Batch History"
              subtitle="История всех запусков discovery и raw sync"
            />
            <BatchHistory />
          </>
        )}
        {tab === "fields" && (
          <>
            <SectionHeader
              ...
              subtitle="Какие поля AlphaCRM присутствуют в сырых записях"
            />
        ...
        {tab === "issues" && (
          <>
            <SectionHeader
              ...
              subtitle="Проблемы привязки — payment_unlinked, lesson_subject_missing и т.д."
            />
            <IssuesPanel />
          </>
        )}
        {tab === "raw" && (
          <>
            <SectionHeader
              icon={Eye}
              title="Raw Records Browser"
              subtitle="Просмотр полных JSON-payload из alpha_raw_records"
            />
            <RawRecordsBrowser />
          </>
        )}
        {tab === "audit-norm" && (
          <>
            <SectionHeader
              icon={FlaskConical}
              title="P7 — Нормализация (Raw → Internal)"
              subtitle="Сколько raw-записей стали нормализованными сущностями. Пробелы = данные потеряны при нормализации."
            />
        ...
        {tab === "audit-recon" && (
          <>
            <SectionHeader
              icon={Activity}
              title="P7 — Сверка сущностей"
              subtitle="Raw vs Normalized vs Identity layer. Статус каждой группы сущностей по всем слоям системы."
            />
            <EntityReconciliationPanel branchId={branchId} />
            <div className="mt-6">
              <div className="text-[13px] font-semibold text-gray-700 mb-3">
                💳 Платежи AlphaCRM — Нормализация и аудит (P7.5)
              </div>
              <PaymentAuditPanel branchId={branchId} />
            </div>
            <div className="mt-6">
              <div className="text-[13px] font-semibold text-gray-700 mb-3">
                🧹 Платежи AlphaCRM — Очистка и риск-флаги (P7.6)
              </div>
              <PaymentCleanupPanel branchId={branchId} />
            </div>
            <div className="mt-6">
              <div className="text-[13px] font-semibold text-gray-700 mb-3">
                🪪 Покрытие идентичности в посещениях (P7.4.1)
              </div>
        ...
        {tab === "audit-dups" && (
          <>
            <SectionHeader
              icon={ShieldAlert}
              title="P7 — Дубликаты"
              subtitle="Кандидаты на дублирование студентов, семей, учителей. Детекция только — без авто-слияния."
            />
