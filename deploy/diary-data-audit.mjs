...
export function auditDiary(db) {
  ...
  const result = {
    ...
    teachers: count("SELECT count(*) n FROM users WHERE role='teacher' AND status='active'"),
    ...
    lessonsWithInactiveTeacher: count("SELECT count(*) n FROM lessons l LEFT JOIN users u ON u.id=l.teacher_user_id WHERE l.status NOT IN ('archived','cancelled') AND l.teacher_user_id IS NOT NULL AND l.teacher_user_id!='' AND (u.id IS NULL OR u.status!='active' OR u.role!='teacher')"),
