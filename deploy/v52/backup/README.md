...
Ручной запуск доступен собственнику в «Настройки → Резервные копии» после
установки host bridge из `install-bridge.sh` и подключения его к приложению.
Интерфейс показывает состояние задачи, расписание и историю проверенных копий.
Уведомления о неуспешной суточной копии пока не реализованы. Ограниченный bridge
не выдаёт пользователям shell-доступ и не передаёт Docker socket приложению.
