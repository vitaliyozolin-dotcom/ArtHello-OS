...
export type ReleaseGateState={id?:string;status:string;required:boolean|number};
...
export function prerequisiteGatesPassed(gates:ReleaseGateState[]){
  return requiredGatesPassed(gates.filter(gate=>gate.id!=="GATE-T-APPROVAL"));
}

export function manualEvidence(value:unknown){
  const evidence=typeof value==="string"?value.trim():"";
  if(evidence.length<8)return{ok:false as const,error:"Добавьте конкретное проверяемое основание — минимум 8 символов"};
  if(/CONTROL:EVIDENCE(?::|$)/i.test(evidence))return{ok:false as const,error:"Автоматическая метка не является доказательством"};
  return{ok:true as const,evidence:evidence.slice(0,1000)};
}
