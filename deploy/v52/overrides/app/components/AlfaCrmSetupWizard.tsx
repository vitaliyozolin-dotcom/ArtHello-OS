...
export function AlfaCrmSetupWizard({ roleCode, close, notify }: {
  ...
  const [payload, setPayload] = useState<AlfaPayload>({
    state: emptyState(), localBranches: [], credentialStored: false, canManage: false,
    ...
    <div className="ahIntegrationModalLayer ahAlfaCrmLayer">
      ...
      <section ref={dialogRef} className="ahAlfaCrmWizard" role="dialog" aria-modal="true" aria-labelledby="alfacrm-wizard-title" tabIndex={-1}>
          ...
          {!loading ? <section className="ahAlfaCrmPanel">
            ...
            {state.connected && !connectionEdit ? <div className="ahAlfaCrmConnectionSummary">
              ...
              <div className="ahAlfaCrmSummaryActions">
                <button type="button" disabled={Boolean(busy)} onClick={() => void post({ action: "refreshBranches" }, "refresh")}>{busy === "refresh" ? "Проверяю…" : "Проверить соединение"}</button>
                <button type="button" className="secondary" onClick={() => setConnectionEdit(true)}>Заменить ключи</button>
              </div>
            </div> : <form className="ahAlfaCrmConnectForm" onSubmit={(event) => {
          ...
          {!loading && state.connected ? <section className="ahAlfaCrmPanel">
            ...
            <div className="ahAlfaCrmBranchTable" role="table" aria-label="Сопоставление филиалов">
              ...
              {state.remoteBranches.map((branch) => <div className="ahAlfaCrmBranchRow" role="row" key={branch.id}>
                ...
                <select value={mappings[branch.id] ?? ""} onChange={(event) => setMappings({ ...mappings, [branch.id]: event.target.value })}>
            ...
            <div className="ahAlfaCrmPanelActions"><button type="button" disabled={Boolean(busy) || !Object.values(mappings).some(Boolean)} onClick={() => void post({ action: "saveBranchMappings", mappings }, "mapping")}>{busy === "mapping" ? "Сохраняю…" : "Сохранить сопоставление филиалов"}</button></div>
          </section> : null}
        ...
        <footer className="ahAlfaCrmFooter">
          ...
          <div>
            {state.connected ? <button type="button" className="danger" disabled={Boolean(busy)} onClick={() => {
...
function ModuleCard({ definition, state, period, setPeriod, busy, preview, importData }: {
  ...
  const canImport = Boolean(moduleState.previewToken) && !dependencyMissing;
  ...
  return <article className={`ahAlfaCrmModule ${imported ? "complete" : ""}`}>
    ...
    <div className="ahAlfaCrmModuleActions">
      <button type="button" className="secondary" disabled={Boolean(busy) || Boolean(dependencyMissing)} onClick={() => void preview()}>{previewBusy ? "Считаю…" : imported ? "Пересчитать" : "Проверить данные"}</button>
