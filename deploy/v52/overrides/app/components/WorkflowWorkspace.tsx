...
function WorkTab({ detail, action, createSubtask }: { detail: TaskDetail; action: (payload: Record<string, unknown>, message: string) => Promise<void>; createSubtask: () => void }) {
  return <>
    <section className="task-summary"><p>{detail.task.description || "Описание не добавлено."}</p><dl><div><dt>Ответственный</dt><dd>{detail.task.owner}<small>{detail.task.assigneeEntityId || "без связи со справочником"}</small></dd></div><div><dt>Срок</dt><dd>{detail.task.dueDate ? formatDate(detail.task.dueDate) : "Без срока"}<small>{detail.task.recurrenceRule || "однократно"}</small></dd></div><div><dt>Источник</dt><dd>{detail.task.sourceType}<small>{detail.task.sourceId}</small></dd></div><div><dt>Результат</dt><dd>{detail.task.result || "Ещё не передан"}<small>{detail.task.resultEvidence || "нет основания"}</small></dd></div></dl></section>
