...
function SafetyOverview({ data, hasData }: { data: SafetyData; hasData: boolean }) {
  ...
  return <div className="ahSafetyLayout">
    <Card className="ahSafetyPanel">
      ...
      <div className="ahSafetySystemList">{data.systems.map((system, index) => <CompactListCard key={system.id} index={String(index + 1).padStart(2, "0")} title={system.name} description={`${system.systemType} · ${system.objectEntityId} · ${system.status}`} />)}</div>
    </Card>
    <Card className="ahSafetyPanel">
      ...
      ].map(([label, value], index) => <CompactListCard key={label} index={String(index + 1).padStart(2, "0")} title={label} description={value || "не создано"} />)}</div>
    </Card>
  </div>;
}
...
function SafetySystems({ data, busy, schedule }: { data: SafetyData; busy: string; schedule: (equipmentId: string) => void }) {
  ...
  return <div className="ahSafetySystemGrid">{data.systems.map((system) => <Card key={system.id} className="ahSafetyPanel">
    ...
    <div className="ahSafetySystemMeta"><span>Схема <strong>{system.schemeRef || "не приложена"}</strong></span><span>Журнал <strong>{system.journalRef || "не создан"}</strong></span></div>
    <div className="ahSafetyEquipmentList">{data.equipment.filter((item) => item.systemId === system.id).map((item) => <article key={item.id}>
      <div><strong>{item.name}</strong><small>{item.inventoryNumber} · {item.location}</small></div><em>{item.criticality}</em><span>{item.nextCheckAt}</span>
...
function SafetyIncidents({ data }: { data: SafetyData }) {
  ...
  return <div className="ahSafetyLayout">
    <Card className="ahSafetyPanel"><PanelHead eyebrow="Инциденты" title="Событие → реакция" note={`${data.incidents.length} записей`} /><div className="ahSafetyIncidentList">{data.incidents.map((incident) => <article key={incident.id}><header><strong>{incident.category}</strong><em>{incident.severity}</em></header><p>{incident.description}</p><small>{incident.response}</small><footer>{incident.objectEntityId} · {incident.status}</footer></article>)}</div></Card>
    <Card className="ahSafetyPanel"><PanelHead eyebrow="Охрана" title="Посты и смены" note="журнал" /><div className="ahSafetyGuardList">{data.guardShifts.map((shift, index) => <CompactListCard key={shift.id} index={String(index + 1).padStart(2, "0")} title={shift.post} description={`${data.entityNames[shift.employeeEntityId] ?? shift.employeeEntityId} · ${shift.startedAt.slice(11, 16)}–${shift.endedAt.slice(11, 16)} · ${shift.status}`} />)}</div></Card>
  </div>;
}
...
function SafetyRepairs({ data, busy, complete, onOpenFinance }: { data: SafetyData; busy: string; complete: (repairId: string) => void; onOpenFinance: () => void }) {
  ...
  return <Card className="ahSafetyPanel"><PanelHead eyebrow="Ремонты и документы" title="Закрытие неисправностей" note={`${data.repairs.length} записей`} /><div className="ahSafetyRepairTable"><table><thead><tr><th>Ремонт</th><th>Подрядчик</th><th>Стоимость</th><th>Акт</th><th>Оплата</th><th>Статус</th><th>Действие</th></tr></thead><tbody>{data.repairs.map((repair) => <tr key={repair.id}>
    <td data-label="Ремонт">{repair.id}<small>{repair.actionType} · {repair.faultId}</small></td>
    <td data-label="Подрядчик">{data.entityNames[repair.contractorId] ?? repair.contractorId}</td>
    <td data-label="Стоимость">{rub(repair.costMinor)}</td>
    <td data-label="Акт">{repair.actDocumentId || "Не приложен"}</td>
    <td data-label="Оплата">{repair.paymentOperationId || "Заблокирована"}</td>
...
function SafetyRepairModal({ repairId, entities, busy, close, save }: { repairId: string; entities: Array<[string, string]>; busy: boolean; close: () => void; save: (body: Record<string, unknown>) => Promise<void> }) {
  ...
  return createPortal(<div className="ahSafetyModalLayer"><button className="ahSafetyModalScrim" type="button" onClick={close} aria-label="Закрыть форму" /><form className="ahSafetyModal ahSafetyModalCompact" onSubmit={submit}>
    <header><div><p>{repairId}</p><h2>Завершить ремонт</h2></div><button type="button" onClick={close} aria-label="Закрыть">×</button></header>
    ...
    <label><span>Рабочий акт *</span><input name="actDocumentId" required minLength={5} placeholder="Номер или ID фактического акта" /></label>
