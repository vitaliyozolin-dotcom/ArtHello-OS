...
export function CompactListCard({ index, title, description, className }: {
  ...
  return <article className={join("ahCard", "ahCompactListCard", className)} data-ah-compact-card="true">
    {index ? <span data-ah-compact-index>{index}</span> : null}
    <div><strong data-ah-compact-title>{title}</strong>{description ? <small data-ah-compact-body>{description}</small> : null}</div>
  </article>;
}
...
export function EmptyState({ title, description, action, className, density = "default" }: {
  title: string;
  description: string;
  action?: ReactNode;
  className?: string;
  ...
  return <div className={join("ahEmptyState", className)} data-ah-compact-card={density === "compact" ? "true" : undefined}><h3 data-ah-compact-title>{title}</h3><p data-ah-compact-body>{description}</p>{action}</div>;
}
...
export const designSystemVersion = "1.4-management-insight";
