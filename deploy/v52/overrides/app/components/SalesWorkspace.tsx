...
export function SalesWorkspace({ workspace, role, notify, onTasksChanged, onOpenFinance, focusId }: { workspace: "sales" | "clients"; role: string; notify: (message: string) => void; onTasksChanged: () => void; onOpenFinance: () => void; focusId?: string }) {
  ...
  return (
    <section className="page sales-workspace">
      ...
      </div> : <div data-ah-compact-card="true" className="manual-module-empty"><span>↗</span><h2>Сквозная цепочка ещё не собрана</h2><p>Для лида {chainLead.id} пока нет связанного договора, начисления или платежа. Раздел продолжает работать без фиктивных связей.</p></div> : null}
