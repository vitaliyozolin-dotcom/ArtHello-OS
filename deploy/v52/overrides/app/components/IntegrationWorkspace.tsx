...
export function IntegrationWorkspace({
  ...
  return <PageContainer className="ahIntegrationPage">
    ...
    {wizardId && data.capabilities.canManageSetup && currentBankCapability(wizardId, data.capabilities) ? <ConnectionWizard
  ...
  return createPortal(<div className="ahIntegrationModalLayer">
    ...
    <form ref={dialogRef} className="setup-wizard ahIntegrationSetupWizard" onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="setup-title" tabIndex={-1}>
      ...
      <div className="setup-grid">
        ...
        {tochka && customerChoices.length ? <label><span>Компания в Точке</span><select value={customerChoiceId} onChange={(event) => setCustomerChoiceId(event.target.value)} required><option value="">Выберите компанию</option>{customerChoices.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select><small>Показаны только названия компаний. Служебный код не передаётся в браузер и журнал.</small></label> : null}
