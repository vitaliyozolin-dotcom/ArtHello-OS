...
async function createPlanItem(actor: string, body: Record<string, unknown>) {
  ...
  if (!scheduledAt || Number.isNaN(new Date(scheduledAt).getTime()) || !accountId || !authorEntityId || isDemoReference(authorEntityId) || !formats.has(format) || topic.length < 5 || brief.length < 10) {
    return Response.json({ error: "Заполните дату, рабочий канал, автора, формат, тему и содержательный бриф" }, { status: 400 });
  }
  const db = getDb();
  const [account] = await db.select({ id: marketingAccounts.id }).from(marketingAccounts).where(eq(marketingAccounts.id, accountId)).limit(1);
  const [author] = await db.select({ id: entities.id }).from(entities).where(eq(entities.id, authorEntityId)).limit(1);
  if (!account || !author) return Response.json({ error: "Канал или автор не найдены в рабочем справочнике" }, { status: 404 });
  const id = `PLAN-${crypto.randomUUID().slice(0, 8).toUpperCase()}`;
  const [item] = await db.insert(contentPlanItems).values({ id, scheduledAt: new Date(scheduledAt).toISOString(), accountId, authorEntityId, format, topic, brief, offerId: clean(body.offerId, 80), campaignId: clean(body.campaignId, 80), status: "Черновик", createdBy: actor }).returning();
  await db.insert(auditEvents).values({ actor, action: "content.plan_created", entityType: "content_plan_item", entityId: id, payload: JSON.stringify({ accountId, format, topic, authorEntityId }) });
...
async function publishItem(actor: string, body: Record<string, unknown>) {
  ...
  if (publicationRef.length < 6 || isDemoReference(publicationRef)) return Response.json({ error: "Укажите рабочую ссылку или ID опубликованного материала" }, { status: 400 });
  const db = getDb();
  ...
  const [publication] = await db.insert(contentPublications).values({ id, planItemId, publishedAt: new Date().toISOString(), publicationRef, reach: 0, views: 0, reactions: 0, clicks: 0, leads: 0, contracts: 0, revenueMinor: 0, sourceType: "MANUAL", dataQuality: "Публикация подтверждена вручную; метрики ещё не получены" }).returning();
