...
import {
  ...
  educationLessons, educationPrograms, educationProgress, educationStudents, entities,
} from "../../../db/schema";
...
export async function GET(request: Request) {
  ...
  if (scope.kind === "none") return Response.json({ error: "Нет доступа к образовательному контуру" }, { status: 403 });
  try {
    ...
    if (scope.kind === "teacher") groups = scope.id ? allGroups.filter((row) => row.teacherEntityId === scope.id) : [];
    if (scope.kind === "family") students = scope.id ? allStudents.filter((row) => row.familyEntityId === scope.id) : [];
    if (scope.kind !== "all") {
      const scopedGroupIds = new Set(scope.kind === "teacher" ? groups.map((row) => row.id) : students.map((row) => row.groupId));
      groups = allGroups.filter((row) => scopedGroupIds.has(row.id));
      students = allStudents.filter((row) => scopedGroupIds.has(row.groupId) && (scope.kind !== "family" || row.familyEntityId === scope.id));
    }
    ...
    const communications = allCommunications.filter((row) => row.audienceType === "Группа" ? groupIds.has(row.audienceId) : scope.kind === "all" || row.audienceId === scope.id);
    const visiblePrograms = scope.kind === "all" ? programs : programs.filter((row) => groups.some((group) => group.programId === row.id));
