...
import { probeTochkaJwt, validateTochkaJwt } from "../lib/integrations.ts";
...
async function loadIntegrationActionsRoute(stubs) {
  ...
  const preamble = `
    const {
      ensureCoreTables,getDb,getIntegrationSetups,readIntegrationCredential,
      ensureOperatingIntegrationCatalog,
      revokeTochkaIntegrationCredential,saveIntegrationSetup,saveTochkaSetupWithCredential,
      validateIntegrationSetupReferences,canResolveConflict,probeTochkaJwt,retryDecision,
      validateTochkaJwt,getAuthenticatedRequestContext,isCanonicalOwnerContext,
      verifyAuthenticatedRequestCsrf,getRequestUser,findScopedAutomationTask,scopedAutomationTaskResponse,
...
function actionRouteStubs(overrides = {}) {
  return {
    ...
    revokeTochkaIntegrationCredential: async () => null,
...
function integrationActionRequest(action, role = "OWNER", csrf = "csrf-ok") {
  return new Request("https://example.test/api/integration-actions", {
    ...
    body: JSON.stringify({ action, connectionId: "INT-T-TOCHKA" }),
  });
}
...
test("Tochka probe requires an explicit customerCode when JWT exposes multiple customers", async () => {
  ...
  assert.match(result.reason, /нескольким customerCode/i);
...
test("Tochka probe fails safely when the bank returns no customerCode", async () => {
  ...
  assert.equal(result.valid, false);
  assert.match(result.reason, /доступных customerCode нет/i);
  assert.equal(result.accountCountScope, "none");
  assert.equal(result.customerCode, "");
  assert.deepEqual(result.customerChoices, []);
...
test("Tochka probe rejects a requested customerCode not granted to the JWT", async () => {
  ...
  assert.equal(result.valid, false);
  assert.match(result.reason, /customerCode не доступен/i);
  assert.equal(result.accountCountScope, "none");
  assert.equal(result.customerCode, "");
...
test("Tochka probe rejects a successful response when no supported account array contains accounts", async () => {
  ...
  for (const payload of [{ Data: { Account: [] } }, { Data: { Accounts: [] } }]) {
    ...
    assert.deepEqual(result, {
      valid: false,
      reason: "JWT подтверждён, но доступных счетов нет",
      expiresAt: new Date(fixedNow + 3_600_000).toISOString(),
      accountCount: 0,
...
test("owner credential UI is masked, transient and sends the production CSRF token", async () => {
  ...
  assert.match(workspace, /JWT-ключ Точки[\s\S]*?<input\s+type="password"/);
  ...
  assert.match(workspace, /const tochkaRedirectUrl = useMemo\(/);
  assert.match(workspace, /typeof window === "undefined" \? "\/api\/integrations\/tochka\/callback"/);
  assert.match(workspace, /window\.location\.origin\}\/api\/integrations\/tochka\/callback/);
...
test("all Tochka mutations and bank discovery require the canonical owner with CSRF verification", async () => {
  ...
  assert.match(actions, /const tochkaOwnerActions = new Set\(\[[\s\S]*?"saveSetup"[\s\S]*?"testConnection"[\s\S]*?"retrySync"[\s\S]*?"pauseConnection"[\s\S]*?"resumeConnection"[\s\S]*?"revokeCredential"/);
  assert.match(actions, /context = await getAuthenticatedRequestContext\(request\)[\s\S]*?!editors\.has\(context\.apiRole\)/);
  assert.match(actions, /connectionId === tochkaConnectionId && tochkaOwnerActions\.has\(action\)[\s\S]*?authorizeTochkaOwnerMutation\(request, context\)[\s\S]*?await ensureCoreTables\(\)/);
  assert.match(actions, /function authorizeTochkaOwnerMutation\(request: Request, requester: RequestContext\)/);
...
test("a non-owner cannot reuse the stored owner JWT for discovery or revoke it", async () => {
  let ensureCalls = 0;
  ...
  const route = await loadIntegrationActionsRoute(actionRouteStubs({
    ...
    getAuthenticatedRequestContext: async () => ({ actor: "DIRECTOR-LIVE", apiRole: "DIRECTOR" }),
    isCanonicalOwnerContext: () => false,
    ...
    revokeTochkaIntegrationCredential: async () => { revokeCalls += 1; return null; },
  }));
  try {
    for (const action of ["testConnection", "retrySync", "revokeCredential", "pauseConnection", "resumeConnection"]) {
      const response = await route.POST(integrationActionRequest(action, "DIRECTOR"));
      assert.equal(response.status, 403, `${action} must be owner-only for Tochka`);
    ...
    assert.equal(csrfCalls, 0, "role denial precedes CSRF validation");
  } finally {
    delete globalThis.__ARTHELLO_TOCHKA_ACTION_TEST__;
  }
});

test("owner revoke is CSRF-protected, deletes the JWT without returning it and is auditable", async () => {
  let ensureCalls = 0;
  let revokeCalls = 0;
  let csrfCalls = 0;
  let revokedBy = "";
  const route = await loadIntegrationActionsRoute(actionRouteStubs({
    ensureCoreTables: async () => { ensureCalls += 1; },
    getAuthenticatedRequestContext: async () => ({ actor: "OWNER-LIVE", apiRole: "OWNER", appUserId: "USR-OWNER" }),
    ...
    revokeTochkaIntegrationCredential: async (actor) => {
      revokeCalls += 1;
      revokedBy = actor;
  ...
  try {
    ...
    assert.match(payload.message, /удалён из защищённого хранилища/i);
...
test("credential rotation and revoke are atomic, scoped and leave no orphan JWT", async () => {
  ...
  try {
    ...
    assert.deepEqual({ ...connection }, {
      ...
      auth_status: "JWT отозван владельцем",
      verified_transfer: 0,
      is_enabled: 0,
    });
...
test("a candidate JWT is probed before atomic setup and credential replacement", async () => {
  ...
  const probeAt = saveAction.indexOf("candidateProbe = await probeTochkaJwt(");
  ...
  assert.match(saveAction.slice(probeAt, rejectionAt), /clean\(setupInput\.customerCode, 80\)/);
  ...
  const atomicStart = database.indexOf("export async function saveTochkaSetupWithCredential");
  const atomicEnd = database.indexOf("export async function hasIntegrationCredential", atomicStart);
  const atomic = database.slice(atomicStart, atomicEnd);
  assert.match(atomic, /await env\.DB\.batch\(\[/);
  assert.match(atomic, /DELETE FROM system_runtime_state WHERE state_key LIKE \? AND state_key<>\?/);
  assert.match(atomic, /integrationCredentialConnectionPattern\(setup\.connectionId\)/);
  assert.match(atomic, /integration\.credential_replaced/);
});
...
test("reader roles receive a factual view without management actions", async () => {
  ...
  assert.match(workspace, /wizardId && data\.capabilities\.canManageSetup && \(wizardId !== TOCHKA_CONNECTION_ID \|\| data\.capabilities\.canManageTochka\)/);
  assert.match(workspace, /data\.capabilities\.canRun \? <Button variant="primary"/);
  assert.match(workspace, /const canRunCurrent = data\.capabilities\.canRun && currentTochkaAllowed/);
  assert.match(workspace, /const canChangeCurrentState = data\.capabilities\.canChangeState && currentTochkaAllowed/);
...
test("credential storage is encrypted and scoped by provider, legal entity and verified customerCode", async () => {
  ...
  assert.match(auditPayload, /customerCode/);
...
test("Tochka copy removes administrator hand-off and honestly scopes all JWT-visible accounts", async () => {
  ...
  assert.match(workspace, /Один JWT для выбранной карточки юрлица/);
  assert.match(workspace, /Все счета, разрешённые JWT/);
  ...
  assert.match(workspace, /Банк не подтверждает их принадлежность выбранной карточке/);
  assert.match(workspace, /JWT и список доступных счетов/);
  assert.match(workspace, /startDate: tochka \? "" : startDate/);
  assert.match(workspace, /syncIntervalMinutes: tochka \? 0 : interval/);
  assert.match(workspace, /dataScopes: tochka \? \["Счета"\] : dataScopes/);
  assert.match(workspace, /Загрузка выписок, расписание синхронизации и правила распределения операций ещё не запущены/);
  assert.match(workspace, /Сохранить JWT и проверить доступные счета/);
});
...
test("account discovery does not claim that transaction import is connected", async () => {
  ...
  assert.match(discovery, /status:\s*"Доступ к счетам подтверждён"/);
  assert.match(discovery, /verifiedTransfer:\s*false/);
  assert.match(discovery, /isEnabled:\s*false/);
  assert.match(discovery, /acceptedCount:\s*0/);
  assert.match(discovery, /dryRun:\s*true/);
  assert.match(discovery, /lastSuccessAt:\s*""/);
  assert.doesNotMatch(discovery, /lastSuccessAt:\s*current/);
  assert.match(discovery, /импорт операций ещё не запускался/i);
  assert.match(integrationsApi, /JWT, customerCode и доступ к счетам подтверждены/);
});
