...
@media(max-width:720px){
  ...
  .page :is(article,section,[class*=panel],[class*=card]){box-sizing:border-box}

  /* One card geometry everywhere. */
  .safety-boundary,
  .safety-kpis>*,
  ...
  /* Dashboard KPI strip: fixed icon column + safe text column, no edge bleed. */
  ...
  [data-help-block="kpis"] :is(strong,span,p,small,em){
    max-width:100%!important;
    min-width:0!important;
  ...
  /* Families: same page gutter, one search card, one list card. */
  .family-workspace{
  ...
  /* Contractors: exactly one outer card; never frame text descendants. */
  ...
  .contractor-workspace .page>:last-child :is(h1,h2,h3,h4,p,strong,span){
    max-width:100%!important;
    overflow-wrap:anywhere!important;
  }

  .contractor-workspace .page>:last-child button:empty,
  .contractor-workspace .page>:last-child div:empty{display:none!important}

  /* Safety uses the same radius and spacing as every other module. */
  .safety-boundary,
  .safety-kpis>*,
  .operational-empty-card,
  .operational-inline-empty{
  ...
  /* Finance period is a compact filter, not a second hero card. */
  .finance-period-bar{
    display:grid!important;
  ...
  .finance-period-bar>div:first-child small{display:none!important}
