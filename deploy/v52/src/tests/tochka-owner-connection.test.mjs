import { acquireTochkaStatementState, tochkaStatementLeaseGuardSql, TochkaStatementStateChanged } from '../lib/tochka-statement-state.ts';
...
async function loadIntegrationActionsRoute(stubs) {
  ...
  const preamble = `
    const {
      consumeTochkaCompanySelectionHandle,commitIntegrationBankProbe,commitTochkaReadOnlySync,createTochkaCompanySelectionHandles,openTochkaStatementState,TochkaStatementStateChanged,
...
test("Tochka statement commit is idempotent and preserves manual finance classification", async () => {
  ...
  try {
    ...
    const first = await dbModule.commitTochkaReadOnlySync("OWNER-LIVE", setup, sync, "Первичная загрузка", statementState.fence);
