...
test("D176 presents one compact bank summary and compact account rows", async () => {
  const [workspace, styles] = await Promise.all([
    source("../app/components/FinanceWorkspace.tsx"),
    source("../app/components/FinanceWorkspace.ds.css"),
  ]);

  assert.match(workspace, /data-d176-marker="D176_MONEY_MOBILE_HISTORY"/);
...
test("D176 replaces the wide bank table with a date-grouped responsive history", async () => {
  const [workspace, styles] = await Promise.all([
    source("../app/components/FinanceWorkspace.tsx"),
    source("../app/components/FinanceWorkspace.ds.css"),
  ]);
