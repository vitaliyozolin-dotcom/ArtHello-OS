...
test("owner can show, hide, reorder and resize dashboard widgets", () => {
  ...
  assert.match(dashboard, /reorderDashboardWidgets\(current, id, target\.id, directionToMove === -1 \? "before" : "after"\)/);
  assert.doesNotMatch(dashboard, /\[next\[index\], next\[nextIndex\]\] = \[next\[nextIndex\], next\[index\]\]/);
  ...
  assert.match(dashboard, /<span className=\{styles\.dragHandle\}[^>]+draggable=\{editing\}/);
  assert.doesNotMatch(dashboard, /<button className=\{styles\.dragHandle\}/);
  ...
  assert.equal((dashboard.match(/if \(!editing \|\| !draggedWidgetRef\.current\) return;/g) ?? []).length, 2);
