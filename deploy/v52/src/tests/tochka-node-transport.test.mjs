...
test("production runtime binds the worker to the Node Tochka transport", () => {
  ...
  assert.match(route, /syncTochkaReadOnly\(\{[\s\S]*?request:\s*async\s*\(input, init\)\s*=>\s*\{\s*await statementState\.assertCurrent\(\);\s*return tochkaTransportFetch\(input, init\);/);
});
