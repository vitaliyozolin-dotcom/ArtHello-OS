...
import { buildFunnel, buildLtvPlan, nextSalesStage, riskExplanation, scoreCampaigns } from "../lib/sales.ts";
...
test("LTV plan separates paid fact from the risk-adjusted forecast", () => {
  const plan = buildLtvPlan([{ familyEntityId: "FAM-1", monthlyValueMinor: 10000, ltvMinor: 30000, churnRiskScore: 20 }]);
  assert.equal(plan.actualLtvMinor, 30000);
  assert.equal(plan.baseNext12MonthsMinor, 120000);
  assert.equal(plan.riskAdjustedNext12MonthsMinor, 96000);
  assert.equal(plan.forecastLtvMinor, 126000);
  assert.equal(plan.families[0].retentionProbability, 80);
  assert.match(plan.method, /не финансовый факт/);
});
