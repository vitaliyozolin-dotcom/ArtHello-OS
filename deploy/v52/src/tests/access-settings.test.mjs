...
test("the operating branch directory matches the real ArtHello structure", async () => {
  const [settings, integrations, integrationsApi] = await Promise.all([
  ...
  assert.match(integrationsApi, /organizationBranches\.status, "Активен"/);
  assert.match(integrationsApi, /branches:\s*branchRows/);
  assert.match(integrations, /branches\.map\(\(branch\) => <option key=\{branch\.id\} value=\{branch\.id\}>\{branch\.name\}/);
