...
export function buildLtvPlan(rows: LtvPlanInput[]) {
  const families = rows.map((row) => {
    const retentionProbability = Math.max(25, Math.min(98, 100 - row.churnRiskScore));
    const baseNext12MonthsMinor = row.monthlyValueMinor * 12;
    const riskAdjustedNext12MonthsMinor = Math.round(baseNext12MonthsMinor * retentionProbability / 100);
    return {
      familyEntityId: row.familyEntityId,
      actualLtvMinor: row.ltvMinor,
      monthlyValueMinor: row.monthlyValueMinor,
      retentionProbability,
      baseNext12MonthsMinor,
      riskAdjustedNext12MonthsMinor,
      forecastLtvMinor: row.ltvMinor + riskAdjustedNext12MonthsMinor,
    };
  });
  const actualLtvMinor = families.reduce((sum, row) => sum + row.actualLtvMinor, 0);
  const baseNext12MonthsMinor = families.reduce((sum, row) => sum + row.baseNext12MonthsMinor, 0);
  const riskAdjustedNext12MonthsMinor = families.reduce((sum, row) => sum + row.riskAdjustedNext12MonthsMinor, 0);
  const forecastLtvMinor = families.reduce((sum, row) => sum + row.forecastLtvMinor, 0);
  return {
    families,
    actualLtvMinor,
    averageActualLtvMinor: families.length ? Math.round(actualLtvMinor / families.length) : 0,
    baseNext12MonthsMinor,
    riskAdjustedNext12MonthsMinor,
    forecastLtvMinor,
    method: "Факт LTV + ежемесячная стоимость × 12 месяцев × (100% − риск ухода). Риск — управленческое допущение, а не финансовый факт.",
  };
}
