...
type DetailEvidence = { parentName?: string; hasDuplicate?: boolean };
...
export function familyCardState(family: FamilyStateInput, detail?: DetailEvidence) {
  ...
  if (detail && (!detail.parentName?.trim() || detail.parentName.trim() === 'Представитель семьи')) issues.push({code:'parent-missing',reason:'Не указан родитель или представитель',action:'Укажите ФИО представителя в разделе «Родитель / представитель» и проверьте, что он относится к этому ребёнку.',target:'edit'});
