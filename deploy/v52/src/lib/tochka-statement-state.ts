...
export type TochkaStatementLeaseFence = { key: string; owner: string; requireAutomatic?: boolean };
...
export async function acquireTochkaStatementState(db: Database, scope: CredentialScope) {
  ...
  const fence = { key: 'tochka-statement-lease:v1:' + scope.connectionId, owner: crypto.randomUUID(), ...(scope.requireAutomatic ? { requireAutomatic: true } : {}) };
  const now = Date.now();
