...
const entityApi = read("app/api/entities/route.ts");
const hrActions = read("app/api/hr-actions/route.ts");
const salesActions = read("app/api/sales-actions/route.ts");
const legalActions = read("app/api/legal-actions/route.ts");
...
requireText(finance, 'data-d176-marker="D176_MONEY_MOBILE_HISTORY"', "finance compact turnover summary is missing");
...
requireText(family, "Карточка не равна доступу", "family access boundary is missing");
requireText(registry, "Создано вручную", "manual registry provenance is missing");
requireText(registry, "отдельная самопроверка не нужна", "manual registry still asks for self-review");
requireText(registry, "Основание сверки с источником", "external review evidence is missing");
forbid(registry, />Качество<|Любое качество|Фильтр по качеству/, "registry still exposes the ambiguous quality wording");
requireText(entityApi, "initialEntityDataQuality(sourceSystem)", "entity API does not apply the provenance policy");
requireText(entityApi, "needsExternalReviewEvidence", "external verification can lose its evidence");
