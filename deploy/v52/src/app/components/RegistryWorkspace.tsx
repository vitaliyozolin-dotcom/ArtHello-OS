...
export function RegistryWorkspace({ notify, capabilities }: { notify: (value: string) => void; capabilities: RegistryCapabilities }) {
  ...
  return <PageContainer className="ahRegistryPage">
    ...
    <Card className="ahRegistryShell">
      ...
      {state === "loading" ? <RegistryState title="Загружаем реестр" text="Проверяем карточки, источники и единые номера." /> : null}
      ...
      {state === "ready" && entities.length > 0 ? <div className="ahRegistryTable"><table className="registry-table"><thead><tr><th>Карточка</th><th>Тип</th><th>Используется в</th><th>Источник</th><th>Состояние данных</th><th>Статус</th><th /></tr></thead><tbody>
        {entities.map((entity) => <tr key={entity.id} onClick={() => setSelectedId(entity.id)}>
          <td><strong>{entity.displayName}</strong><small>{recordLabel("Карточка", entity.id)}</small></td>
          <td><span className="entity-type-badge">{entity.entityType}</span></td>
          <td><strong>{entity.scope}</strong><small>единая карточка во всех разделах</small></td>
          <td><strong>{sourceLabel(entity.sourceSystem)}</strong><small>{entity.sourceRecordId ? `Запись источника ${recordNumber(entity.sourceRecordId)}` : "Создано в ArtHello OS"}</small></td>
...
export function EntityPanel({ entityId, close, notify, refreshList, onNavigate, capabilities, readOnly = false, backLabel, initialTab = "overview" }: { entityId: string; close: () => void; notify: (value: string) => void; refreshList?: () => Promise<void>; onNavigate?: (id: string) => void; capabilities?: RegistryCapabilities; readOnly?: boolean; backLabel?: string; initialTab?: "overview" | "relations" | "documents" | "history" }) {
  ...
  async function done(id?: string) {
    ...
    if (id && id !== entityId) notify(`Основной стала ${recordLabel("карточка", id).toLocaleLowerCase("ru")}`);
  }
  ...
  return createPortal(<div className="registry-drawer-layer"><button className="drawer-scrim" onClick={close} aria-label="Закрыть карточку" /><aside className="entity-panel" role="dialog" aria-modal="true" aria-labelledby={`entity-title-${entityId}`}>
    ...
    {state === "ready" && detail ? <>
      <div className="entity-panel-head"><div><span className="entity-type-badge">{detail.entity.entityType}</span><h2 id={`entity-title-${entityId}`}>{detail.entity.displayName}</h2><p>{recordLabel("Карточка", detail.entity.id)} · {detail.entity.status}</p></div><button ref={closeRef} className={backLabel ? "entity-panel-back" : ""} onClick={close} aria-label={backLabel || "Закрыть"}>{backLabel ? <><span>←</span>{backLabel}</> : "×"}</button></div>
      ...
      <div className="entity-panel-body">
        {tab === "overview" ? <>
          <dl className="entity-facts"><div><dt>Область</dt><dd>{detail.entity.scope}</dd></div><div><dt>Источник записи</dt><dd>{sourceLabel(detail.entity.sourceSystem)}</dd></div><div><dt>Номер в источнике</dt><dd>{detail.entity.sourceRecordId ? recordNumber(detail.entity.sourceRecordId) : "Не указан"}</dd></div><div><dt>Последнее изменение</dt><dd>{detail.entity.updatedAt}</dd></div><div><dt>Создано</dt><dd>{detail.entity.createdAt}</dd></div><div><dt>Автор записи</dt><dd>{auditActor(detail.entity.createdBy)}</dd></div></dl>
          {detail.mergedCards.length ? <section className="merged-box"><p>Объединённые карточки</p>{detail.mergedCards.map((merge) => <div key={merge.id}><strong>{recordLabel("Карточка", merge.duplicateId)}</strong><span>{merge.reason}</span></div>)}</section> : null}
        ...
        {tab === "relations" ? <section className="entity-list-section"><div className="entity-section-head"><div><p>Сквозная модель</p><h3>Связанные сущности</h3></div>{!readOnly && capabilities?.relation ? <button onClick={() => setLocalAction("relation")}>+ Добавить связь</button> : null}</div>{detail.relations.length ? <div className="relation-list">{detail.relations.map((relation) => <button type="button" disabled={!relation.peer} key={relation.id} onClick={() => relation.peer && onNavigate?.(relation.peer.id)}><span>↔</span><div><strong>{relation.peer?.displayName || "Карточка объединена"}</strong><small>{relation.relationType} · {relation.direction}</small></div><em>{relation.peer ? "Открыть →" : "—"}</em></button>)}</div> : <EmptyDetail text="Связей пока нет." />}</section> : null}
        {tab === "documents" ? <section className="entity-list-section"><div className="entity-section-head"><div><p>Метаданные без загрузки файла</p><h3>{detail.entity.entityType === "Семья" ? "Договоры семьи" : "Связанные документы"}</h3></div>{!readOnly && capabilities?.document ? <button onClick={() => setLocalAction("document")}>{detail.entity.entityType === "Семья" ? "+ Добавить договор" : "+ Связать документ"}</button> : null}</div>{detail.documents.length ? <div className="document-list">{detail.documents.map((document) => <article key={document.id}><div><strong>{document.title}</strong><small>{recordLabel(document.documentType, document.id)} · {sourceLabel(document.source)}</small></div><span className={`quality-badge ${document.status === "Актуален" ? "verified" : "review"}`}>{document.status}</span><em>{document.validUntil || "Без срока"}</em></article>)}</div> : <EmptyDetail text={detail.entity.entityType === "Семья" ? "Договоров пока нет. Добавьте номер, срок и статус." : "Документы пока не связаны."} />}</section> : null}
        {tab === "history" ? <section className="entity-list-section"><div className="entity-section-head"><div><p>Неизменяемый аудит</p><h3>История карточки</h3></div></div>{detail.history.length ? <div className="entity-history">{detail.history.map((event) => <article key={event.id}><span /><div><strong>{auditTitle(event.action)}</strong><small>{auditDescription(event)}</small><em>{auditActor(event.actor)} · {event.createdAt}</em></div></article>)}</div> : <EmptyDetail text="История начнётся с первого изменения." />}</section> : null}
      </div>
...
function RegistryActionModal({ action, detail, close, onDone, notify }: { action: RegistryAction; detail?: DetailPayload; close: () => void; onDone: (id?: string) => Promise<void>; notify: (value: string) => void }) {
  ...
  return createPortal(<div className="modal-layer registry-modal-layer"><button className="drawer-scrim" onClick={close} aria-label="Закрыть форму" /><form className="task-modal registry-modal" onSubmit={submit} role="dialog" aria-modal="true" aria-label={actionTitle(action)}>
    ...
    {action === "create" ? <>
      ...
      <div className="merge-warning"><strong>Номер назначит система</strong><span>Карточка получит постоянный понятный номер после сохранения. Источник записи — ручной ввод.</span></div>
    </> : null}
    {action === "edit" && entity ? <>
    ...
    {action === "relation" && detail ? <><label><span>Связать с карточкой</span><select name="toEntityId" required defaultValue=""><option value="" disabled>Выберите карточку</option>{detail.relationCandidates.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.displayName} · {recordLabel("карточка", candidate.id).toLocaleLowerCase("ru")}</option>)}</select></label><label><span>Тип связи</span><select name="relationType" defaultValue={relationTypes[0]}>{relationTypes.map((item) => <option key={item}>{item}</option>)}</select></label></> : null}
...
function auditTitle(action: string) { return ({ "entity.created": "Карточка создана", "entity.updated": "Карточка изменена", "entity.provenance_normalized": "Источник ручной записи уточнён", "entity.relation_added": "Добавлена связь", "entity.document_linked": "Связан документ", "entity.merge_survivor": "Присоединён дубль", "entity.merged": "Карточка объединена" } as Record<string, string>)[action] || "Изменение карточки"; }
function auditDescription(event: AuditEvent) { return event.payload ? "Подробности изменения сохранены в журнале" : "Событие записано"; }
function auditActor(actor: string) { return actor.startsWith("system-") ? "Система ArtHello OS" : actor || "Пользователь"; }
