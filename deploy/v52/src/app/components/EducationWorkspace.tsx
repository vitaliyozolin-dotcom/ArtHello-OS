...
import { humanPeriodLabel, humanReferenceLabel, recordLabel, taskRecordLabel } from "../../lib/record-labels";
...
export function EducationWorkspace({ workspace, role, notify, onTasksChanged, onOpenIntegrations, focusId }: {
  ...
  return <PageContainer className="ahEducationPage">
    <PageHeader
      ...
      actions={<div className="ahEducationHeaderActions">{workspace === "education" ? <Button variant="secondary" aria-label="Открыть дневник" onClick={() => window.location.assign(SCHOOL_DIARY_SSO_URL)}>Дневник 1–11</Button> : null}{workspace === "education" ? <Button variant="secondary" onClick={() => window.location.assign("/api/atlas-sso/open")}>Дневник Атласа</Button> : null}{canManage ? <><Button variant="secondary" onClick={() => setImportOpen(true)}>Импортировать</Button><Button variant="primary" onClick={() => setEditor(data.programs.length ? "group" : "program")}>{data.programs.length ? "Добавить" : "Создать программу"}</Button></> : null}</div>}
    />
    ...
    {tab === "Структура" ? <div className="edu-structure">
      <article className="edu-panel edu-groups-panel"><Head p="Структура филиала" h="Классы и группы" s={`${visibleGroups.length} записей`} /><div className="edu-group-grid">{visibleGroups.map((group) => {
        ...
        return <button key={group.id} onClick={() => { setSelectedGroupId(group.id); setTab("Сегодня"); }}><header><span>{EDUCATION_BRANCHES.find((branch) => branch.id === educationBranchId(group.unitEntityId))?.label}</span><em>{group.status}</em></header><h3>{group.name}</h3><p>{program?.title ?? recordLabel("Программа", group.programId)}</p><dl><div><dt>Ученики</dt><dd>{students.length}</dd></div><div><dt>Педагог</dt><dd>{data.entityNames[group.teacherEntityId] ?? humanReferenceLabel(group.teacherEntityId, "Педагог")}</dd></div><div><dt>Занятия</dt><dd>{lessons.length}</dd></div><div><dt>Кабинет</dt><dd>{group.room || "—"}</dd></div></dl><footer>Открыть группу →</footer></button>;
      })}</div></article>
    ...
    {tab === "Сегодня" ? <div className="edu-layout">
      <article className="edu-panel"><Head p="Расписание" h={focusedGroup ? focusedGroup.name : "Учебный день"} s={`${visibleLessons.length} занятий`} /><div data-ah-compact-card="true" className="lesson-list">{visibleLessons.map((item) => <article key={item.id}><time>{fmt(item.scheduledAt)}</time><div><strong>{item.topic}</strong><small>{data.groups.find((group) => group.id === item.groupId)?.name} · {item.room}</small></div><span>{data.entityNames[item.substituteEntityId || item.teacherEntityId] ?? humanReferenceLabel(item.substituteEntityId || item.teacherEntityId, "Педагог")}{item.substituteEntityId ? " · замена" : ""}</span><em>{item.status}</em></article>)}</div>{canManage ? <button className="edu-inline-add" onClick={() => setEditor("lesson")}>+ Добавить занятие</button> : null}</article>
      {focusedGroup || lesson || activeProgram ? <article className="edu-panel edu-chain"><Head p="Приёмочный маршрут" h="От программы до улучшения" s="связи сохранены" /><div data-ah-compact-card="true">{[
        ["Программа", activeProgram ? recordLabel("Программа", activeProgram.id) : "", activeProgram ? `v${activeProgram.version}` : "не назначена"],
        ["Педагог", focusedGroup ? data.entityNames[focusedGroup.teacherEntityId] ?? humanReferenceLabel(focusedGroup.teacherEntityId, "Педагог") : "", focusedGroup ? "назначен" : "не назначен"],
        ["Группа", focusedGroup ? recordLabel("Группа", focusedGroup.id) : "", `${visibleStudents.length} учеников`],
        ["Занятие", lesson ? recordLabel("Занятие", lesson.id) : "", lesson?.topic],
        ["Посещаемость", chainAttendance ? recordLabel("Посещаемость", chainAttendance.id) : "", `${attendancePercent}%`],
        ["Результат", chainProgress ? recordLabel("Результат", chainProgress.id) : "", chainProgress?.trend],
        ["Отзыв", chainFeedback ? recordLabel("Отзыв", chainFeedback.id) : "", chainFeedback ? "семья" : "не добавлен"],
    ...
    {tab === "Журнал" ? <article className="edu-panel"><Head p="Электронный журнал" h="Посещаемость, оценка и результат" s={`${visibleAttendance.length} записей`} /><div className="ahEducationTable"><table><thead><tr><th>Ученик</th><th>Занятие</th><th>Статус</th><th>Оценка</th><th>Наблюдаемый результат</th><th>Действие</th></tr></thead><tbody>{visibleAttendance.map((row) => <tr key={row.id}><td>{data.entityNames[data.students.find((student) => student.id === row.studentId)?.childEntityId ?? ""] ?? recordLabel("Ученик", row.studentId)}</td><td>{recordLabel("Занятие", row.lessonId)}</td><td>{row.attendanceStatus}</td><td>{row.grade || "—"}</td><td>{row.result}</td><td><button disabled={busy === row.id} onClick={() => void action({ action: "recordAttendance", id: row.id, attendanceStatus: row.attendanceStatus === "Присутствовал" ? "Опоздал" : "Присутствовал", result: "Статус уточнён педагогом на занятии" }, row.id)}>Уточнить</button></td></tr>)}</tbody></table></div><footer>Домашнее задание: {lesson?.homework || "не задано"}</footer></article> : null}

    {tab === "Прогресс" ? <div className="progress-grid">{visibleProgress.map((row) => <article key={row.id}><header><span>{row.score}</span><div><strong>{data.entityNames[data.students.find((student) => student.id === row.studentId)?.childEntityId ?? ""] ?? recordLabel("Ученик", row.studentId)}</strong><small>{humanPeriodLabel(row.period)} · {row.trend}</small></div><em>{row.band}</em></header><h2>{row.metric}</h2><p>{row.evidence}</p><footer>Сигнал для педагога и семьи, не автоматическое решение</footer></article>)}</div> : null}

    {tab === "Программы" ? data.programs.length ? <div className="program-grid">{data.programs.filter((item) => visibleGroups.some((group) => group.programId === item.id) || branchId === "all").map((item) => <article key={item.id}><header><span>{recordLabel("Программа", item.id)}</span><em>{item.status}</em></header><h2>{item.title} · v{item.version}</h2><p>{item.expectedResult}</p><dl><div><dt>Автор</dt><dd>{data.entityNames[item.authorEntityId] ?? humanReferenceLabel(item.authorEntityId, "Автор")}</dd></div><div><dt>Методист</dt><dd>{data.entityNames[item.methodistEntityId] ?? humanReferenceLabel(item.methodistEntityId, "Методист")}</dd></div><div><dt>Материал</dt><dd>{item.materialRef || "Не указан"}</dd></div><div><dt>Область</dt><dd>{item.scope}</dd></div></dl><button disabled={busy === item.id} onClick={() => void action({ action: "createProgramVersion", programId: item.id, note: "Обновление по результату занятия и обратной связи семьи" }, item.id)}>+ Версия с основанием</button></article>)}</div> : <div data-ah-compact-card="true" className="manual-module-empty"><span>＋</span><h2>Учебных программ пока нет</h2><p>Создайте первую рабочую программу. После сохранения её можно будет выбрать при создании класса или группы.</p>{canManage ? <button className="primary-action" onClick={() => setEditor("program")}>Создать программу</button> : null}</div> : null}

    {tab === "Семья и коммуникации" ? <div className="edu-layout"><article className="edu-panel"><Head p="Обратная связь" h="Отзывы и рекомендации" s={`${data.feedback.length} записей`} /><div className="feedback-list">{data.feedback.map((row) => <article key={row.id}><header><strong>{data.entityNames[row.familyEntityId] ?? humanReferenceLabel(row.familyEntityId, "Семья")}</strong><span>{"★".repeat(row.rating)}</span><em>{row.status}</em></header><p>{row.comment}</p><div>{row.recommendation}</div><footer>{row.relatedTaskId ? <span>{taskRecordLabel(row.relatedTaskId)}</span> : <button disabled={busy === row.id} onClick={() => void action({ action: "createFeedbackTask", feedbackId: row.id }, row.id)}>+ Методическая задача</button>}</footer></article>)}</div></article><article className="edu-panel"><Head p="Личный кабинет" h="Новости, события и чат" s="по области доступа" /><div className="comm-list">{data.communications.map((row) => <article key={row.id}><span>{row.communicationType.slice(0, 1)}</span><div><strong>{row.title}</strong><p>{row.body}</p><small>{row.audienceType} · {humanReferenceLabel(row.audienceId, "Получатель")}{row.eventAt ? ` · ${fmt(row.eventAt)}` : ""}</small></div></article>)}</div></article></div> : null}
...
function EducationEditor({ kind, data, defaultGroupId, busy, close, submit, setKind }: { kind: EditorKind; data: Data; defaultGroupId: string; busy: string; close: () => void; submit: (body: Record<string, unknown>) => void; setKind: (kind: EditorKind) => void }) {
  ...
  const teacherOptions = namedEntityOptions([
    ...data.groups.map((group) => group.teacherEntityId),
    ...data.lessons.flatMap((lesson) => [lesson.teacherEntityId, lesson.substituteEntityId]),
  ], data.entityNames);
  const childOptions = namedEntityOptions(data.students.map((student) => student.childEntityId), data.entityNames);
  const familyOptions = namedEntityOptions(
    data.students.filter((student) => student.childEntityId === selectedChildId).map((student) => student.familyEntityId),
    data.entityNames,
  );
  const focusedGroupTeacherId = data.groups.find((group) => group.id === defaultGroupId)?.teacherEntityId ?? "";
  const defaultTeacherId = teacherOptions.some((option) => option.id === focusedGroupTeacherId) ? focusedGroupTeacherId : "";
  const editorBlocked = Boolean(busy)
    || ((kind === "group" || kind === "lesson") && !teacherOptions.length)
    || (kind === "student" && (!selectedChildId || !selectedFamilyId));

  function selectChild(childId: string) {
    const firstFamilyId = data.students.find((student) => student.childEntityId === childId && data.entityNames[student.familyEntityId]?.trim())?.familyEntityId ?? "";
  ...
  return createPortal(<div className="modal-layer edu-modal-layer ahEducationModalLayer"><button className="drawer-scrim" onClick={close} aria-label="Закрыть" /><form className="task-modal edu-editor ahEducationModal" onSubmit={save} role="dialog" aria-modal="true" aria-label="Ручное добавление в обучение"><header className="drawer-head"><div><p>Обучение · ручное добавление</p><h2>{kind === "program" ? "Учебная программа" : kind === "group" ? "Класс или группа" : kind === "lesson" ? "Занятие" : "Назначение ученика"}</h2></div><button type="button" onClick={close} aria-label="Закрыть">×</button></header><nav>{(["program", "group", "lesson", "student"] as EditorKind[]).map((item) => <button type="button" key={item} disabled={item !== "program" && !data.programs.length} className={kind === item ? "active" : ""} onClick={() => setKind(item)}>{item === "program" ? "Программа" : item === "group" ? "Группа" : item === "lesson" ? "Занятие" : "Ученик"}</button>)}</nav><div className="edu-editor-body">
    {kind === "program" ? <><label className="wide"><span>Название программы</span><input name="title" required placeholder="Название рабочей программы" /></label><label><span>Область</span><input name="scope" required placeholder="Возраст, предмет или направление" /></label><label><span>Методист</span><select name="methodistEntityId" defaultValue=""><option value="">Автор программы</option>{teacherOptions.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select></label><label className="wide"><span>Ожидаемый результат</span><textarea name="expectedResult" required placeholder="Какой проверяемый результат должна дать программа" /></label><label className="wide"><span>Материал или документ</span><input name="materialRef" placeholder="Ссылка или номер документа — необязательно" /></label></> : null}
    {kind === "group" ? <><label><span>Название</span><input name="name" required placeholder="3А / Старшая группа" /></label><label><span>Филиал</span><select name="branchId" required>{EDUCATION_BRANCHES.map((branch) => <option key={branch.id} value={branch.id}>{branch.label}</option>)}</select></label><label><span>Программа</span><select name="programId" required>{data.programs.map((program) => <option key={program.id} value={program.id}>{program.title} · v{program.version}</option>)}</select></label><label><span>Педагог</span><select name="teacherId" required defaultValue="" disabled={!teacherOptions.length}><option value="">{teacherOptions.length ? "Выберите педагога" : "Нет доступных педагогов"}</option>{teacherOptions.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select></label>{!teacherOptions.length ? <div className="edu-editor-note wide" role="status"><strong>Доступных педагогов пока нет</strong><span>Сначала импортируйте или подтвердите назначение педагога в учебном контуре.</span></div> : null}<label><span>Кабинет</span><input name="room" placeholder="Кабинет 12" /></label></> : null}
    {kind === "lesson" ? <><label><span>Класс / группа</span><select name="groupId" required defaultValue={defaultGroupId}>{!defaultGroupId ? <option value="">Выберите группу</option> : null}{data.groups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label><label><span>Дата и время</span><input type="datetime-local" name="scheduledAt" required /></label><label className="wide"><span>Тема занятия</span><input name="topic" required /></label><label><span>Педагог</span><select name="teacherId" required defaultValue={defaultTeacherId} disabled={!teacherOptions.length}><option value="">{teacherOptions.length ? "Выберите педагога" : "Нет доступных педагогов"}</option>{teacherOptions.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select></label>{!teacherOptions.length ? <div className="edu-editor-note wide" role="status"><strong>Доступных педагогов пока нет</strong><span>Сначала импортируйте или подтвердите назначение педагога в учебном контуре.</span></div> : null}<label><span>Кабинет</span><input name="room" /></label><label className="wide"><span>Домашнее задание</span><textarea name="homework" /></label></> : null}
    {kind === "student" ? <><div className="edu-editor-note wide"><strong>Ребёнок здесь не создаётся</strong><span>Выберите проверенные карточки ребёнка и его семьи, уже доступные в учебном контуре.</span></div><label><span>Ребёнок</span><select name="childId" required value={selectedChildId} disabled={!childOptions.length} onChange={(event) => selectChild(event.target.value)}><option value="">{childOptions.length ? "Выберите ребёнка" : "Нет доступных карточек детей"}</option>{childOptions.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select></label><label><span>Семья</span><select name="familyId" required value={selectedFamilyId} disabled={!familyOptions.length} onChange={(event) => setSelectedFamilyId(event.target.value)}><option value="">{selectedChildId ? "Нет связанной семьи" : "Сначала выберите ребёнка"}</option>{familyOptions.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select></label>{!childOptions.length ? <div className="edu-editor-note wide" role="status"><strong>Доступных карточек детей пока нет</strong><span>Сначала загрузите проверенную связку ребёнка и семьи через контролируемый импорт.</span></div> : selectedChildId && !familyOptions.length ? <div className="edu-editor-note wide" role="status"><strong>У ребёнка нет доступной карточки семьи</strong><span>Проверьте связь ребёнка с семьёй перед назначением в группу.</span></div> : null}<label className="wide"><span>Класс / группа</span><select name="groupId" required defaultValue={defaultGroupId}>{!defaultGroupId ? <option value="">Выберите группу</option> : null}{data.groups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label></> : null}
  </div><footer className="modal-actions"><button type="button" onClick={close}>Отмена</button><button disabled={editorBlocked}>{busy ? "Сохраняем…" : "Сохранить на проверку"}</button></footer></form></div>, document.body);
}

function namedEntityOptions(ids: string[], entityNames: Record<string, string>) {
  return [...new Set(ids)]
    .map((id) => ({ id, name: entityNames[id]?.trim() ?? "" }))
...
function EducationImport({ busy, close, openIntegrations, submit }: { busy: string; close: () => void; openIntegrations: () => void; submit: (rows: Array<Record<string, string>>) => void }) {
  ...
  return createPortal(<div className="modal-layer edu-modal-layer ahEducationModalLayer"><button className="drawer-scrim" onClick={close} aria-label="Закрыть" /><section className="task-modal edu-import ahEducationModal" role="dialog" aria-modal="true" aria-label="Импорт данных обучения"><header className="drawer-head"><div><p>Обучение · контролируемый импорт</p><h2>Загрузка данных</h2></div><button onClick={close} aria-label="Закрыть">×</button></header><div data-ah-compact-card="true" className="edu-import-body"><article><span>01</span><div><strong>Семьи и дети из AlfaCRM</strong><p>Сначала попадают в «Клиенты» на проверку. После проверки ребёнок назначается в нужную группу.</p><button onClick={openIntegrations}>Настроить AlfaCRM →</button></div></article><article><span>02</span><div><strong>Группы, занятия и назначения из CSV</strong><p>Поддерживаются строки group/группа, lesson/занятие и student/ученик. Ни одна строка не становится активной автоматически.</p><div className="edu-file-row"><label><input type="file" accept=".csv,text/csv" onChange={(event) => void readFile(event)} /><span>{fileName || "Выбрать CSV"}</span></label><button onClick={template}>Скачать шаблон</button></div>{fileName ? <div className="edu-import-preview"><strong>{rows.length} строк распознано</strong><span>{rows.length ? "После подтверждения они получат статус «На проверке»." : "Проверьте заголовки и разделитель файла."}</span></div> : null}</div></article><article><span>03</span><div><strong>Контроль перед записью</strong><p>При ошибке хотя бы в одной строке пакет останавливается целиком. Доступы ученикам и родителям не выдаются.</p></div></article></div><footer className="modal-actions"><button onClick={close}>Отмена</button><button disabled={!rows.length || busy === "import"} onClick={() => submit(rows)}>{busy === "import" ? "Импортируем…" : `Импортировать ${rows.length || ""} на проверку`}</button></footer></section></div>, document.body);
}
