...
type Overview = {
  ...
  permissions: { canManageAll: boolean; canApprove: boolean; canManageDocuments: boolean };
};
type TaskDetail = {
  ...
  assignees: Entity[];
  permissions: { canView: boolean; canManage: boolean; canComment: boolean; canApprove: boolean };
};
type View = "queue" | "board" | "notifications" | "documents";

const statuses = [...taskStatuses];
const emptyOverview: Overview = { tasks: [], notifications: [], escalations: [], documents: [], obligations: [], assignees: [], permissions: { canManageAll: false, canApprove: false, canManageDocuments: false }, stats: { open: 0, overdue: 0, waitingApproval: 0, escalations: 0 } };

function technicalKind(value: string) {
  const prefix = value.toUpperCase();
  if (prefix.startsWith("DOG") || prefix.startsWith("CONTRACT")) return "Договор";
  if (prefix.startsWith("ACT")) return "Акт";
  if (prefix.startsWith("DOC")) return "Документ";
  if (prefix.startsWith("SAFE-FLT")) return "Неисправность";
  if (prefix.startsWith("LEAD")) return "Лид";
  if (prefix.startsWith("EMP")) return "Сотрудник";
  return "Запись";
}

function humanReference(value: string, fallback = "Запись") {
  const source = value.trim();
  if (!source) return "Не указано";
  if (/^MANUAL(?::|$)/i.test(source)) return "Добавлено вручную";
  if (/^SYNTHETIC(?::|$)/i.test(source)) return "Демонстрационный пример";
  const task = /^TASK:(\d+)$/i.exec(source);
  if (task) return taskRecordLabel(Number(task[1]));
  if (/^(?:TSK|TASK)(?:-|$)/i.test(source)) return taskRecordLabel(source);
  if (/^[A-ZА-Я0-9]+(?:[-:_][A-ZА-Я0-9]+)+$/i.test(source)) {
    const kind = technicalKind(source);
    return recordLabel(kind === "Запись" ? fallback : kind, source);
  }
  return source;
}
...
function humanText(value: string) {
  return value
    .replace(/\bcontract_expiry\b/gi, "контроль срока договора")
    .replace(/\b[A-ZА-Я]{2,}(?:[-:][A-ZА-Я0-9]+){1,}\b/gi, (token) => humanReference(token));
}

function taskSourceLabel(sourceType: string, sourceId: string) {
  if (sourceType === "Ручная задача" || /^MANUAL(?::|$)/i.test(sourceId)) return "Создано вручную";
  if (sourceType === "Повторяющаяся задача" || /^TASK:/i.test(sourceId)) return humanReference(sourceId, "Задача");
  return sourceId ? humanReference(sourceId, sourceType || "Основание") : "Без связанного основания";
}

function actorLabel(value: string) {
  if (!value) return "Система";
  if (value === "system-automation") return "Система";
  if (value.startsWith("system-")) return "Демонстрационный контур";
  const roles: Record<string, string> = {
    "ROLE:DIRECTOR": "Директор",
    "ROLE:OWNER": "Собственник",
    "ROLE:REPRESENTATIVE": "Представитель собственника",
    "ROLE:FINANCE": "Ответственный за финансы",
    "ROLE:SAFETY": "Ответственный по безопасности",
    "ROLE:LEGAL": "Юрист",
    "ROLE:HR": "Ответственный за команду",
  };
  if (roles[value]) return roles[value];
  return humanReference(value, "Участник");
}

function documentLabel(document: Pick<WorkflowDocument, "id" | "documentType">) {
  return recordLabel(document.documentType || "Документ", document.id);
}

export function WorkflowWorkspace({ notify, onChanged }: { role: string; notify: (value: string) => void; onChanged: () => Promise<void> }) {
  ...
  return <PageContainer className="ahWorkflowPage">
    ...
    {expiry ? <button className="workflow-risk" onClick={() => expiryTask && setSelectedId(expiryTask.id)}><span>!</span><div><small>Автоматический контроль обязательства</small><strong>{recordLabel("Договор", expiry.documentId)} истекает {formatDate(expiry.dueDate)}</strong><p>Уведомление руководителю и задача ответственному созданы системой. Нажмите, чтобы проверить цепочку.</p></div><em>Открыть риск →</em></button> : null}
    ...
    <Card className="ahWorkflowShell">
      <div className="ahWorkflowViewbar">
        ...
        {(view === "queue" || view === "board") ? <div className="ahWorkflowFilters"><SearchField value={query} onChange={setQuery} placeholder="Название, номер, источник или ответственный" label="Поиск задач" /><select value={status} onChange={(event) => setStatus(event.target.value)} aria-label="Фильтр статуса"><option value="">Все статусы</option>{statuses.map((item) => <option key={item}>{item}</option>)}</select></div> : null}
      </div>
      ...
      {state === "ready" && view === "documents" ? <DocumentView documents={data.documents} obligations={data.obligations} assignees={data.assignees} canManage={data.permissions.canManageDocuments} create={() => setDocumentAction({ mode: "create" })} version={(document) => setDocumentAction({ mode: "version", document })} /> : null}
    </Card>

    {selectedId ? <TaskPanel taskId={selectedId} close={() => setSelectedId(null)} notify={notify} changed={changed} createSubtask={() => setCreateParent(selectedId)} /> : null}
...
function QueueView({ tasks, open, create }: { tasks: Task[]; open: (id: number) => void; create: () => void }) {
  ...
  return <div className="ahWorkflowTable"><table className="workflow-table"><thead><tr><th>Задача</th><th>Цепочка</th><th>Ответственный</th><th>Срок</th><th>Статус</th><th /></tr></thead><tbody>{tasks.map((task) => <tr key={task.id} onClick={() => open(task.id)}><td><div className="task-title-cell"><span className={`priority-dot ${priorityClass(task.priority)}`} /><div><strong>{humanText(task.title)}</strong><small>{taskRecordLabel(task.id)} · {humanText(task.kind)}{task.parentTaskId ? " · подзадача" : ""}</small></div></div></td><td><strong>{humanText(task.sourceType)}</strong><small>{taskSourceLabel(task.sourceType, task.sourceId)}</small></td><td><strong>{humanText(task.owner)}</strong><small>{task.assigneeEntityId ? "Связан с карточкой сотрудника" : "не связан с карточкой"}</small></td><td><strong className={isOverdue(task) ? "due-over" : ""}>{task.dueDate ? formatDate(task.dueDate) : "Без срока"}</strong><small>{task.recurrenceRule || "однократно"}</small></td><td><span className={`status-pill ${statusClass(task.status)}`}>{task.status}</span></td><td><button aria-label={`Открыть задачу ${humanText(task.title)}`}>→</button></td></tr>)}</tbody></table></div>;
}

function BoardView({ tasks, open }: { tasks: Task[]; open: (id: number) => void }) { return <div data-ah-compact-card="true" className="process-board">{statuses.map((status) => { const list = tasks.filter((task) => task.status === status); return <section key={status}><header><strong>{status}</strong><span>{list.length}</span></header><div>{list.length ? list.map((task) => <button key={task.id} onClick={() => open(task.id)}><i className={priorityClass(task.priority)} /><strong>{humanText(task.title)}</strong><small>{taskRecordLabel(task.id)} · {humanText(task.owner)}</small><em>{task.dueDate ? formatDate(task.dueDate) : "Без срока"}</em></button>) : <p>Нет задач</p>}</div></section>; })}</div>; }

function NotificationView({ notifications, escalations, openTask, markRead }: { notifications: Notification[]; escalations: Escalation[]; openTask: (id: number) => void; markRead: (id: number) => Promise<void> }) { return <div className="notification-layout"><section><div className="workflow-section-head"><div><small>Системные события</small><h2>Уведомления</h2></div><span>{notifications.length}</span></div>{notifications.length ? <div className="notification-list">{notifications.map((item) => <article key={item.id}><span className={item.status === "Новое" ? "new" : ""} /><div><strong>{humanText(item.title)}</strong><p>{humanText(item.body)}</p><small>{humanText(item.notificationType)} · {item.sourceType === "task" ? taskRecordLabel(item.sourceId) : humanReference(item.sourceId, "Основание")} · {item.createdAt}</small></div><button disabled={item.status !== "Новое"} onClick={() => void markRead(item.id)}>{item.status === "Новое" ? "Прочитано" : item.status}</button></article>)}</div> : <WorkflowState title="Уведомлений нет" text="Новые события появятся здесь." />}</section><aside><div className="workflow-section-head"><div><small>Сроки и контроль</small><h2>Эскалации</h2></div><span>{escalations.length}</span></div>{escalations.map((item) => <button key={item.id} onClick={() => openTask(item.taskId)}><span>Уровень {item.level}</span><div><strong>{humanText(item.reason)}</strong><small>{actorLabel(item.recipientEntityId)} · {item.status}</small></div><em>→</em></button>)}</aside></div>; }

function DocumentView({ documents, obligations, assignees, canManage, create, version }: { documents: WorkflowDocument[]; obligations: Obligation[]; assignees: Entity[]; canManage: boolean; create: () => void; version: (document: WorkflowDocument) => void }) { return <div className="document-workspace"><section><div className="workflow-section-head"><div><small>Контролируемые версии</small><h2>Документы</h2></div>{canManage ? <button onClick={create}>+ Новый документ</button> : null}</div><div data-ah-compact-card="true" className="workflow-doc-list">{documents.map((document) => <article key={document.id}><span>ДОК</span><div><strong>{humanText(document.title)}</strong><small>{documentLabel(document)} · источник: {humanReference(document.source)}</small></div><b>версия {document.currentVersion}</b><em className={document.status === "Истекает" ? "warn" : ""}>{document.status}</em>{canManage ? <button onClick={() => version(document)}>+ версия</button> : null}</article>)}</div></section><aside><div className="workflow-section-head"><div><small>Сроки действия</small><h2>Обязательства</h2></div><span>{obligations.length}</span></div>{obligations.map((item) => <article key={item.id}><strong>{humanText(item.title)}</strong><small>{recordLabel("Документ", item.documentId)}</small><p>{formatDate(item.dueDate)} · {assignees.find((entity) => entity.id === item.ownerEntityId)?.displayName || "Ответственный не указан"}</p><span>{item.status}</span></article>)}</aside></div>; }

function TaskPanel({ taskId, close, notify, changed, createSubtask }: { taskId: number; close: () => void; notify: (value: string) => void; changed: (message: string) => Promise<void>; createSubtask: () => void }) {
  ...
  useEffect(() => { const timer = window.setTimeout(() => void load(), 0); return () => window.clearTimeout(timer); }, [load]);
  async function action(payload: Record<string, unknown>, message: string) { try { const response = await fetch("/api/task-actions", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...payload, taskId }) }); const body = (await response.json()) as { error?: string }; if (!response.ok) throw new Error(body.error || "Действие не выполнено"); await load(); await changed(message); } catch (error) { notify(error instanceof Error ? error.message : "Действие не выполнено"); } }
  ...
  return createPortal(<div className="workflow-drawer-layer ahWorkflowDialogLayer"><button className="drawer-scrim" onClick={close} aria-label="Закрыть задачу" /><aside className="workflow-drawer ahWorkflowDialog" role="dialog" aria-modal="true" aria-label={detail?.task.title ?? "Карточка задачи"}>{state === "loading" ? <WorkflowState title="Открываем задачу" text="Собираем всю цепочку исполнения." /> : null}{state === "error" ? <WorkflowState title="Задача недоступна" text="Повторите загрузку." action="Повторить" onAction={() => void load()} /> : null}{state === "ready" && detail ? <><header className="workflow-drawer-head"><div><span>{humanText(detail.task.kind)}</span><h2>{humanText(detail.task.title)}</h2><p>{taskRecordLabel(detail.task.id)} · {humanText(detail.task.sourceType)} · {taskSourceLabel(detail.task.sourceType, detail.task.sourceId)}</p></div><button onClick={close} aria-label="Закрыть">×</button></header><ProcessRail status={detail.task.status} /><nav className="workflow-tabs"><button className={tab === "work" ? "active" : ""} onClick={() => setTab("work")}>Работа</button><button className={tab === "people" ? "active" : ""} onClick={() => setTab("people")}>Участники {detail.watchers.length + 1}</button><button className={tab === "documents" ? "active" : ""} onClick={() => setTab("documents")}>Документы {detail.documents.length}</button><button className={tab === "history" ? "active" : ""} onClick={() => setTab("history")}>История {detail.history.length}</button></nav><div className="workflow-drawer-body">{tab === "work" ? <WorkTab detail={detail} action={action} createSubtask={createSubtask} /> : null}{tab === "people" ? <PeopleTab detail={detail} action={action} /> : null}{tab === "documents" ? <TaskDocuments detail={detail} /> : null}{tab === "history" ? <TaskHistory events={detail.history} /> : null}</div><footer className="workflow-drawer-footer"><div><span className={`priority-dot ${priorityClass(detail.task.priority)}`} /><p><strong>{detail.task.priority} приоритет</strong><small>{detail.task.dueDate ? `Срок ${formatDate(detail.task.dueDate)}` : "Срок не установлен"}</small></p></div>{detail.permissions.canManage && nextStatus(detail.task.status) ? (nextStatus(detail.task.status) === "На проверке" ? <button onClick={() => setShowResult(true)}>Передать результат →</button> : <button onClick={() => void transition()}>{transitionLabel(detail.task.status)} →</button>) : detail.task.status === "Выполнено" ? <span className="done-label">Результат подтверждён</span> : <span className="done-label">Только наблюдение</span>}</footer>{showResult ? <div className="result-layer"><form onSubmit={(event) => void transition(event)}><div><small>Подтверждение результата</small><h3>Что сделано</h3></div><label><span>Результат</span><textarea name="result" required minLength={8} defaultValue={detail.task.result} placeholder="Конкретный проверяемый результат" /></label><label><span>Основание / ссылка</span><input name="resultEvidence" defaultValue={detail.task.resultEvidence} placeholder="Документ, событие или запись" /></label><div><button type="button" onClick={() => setShowResult(false)}>Отмена</button><button type="submit">Передать на проверку</button></div></form></div> : null}</> : null}</aside></div>, document.body);
}

function ProcessRail({ status }: { status: string }) { const current = statuses.findIndex((item) => item === status); return <div className="process-rail">{statuses.map((item, index) => <div key={item} className={index < current ? "done" : index === current ? "current" : ""}><span>{index < current ? "✓" : index + 1}</span><small>{item}</small></div>)}</div>; }

function WorkTab({ detail, action, createSubtask }: { detail: TaskDetail; action: (payload: Record<string, unknown>, message: string) => Promise<void>; createSubtask: () => void }) {
  return <>
    <section className="task-summary"><p>{humanText(detail.task.description) || "Описание не добавлено."}</p><dl><div><dt>Ответственный</dt><dd>{humanText(detail.task.owner)}<small>{detail.task.assigneeEntityId ? "Связан с карточкой сотрудника" : "без связи со справочником"}</small></dd></div><div><dt>Срок</dt><dd>{detail.task.dueDate ? formatDate(detail.task.dueDate) : "Без срока"}<small>{detail.task.recurrenceRule || "однократно"}</small></dd></div><div><dt>Источник</dt><dd>{humanText(detail.task.sourceType)}<small>{taskSourceLabel(detail.task.sourceType, detail.task.sourceId)}</small></dd></div><div><dt>Результат</dt><dd>{humanText(detail.task.result) || "Ещё не передан"}<small>{detail.task.resultEvidence ? humanReference(detail.task.resultEvidence, "Основание") : "нет основания"}</small></dd></div></dl></section>
    <section className="task-block">
      <div className="workflow-section-head"><div><small>Контроль выполнения</small><h3>Чек-лист</h3></div><span>{detail.checklist.filter((item) => item.isDone).length}/{detail.checklist.length}</span></div>
      <div className="checklist-list">{detail.checklist.map((item) => <label key={item.id}><input type="checkbox" checked={item.isDone} disabled={!detail.permissions.canManage} onChange={(event) => void action({ action: "checklist_toggle", checklistId: item.id, isDone: event.target.checked }, "Чек-лист обновлён")} /><span>{item.title}</span></label>)}</div>
      {detail.permissions.canManage ? <form className="inline-add" onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; const value = new FormData(form).get("title"); void action({ action: "checklist_add", title: value }, "Пункт добавлен").then(() => form.reset()); }}><input name="title" required minLength={3} placeholder="Добавить пункт" /><button>+</button></form> : null}
    </section>
    <section className="task-block">
      <div className="workflow-section-head"><div><small>Декомпозиция</small><h3>Подзадачи</h3></div>{detail.permissions.canManage ? <button onClick={createSubtask}>+ Подзадача</button> : null}</div>
      {detail.subtasks.length ? <div data-ah-compact-card="true" className="subtask-list">{detail.subtasks.map((task) => <article key={task.id}><span className={`status-pill ${statusClass(task.status)}`}>{task.status}</span><div><strong>{humanText(task.title)}</strong><small>{taskRecordLabel(task.id)} · {humanText(task.owner)} · {task.dueDate ? formatDate(task.dueDate) : "без срока"}</small></div></article>)}</div> : <p data-ah-compact-card="true" className="empty-inline">Подзадач нет.</p>}
    </section>
  </>;
}
...
function PeopleTab({ detail, action }: { detail: TaskDetail; action: (payload: Record<string, unknown>, message: string) => Promise<void> }) {
  ...
  return <>
    <section className="task-block">
      ...
      <article className="owner-card"><span>{initials(detail.task.owner)}</span><div><strong>{humanText(detail.task.owner)}</strong><small>{detail.task.assigneeEntityId ? "Связан с единой карточкой" : "не связан с единой карточкой"}</small></div></article>
    </section>
    <section className="task-block">
      ...
      <div className="watcher-list">{detail.watchers.map((watcher) => <span key={watcher.id}>{watcher.entity?.displayName || actorLabel(watcher.entityId)}</span>)}</div>
      {detail.permissions.canManage ? <form className="inline-add" onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; void action({ action: "watcher", entityId: new FormData(form).get("entityId") }, "Наблюдатель добавлен"); }}><select name="entityId" required defaultValue=""><option value="" disabled>Выберите сотрудника</option>{detail.assignees.map((entity) => <option key={entity.id} value={entity.id}>{entity.displayName}</option>)}</select><button>Добавить</button></form> : null}
    </section>
...
function eventTitle(action: string) { return ({ "task.created": "Задача создана", "task.auto_created": "Автозадача создана", "task.status_changed": "Статус изменён", "task.comment_added": "Добавлен комментарий", "task.watcher_added": "Добавлен наблюдатель", "task.checklist_added": "Добавлен пункт чек-листа", "task.checklist_changed": "Чек-лист обновлён", "task.approval_decided": "Решение согласования", "task.document_linked": "Связан документ", "task.recurrence_created": "Создан следующий повтор" } as Record<string, string>)[action] || "Событие задачи"; }
function describePayload(value: string) {
  try {
    const payload = JSON.parse(value) as Record<string, unknown>;
    const labels: Record<string, string> = {
      from: "Было", to: "Стало", sourceId: "Основание", rule: "Правило", taskId: "Задача", previousTaskId: "Предыдущая задача",
      kind: "Тип", recurrenceRule: "Повтор", status: "Статус", comment: "Комментарий", title: "Название", isDone: "Выполнено",
      requiresApproval: "Нужно согласование", documentId: "Документ", assigneeEntityId: "Ответственный", entityId: "Участник",
    };
    return Object.entries(payload).map(([key, item]) => {
      const label = labels[key] || "Сведения";
      if ((key === "taskId" || key === "previousTaskId") && Number.isFinite(Number(item))) return `${label}: ${taskRecordLabel(Number(item))}`;
      if (key === "sourceId" || key === "documentId") return `${label}: ${humanReference(String(item), key === "documentId" ? "Документ" : "Основание")}`;
      if (key === "entityId" || key === "assigneeEntityId") return `${label}: ${item ? "карточка сотрудника выбрана" : "не указан"}`;
      const display = typeof item === "boolean" ? (item ? "да" : "нет") : humanText(String(item));
      return `${label}: ${display}`;
    }).join(" · ") || "Событие сохранено";
  } catch { return "Событие сохранено"; }
}
