...
function UserAccessModal({ data, user, busy, close, submit }: { data: AccessData; user: User; busy: string; close: () => void; submit: (body: Record<string, unknown>) => Promise<void> }) {
  ...
  const [atlasDiary, setAtlasDiary] = useState(systemIds.has("SYS-SCHOOL-ATLAS"));
  ...
  function save(event: FormEvent<HTMLFormElement>) {
    ...
    void submit({ action: "inviteUser", employeeId: user.employeeId, displayName: user.displayName, contact: form.get("contact"), position: form.get("position"), role: form.get("role"), allowedModules: form.getAll("allowedModules"), isAdministrative: form.get("isAdministrative") === "on", branchIds: form.getAll("branchIds"), systemIds: form.getAll("systemIds"), diaryRole: form.get("diaryRole"), atlasDiaryRole: form.get("atlasDiaryRole"), expectedAccessVersion: user.accessVersion });
  }
  return <Modal title={user.hasAccess ? "Изменить доступ" : "Выдать доступ"} subtitle={user.displayName} close={close} wide>
    <form className={styles.accessForm} onSubmit={save}>
      ...
      <fieldset><legend>Системы</legend><div className={styles.choiceGrid}>{data.systems.map((system) => <label key={system.id}><input type="checkbox" name="systemIds" value={system.id} defaultChecked={user.hasAccess ? systemIds.has(system.id) : system.id === "SYS-ARTHELLO-OS"} onChange={system.id === "SYS-SCHOOL-1-11" ? (event) => setDiary(event.target.checked) : system.id === "SYS-SCHOOL-ATLAS" ? (event) => setAtlasDiary(event.target.checked) : undefined} /><span><strong>{system.name}</strong><small>{system.description}</small></span></label>)}</div></fieldset>
      {atlasDiary ? <label><span>Роль в дневнике Атласа</span><SoftSelect name="atlasDiaryRole" ariaLabel="Роль в дневнике Атласа" defaultValue={systemGrants.find(grant => grant.systemId === "SYS-SCHOOL-ATLAS")?.role ?? "teacher"} options={data.systemRoleOptions["SYS-SCHOOL-ATLAS"] ?? []} /><small>Нужен доступ к школе Атлас и ArtHello OS. Классы и предметы назначаются внутри дневника.</small></label> : null}
