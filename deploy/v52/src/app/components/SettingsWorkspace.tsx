          ...
          {tab === "Доступы" ? <div className="settings-grid users-grid access-users-full">
            ...
            <article className="settings-card wide">
              ...
              <div className="access-directory-toolbar" role="search" aria-label="Фильтры доступов">
                ...
                <label><span>Статус доступа</span><SoftSelect ariaLabel="Статус доступа" value={accessStatus} onChange={setAccessStatus} options={[{ value: "all", label: "Все статусы" }, { value: "active", label: "Доступ активен" }, { value: "unassigned", label: "Доступ не выдан" }, { value: "blocked", label: "Заблокирован" }]} /></label>
                <label><span>Филиал доступа</span><SoftSelect ariaLabel="Филиал доступа" value={accessBranch} onChange={setAccessBranch} options={[{ value: "", label: "Все филиалы" }, ...data.branches.map((branch) => ({ value: branch.id, label: branch.name }))]} /></label>
                <label><span>Роль</span><SoftSelect ariaLabel="Роль доступа" value={accessRole} onChange={setAccessRole} options={[{ value: "", label: "Все роли" }, ...[...new Set(data.users.map((user) => user.role))].sort((a, b) => a.localeCompare(b, "ru")).map((role) => ({ value: role, label: role }))]} /></label>
              </div>
...
function AccessAssignmentForm({ user, data, busy, headingRef, close, submit }: { user: SettingsUser; data: SettingsData; busy: string; headingRef: RefObject<HTMLHeadingElement | null>; close: () => void; submit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  return <form className="settings-card settings-form invite-form access-assignment-form" onSubmit={submit}>
    ...
    <div className="access-form-columns">
      ...
      <fieldset><legend>Доступ к филиалам</legend>{data.branches.map((branch) => <label key={branch.id}><input type="checkbox" name="branchIds" value={branch.id} defaultChecked={branchIds.has(branch.id)} /><span>{branch.name}</span></label>)}<small>Владелец может назначить любой действующий филиал. Для редактирования общей карточки семьи отметьте все филиалы, к которым она относится.</small></fieldset>
