...
export async function POST(request: Request) {
  ...
  try {
    ...
    const resolvedDataQuality = isManualEntitySource(survivor.sourceSystem)
      ? "Проверено"
      : survivor.dataQuality === "Проверено" ? "Проверено" : "На проверке";
    await db.update(entities).set({ dataQuality: resolvedDataQuality, updatedAt: sql`CURRENT_TIMESTAMP` }).where(eq(entities.id, survivorId));
    await db.insert(auditEvents).values({ actor, action: "entity.merge_survivor", entityType: "entity", entityId: survivorId, payload: JSON.stringify({ duplicateId, reason, dataQualityFrom: survivor.dataQuality, dataQualityTo: resolvedDataQuality }) });
