...
export async function PATCH(request:Request){
  ...
  try{
    ...
    const links=await db.select().from(entityLinks).where(eq(entityLinks.fromEntityId,familyId));
    if(![parentId,childId].every(id=>links.some(link=>link.toEntityId===id)))return Response.json({error:"Родитель или ребёнок не принадлежит этой семье"},{status:409});
    ...
    await db.update(entities).set({displayName:values.familyName,status,dataQuality:quality,scope:values.branch,metadata:JSON.stringify({...metadata(currentFamily.metadata),...values.familyOther,localArchive:metadata(currentFamily.metadata).localArchive,remoteBranchId:metadata(currentFamily.metadata).remoteBranchId,alfaCustomerId:metadata(currentFamily.metadata).alfaCustomerId,branch:values.branch,address:values.address,familyManager:values.familyManager,contractNumber:values.contractNumber,billingPlan:values.billingPlan,financialNote:values.financialNote,tags:values.tags,note:values.note}),updatedAt:now}).where(eq(entities.id,familyId));
    await db.update(entities).set({displayName:values.parentName,status,dataQuality:quality,scope:values.branch,metadata:JSON.stringify({...values.parentOther,...metadata(currentParent.metadata),phone:values.parentPhone,email:values.parentEmail,relation:values.parentRelation}),updatedAt:now}).where(eq(entities.id,parentId));
    await db.update(entities).set({displayName:values.childName,status,dataQuality:quality,scope:values.branch,metadata:JSON.stringify({...values.childOther,...metadata(currentChild.metadata),birthDate:values.birthDate,className:values.className,groupName:values.groupName,coreActivities:values.coreActivities,extraActivities:values.extraActivities}),updatedAt:now}).where(eq(entities.id,childId));
...
async function syncFamilyOperationalLinks(actor:string,familyId:string,childId:string,operationIds:string[],groupName:string){
  ...
  const[linkedFamily]=await db.select().from(entities).where(eq(entities.id,familyId)).limit(1);const candidates=groupName?await db.select().from(educationGroups).where(eq(educationGroups.name,groupName)):[];const localBranches=await db.select().from(organizationBranches).where(eq(organizationBranches.name,linkedFamily?.scope??""));const scopedGroups=candidates.filter(group=>group.status==="Активна"&&localBranches.some(branch=>branch.id===group.unitEntityId));const matchingGroup=scopedGroups.length===1?scopedGroups[0]:undefined;const[student]=await db.select().from(educationStudents).where(eq(educationStudents.familyEntityId,familyId)).limit(1);if(student&&matchingGroup)await db.update(educationStudents).set({childEntityId:childId,groupId:matchingGroup.id,status:"Активен"}).where(eq(educationStudents.id,student.id));else if(!student&&matchingGroup)await db.insert(educationStudents).values({id:`STU-M-${crypto.randomUUID().slice(0,8).toUpperCase()}`,childEntityId:childId,familyEntityId:familyId,groupId:matchingGroup.id,cabinetStatus:"Доступ не выдан",status:"На проверке"});
