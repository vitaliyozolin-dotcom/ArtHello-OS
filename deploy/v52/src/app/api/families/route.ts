...
export async function PATCH(request:Request){
  ...
  try{
    ...
    await db.update(entities).set({displayName:values.parentName,status,dataQuality:quality,scope:currentParent.sourceSystem==="ALFACRM"?currentParent.scope:values.branch,metadata:JSON.stringify({...values.parentOther,...metadata(currentParent.metadata),phone:values.parentPhone,email:values.parentEmail,relation:values.parentRelation}),updatedAt:now}).where(eq(entities.id,parentId));
    await db.update(entities).set({displayName:values.childName,status,dataQuality:quality,scope:currentChild.sourceSystem==="ALFACRM"?currentChild.scope:values.branch,metadata:JSON.stringify({...values.childOther,...metadata(currentChild.metadata),birthDate:values.birthDate,className:values.className,groupName:values.groupName,coreActivities:values.coreActivities,extraActivities:values.extraActivities}),updatedAt:now}).where(eq(entities.id,childId));
