...
import { auditEvents, clientAccruals, clientLifecycles, educationGroups, educationStudents, entities, entityLinks, financialOperations, legalContracts, legalDocumentItems, organizationBranches, salesLeads, salesStageEvents, salesTouchpoints } from "../../../db/schema";
...
export async function GET(request:Request){
  ...
  try{
    ...
    const[operations,students,groups,leads,lifecycles,accruals,contracts,familyCandidates]=await Promise.all([
      db.select().from(financialOperations).where(inArray(financialOperations.counterpartyEntityId,familyIds)).orderBy(asc(financialOperations.operationDate)),
      db.select().from(educationStudents).where(inArray(educationStudents.familyEntityId,familyIds)),db.select().from(educationGroups),
      db.select().from(salesLeads).where(inArray(salesLeads.familyEntityId,familyIds)),
      db.select().from(clientLifecycles).where(inArray(clientLifecycles.familyEntityId,familyIds)),
      db.select().from(clientAccruals).where(inArray(clientAccruals.familyEntityId,familyIds)),
      db.select().from(legalContracts).where(inArray(legalContracts.partyEntityId,familyIds)),
    ...
    const duplicate=familyCandidates.find(candidate=>candidate.id!==family.id&&candidate.status!=="Объединена"&&candidate.status!=="Архив"&&normalizeEntityName(candidate.displayName)===normalizeEntityName(family.displayName));
...
export async function PATCH(request:Request){
  ...
  try{
    ...
    const currentParent=currentMembers.find(member=>member.id===parentId&&member.entityType==="Клиент"),currentChild=currentMembers.find(member=>member.id===childId&&member.entityType==="Ребёнок");
    if(!currentParent||!currentChild)return Response.json({error:"Связи семьи требуют сверки"},{status:409});
    if(currentFamily.sourceSystem==="ALFACRM"&&values.branch!==currentFamily.scope)return Response.json({error:"Филиал импортированной семьи меняется по подтверждённым данным AlfaCRM"},{status:409});
    const familyCandidates=await db.select({id:entities.id,displayName:entities.displayName,status:entities.status}).from(entities).where(eq(entities.entityType,"Семья")),duplicate=familyCandidates.find(candidate=>candidate.id!==familyId&&candidate.status!=="Объединена"&&candidate.status!=="Архив"&&normalizeEntityName(candidate.displayName)===normalizeEntityName(values.familyName));
    ...
    await db.update(entities).set({displayName:values.familyName,status,dataQuality:quality,scope:values.branch,metadata:JSON.stringify({...metadata(currentFamily.metadata),...editableFamilyExtras(values.familyOther),canonicalId:metadata(currentFamily.metadata).canonicalId,branchAssignments:metadata(currentFamily.metadata).branchAssignments,identitySourceStatus:metadata(currentFamily.metadata).identitySourceStatus,localArchive:metadata(currentFamily.metadata).localArchive,remoteBranchId:metadata(currentFamily.metadata).remoteBranchId,alfaCustomerId:metadata(currentFamily.metadata).alfaCustomerId,branch:values.branch,address:values.address,familyManager:values.familyManager,contractNumber:values.contractNumber,billingPlan:values.billingPlan,financialNote:values.financialNote,tags:values.tags,note:values.note}),updatedAt:now}).where(eq(entities.id,familyId));
    await db.update(entities).set({displayName:values.parentName,status,dataQuality:quality,scope:currentParent.sourceSystem==="ALFACRM"?currentParent.scope:values.branch,metadata:JSON.stringify({...metadata(currentParent.metadata),...editableFamilyExtras(values.parentOther),phone:values.parentPhone,email:values.parentEmail,relation:values.parentRelation}),updatedAt:now}).where(eq(entities.id,parentId));
    await db.update(entities).set({displayName:values.childName,status,dataQuality:quality,scope:currentChild.sourceSystem==="ALFACRM"?currentChild.scope:values.branch,metadata:JSON.stringify({...metadata(currentChild.metadata),...editableFamilyExtras(values.childOther),birthDate:values.birthDate,className:values.className,groupName:values.groupName,coreActivities:values.coreActivities,extraActivities:values.extraActivities}),updatedAt:now}).where(eq(entities.id,childId));
    const linkResult=status==="Архив"?{}:await syncFamilyOperationalLinks(actor,familyId,childId,values.operationIds,values.groupName);
    await audit(actor,"family.updated","entity",familyId,{before:{familyName:currentFamily.displayName,branch:currentFamily.scope,status:currentFamily.status,parentName:currentParent.displayName,childName:currentChild.displayName},after:{familyName:values.familyName,branch:values.branch,status,parentName:values.parentName,childName:values.childName},quality,sourceConfirmed,duplicateId:duplicate?.id??"",parentId,childId,...linkResult});const [updatedFamily]=await db.select().from(entities).where(eq(entities.id,familyId)).limit(1);return Response.json({familyId,quality,status,family:updatedFamily});
...
async function syncFamilyOperationalLinks(actor:string,familyId:string,childId:string,operationIds:string[],groupName:string){
  const identity=await readIdentityIndex(env.DB),familyIds=identity.members(familyId);const db=getDb(),currentlyLinked=await db.select({id:financialOperations.id,counterpartyEntityId:financialOperations.counterpartyEntityId}).from(financialOperations).where(inArray(financialOperations.counterpartyEntityId,familyIds)),requested=operationIds.length?await db.select({id:financialOperations.id,counterpartyEntityId:financialOperations.counterpartyEntityId}).from(financialOperations).where(inArray(financialOperations.id,operationIds)):[],found=new Set(requested.map(row=>row.id)),missing=operationIds.filter(id=>!found.has(id));
  ...
  const newLinks=requested.filter(row=>!familyIds.includes(row.counterpartyEntityId)).map(row=>row.id);if(newLinks.length)await db.update(financialOperations).set({counterpartyEntityId:familyId,updatedAt:new Date().toISOString()}).where(inArray(financialOperations.id,newLinks));
  ...
  const[linkedFamily]=await db.select().from(entities).where(eq(entities.id,familyId)).limit(1);const candidates=groupName?await db.select().from(educationGroups).where(eq(educationGroups.name,groupName)):[];const localBranches=await db.select().from(organizationBranches).where(eq(organizationBranches.name,linkedFamily?.scope??""));const scopedGroups=candidates.filter(group=>group.status==="Активна"&&localBranches.some(branch=>branch.id===group.unitEntityId));const matchingGroup=scopedGroups.length===1?scopedGroups[0]:undefined;const students=await db.select().from(educationStudents).where(and(inArray(educationStudents.familyEntityId,familyIds),eq(educationStudents.childEntityId,childId)));const student=students.find(row=>row.groupId===matchingGroup?.id);if(student&&matchingGroup)await db.update(educationStudents).set({childEntityId:childId,groupId:matchingGroup.id,status:"Активен"}).where(eq(educationStudents.id,student.id));else if(!students.length&&matchingGroup)await db.insert(educationStudents).values({id:`STU-M-${crypto.randomUUID().slice(0,8).toUpperCase()}`,childEntityId:childId,familyEntityId:familyId,groupId:matchingGroup.id,cabinetStatus:"Доступ не выдан",status:"На проверке"});
