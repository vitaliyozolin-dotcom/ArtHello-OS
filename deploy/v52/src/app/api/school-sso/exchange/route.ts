import {
  acquireSchoolSsoExchangeRateLease,
  exchangeSchoolSsoCode,
  releaseSchoolSsoExchangeRateLease,
  schoolPublicOrigin,
  type SchoolSsoExchangeRateLease,
} from "../../../../lib/school-sso";
...
function envelopeIssue(request: Request) {
  ...
  try {
    trustedOrigin = schoolPublicOrigin();
  } catch {
...
export async function POST(request: Request) {
  ...
  let lease: SchoolSsoExchangeRateLease | null = null;
  try {
    lease = await acquireSchoolSsoExchangeRateLease(request);
    if (!lease) {
      return json(
    ...
    console.error("school_sso.exchange_failed");
    return json({ error: "Обмен кода временно недоступен" }, 503);
  } finally {
    if (lease) {
      try {
        await releaseSchoolSsoExchangeRateLease(lease);
      } catch {
        console.error("school_sso.exchange_rate_release_failed");
      }
    }
  }
}
