import { isTochkaAutosyncRequest, runScheduledTochkaSync, runTochkaSyncStage, type TochkaSyncObserver } from '../../../lib/tochka-autosync';
// TOCHKA_AUTOMATIC_READONLY_V1
import { TochkaStatementStateChanged } from '../../../lib/tochka-statement-state';
...
async function saveSetup(actor: string, body: Record<string, unknown>) {
  ...
  if (credential) {
    if (connectionId === tochkaConnectionId) {
      ...
      return runTochkaStatementSync(actor, setup, "Первичная загрузка владельцем", credential);
    }
  ...
  if (setup.connectionId === tochkaConnectionId && setup.authMethod === "JWT") {
    return privateJson({
      ...
      message: setup.secretStatus === "stored"
        ? "Распределение и расписание сохранены. Если подключение включено, сервер загрузит данные автоматически; ручная проверка также доступна."
...
async function verifyTochkaConnection(actor: string, setup: IntegrationSetup, trigger: string) {
  ...
  return runTochkaStatementSync(actor, setup, trigger, token);
}
...
async function runTochkaStatementSync(actor: string, setup: IntegrationSetup, trigger: string, token: string, automatic = false, observe?: TochkaSyncObserver) {
  const statementState = await runTochkaSyncStage('statement_state_open', () => openTochkaStatementState(setup, automatic), observe);
  ...
  try {
    const sync = await runTochkaSyncStage('bank_sync', () => syncTochkaReadOnly({
      token,
    ...
    await runTochkaSyncStage('statement_fence', () => statementState.assertCurrent(), observe);
    return await recordTochkaSync(actor, setup, trigger, sync, statementState, observe);
  } catch (error) {
    if (error instanceof TochkaStatementStateChanged) return privateJson({ error: error.message }, 409);
    throw error;
  } finally {
    await runTochkaSyncStage('statement_release', () => statementState.release(), observe);
  }
}
...
async function recordTochkaSync(actor: string, setup: IntegrationSetup, trigger: string, sync: TochkaReadOnlySyncResult,
  ...
  const commit = await runTochkaSyncStage('sync_commit', () => commitTochkaReadOnlySync(actor, setup, sync, trigger, statementState.fence), observe);
  if (!commit.committed) {
