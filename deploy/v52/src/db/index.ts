...
export async function commitTochkaReadOnlySync(
  ...
  const guard = `EXISTS (
    ...
    WHERE saved_setup.state_key=?
      AND json_extract(saved_setup.state_value,'$.credentialGeneration')=?
  ) AND EXISTS (SELECT 1 FROM integration_connections WHERE id=?)${statementLease ? ` AND ${tochkaStatementLeaseGuardSql}` : ""}${statementLease?.requireAutomatic ? " AND EXISTS (SELECT 1 FROM integration_connections WHERE id='INT-T-TOCHKA' AND status <> 'На паузе' AND (is_enabled=1 OR status='Ошибка подключения'))" : ""}`;
