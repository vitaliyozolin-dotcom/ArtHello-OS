...
async function ensureCoreTablesOnce() {
  ...
  if (marker?.state_value === CORE_SCHEMA_VERSION) {
    await ensurePaymentDerivedCounterparties();
    if (await getSystemDataMode() === "source_only") {
  ...
  if (Number(existing?.table_count ?? 0) !== REQUIRED_CORE_TABLES.length) {
...
export type SystemDataMode = "test" | "source_only";
...
export async function getSystemDataMode(): Promise<SystemDataMode> {
  ...
  return row?.state_value === "test" ? "test" : "source_only";
}
