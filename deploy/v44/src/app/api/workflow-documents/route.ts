...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!/^[A-ZА-Я0-9-]{5,40}$/.test(id) || title.length < 3 || documentType.length < 3) return Response.json({ error: "Заполните реквизиты документа" }, { status: 400 });
    ...
    await db.insert(documentVersions).values({ documentId: id, version: 1, note: "Первая версия", reference: `MANUAL:${id}:v1`, createdBy: actor });
    ...
    if (message.includes("UNIQUE constraint failed")) return Response.json({ error: "Документ с таким ID уже существует" }, { status: 409 });
