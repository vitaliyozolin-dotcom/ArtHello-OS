...
import { auditEvents, salesLeads, salesStageEvents, salesTouchpoints, tasks } from "../../../db/schema";
...
async function createFollowupTask(actor: string, body: Record<string, unknown>) {
  ...
  const [task] = await db.insert(tasks).values({
    ...
    owner: lead.managerEntityId || "Руководитель продаж",
    dueDate: "2026-08-24",
    ...
    assigneeEntityId: lead.managerEntityId,
    kind: "Автозадача",
    automationKey,
    createdBy: actor,
  }).returning();
