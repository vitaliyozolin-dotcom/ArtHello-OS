...
import { auditEvents, notifications, taskApprovals, taskChecklist, taskComments, taskDocuments, tasks, taskWatchers, workflowDocuments } from "../../../db/schema";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "approval") {
      const role = request.headers.get("x-arthello-test-role") || "";
      if (!new Set(["OWNER", "DIRECTOR", "REPRESENTATIVE"]).has(role)) return Response.json({ error: "Недостаточно прав для решения согласования" }, { status: 403 });
      ...
      await db.insert(notifications).values({ recipientEntityId: task.assigneeEntityId || task.owner, notificationType: "Согласование", title: `${task.title}: ${status}`, body: comment || "Решение руководителя сохранено.", sourceType: "task", sourceId: String(taskId) });
      return Response.json({ ok: true });
    }
