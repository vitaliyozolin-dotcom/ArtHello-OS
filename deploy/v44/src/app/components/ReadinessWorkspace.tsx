...
export function ReadinessWorkspace({ role, notify }: { role: string; notify: (value: string) => void }) {
  ...
  return <section className="page readiness-workspace">
    <header className="readiness-heading">
      <div><p className="eyebrow">Этап 19 · единая визуальная оболочка</p><h1>Готовность ArtHello OS</h1><p>Айдентика, 25 разделов, персональные дашборды, навигация, тестовые данные и сквозные доказательства собраны в закрытом контуре.</p></div>
      <div><span className="readiness-release ready">ГОТОВО К ИТОГОВОЙ ПРОВЕРКЕ</span><button disabled={!canRun || busy} onClick={() => void runAll()}>{busy ? "Проверяем…" : "Запустить 10 сценариев"}</button></div>
    </header>
    <div className="readiness-alert"><strong>PRODUCTION ЗАКРЫТ</strong><span>{data.productionDecision}</span></div>
    <div className="readiness-kpis">
      <article><span>Разделы</span><strong>25/25</strong><small>открытие с первого клика</small></article>
      <article><span>Действия</span><strong>181</strong><small>аудируемый реестр</small></article>
      <article><span>Автотесты</span><strong>84/84</strong><small>lint и production build пройдены</small></article>
    ...
    {tab === "Визуальная оболочка" ? <VisualHandoff /> : null}
    ...
    {tab === "Решение представителя" ? <div className="readiness-decision">
      <section><p>Контроль собственника</p><h2>Итоговая приёмка визуальной оболочки</h2><div className="production-seal">НЕ ЯВЛЯЕТСЯ РАЗРЕШЕНИЕМ НА PRODUCTION</div><label><span>Комментарий представителя</span><textarea value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Принято для тестового контура или конкретный список замечаний" /></label>{role !== "Представитель Виталия" ? <small>Для решения выберите роль «Представитель Виталия».</small> : null}<footer><button disabled={role !== "Представитель Виталия" || busy || comment.trim().length < 8} onClick={() => void decide("ОТКЛОНЕНО ПРЕДСТАВИТЕЛЕМ")}>Отклонить оболочку</button><button disabled={role !== "Представитель Виталия" || busy || comment.trim().length < 8} onClick={() => void decide("ПРИНЯТО ПРЕДСТАВИТЕЛЕМ")}>Принять оболочку</button></footer></section>
