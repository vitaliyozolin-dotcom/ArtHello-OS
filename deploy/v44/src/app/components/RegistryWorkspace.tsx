...
export function RegistryWorkspace({ notify }: { notify: (value: string) => void }) {
  ...
  return <section className="page registry-page">
    ...
    <div className="registry-kpis">
      ...
      <RegistryKpi label="Источники" value={String(stats.sources)} note="Ручной ввод · XLSX · интеграции" tone="blue" />
    </div>
...
function RegistryActionModal({ action, detail, close, onDone, notify }: { action: RegistryAction; detail?: DetailPayload; close: () => void; onDone: (id?: string) => Promise<void>; notify: (value: string) => void }) {
  ...
  return <div className="modal-layer registry-modal-layer"><button className="drawer-scrim" onClick={close} aria-label="Закрыть форму" /><form className="task-modal registry-modal" onSubmit={submit}>
    ...
    {action === "create" ? <>
      <div className="form-row"><label><span>Тип сущности</span><select name="entityType" defaultValue="Семья">{entityTypes.map((item) => <option key={item}>{item}</option>)}</select></label><label><span>Стабильный ID</span><input name="id" placeholder="Назначит система" pattern="[A-Z]{2,8}-T-[0-9]{3,6}" /></label></div>
      <label><span>Название карточки</span><input name="displayName" required minLength={3} placeholder="Например: Проект T-002" /></label>
      <label><span>Область использования</span><input name="scope" required placeholder="Подразделение, объект или проект" /></label>
      <div className="form-row"><label><span>Источник</span><select name="sourceSystem" defaultValue="MANUAL"><option value="MANUAL">Ручной ввод</option><option value="XLSX_MASKED">XLSX</option></select></label><label><span>ID в источнике</span><input name="sourceRecordId" placeholder="Необязательно для ручного ввода" /></label></div>
    </> : null}
    {action === "edit" && entity ? <>
...
function sourceLabel(source: string) { return source === "XLSX_MASKED" ? "XLSX · обезличено" : source === "MANUAL" ? "Ручной ввод" : "Интеграция"; }
