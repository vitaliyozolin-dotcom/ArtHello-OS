...
test("desktop typography and KPI cards do not depend on clipped microcopy", () => {
  ...
  assert.match(tokens, /--font-ui:\s*"Cygre"/);
  assert.match(css, /font-family:\s*"Cygre"/);
  assert.match(css, /3ed1693da2d7d3c28d28b79a8a1a6b0e\.woff2/);
  assert.match(css, /1e49c6ca6f3921df2a72128b76217e48\.woff2/);
...
test("owner cash flow is a source-backed interactive chart instead of decorative lines", () => {
  assert.match(ownerDashboard, /finance\.monthly\.filter/);
...
test("finance operation opens in a viewport portal and drills into linked entities without inventing family attribution", () => {
  ...
  assert.match(finance, /Тестовая семья · не детализация XLSX/);
  assert.match(finance, /не считается доказанной частью этой операции/);
  assert.match(registry, /onNavigate\?\.\(relation\.peer\.id\)/);
  assert.match(db, /Тестовый пример семьи · не детализация XLSX/);
