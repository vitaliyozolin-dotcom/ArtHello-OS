...
Production-доставка собирает базовый автономный runtime v3 и проверяемое
обновление v4. Перед переключением релиза создаётся резервная копия базы. После
успешного healthcheck скрипт идемпотентно загружает расписание 2026/27 и
проверяет 189 активных уроков, распределение по 1–6 классам и пять дней
математики во 2 классе. Если в базе найдено другое действующее расписание,
автоматическая замена останавливается до ручной сверки.
