#!/usr/bin/env bash
# D186: restore the central Atlas SSO boundary and publish the accepted Atlas UI
...
test "$CUTOVER_CONFIRMATION" = "DEPLOY D186 TO PRODUCTION"
...
receipt="$state_root/production-d186-$CONTROLLER_SHA.json"
run_key="$GITHUB_RUN_ID-$GITHUB_RUN_ATTEMPT"
candidate="arthello-direct-$run_key"
atlas_rollback="${atlas_service}-d186-rollback-$run_key"
work="$(mktemp -d "$state_root/.atlas-d186-$run_key.XXXXXXXX")"
...
route_backup="/data/external-routes.caddy.pre-d186-$run_key"
route_stage="/data/.external-routes.d186-$run_key"
...
rollback() {
  ...
  if [ "$success" -ne 1 ]; then
    printf 'ATLAS_D186_ROLLBACK=STARTED\n' >&2
...
"$runtime_node" "$controller_root/deploy/atlas-runtime-recovery-d186.mjs" env \
...
"$runtime_node" "$controller_root/deploy/atlas-runtime-recovery-d186.mjs" route \
...
test "$atlas_live_source" = "$atlas_source_before"
atlas_changed=1
...
test "$(current_main)" = "$CONTROLLER_SHA"
printf 'ATLAS_D186_PREDECESSOR=VERIFIED central=%s route=%s atlas=%s\n' "$central_release" "$expected_route_sha" "$atlas_live_source"

bash "$controller_root/deploy/upgrade-atlas-d186.sh"
...
atlas_upgrade_receipt="$ATLAS_BUNDLE_DIR/atlas-d186-production-receipt.json"
...
jq -e '
  ...
  (.backup.sha256|test("^[a-f0-9]{64}$")) and
  .backup.integrity=="ok" and .backup.journalMode=="delete" and
  ((.backup.walIncluded|type)=="boolean")
' "$atlas_upgrade_receipt" >/dev/null
test "$(jq -er '.backup.file' "$atlas_upgrade_receipt")" = "pre-d186-$run_key.sqlite"
test "$(docker inspect "$atlas_rollback" --format '{{.State.Running}}')" = false
printf 'ATLAS_D186_BACKUP=VERIFIED data_volume=%s backups_volume=%s\n' "$atlas_data" "$atlas_backups"
...
docker run -d --name "$candidate" --restart no --read-only \
  ...
  --label "arthello.config.decision=D186" \
...
printf 'ATLAS_D186_CENTRAL_SSO_OPEN=VERIFIED status=303\n'
...
printf 'ATLAS_D186_OWNER_SSO=VERIFIED role=director\n'
...
jq -n \
  ...
  --argjson atlasBackup "$(jq '.backup' "$atlas_upgrade_receipt")" \
  ...
  --argjson owner "$owner_result" '
    {schemaVersion:1,state:"verified",decision:"D186",controllerSha:$controller,
     ...
     atlas:{sourceSha:$atlasSource,sourceTree:$atlasTree,containerId:$atlasContainer,dataVolume:"atlas-school-diary-data",dataVolumePreserved:true,imageProof:$atlasImageProof,backup:$atlasBackup,ownerSso:$owner}}
  ' > "$production_receipt"
...
rm -f -- "$runtime_before" "$runtime_env"
trap - EXIT INT TERM HUP
printf 'ATLAS_D186_PRODUCTION=VERIFIED controller=%s central=%s atlas=%s candidate=%s\n' \
