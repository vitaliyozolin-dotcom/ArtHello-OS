...
SSO exchange должен вернуть `{ok:true, systemId:"SYS-SCHOOL-ATLAS", branchId:"BR-ATLAS-SCHOOL", identity:{centralUserId,displayName,contact,role,accessVersion}}`. Код ожидает отдельные центральные маршруты. В этой ветке нет изменений центрального сервера: до их реализации и подключения реальный сотрудник не войдёт в новый экземпляр. Проверка redirect/контракта в изолированных тестах не равна прохождению реального SSO.
