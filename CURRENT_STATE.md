...
- Первый D122 protected run `34520906921/103018492882` остановился до контейнеров и Caddy: artifact/checksums прошли, gateway Docker не разрешил raw image-ID lookup после load. D124 сохраняет тот же receipt и добавляет обязательное разрешение через immutable tag с немедленной сверкой tag→image ID; production Atlas ещё не принят.
