...
Release `511d467763b7ca050c096df23cfcdcd1f62fe51d` опубликован protected run `34799802826`. Подтверждены rollback snapshot, четыре банковских счёта с четырьмя остатками, один административный Pay grant, live AlfaCRM state с 2887 семьями, 193 сотрудниками и 60 абонементами, 65 секунд стабильности и независимые ArtHello/School/Pay `200`.

## D181 — Доступный выбор филиала в мобильных «Финансах» (2026-09-14, кандидат)
...
D181 добавляет компактный выбор филиала непосредственно в `FinanceWorkspace` для заблокированного, загрузочного, ошибочного и рабочего состояний. Оба переключателя используют единый `selectedBranch/changeBranch`; общий отчёт `ALL` остаётся выключенным. Production browser flow теперь воспроизводит дефект: устанавливает `ALL`, меняет viewport на 390×844, требует видимый touch-sized selector и выбирает филиал до продолжения финансового сценария.

Локально TypeScript, lint без ошибок и production build прошли; связанные финансовые и release-контракты зелёные. Полный набор дал 835 passed и 2 skipped; 6 неизменённых activation tests остановились только на запрете `chown` managed filesystem (`EINVAL`) и остаются обязательными в hosted Verify. Кандидат опирается на exact D180 production receipt; до successful Quality/Proof/Verify, rollback snapshot, 65-секундной стабильности и независимого внешнего probe выпуск не считается завершённым.
