...
## D181 — Настройки при большом каталоге семей (2026-09-14, production)
...
Красный тест подтвердил прежний `500`; после исправления тот же каталог даёт `200`, пустой быстрый основной ответ, страницу из 25 семей, общий счётчик 2 887 и точный результат поиска. Typecheck, production build и lint прошли; 17/17 целевых runtime/release-тестов зелёные. Первые protected runs `34810891708` и `34811820022` безопасно остановились до image load из-за неточного source guard. Исправленный exact release `eea35ebe384f1039324ad083fcdfa74e0d2217c4` опубликован run `34812906091`: подтверждены hosted image, AlfaCRM families=2 887/staff=193/subscriptions=60, rollback snapshot, четыре банковских счёта с остатками, 65 секунд стабильности, locked route и внешние ArtHello/School/Pay `200`.

## D182 — Доступный выбор филиала в мобильных «Финансах» (2026-09-14, production)

IMG_0270/IMG_0271 выявили production-дефект: обязательный филиал не выбран, а единственный глобальный selector находится в `.top-actions`, полностью скрытых мобильным media query. Поэтому early state «Выберите филиал» не содержал доступного действия и не позволял открыть отчёт.

D182 добавляет компактный выбор филиала непосредственно в `FinanceWorkspace` для заблокированного, загрузочного, ошибочного и рабочего состояний. Оба переключателя используют единый `selectedBranch/changeBranch`; общий отчёт `ALL` остаётся выключенным. Неадминистративному пользователю показываются только филиалы из действующих grants. При смене филиала старые данные сразу скрываются, а поздние ответы предыдущего запроса игнорируются. Production browser flow воспроизводит `ALL → филиал` на 390×844, контролируемый сетевой отказ, отсутствие данных прежнего филиала и успешный повтор.

D182 опирается на фактически активный exact D181 production receipt/release `eea35ebe384f1039324ad083fcdfa74e0d2217c4`. Cutover разрешён только ручным запуском владельца из `main` с exact release SHA, ID успешного first-attempt Verify для того же SHA и typed confirmation `DEPLOY D182 TO PRODUCTION`; защищённое окружение `production-ru` и повторная проверка подтверждения непосредственно перед `docker stop` обязательны. Отдельный D182 artifact-delivery helper принимает только этот точный ручной контекст; общий исторический R17 helper остаётся побайтово неизменным и принимает только `workflow_run`. До successful exact-head Quality/Proof/Verify, rollback snapshot, 65-секундной стабильности и независимого внешнего probe выпуск не считается завершённым.

Exact release `95873e519113e93d9d52ac08166eb46b317e5c6e` опубликован protected run `34819003014`.

## D186 — WAL-полная Atlas backup (2026-09-14, кандидат)
...
D184 exact main `d776bd2f46c763cec7f71479125b60bdf68bbe28` прошёл Quality `34830591245`, Proof `34830591229` и v52 verification `34830591231`. Protected run `34831139683/103935120775` доказал переносимую identity: builder/gateway IDs различались, а runtime fingerprint и source/tree labels совпали. После остановки прежнего Atlas и создания immutable backup reader с UID/GID `1001:1001` завершился `unable to open database file`. Это произошло до подтверждения backup, остановки central и route swap; rollback запустил прежний Atlas, D184 receipt не создан. D185 позднее доказал, что root получает тот же отказ и дело не в UID.
