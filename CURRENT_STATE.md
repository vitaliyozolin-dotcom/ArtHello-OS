...
Красный тест подтвердил прежний `500`; после исправления тот же каталог даёт `200`, пустой быстрый основной ответ, страницу из 25 семей, общий счётчик 2 887 и точный результат поиска. Typecheck, production build и lint прошли; 17/17 целевых runtime/release-тестов зелёные. Полный app-набор: 835 passed, 6 прежних owner/chown-тестов несовместимы с managed filesystem (`EINVAL`), 2 штатно пропущены. Exact-main Quality/Proof/Verify прошли дважды, но protected runs `34810891708` и `34811820022` не дошли до переключения: source-проверка не совпала с фактической строкой route; второй коммит добавил bounded ref retry, но сохранил эту неточность. Production остался на D180; исправление D181 включено в следующий D182.
...
D182 включает уже проверенное исправление D181 для каталога семей и исправляет его неточную source-проверку. Кандидат опирается на фактически активный exact D180 production receipt; до successful exact-head Quality/Proof/Verify, rollback snapshot, 65-секундной стабильности и независимого внешнего probe выпуск не считается завершённым.
