...
D182 опирается на фактически активный exact D181 production receipt/release `eea35ebe384f1039324ad083fcdfa74e0d2217c4`. Cutover разрешён только ручным запуском владельца из `main` с exact release SHA, ID успешного first-attempt Verify для того же SHA и typed confirmation `DEPLOY D182 TO PRODUCTION`; защищённое окружение `production-ru` и повторная проверка подтверждения непосредственно перед `docker stop` обязательны. Загрузчик verified artifact сохраняет прежний `workflow_run` default и разрешает manual mode только точному D182 workflow с той же фразой. До successful exact-head Quality/Proof/Verify, rollback snapshot, 65-секундной стабильности и независимого внешнего probe выпуск не считается завершённым.
