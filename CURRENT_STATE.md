...
## D176 — Компактные банковские итоги, счета и мобильная история (2026-09-13, кандидат)
...
Пятиколоночная таблица удалена только из банковской вкладки: дата, контрагент, назначение, счёт, сумма и статус теперь помещаются в адаптивную строку без горизонтального скролла. Состав операций, период, остатки, синхронизация, границы «Денег»/«Эквайринга» и вход в Pay не меняются. Кандидат проходит целевые контракты и TypeScript; до объявления production нужны hosted image, snapshot, 65-секундная стабильность и независимый внешний probe.
