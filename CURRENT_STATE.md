...
## D175 — Единый вход в Pay и устойчивый мобильный поиск (2026-09-13, кандидат)
...
Причина закрытия клавиатуры после одной буквы — полная перерисовка `renderShell()` через 260 мс при запуске поиска и после ответа. D175 сохраняет тот же input и обновляет только `.customer-search-results`; защита от устаревших ответов остаётся. Реальные платежи и фискализация не включаются. До объявления production нужны exact-head CI, защищённый выпуск и независимый внешний probe.
