...
- D125 protected run `34523308648/103026645872` снова остановился после artifact checksums и до контейнеров/Caddy; отсутствие Docker stderr указывает на fail-closed comparison, но не доказывает конкретную строку. D126 устраняет возможный stale-tag drift перед load и добавляет безопасную явную диагностику, сохраняя обязательные tag→image ID и revision сверки; production Atlas ещё не принят.
