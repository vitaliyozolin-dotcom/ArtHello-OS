...
### D-080 — Подтверждённый Education403 и подготовка проверки кандидата
...
- Первоначальный R9 без изменения порядка не мог доставить это исправление: он требовал полного PASS старого приложения. D080 открыто меняет порядок, сохраняя FAIL baseline и требуя полного естественного прохода того же сотрудника на точном кандидате до открытия обычных запросов. Описание протокола и границ восстановления — DECISIONS D080. Окно обслуживания ещё не включалось.
