...
Подготовлены ровно шесть source overlays и четыре документа поверх finalized D092. Локально: 30 browser policy и 15 retirement tests PASS, независимые reviews без блокирующих замечаний. Маршрут: сверить фактический D092 main → exact-source CI/hosted real image smoke → прежний D076 protected browser run → четыре фиксированных результата и финальный cleanup. Hosted Chromium и фактические employee flags пока pending. Метрика завершения — проверяемый terminal результат с настоящей сессией, а не наличие тестового fixture.
