...
## Приложение D128 — филиальный учёт с 01.09.2026 (2026-09-10)

Владелец — Виталий; исполнитель — Codex. Цель — вести финансовый контур только с 1 сентября 2026 года и показывать самостоятельные ОДДС/ОПиУ по каждому филиалу без консолидации. Входы: неизменяемые банковские операции, утверждённый каталог из 148 активных статей, юрлицо и явный филиальный ключ. Выходы: филиальный реестр, ОДДС, ОПиУ и автоматические назначения только для доказанных правил Атласа.

AI-контракт: разрешено читать банковские факты с даты старта, записывать только управленческую проекцию однозначного назначения школы/садика и показывать один доступный активный филиал. Запрещено учитывать даты до границы, формировать общий отчёт, распределять общий остаток юрлица, угадывать филиал, менять банковские факты или справочник без нового решения. Ответственный за бизнес-правила и ручные исключения — Виталий; технические ограничения исполняет детерминированный код. Новых сервисов нет, дополнительная стоимость — ноль, стоимость штатной платформы неизвестна.

Метрики: ноль строк другого филиала и дат до 01.09.2026 в отчёте; 100% финансовых запросов требуют конкретный доступный филиал; каждая автоматическая запись одновременно содержит филиал, статьи ДДС/ОПиУ, класс и период. Автостоп — отсутствующий/общий/недоступный филиал, неготовый каталог, неизвестное юрлицо, нерублёвая или расходная операция, конфликт либо отсутствие признаков. При отказе банковский факт сохраняется, операция остаётся ручной, данные не удаляются. Пересмотр — после защищённого выпуска и фактической сверки новых операций.
