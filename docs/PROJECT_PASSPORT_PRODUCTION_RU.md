...
## D-077 — R8: приёмка из настоящего браузера и сохранённый путь выпуска

- Статус preparation; дата2026-09-08; владелец Виталий, исполнитель Codex. Это техническое продолжение уже разрешённого выпуска, без получения новых host/admin полномочий.
- Проблема: R7 остановился до image import/cutover из-за отсутствия подходящего browser receipt. Дневник вручную работает у владельца; это не подменяет машинную identity-bound приёмку. D076 bundle теперь собирается отдельно, сохраняя namespace sandbox, stdin credentials, TLS и оба fixed origins.
- Входы: отдельные exact R8 PR/branch/merge SHA; current main и успешные Quality/Proof/v52 Verify с provenance до checkout; production-ru и оба shared locks; проверенная предыдущая R5 installation, R6 abort и R7 pre-cutover abort; фактический D076 protected pass и pinned portable browser fingerprint. Старые R1–R7 не переармляются и не изменяются.
- Изменения: перед импортом release image и после public cutover запускается тот же ограниченный browser flow; read-only School diagnostic/repair receipt и live ArtHello labels связываются с настоящим результатом. Schema3 validator сохраняется, включая freshness45 минут, время после repair, role/contact matching в браузере, no session injection/no constructed callback и exact steps. После cutover live ArtHello SHA должен быть R8 candidate.
- Сохраняется весь R7 cutover step: canonical active D1/FD proof; portable immutable runtime; clone preflight; проверенный ordinary-UID backup до live stop; separate backup/control volumes; isolated UID1002 activation writer; current-main/School secret checks; durable public-write boundary. Никаких host package installs, root/sudo, изменения School routes/volumes или новых payment полномочий.
- Отказ: нет образа/места/доступа/совпадения source/receipt/sandbox — остановка до cutover. Начавшийся предыдущий cutover или неполная история не разрешает replay. Ошибка после public activation сохраняет candidate/current DB/rollback volumes и записи пользователей; автоматический snapshot restore запрещён.
- Приёмка: hosted структурное сравнение R8/R7 границ и полного cutover; негативные replay/receipt tests; полный exact image build/760+ application tests и реальные Docker backup/restore fixtures; завершённое exact-head review; настоящий pre/post browser и production backup/activation evidence. Source tests и один login не объявляют финансовую сверку или все пользовательские сценарии успешными.
- AI-контракт: цена — существующий CI/runner, без покупки инфраструктуры; метрика — проверенный опубликованный кандидат с сохранёнными данными и конкретными результатами сценариев. Стоп владельца прекращает новые запуски; после принятия публичных записей сохраняется восстановимое состояние. Следующий шаг при реальном resource blocker — конкретные измерения и необходимое действие владельца, а не повторный запрос общего разрешения.
