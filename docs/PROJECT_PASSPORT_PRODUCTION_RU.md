...
## Решение владельца: обращения и обратный переход из дневника — 2026-09-09

Источник: прямое поручение Виталия в чате OS ArtHello; снимок существующего обращения «кнопка выхода из дневника».

- Отдельный раздел «Обращения и улучшения» утверждён для бэклога: собственник просматривает все предложения и ошибки. Текущее окно «Разработчикам» пока сохраняется. Реализация раздела отложена; не объявлять опубликованным.
- Приоритетная небольшая правка: сотрудники возвращаются из дневника в ArtHello OS по заметной ссылке «Вернуться в ArtHello OS», включая телефон. Это навигация в той же вкладке, без logout, удаления cookies и выдачи новых прав. Родительский и ученический интерфейсы не получают ссылку на рабочую OS.
- Приоритет интеграций сохраняется: Точка, затем выборочный импорт АльфаCRM. Активный банковский выпуск и main не менять ради этой правки.
- Критерий завершения кнопки: обычный вход ArtHello → дневник → возврат ArtHello с действующей сессией, мобильная/десктопная видимость, отсутствие горизонтального переполнения; истёкшую сессию обрабатывает штатная авторизация ArtHello. Сборка и публикация отмечаются отдельно.
- Исполнитель: Codex; владелец решения: Виталий. Срок не назначен. Статус: подготовка отдельного кандидата; production ещё не изменён.


## Уточнение владельца: «Атлас» и «Школа 1–11» — разные школы — 2026-09-10

Источник: Виталий в текущем чате: «Это разные школы». Владелец ранее согласовал выполнение запроса Екатерины Грудановой, руководителя учебной части школы и садика «Атлас». Исполнитель — Codex.

Это уточнение исправляет ошибочную привязку задачи Кати к коду дневника «Школы 1–11». Вопрос о тождестве школ закрыт. Подготовленный PR402 не считается выполнением запроса «Атласа». Разрешение на дальнейшую работу сохраняется; новая инфраструктура и центральные идентификаторы по догадке не создаются.

### Что подтверждено исходниками

| Объект | Существующий идентификатор | Источник |
|---|---|---|
| Атлас — школа | BR-ATLAS-SCHOOL | deploy/v52/src/lib/education.ts:58 |
| Атлас — садик | BR-KINDERGARTEN | deploy/v52/src/lib/education.ts:58 |
| Школа 1–11 | BR-SCHOOL | deploy/v52/src/lib/education.ts:58 |
| Система существующего школьного входа | SYS-SCHOOL-1-11 | deploy/v52/src/lib/school-sso.ts:62; deploy/v52/src/lib/staff-access-sync.ts:5 |

Это карта исходников, прочитанных 2026-09-10, а не проверка рабочих строк БД или CRM-привязки. AlfaCRM branch IDs должны сопоставляться по D-068; нельзя объединять учреждения по похожим названиям.

Существующие связи ArtHello:
- organization_branches.id — ключ подразделения; user_branch_access уникален по user_id + branch_id.
- user_system_access уникален по user_id + system_id: доступ к разделу/филиалу не равнозначен доступу в другую систему.
- education_groups.unit_entity_id связывает группу с подразделением; education_students.group_id — учебную запись ребёнка с группой; education_lessons.group_id — занятие с группой.
- GET /api/education читает действующие branch grants и фильтрует группы и детей сервером. Для isAdministrative предусмотрены все активные подразделения: этот широкий флаг не является способом ограничить Катю «Атласом».
- В отдельном School-приложении deputy видит всех активных учеников экземпляра. Его нельзя выдать Кате как замену прав в «Атласе».
- В EducationWorkspace.tsx:30,103 общая кнопка «Открыть дневник» всегда ведёт на school-188-225-38-55.sslip.io/auth/central/start. Выбор филиала эту цель не меняет. Наличие этого перехода не подтверждает отдельного дневника «Атласа».

### Задача ATLAS-ACADEMIC-LEAD

Цель: Катя составляет расписание в школе или садике «Атлас» → назначенный педагог видит занятие → связанный родитель видит опубликованный результат.

Рабочая область: школа «Атлас» и садик «Атлас», по подтверждённым назначениям её центральной учётной записи. Школа 1–11, Небо и Лиственная не включаются этим поручением. Не изменять существующие права других сотрудников.

Необходимое поведение:
1. Расписание: создавать и редактировать занятия, назначать педагогов и замены, проверять пересечения в выбранном учреждении.
2. События: указывать школу/садик и конкретные классы/группы либо всё выбранное учреждение. Текстовая подпись аудитории сама по себе не задаёт доступ.
3. Программы: смотреть, загружать, проверять и утверждать в пределах выданных полномочий. Дошкольные группы и планы не получать школьные оценки и КТП автоматически.
4. Контроль: доступные классы/группы, посещаемость, учебные результаты и опубликованные комментарии педагогов.
5. Предпросмотр родителя: выбранный ребёнок только из доступного учреждения; заметная плашка и возврат в кабинет Кати; учебные сведения без действий от имени родителя, личной переписки, финансов или отметки уведомлений прочитанными. Отдельные финансовые/переписочные права этим поручением не выдаются.

### Статус и проверка

- Подтверждено владельцем: учреждения разные; запрос относится к «Атласу».
- Подтверждено кодом: в ArtHello уже есть раздельные branch IDs; существующий SSO указывает на «1–11».
- Подготовлено ранее: PR402 — учебный preview в исходниках «1–11», 39/39 локальных проверок и production-сборка. Эти результаты относятся только к кандидату «1–11» и не являются приёмкой «Атласа».
- Открыто: адрес/проект и system ID рабочего дневника «Атласа» либо решение о создании нового; фактическая центральная запись Екатерины и её назначения. В этой сессии рабочая учётная запись не прочитана и не изменена.
- Критерии завершения «Атласа»: полный пользовательский сценарий выше, отдельный проход школы и садика, отказ доступа к «1–11» прямым запросом и через выбор ребёнка, проверка отсутствия действий от имени родителя. Подготовка кода, локальная проверка и публикация отмечаются отдельно.
- Следующий шаг: определить правильный проект/адрес дневника «Атласа» и проверить существующие назначения; затем доработать недостающее в этом контуре. Не продолжать реализацию задачи Кати в «1–11».
- Ресурсы: использовать существующий проект и доступы; новый сервер или платный сервис не выбран. Срок не назначен. Приоритет интеграций сохраняется: Точка → АльфаCRM.
- PR401 с возвратом из дневника «1–11» самостоятельный; уточнение учреждений не отменяет полученное разрешение на его выпуск после необходимых проверок. Production этой корректировкой не изменяется.


## Решение владельца: новый дневник «Атласа» по шаблону «1–11» — 2026-09-10

Источник: Виталий — «У Атласа своего дневника нет, нужно будет сделать по шаблону 1-11». Владелец — Виталий; исполнитель — Codex. Решение подтверждено. supersedes: в уточнении от 2026-09-10 заменяется только открытый вопрос о наличии отдельного дневника и поиске его адреса. Учреждения остаются разными.

У «Атласа» дневника пока нет. Создать новый на основе дневника «Школы 1–11». Поиск существующего адреса закрыт ответом владельца; новый адрес и системная привязка являются частью реализации, а не недостающими данными от пользователя.

Область задачи ATLAS-ACADEMIC-LEAD:
- Использовать интерфейс и пригодные механизмы «1–11»: кабинеты, расписание, программы, журнал, события, родительский просмотр и возврат в ArtHello OS.
- У «Атласа» свои дети, семьи, педагоги, учебные данные, настройки и назначения прав. Не переносить рабочую базу, пользователей, расписание, КТП и секреты «1–11» как стартовые данные.
- Предусмотреть школу BR-ATLAS-SCHOOL и садик BR-KINDERGARTEN. Катя руководит учебной частью этих подразделений; доступ к данным «1–11» из этого назначения не следует.
- Для школы применять школьные сценарии; для садика — группы, занятия, посещаемость и обратную связь, без автоматического навязывания школьных оценок и КТП.
- Переход из ArtHello должен выбирать правильное учреждение. Новый вход и синхронизация не могут использовать SYS-SCHOOL-1-11 как идентичность «Атласа».

Маршрут:
1. Подготовить переносимый шаблон без привязок к данным, бренду, конфигурации входа и релизным импортам «1–11».
2. Подключить изолированные данные и назначения «Атласа», затем адаптировать школу/садик и кабинет Кати. Технический способ изоляции и адрес фиксируются при реализации; новый платный сервер этим решением не выбран.
3. После сопоставления филиалов AlfaCRM загружать выбранные данные по D-068. Импорт не выдаёт права автоматически. Подготовка пустого шаблона не требует импорта CRM.
4. Пройти сценарий Катя → педагог → родитель отдельно для школы и садика, проверки границ учреждений, пустого состояния, ошибок и возврата на телефоне и компьютере; затем выпустить проверенный кандидат с сохранением существующих данных.

Основной приоритет интеграций остаётся Точка → АльфаCRM. Новый дневник зафиксирован как последующая работа; подготовку шаблона можно выполнять отдельно, не задерживая интеграции. Срок и бюджет не назначены. Метрика результата — рабочий сценарий «Атласа» и отсутствие доступа к чужому учреждению; число PR или сборок не означает завершение.

PR402 можно использовать как исходный материал для адаптации родительского предпросмотра: сейчас он всё ещё исполняется в коде «1–11», и его прежние проверки не являются проверками «Атласа». PR401 с возвратом — самостоятельная задача. На этом шаге обновлены только постановка и история решений; новый дневник ещё не реализован и не опубликован, права Кати не менялись.


## Результат проверки учебных сценариев — 2026-09-10

По запросу Виталия проверены расписание, импорт годового КТП, распределение тем и смежные функции. Отчёт и точные границы доказательств: docs/acceptance/2026-09-10-education-workflows-audit.md.

School SHA 54242340 совпадает с версией из R16 receipt (10.09.2026 09:41:39 UTC). Локально 44/44 существующих проверок прошли: XLSX, календарное распределение, изолированный импорт и ролевые контракты. Это не подтверждение личных прав Екатерины/Наталии и не сквозная браузерная приёмка. Рабочий UI из облачного браузера вернул 502; карточки сотрудников не прочитаны, данные и права не менялись.

Воспроизведён дефект посещаемости: две недели одного lesson_id + student_id заменяют одну запись; дата конкретного занятия отсутствует. Рабочего ввода посещаемости в текущем UI нет. Выявлены также отсутствие редактора учебного года/каникул, разовой замены по дате и пользовательской загрузки XLSX расписания; аудитория событий не фильтрует серверный список.

Функции КТП для deputy/director существуют; methodist не утверждает КТП и не правит расписание, admin не импортирует КТП. Учебный год 2026/27 фиксирован. Прежнее решение о запрете автоматического сдвига всех тем сохраняется. «Атлас» остаётся отдельным новым дневником по шаблону «1–11».

Статус: аудит кода и тестов завершён, личные доступы и сквозной рабочий сценарий открыты. Найденные недостатки записаны в BACKLOG; это результаты проверки, а не опубликованные исправления. Следующий предлагаемый шаг — датированная посещаемость и проверка назначений; сроки/новый бюджет не назначены.
