...
## D099 — Доказать сохранение операций Точки и равенство сумм ДДС

Владелец — Виталий; исполнитель — Codex. Draft подготовлен в рамках разрешённого доведения Точки до работы. Входы: D097, реальный успешный receipt будущего R14, четыре уже настроенных счёта и каноническая production D1. Выход: завершённый read-only отчёт по фактическому принятому runtime с покрытием периода, реальными операциями, account-scoped антидублями и согласованными доходами/расходами банка и связанных строк ДДС. До успешного R14 и закрепления его реальных pins запуск нового наблюдения блокируется.

Разрешены подготовка и review исходников, существующие hosted CI, после приёмки R14 — существующий protected D075 read-only путь. Запрещены raw financial exports, произвольные ошибки/секреты в отчётах, платежи, ручные SQL writes, смена ролей/ключей/scope и обход scheduler lease/backoff. Метрика пользы — подтверждённая банковская загрузка с равными суммами и без дублей; тестовые fixtures и CI не заменяют бизнес-приёмку. Стоимость — существующие CI/серверы, точная стоимость выполнения неизвестна; новых услуг нет. Пересмотр после terminal release и actual observation. Стоп — source/runtime/consumer drift, неподтверждённые pins, несогласованный отчёт или отказ владельца. При отказе новые проверки не запускаются, существующие данные и приложение сохраняются.
