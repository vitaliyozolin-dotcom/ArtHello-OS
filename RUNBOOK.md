...
## D194 — выпуск Альфы и приёмника Атласа

После successful exact-main Quality/Proof/Verify открыть существующий `Build closed RU release` (`deploy-ru.yml`), выбрать main, `operation=alfa-central`, передать полный текущий SHA в `ref` и фразу `DEPLOY ALFA UPDATE TO PRODUCTION` в `confirmation`. Это ранее разрешённая владельцем установка исходников D187–D193; импорт данных не запускается. После terminal outcome отдельно выполнить `alfa-atlas` с тем же main. Не менять main во время установки.

Установщик повторно проверяет first-attempt provenance, actual predecessor, готовый image/fingerprint и свободное место. Точка сохраняет прежнее окружение и activation volume без изменения. Новый source фиксируется в image/container labels; сохранённый runtime RELEASE_SHA относится к банковской активации. Старый контейнер остаётся остановленным; резервная копия остаётся в отдельном именованном томе. После публичного запуска запрещено восстанавливать старую базу поверх текущей: rollback возвращает только контейнер, сохраняя текущий /data.

При BLOCKED изучить последний `D194_STAGE` и фиксированный `reason`; не повторять старый job и не ослаблять guard. Установка School 1–11 и бизнес-импорт не входят в эти две операции. Для них нужны проверка отдельного School runtime, свежий предпросмотр Альфы, сохранение legacy evidence, применение D190/D191 и отдельная проверка обоих дневников. Браузерный preflight 35641981048 заблокирован настройками тестового окружения; выдавать его за production acceptance нельзя.

## D189 — подтверждённая идентичность

Порядок выпуска и отката: `docs/acceptance/2026-09-15-d189-unified-identity.md`. Старые карточки не удалять: это ссылки на источники и историю. Сначала резервная копия и выпуск финального проверенного SHA, затем полный импорт семей/сотрудников/групп, сравнение активных основных ID и филиальных назначений, повторная загрузка. Разные ID подтверждать через реестр с основанием; семьи и детей проверять отдельно. Перед downgrade остановить автоимпорт; старая версия может оживить объединённые карточки. Оба дневника проверять отдельно.
...
## D187 — AlfaCRM: серверное автообновление

После проверенного выпуска кандидата передать `ALFACRM_AUTOSYNC_ENABLED=1` в центральный runtime, сохранив `ALFACRM_IMPORT_ENABLED` и существующее защищённое подключение. Флаг отсутствует по умолчанию; ключ AlfaCRM повторно вводить не требуется. Включение флага само по себе не запускает импорт: сохранённый scope задаётся собственником в AlfaCRM → «Автоматическое обновление» после сверки первичной загрузки. Не активировать поверх обнаруженной путаницы «Атлас — садик» / «Атлас — школа».

Выбрать только полностью загруженные семьи, сотрудников, группы, остатки. Группы требуют сотрудников, остатки — семей. Статус показывает последний полностью успешный цикл и следующий шаг; этапы, HTTP-код, повторы и остановки пишутся в audit_events действием `integration.alfacrm_autosync_tick`. Каждую минуту таймер проверяет срок, но не обращается к AlfaCRM, если запуск ещё не наступил. Период полного обновления — час после окончания предыдущего цикла. Ручной импорт во время автоматического обновления отклоняется; сначала остановить обновление. Остановка ждёт завершения текущего сериализованного этапа. Смена филиалов останавливает следующий этап до повторной сверки. Прогресс и backoff сохраняются при рестарте.

Применимо к текущей архитектуре с одним worker на общей БД. До горизонтального масштабирования заменить внутрипроцессный mutex межпроцессной арендой. Исторические занятия из AlfaCRM и CRM-проводки в автоматическую очередь не входят. Для расписания 1–11 использовать утверждённый Лист2 с 01.09.2026. После активации отдельно сверить данные ОС и обоих дневников; вход через SSO не является проверкой передачи учеников.

### D188: сверка филиалов и архив семей (после проверенного выпуска)

1. Снять проверенную резервную копию; автообновление AlfaCRM остановить. Проверить явное сопоставление удалённых ID филиалов с локальными, исключив online при отсутствии такого направления в ОС.
2. В AlfaCRM wizard открыть «Сверить сохранённые данные по филиалам». Это аудит последних ответов, а не свежая выгрузка. Сохранить количества/время источника и сопоставить с текущими клиентами в AlfaCRM; около 200 — оценка собственника.
3. Повторить полный предпросмотр и импорт семей, сотрудников, затем групп. Не обходить ошибку неизвестной принадлежности. Полный принятый снимок архивирует чужие и ушедшие записи только выбранных филиалов. Сверить количества, ID клиентов, семей и назначений отдельно. Остатки обновлять после семей. Не загружать расписание AlfaCRM поверх Лист2.
4. В «Клиенты и семьи» текущие и архивные карточки разделены. «В архив» сохраняет историю, прекращает учебные назначения и защищает от авто-восстановления импортом. «Восстановить» для AlfaCRM требует подтверждения источника, затем назначение сверяется отдельно. Не объединять семьи автоматически по телефону.
5. После успешной сверки включить D187 на сервере (`ALFACRM_AUTOSYNC_ENABLED=1`) и выбранные модули в wizard. Новый контракт требует повторной полной загрузки, а не старого статуса «импортировано». Проверить полный цикл без открытого браузера и отсутствие роста дублей.
