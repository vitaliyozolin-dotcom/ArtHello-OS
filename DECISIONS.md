...
Одноразово для релиза из `vitaliyozolin-dotcom/ArtHello-OS:codex/owner-dashboard-tochka-20260902` D-040 уточняется только в части запуска: вместо недоступного `workflow_dispatch` используется `workflow_run` после успешной hosted-проверки `Verify ArtHello v52 release` на точном подписанном squash-коммите, который в момент каждого необратимого шага остаётся текущим `main`. Защищённое Environment `production-ru`, обязательное независимое подтверждение без self-review, ограничения владельца/репозитория/ветки/actor, точные Quality и Verify gates, изоляция self-hosted runner, backup-first cutover и автоматический rollback остаются обязательными. Повтор успешной выкладки того же SHA и повторный attempt production-run запрещены. Это исключение не распространяется на следующие релизы.
