...
До открытия записи выполняются offline build, clone-preflight, остановка старого контейнера, maintenance gate, финальный проверенный SQLite snapshot, live import и сверка защищённого scope. Любая ошибка до commit boundary восстанавливает snapshot и прежний контейнер. После успешной полной сверки единый синхронизированный marker в отдельном rollback volume атомарно проходит состояния `ROLLBACK_READY → ROLLBACK_DISABLED → RELEASE_VERIFIED`; labels связывают его с exact release, run и attempt. Файл `commit-state` в `/tmp` используется только как быстрый сигнал текущему workflow и не является источником истины после перезагрузки. До состояния `RELEASE_VERIFIED` кандидат сохраняет `restart=no`; штатный автозапуск возвращается только после внутренних и публичных проверок. Поэтому перезагрузка между снятием maintenance gate и окончанием проверки не поднимает неподтверждённый кандидат на запись.

При любой ошибке deploy отдельный `always()` re-gate сначала оставляет exact cancel fence в общем production lock, ждёт тот же `flock`, проверяет готовность Docker и только затем читает durable state. Подтверждённый `ROLLBACK_READY` допускает прежний production, а `ROLLBACK_DISABLED`, `RELEASE_VERIFIED`, известный commit или любое противоречивое/нечитаемое доказательство сначала устанавливают и независимо проверяют maintenance gate, в том числе для остановленного контейнера. Старый контейнер и rollback volume сохраняются остановленными как операционное доказательство и средство ручного восстановления.
