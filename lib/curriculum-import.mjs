...
export async function parseCurriculumWorkbook(input) {
  // The production release importer reuses the calendar/allocation helpers
  // from this module but consumes normalized JSON, not XLSX. Load ExcelJS only
  // for the interactive XLSX path so the offline release image does not need
  // to carry the parser dependency.
