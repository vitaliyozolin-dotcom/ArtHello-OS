...
async function schemaSnapshot(client) {
  const { rows } = await client.query(`
    SELECT table_name, column_name, ordinal_position, data_type, is_nullable, COALESCE(column_default, '') AS column_default
...
try {
  ...
  if (fullAfter.digest !== fullBefore.digest) {
    throw new Error(`Schema digest mismatch after reapply: before=${fullBefore.digest} after=${fullAfter.digest}`);
  }
