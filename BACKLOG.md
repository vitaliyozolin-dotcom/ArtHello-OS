...
### D108 — статьи и разнесение, 10.09.2026

- [ ] Принять справочник ДДС/ОПиУ и назначение через утверждённые статьи: очередь без статьи, атомарный аудит, конкуренция, проверка копеек и DDS drilldown. Кандидат: `docs/acceptance/2026-09-10-finance-articles.md`.
...
- [x] Production central и отдельный Atlas runtime выпущены D129; D130 protected receipt подтвердил полный redirect exchange и авторизованный Atlas API владельца.
- [ ] D185: повторить отдельный Atlas release с переносимой image identity и изолированной root-проверкой immutable backup; D183/D184 безопасно откатились до central/route cutover.
...
- [x] Добавить отдельные статьи управленческих услуг и организации праздников в ДДС и ОПиУ.
- [x] Ограничить автопроход ООО «АртХелло» только однозначными поступлениями школы/сада; конфликт и общий «Атлас» оставлять без назначения.
- [ ] Проверить вручную оставшиеся неоднозначные операции ООО «АртХелло».
