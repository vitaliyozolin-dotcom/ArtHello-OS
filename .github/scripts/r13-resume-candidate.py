"""Verify the held R13 application and sealed accepted-R12 backup adoption."""
import argparse
import hashlib
import fcntl
import json
...
state = checked_module('r13_candidate_state', 'r13-candidate-state.py', '7387d5d189c8ba1c1801899756fabad3428dd8661c8d0fc6ff1fe506cfad355c')
adoption = state.adoption
backup = adoption.r7
controller = checked_module('r13_backup_controller', 'r13-backup-controller.py', '1c480194f7f301b7956e8b1ee0e05b172fb57785c6a2ab1ff9094bab045e3d10')
...
def verify(path, *, release, tree, run, attempt, durable_root, hold=False, docker=None, probe=True):
    ...
    require(previous.get('Id') == adoption.ACCEPTED_APP_ID and previous.get('Image') == adoption.ACCEPTED_IMAGE
            and previous.get('Config', {}).get('Labels', {}).get('arthello.release.sha') == adoption.ACCEPTED_SHA
