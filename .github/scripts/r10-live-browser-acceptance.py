...
def write_acceptance(record, target, *, run, attempt, durable_root):
    ...
    assert path.name == 'r10-acceptance-' + run + '-' + attempt + '.json'
...
def main():
    try:
        ...
        raise SystemExit('ARTHELLO_R10_SSO_ACCEPTANCE=BLOCKED: browser, source or repair evidence is absent, stale or mismatched')
    print(json.dumps(record, separators=(',', ':')))
    print('ARTHELLO_R10_SSO_ACCEPTANCE=VERIFIED')
