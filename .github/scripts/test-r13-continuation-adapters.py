...
resume = load('r13_resume_test', 'r13-resume-candidate.py')
state = resume.state
adoption = state.adoption
public = load('r13_public_test', 'r13-public-audit.py')
activation = load('r13_activation_fixture', 'd063-activation-state.py')
old_tests = load('r13_held_fixture', 'test-d083-resume-candidate.py')
backup_tests = load('r13_backup_fixture', 'test-r13-backup-adoption.py')
controller_tests = load('r13_controller_fixture', 'test-r13-backup-controller.py')
private, json_bytes = old_tests.private, old_tests.json_bytes
REFUSED = (ValueError, adoption.r7.Refused, public.state.adoption.r7.Refused, resume.controller.r7.Refused, OSError, AssertionError)


class HeldR13(old_tests.HeldCandidate):
    def __init__(self, root):
        ...
        self.state_path.unlink()  # Discard synthetic D083 state before real R13 begin.
        c = self.context
        ...
        c['previousContainerId'] = adoption.ACCEPTED_APP_ID
        c['previousName'] = 'arthello-direct-' + adoption.ACCEPTED_RUN + '-1'
        self.previous.update(Id=c['previousContainerId'], Name='/' + c['previousName'], Image=adoption.ACCEPTED_IMAGE,
                             Config={'Labels': {'arthello.release.sha': adoption.ACCEPTED_SHA}})
...
class AdapterTests(unittest.TestCase):
    def setUp(self):
        ...
        self.f = HeldR13(Path(self.temp.name).resolve())

    def test_split_identity_context_is_valid(self):
        context = dict(self.f.context)
        target = state
        if os.environ.get('R13_TEST_FROZEN') == '1':
