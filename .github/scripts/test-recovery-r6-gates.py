...
acceptance = load('acceptance', 'check-school-live-acceptance-r6.py')
replay = load('replay', 'd068-replay-guard.py')
...
class PreservedInstallationTests(unittest.TestCase):
    def setUp(self):
        self.repository = 'vitaliyozolin-dotcom/ArtHello-OS'
        self.run = {'id':replay.INSTALL_RUN, 'head_sha':replay.INSTALL_SHA, 'path':replay.INSTALL_PATH,
            'repository':{'full_name':self.repository}, 'head_repository':{'full_name':self.repository},
            'event':'workflow_run', 'head_branch':'main', 'actor':{'login':'vitaliyozolin-dotcom'},
            'triggering_actor':{'login':'vitaliyozolin-dotcom'}, 'run_attempt':1, 'status':'completed', 'conclusion':'failure'}
        self.job = {'id':replay.INSTALL_JOB, 'name':'deploy', 'run_attempt':1, 'status':'completed', 'conclusion':'failure', 'steps':[
            {'name':'Repair School egress configuration with durable receipt','conclusion':'success'},
            {'name':'Read-only School diagnostic and real browser acceptance','conclusion':'failure'},
            {'name':'Clone preflight and guarded production cutover','conclusion':'skipped'}]}

    def check(self, run=None, job=None):
        return replay.validate_installation(run or self.run, {'total_count':1,'jobs':[job or self.job]}, self.repository)

    def test_exact_installed_r5_is_a_readonly_prerequisite(self):
        self.check()

    def test_altered_or_replayed_installation_is_rejected(self):
        import copy
        for key, value in [('id',34208952717),('head_sha','a'*40),('path',replay.CONSUMER_PATH),('run_attempt',2),
                           ('conclusion','success'),('status','in_progress'),('head_branch','other'),('event','pull_request')]:
            with self.subTest(key=key), self.assertRaises(AssertionError):
                self.check(dict(self.run, **{key:value}))
        for key in ('repository','head_repository'):
            with self.subTest(key=key), self.assertRaises(AssertionError):
                self.check(dict(self.run, **{key:{'full_name':'foreign/repository'}}))
        for key in ('actor','triggering_actor'):
            with self.subTest(key=key), self.assertRaises(AssertionError):
                self.check(dict(self.run, **{key:{'login':'unapproved'}}))
        for index in range(3):
            for outcome in ('failure','cancelled','success','skipped',None):
                job = copy.deepcopy(self.job)
                if job['steps'][index]['conclusion'] == outcome:
                    continue
                job['steps'][index]['conclusion'] = outcome
                with self.subTest(index=index,outcome=outcome), self.assertRaises(AssertionError):
                    self.check(job=job)

    def test_missing_duplicate_or_incomplete_history_is_rejected(self):
        for jobs in ({'total_count':0,'jobs':[]},{'total_count':2,'jobs':[self.job]}, {'total_count':2,'jobs':[self.job,self.job]}):
            with self.subTest(jobs=jobs['total_count']), self.assertRaises(AssertionError):
                replay.validate_installation(self.run,jobs,self.repository)
        for steps in ([],self.job['steps']+[self.job['steps'][0]],self.job['steps'][:-1]):
            with self.subTest(length=len(steps)), self.assertRaises(AssertionError):
                self.check(job=dict(self.job,steps=steps))

    def test_candidate_cutover_is_byte_identical_to_r5(self):
        path=Path(__file__).resolve().parents[1]/'workflows/deploy-arthello-recovery-r5-20260908.yml'
        lines=path.read_text().splitlines()
        identity=lines.index('      id: cutover')
        step_start=max(i for i in range(identity+1) if lines[i].startswith('    - name:'))
        start=lines.index('      run: |',step_start)+1
        end=next((i for i in range(start,len(lines)) if lines[i] and not lines[i].startswith('        ')),len(lines))
        self.assertEqual(cutover_script(),'\n'.join(line[8:] if line else '' for line in lines[start:end])+'\n')

    def test_empty_missing_or_duplicate_current_run_history_cannot_skip_prior_attempts(self):
        import io
        current={'id':999,'head_sha':'a'*40,'repository':{'full_name':self.repository},'path':replay.CONSUMER_PATH,'run_attempt':2}
        environment={'EXPECTED_REPOSITORY':self.repository,'RELEASE_SHA':'a'*40,'GITHUB_RUN_ID':'999','GITHUB_RUN_ATTEMPT':'2','GH_TOKEN':'synthetic-test-token'}
        for runs in [[],[dict(current,id=998)], [current,current]]:
            def response(request,timeout):
                path=request.full_url
                if path.endswith('/actions/runs/'+str(replay.INSTALL_RUN)):
                    value=self.run
            ...
            with self.subTest(count=len(runs)),mock.patch.dict(os.environ,environment),mock.patch.object(replay.urllib.request,'urlopen',side_effect=response),self.assertRaises(AssertionError):
                replay.run()

    def test_gateway_capability_reports_live_source_and_stops_on_missing_authority(self):
        path=Path(__file__).resolve().parents[1]/'workflows/deploy-arthello-recovery-r6-20260908.yml'
        lines=path.read_text().splitlines()
        step=lines.index('    - name: Check existing gateway backup installation capability')
        ...
        with tempfile.TemporaryDirectory() as temporary:
            fixture='''
set -Eeuo pipefail
test_work="$1"; fixture_uid="$2"; fixture_sudo="$3"
id() { printf '%s\\n' "$fixture_uid"; }
docker() { if [ "$1" = ps ]; then printf '%064d\\n' 1; elif [ "$1" = inspect ]; then printf '%040d\\n' 2; else echo bad-docker >> "$test_work/calls"; return 88; fi; }
sudo() { test "$*" = '-n true'; echo sudo-check >> "$test_work/calls"; return "$fixture_sudo"; }
'''
            for uid,sudo_rc,success in [('1000','1',False),('1000','0',True),('0','1',True)]:
                callfile=Path(temporary)/'calls'
                callfile.unlink(missing_ok=True)
                result=subprocess.run(['bash','-c',fixture+script,'fixture',temporary,uid,sudo_rc],capture_output=True,text=True)
                self.assertEqual(result.returncode==0,success,result.stderr)
                self.assertIn('ARTHELLO_OBSERVED_LIVE_RELEASE_SHA='+'0'*39+'2',result.stdout)
                data=json.loads(result.stdout.splitlines()[1])
                self.assertEqual(data['gatewayUid'],int(uid))
                self.assertEqual(data['backupInstallCapability'],success)
                self.assertEqual(callfile.read_text() if callfile.exists() else '', '' if uid=='0' else 'sudo-check\n')
