"""R16 history gate with unchanged R14 durable protocol: accepted R15 success plus preserved candidate replay rules."""
import ast
import hashlib
import json
import os
...
ACCEPTED_SOURCE = '4a0713b4a7d87f132e49836fe0ce9ca9258bc1ec'
ACCEPTED_TREE = 'ac0ec2a2845eee6254ba7cf8c0e0b83d0e5848de'
ACCEPTED_IMAGE = 'sha256:0f15a32c9dff9cd7278a1449bd66f282abc514f0392edfd57262ed25ef247a6f'
ACCEPTED_RUN = 34445017241
ACCEPTED_PATH = '.github/workflows/deploy-arthello-tochka-r15-20260910.yml'
CONSUMER_PATH = '.github/workflows/deploy-arthello-tochka-r16-20260910.yml'
...
FORBIDDEN_RUNS = {34340461537, 34443217193, 34391105865, ACCEPTED_RUN, 34326582961, 34208952716, 34221458013, 34227595623,
                  34283447507, 34318973875, 34322039891, 34324235442}
ACCEPTED_JOBS = {"bundle":102767831023,"deploy":102768250585}
LABELS = {'bundle': ['ubuntu-latest'], 'deploy': ['self-hosted', 'linux', 'x64', 'arthello-gateway']}
# Complete observed /attempts/1/jobs graph, independently read from accepted R15.
ACCEPTED_STEPS = {"bundle":[["Set up job",1,"success"],["Run actions/checkout@11d5960a326750d5838078e36cf38b85af677262",2,"success"],["Run actions/setup-node@49933ea5288caeca8642d1e84afbd3f7d6820020",3,"success"],["Test browser policy and install locked package off the VPS",4,"success"],["Build immutable bundle and exercise Chromium sandbox and natural flow fixture",5,"success"],["Run actions/upload-artifact@ea165f8d65b6e75b540449e92b4886f43607fa02",6,"success"],["Post Run actions/setup-node@49933ea5288caeca8642d1e84afbd3f7d6820020",11,"success"],["Post Run actions/checkout@11d5960a326750d5838078e36cf38b85af677262",12,"success"],["Complete job",13,"success"]],"deploy":[["Set up job",1,"success"],["Verify R14 identity and accepted R13 success history",2,"success"],["Wait for exact main Quality Proof and v52 verification",3,"success"],["Check out exact verified release",4,"success"],["Verify R14 checkout and release contracts",5,"success"],["Verify retained R14 candidate before any replay skip",6,"skipped"],["Verify accepted R13 baseline and preserved School relay",7,"success"],["Check existing gateway Docker authority",8,"success"],["Check current release and existing browser runtime",9,"success"],["Retire only the exact unused R14 browser image",10,"success"],["Measure import capacity before downloading the verified archive",11,"success"],["Run actions/download-artifact@d3f86a106a0bac45b974a628896c90dbdf5c8093",12,"success"],["Verify archive, import off-host bundle and verify portable identity",13,"success"],["Read-only School diagnostic before candidate acceptance",14,"success"],["Download and load exact hosted-verified image",15,"success"],["Verify and load exact hosted-verified image",16,"success"],["Verify actual gateway Caddy with isolated candidate fixture",17,"success"],["Resolve authoritative School sync secret",18,"success"],["Clone fresh snapshot and guarded R14 production cutover",19,"success"],["School diagnostic and real browser acceptance after R14 cutover",20,"success"],["Remove only this downloaded temporary archive",21,"success"],["Remove temporary School SSH material",22,"success"],["Production summary",23,"success"],["Post Check out exact verified release",46,"success"],["Complete job",47,"success"]]}

IDENTITY_STEP = 'Verify R16 identity and accepted R15 success history'
RESUME_STEP = 'Verify retained R16 candidate before any replay skip'
LIVE_STEP = 'Verify accepted R15 baseline and preserved School relay'
CUTOVER_STEP = 'Clone fresh snapshot and guarded R16 production cutover'
POST_STEP = 'School diagnostic and real browser acceptance after R16 cutover'
PRESTEPS = [
    'Set up job', IDENTITY_STEP, 'Wait for exact main Quality Proof and v52 verification',
    'Check out exact verified release', 'Verify R16 checkout and release contracts', RESUME_STEP, LIVE_STEP,
    'Check existing gateway Docker authority', 'Check current release and existing browser runtime',
    'Retire only the exact unused accepted R15 browser image',
...
def accepted_success(run, jobs, commit):
    ...
    require(type(run.get('id')) is int and run['id'] == ACCEPTED_RUN
            ...
            and run.get('name') == 'Deploy ArtHello Tochka R15 D101'
            and run.get('status') == 'completed' and run.get('conclusion') == 'success', 'R15_SUCCESS_REQUIRED')
    require(isinstance(commit, dict) and commit.get('sha') == ACCEPTED_SOURCE
            and commit.get('tree', {}).get('sha') == ACCEPTED_TREE, 'R15_TREE_INVALID')
    for job in complete_jobs(jobs):
        require(job['id'] == ACCEPTED_JOBS[job['name']] and job.get('run_id') == ACCEPTED_RUN
                ...
                and job.get('status') == 'completed' and job.get('conclusion') == 'success', 'R15_JOB_INVALID')
        steps = job.get('steps')
