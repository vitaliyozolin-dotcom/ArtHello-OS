#!/usr/bin/env python3
"""Read-only accepted R12 identity checks for a fresh R13 continuation.
...
separately verified durable context instead of pretending R12 is still running.
"""
import json
import os
import stat
...
ACCEPTED_NAME = "/arthello-direct-34326582961-1"
...
def validate_live(identities, app, image):
    ...
    return {"kind": "r13-accepted-live-baseline", "result": "verified",
...
def validate_school(content):
    ...
    return {"kind": "r13-accepted-school-identity", "result": "verified",
...
def main(arguments):
    try:
        ...
        print('{"kind":"r13-live-identity","result":"blocked","reason":"identity_mismatch"}')
        return 1
