#!/usr/bin/env python3
"""Read-only fixed-R12 backup facts for the R13 shell controller."""
import argparse
...
def checked_module():
    path = Path(__file__).with_name('r13-backup-adoption.py')
    source = path.read_bytes()
    if hashlib.sha256(source).hexdigest() != 'fc48aa59e11257bf6ab354afff374aeab006a973711d3b7c12a7b40abe46aaee':
        raise RuntimeError('R13_ADOPTION_DEPENDENCY_DRIFT')
    module = ModuleType('r13_controller_adoption')
...
def opened(args, docker):
    ...
    try:
        ...
        own_name = 'backup-adoption-r13-' + args.run_id + '-' + args.attempt
        try:
...
def canonical_consumers(args, docker, phase):
    ...
    old = docker.inspect('container', adoption.ACCEPTED_APP_ID)
    app_metadata(old, identity=adoption.ACCEPTED_APP_ID, name='arthello-direct-' + adoption.ACCEPTED_RUN + '-1',
                 image=adoption.ACCEPTED_IMAGE, release=adoption.ACCEPTED_SHA,
                 running=phase in ('live', 'live-paused'), paused=phase == 'live-paused')
    expected = {adoption.ACCEPTED_APP_ID, adoption.ACCEPTED_WORKER_ID}
    ...
    if actual == expected:
        ...
        return actual
    require(expected < actual and len(actual - expected) == 1, 'CANONICAL_CONSUMERS_INVALID')
    previous = historical_predecessor(docker)
    require(previous is not None and previous not in expected, 'PREDECESSOR_NOT_PROVEN')
...
def main(arguments=None):
    ...
    try:
        ...
        print(json.dumps({'state': 'preserve' if args.command == 'boundary' else 'refused',
                          'code': 'R13_CONTROLLER_CHECK_FAILED'}, separators=(',', ':')))
        return 1
