...
work="$(mktemp -d "$RUNNER_TEMP/d080-caddy.XXXXXXXX")"
cleanup() {
  case "$work" in "$RUNNER_TEMP"/d080-caddy.*) rm -rf -- "$work" ;; *) return 1 ;; esac
}
trap cleanup EXIT
...
PATH="$work:$PATH" python3 -I .github/scripts/test-d080-maintenance-caddy.py
