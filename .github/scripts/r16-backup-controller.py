...
def checked_module():
    path = Path(__file__).with_name('r16-backup-adoption.py')
    source = path.read_bytes()
    if hashlib.sha256(source).hexdigest() != '3dc30115d9848270b9d6098c8682398fd5955ccfac044e3e9e24a2c6ee58bc21':
...
def checked_history():
    path = Path(__file__).with_name('r14-backup-controller.py')
    raw = path.read_bytes()
    if hashlib.sha256(raw).hexdigest() != '541cd6ad6340d3b97e180bd735824a826673f35c8cf08817a795924cdf70c64b':
...
LIVE_CONTEXT_SHA256 = '9c4ef4d58a8a7d97835bc1f8f00cc4aad44849a8cd62b7bcd4869cce247d96ca'
...
def accepted_live_predecessor(docker):
    """Prove the retained R13 app through accepted R15's exact public context."""
    root = Path.home() / '.config/arthello/release-state'
    directory = adoption.private_directory(root)
    try:
...
def canonical_consumers(args, docker, phase):
    ...
    if actual == expected:
        ...
        return actual
    require(expected < actual and len(actual - expected) <= 3, 'CANONICAL_CONSUMERS_INVALID')
    ...
    if actual != expected:
        ancestor = historical.historical_predecessor(docker)
