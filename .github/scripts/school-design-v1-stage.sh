#!/usr/bin/env bash
set -Eeuo pipefail
exec 9>/var/lock/school-1-11-design-v1-staging.lock
flock -n 9

production=school-1-11
staging=school-1-11-staging
network=school-1-11-staging-local
acceptance="school-1-11-design-acceptance-$RUN_ID"
rollback="school-1-11-staging-design-rollback-$RUN_ID"
new_data="school-1-11_staging_design_${RUN_ID}_data"
success=0
...
design_flag="$(docker image inspect "$CANDIDATE_IMAGE" --format '{{range .Config.Env}}{{println .}}{{end}}' | sed -n 's/^NEXT_PUBLIC_SCHOOL_DESIGN_V1=//p' | head -n1)"
test "$design_flag" = true
printf 'SCHOOL_DESIGN_IMAGE_FLAG=ENABLED\n'
...
docker volume create \
  ...
  --label school.purpose=design-v1-candidate-data \
...
const designResponse = await fetch('http://127.0.0.1:3000/login');
const designHtml = await designResponse.text();
if (!designResponse.ok || !designHtml.includes('data-design-code="v1"'))
  throw new Error('Design v1 flag is absent from acceptance HTML');
console.log('SCHOOL_DESIGN_ACCEPTANCE_HTML=V1');
...
console.log('SCHOOL_DESIGN_PROXY_ORIGIN=ACCEPTED');
console.log('SCHOOL_DESIGN_FOREIGN_ORIGIN=REJECTED');
VERIFY_PROXY_AUTH
...
if (
  ...
  html.length < 1000 ||
  !html.includes('<html') ||
  !html.includes('data-design-code="v1"')
) throw new Error('Staging design login failed');
...
console.log('SCHOOL_DESIGN_STAGING_DB=OK tables=' + tableCount);
console.log('SCHOOL_DESIGN_STAGING_LOGIN=OK');
console.log('SCHOOL_DESIGN_STAGING_HTML=V1');
console.log('SCHOOL_DESIGN_STAGING_SAME_ORIGIN=OK');
VERIFY_STAGING
...
success=1
printf 'SCHOOL_DESIGN_STAGING=OK container=%s\n' "$staging"
printf 'SCHOOL_DESIGN_CANDIDATE_SHA=%s\n' "$CANDIDATE_SHA"
printf 'SCHOOL_DESIGN_STAGING_BIND=%s\n' "$new_binding"
printf 'SCHOOL_DESIGN_STAGING_PUBLIC=no\n'
printf 'SCHOOL_DESIGN_ROLLBACK_READY=%s\n' "$rollback"
printf 'SCHOOL_DESIGN_PRODUCTION_RESTARTED=no\n'
printf 'SCHOOL_DESIGN_PRODUCTION_HEALTH=OK\n'
