...
spec = importlib.util.spec_from_file_location('replay', Path(__file__).with_name('d083-replay-guard.py'))
...
class R10HistoryTests(unittest.TestCase):
    def setUp(self):
        self.release = 'a' * 40
        self.current = 3000
        self.requested = []
    ...
    def make_run(self, identity, attempt, status, conclusion):
        return {**copy.deepcopy(R8_OBSERVED['run']), 'id': identity, 'run_attempt': attempt,
                'head_sha': self.release, 'path': '.github/workflows/deploy-arthello-recovery-r10-20260909.yml',
