...
def validate(receipt, manifest, expected, now):
    ...
    assert set(receipt)=={'schemaVersion','state','mode','repairConfigSha256','runtimeConfigSha256','schoolSourceSha','schoolImageId','schoolContainerId','relayContainerId','activatedAtUtc','verifiedAtUtc'}
    ...
    for name in ('runtimeConfigSha256','schoolContainerId','relayContainerId'):
...
if __name__=='__main__':
    ...
    try:
        ...
        raise SystemExit('SCHOOL_R2_REPAIR_RECEIPT=BLOCKED: configuration evidence is absent, stale or mismatched')
    print('SCHOOL_R2_REPAIR_CONFIG=VERIFIED; natural browser SSO remains separate')
