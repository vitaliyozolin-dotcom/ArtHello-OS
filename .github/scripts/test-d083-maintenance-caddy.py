#!/usr/bin/env python3
"""Exercise the D083 renderer through an actual Caddy process and HTTP requests.
...
STROIOS = 'stroios.example.invalid'
...
class BackendHandler(BaseHTTPRequestHandler):
    ...
    def handle_fixture(self):
        ...
        self.send_header('X-D083-Fixture', self.server.name)
...
{SCHOOL} {{
    header X-D083-School-Preserved yes
...
class CaddyFixture:
    def __init__(self, binary, root, rendered, backends, expected_candidate_proxies):
        ...
        external_file = root / 'external-routes.caddy'
        private_write(external_file, rendered)
        template_bytes = (HERE / 'test-fixtures/d083-stroios-Caddyfile').read_bytes()
        require(hashlib.sha1(b'blob ' + str(len(template_bytes)).encode() + b'\0' + template_bytes).hexdigest()
                == '37ff734e50f904982407baffe43cbc787dc3ce9f')
        template = template_bytes.decode('utf-8')
        require(template.count('import /data/external-routes.caddy') == 1)
        # Only the fixture import path changes before adaptation. The shared
        # template's actual APP_DOMAIN, directives, nesting and tokens stay intact.
        private_write(config_file, template.replace('import /data/external-routes.caddy', 'import ' + str(external_file)))
        self.environment = {
        ...
        counts = {CANDIDATE: 0, SCHOOL_UPSTREAM: 0, 'stroios': 0}

        def localize_upstreams(node):
            if isinstance(node, dict):
                if node.get('handler') == 'reverse_proxy':
                    if 'dynamic_upstreams' in node:
                        dynamic = node.pop('dynamic_upstreams')
                        require(dynamic.get('source') == 'a' and dynamic.get('name') == 'app'
                                and str(dynamic.get('port')) == '3000' and 'upstreams' not in node)
                        counts['stroios'] += 1
                        node['upstreams'] = [{'dial': '127.0.0.1:' + str(backends['stroios'].server_port)}]
                        return
        ...
        require(counts == {CANDIDATE: expected_candidate_proxies, SCHOOL_UPSTREAM: 1, 'stroios': 1})
    ...
    def check(self, method, path, headers=(), expected=503, host=HOST,
        ...
        try:
            ...
            require(response.getheader('X-D083-Fixture') == backend)
            if expected == 503:
                require(response.getheader('Cache-Control') == 'no-store')
            if host == STROIOS:
                require(response.getheader('X-Frame-Options') == 'DENY')
                require(response.getheader('Referrer-Policy') == 'same-origin')
                require(response.getheader('Strict-Transport-Security') == 'max-age=31536000; includeSubDomains')
            else:
                require(response.getheader('X-D083-Preserved' if host == HOST else 'X-D083-School-Preserved') == 'yes')
        finally:
            connection.close()
...
def check_stroios_route(fixture):
    for method, path in [('GET', '/'), ('HEAD', '/'), ('POST', '/api/auth/login'), ('POST', '/api/finance-actions')]:
        for headers in ((), (WRONG_AUTH,)):
            fixture.check(method, path, headers, 200, STROIOS, 'stroios', headers)
...
def main():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location('d083_renderer', HERE / 'd083-maintenance-route.py')
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        with tempfile.TemporaryDirectory(prefix='d083-caddy-fixture-') as directory:
            root = pathlib.Path(directory)
            stage = 'fixture_setup_failed'
            backends['candidate'] = Backend('candidate')
            backends['school'] = Backend('school')
            backends['stroios'] = Backend('stroios')
            access_log = root / 'access.json'
            module.validate_main_config((HERE / 'test-fixtures/d083-stroios-Caddyfile').read_text(), app_domain=STROIOS)
