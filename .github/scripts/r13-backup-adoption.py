#!/usr/bin/env python3
"""Fixed accepted-R12 backup adoption; no backup/history deletion or DB writes."""
import hashlib
import argparse
import datetime
import fcntl
...
class Adoption:
    def __init__(self, args, docker, state, accepted, boundary):
        ...
        require(args.release_sha != ACCEPTED_SHA and args.image_id != ACCEPTED_IMAGE
                and args.tree_sha != ACCEPTED_TREE and args.run_id != ACCEPTED_RUN, 'FRESH_RELEASE_REQUIRED')
    ...
    def no_duplicates(self):
        ...
        for identity in inventory:
            ...
            if mounted & protected and identity != ACCEPTED_WORKER_ID:
                ...
                require(accepted_app or new_app, 'FOREIGN_BACKUP_CONSUMER')
    ...
    def load(self):
        ...
        require(isinstance(self.state, dict) and self.state.get('schemaVersion') == 13
    ...
    def receipt(self, backup=None):
        return {'schemaVersion': 13, 'state': self.state['phase'], 'releaseSha': self.args.release_sha,
    ...
    def prepare(self):
        ...
        self.state = {'schemaVersion': 13, 'identity': self.owned.identity, 'phase': 'preparing', 'sealed': False,
    ...
    def verify_copyback(self):
        ...
        self.pre_auth()
        return {'schemaVersion': 13, 'state': 'quiesced', 'canonicalReaders': 'none', 'databaseMutation': False}
    ...
    def cleanup(self):
        ...
        self.accepted()
        self.health()
        return {'schemaVersion': 13, 'state': 'cleaned', 'ownedRemoved': result['removed'], 'adoptedRemoved': 0}
