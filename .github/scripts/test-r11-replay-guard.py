...
spec = importlib.util.spec_from_file_location('replay', Path(__file__).with_name('r11-replay-guard.py'))
...
class R11HistoryTests(unittest.TestCase):
    def setUp(self):
        self.release = 'a' * 40
        self.current = 3000
        self.requested = []
    ...
    def make_run(self, identity, attempt, status, conclusion):
        return {**copy.deepcopy(R8_OBSERVED['run']), 'id': identity, 'run_attempt': attempt,
                'head_sha': self.release, 'path': '.github/workflows/deploy-arthello-recovery-r11-20260909.yml',
...
class R10FixtureAbortTests(unittest.TestCase):
    ...
    def test_r11_preserves_existing_r9_retirement_without_new_retirement_scope(self):
        ...
        self.assertEqual([name for name in replay.CANDIDATE_IMPORTS if name.startswith('Retire ')],
                         ['Retire only the exact unused R9 browser image under D084'])
