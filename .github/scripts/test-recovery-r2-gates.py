...
acceptance = load('acceptance', 'check-school-live-acceptance-r2.py')
replay = load('replay', 'd064-replay-guard.py')
activation = load('activation', 'd063-activation-state.py')
transport = load('transport', 'r2-school-repair-remote.py')
receipt_gate = load('receipt_gate', 'check-school-repair-receipt-r2.py')


def step_script(step_id):
    path = Path(__file__).resolve().parents[1] / 'workflows/deploy-arthello-recovery-r2-20260908.yml'
...
class GateTests(unittest.TestCase):
    def setUp(self):
        ...
        self.record = {'schemaVersion':2,'observedSchoolRepairConfigSha256':'f'*64,'intendedCandidateArtHelloSha':'a'*40,'observedLiveSchoolSha':'b'*40,'observedLiveArtHelloSha':'c'*40,'arthelloOrigin':'https://arthello-188-225-38-55.sslip.io','schoolOrigin':'https://school-188-225-38-55.sslip.io','method':'natural-browser-navigation','sessionInjected':False,'callbackUrlConstructed':False,'verifiedSteps':['open_education_in_authenticated_arthello','click_diary_entry','follow_natural_sso_redirects','authenticated_school_diary_visible'],'result':'pass','evidenceReference':'synthetic-test-fixture-only','observedAtUtc':'2026-09-08T07:50:00Z'}

    def check(self, record):
        return acceptance.validate(record, 'a'*40, 'b'*40, 'c'*40, 'f'*64, '2026-09-08T07:45:00Z', self.now)
    ...
    def test_school_receipt_binds_manifest_identity_and_recent_real_probe(self):
        ...
        receipt={'schemaVersion':1,'state':'verified','mode':'created','repairConfigSha256':expected,'runtimeConfigSha256':'a'*64,'schoolSourceSha':receipt_gate.SCHOOL_SHA,'schoolImageId':receipt_gate.SCHOOL_IMAGE,'schoolContainerId':'b'*64,'relayContainerId':'c'*64,'activatedAtUtc':'2026-09-08T07:58:00Z','verifiedAtUtc':'2026-09-08T07:59:00Z'}
        ...
        for patch in [{'state':'refused'},{'repairConfigSha256':'d'*64},{'schoolImageId':'sha256:'+'d'*64},{'schoolContainerId':'c'*64},{'verifiedAtUtc':'2026-09-08T07:30:00Z'},{'activatedAtUtc':'2026-09-08T08:00:00Z'},{'extra':'unexpected'}]:
...
"""
    ...
    def test_each_identity_must_match(self):
        for key in ['intendedCandidateArtHelloSha','observedLiveSchoolSha','observedLiveArtHelloSha','observedSchoolRepairConfigSha256']:
