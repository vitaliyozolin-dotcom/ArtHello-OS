"""R13 binds sealed R12 backup adoption to the preserved durable auth boundary."""
import argparse
...
def checked_module(name, filename, expected):
    ...
    if hashlib.sha256(source).hexdigest() != expected:
        raise RuntimeError('R13_ADAPTER_DEPENDENCY_DRIFT')
    value = ModuleType(name)
    value.__file__ = str(path)
    exec(compile(source, str(path), 'exec'), value.__dict__)
    return value

adoption = checked_module('r13_adoption', 'r13-backup-adoption.py', 'fc48aa59e11257bf6ab354afff374aeab006a973711d3b7c12a7b40abe46aaee')
...
def validate_context(context):
    ...
    if (context['previousName'] != 'arthello-direct-' + adoption.ACCEPTED_RUN + '-1'
            or context['previousContainerId'] != adoption.ACCEPTED_APP_ID
            or context['dataVolume'] != adoption.r7.SOURCE_VOLUME):
        raise ValueError('Previous application must be the accepted R12 runtime')
    if (context['releaseSha'] == adoption.ACCEPTED_SHA or context['sourceTree'] == adoption.ACCEPTED_TREE
            or context['imageId'] == adoption.ACCEPTED_IMAGE or context['runId'] == adoption.ACCEPTED_RUN
            or not 1 <= int(context['runAttempt']) <= 50):
        raise ValueError('Fresh R13 application identity is required')
...
def adoption_paths(context):
    ...
    key = context['runId'] + '-' + context['runAttempt']
    return (root / ('backup-adoption-r13-' + key) / 'backup-runtime-state.json',
...
def validate_adoption(context):
    ...
    try:
        ...
        check(isinstance(saved, dict) and saved.get('schemaVersion') == 13
...
def validate_receipt(record, receipt, attempt, clock):
    ...
    schema = checked_module('r13_schema3', 'check-school-live-acceptance-r7.py', '6fae34638acd1757cad922c288ede1282f8300a8ecfcbe3547661378445cb884')
