...
m = load('r14_controller_test', 'r16-backup-controller.py')
fixture = load('r14_adoption_fixtures', 'test-r16-backup-adoption.py')
REFUSED = (m.r7.Refused, m.historical.r7.Refused, fixture.r7.Refused, OSError, ValueError, KeyError, TypeError)
...
class ControllerTests(unittest.TestCase):
    ...
    def live_context(self):
        ...
        r13 = dict(context, releaseSha=m.adoption.HISTORICAL_SHA, sourceTree=m.adoption.HISTORICAL_TREE,
                   ...
                   imageId=m.adoption.HISTORICAL_IMAGE, previousContainerId=m.adoption.ACCEPTED_APP_ID,
        ...
        self.r13_historical_path = self.root / ('candidate-acceptance-' + m.adoption.HISTORICAL_SHA + '.json')
        private(self.r13_historical_path, self.r13_historical)
        r13_patch = patch.object(m.historical, 'LIVE_CONTEXT_SHA256', self.r13_historical['contextSha256'])
        r13_patch.start()
    ...
    def test_three_exact_r15_r13_r12_context_links_preserve_every_stopped_ancestor(self):
        self.prepare()
        first = self.predecessor()
        r12 = self.retained_r12()
        r13 = self.retained_r13()
        before = copy.deepcopy([first,r12,r13])
        files = [self.live_historical_path,self.r13_historical_path,self.historical_path]
        receipts = [file.read_bytes() for file in files]
        for phase in m.PHASES[:-1]:
            self.set_phase(phase)
            self.assertEqual(m.consumers(self.args,self.docker)['canonicalConsumers'],
                             6 if phase.startswith('candidate') else 5)
        self.set_phase('copyback')
        self.f.subject.quiesce()
        private(self.own,self.f.state.load())
        self.assertEqual(m.consumers(self.args,self.docker)['canonicalConsumers'],5)
        self.assertEqual(copy.deepcopy([first,r12,r13]),before)
        self.assertEqual([file.read_bytes() for file in files],receipts)

    def test_r13_history_reader_and_each_link_cannot_drift_or_add_a_fourth_ancestor(self):
        self.prepare()
        self.predecessor()
        self.retained_r12()
        r13 = self.retained_r13()
        for file in [self.live_historical_path,self.r13_historical_path,self.historical_path]:
