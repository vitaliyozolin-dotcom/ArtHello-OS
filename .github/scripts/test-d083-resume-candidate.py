...
spec = importlib.util.spec_from_file_location('resume', Path(__file__).with_name('d083-resume-candidate.py'))
...
class HeldCandidate:
    ...
    def __init__(self, root):
        ...
        self.gateway_evidence = dict(version=1, gatewayId='5' * 64, gatewayImageId='sha256:' + '6' * 64,
                                     appDomain='stroios.example.invalid', mainConfigSha256=sha(b'gateway-main\n'),
                                     externalRoutesSha256=self.context['originalRouteSha256'])
        self.context['gatewayEvidenceFile'] = private(self.work / 'gateway-evidence.json', json_bytes(self.gateway_evidence))
        self.context['gatewayEvidenceSha256'] = sha(json_bytes(self.gateway_evidence))
        private(self.work / 'Caddyfile.before', b'gateway-main\n')
        ...
        self.caddy = dict(Id='5' * 64, Image=self.gateway_evidence['gatewayImageId'], Name='/stroios-caddy-1', State=dict(Running=True, Paused=False),
    ...
    def process(self, args, *, input=None, maximum=1048576):
        ...
        if args == ['docker', 'container', 'exec', 'stroios-caddy-1', 'caddy', 'reload',
            ...
            self.probe_ok = True
            return ''
        if args == ['python3', '-I', str(Path(resume.__file__).with_name('d083-maintenance-route.py')),
                    'verify-gateway', '--input', self.context['originalRouteFile'],
                    '--main-config', str(self.work / 'Caddyfile.before'),
                    '--gateway-env-evidence', self.context['gatewayEvidenceFile']]:
            resume.require(self.gateway_ok)
            return ''
        if args == ['python3', '-I', str(Path(resume.__file__).with_name('d083-maintenance-route.py')),
    ...
    def repair(self, *, cli=False, mode="--repair-maintenance"):
        ...
        argv = ['d083-resume-candidate.py', '--state', str(self.state_path), mode]
...
class ResumeCandidateTests(unittest.TestCase):
    ...
    def test_swapped_gateway_id_image_env_or_live_main_cannot_resume_or_repair(self):
        original = copy.deepcopy(self.fixture.caddy)
        for field, value in [('Id', '0' * 64), ('Image', 'sha256:' + '0' * 64)]:
            self.fixture.caddy[field] = value
            with self.subTest(field=field), self.assertRaises(ValueError): self.fixture.verify()
            self.fixture.caddy = copy.deepcopy(original)
        self.fixture.gateway_ok = False
        with self.assertRaises(ValueError): self.fixture.verify()
        with self.assertRaises(ValueError): self.fixture.repair()
        self.assertFalse(any(call[0] == 'process' and 'reload' in call[1] for call in self.fixture.calls))

    def test_changed_gateway_sidecar_is_not_accepted_after_job_temp_cleanup(self):
        shutil.rmtree(self.fixture.job_temp)
        changed = dict(self.fixture.gateway_evidence, appDomain='other.example.invalid')
        private(Path(self.fixture.context['gatewayEvidenceFile']), json_bytes(changed))
        with self.assertRaises(ValueError): self.fixture.verify()
