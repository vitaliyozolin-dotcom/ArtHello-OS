...
[[ "$BROWSER_PHASE" == candidate || "$BROWSER_PHASE" == after ]]
...
test "$OBSERVED_LIVE_ARTHELLO_SHA" = "$RELEASE_SHA"
mapfile -t observed_before_ids < <(docker ps --filter 'name=^/arthello-direct-' --format '{{.ID}}')
test "${#observed_before_ids[@]}" -eq 1
observed_container_id="$(docker inspect "${observed_before_ids[0]}" --format '{{.Id}}')"
[[ "$observed_container_id" =~ ^[a-f0-9]{64}$ ]]
test "$(docker inspect "$observed_container_id" --format '{{index .Config.Labels "arthello.release.sha"}}')" = "$RELEASE_SHA"
browser_args=()
if [[ "$BROWSER_PHASE" == candidate ]]; then
  [[ "$BROWSER_CANDIDATE_ID" =~ ^[a-f0-9]{64}$ ]]
  test "$observed_container_id" = "$BROWSER_CANDIDATE_ID"
  browser_args=(--candidate-gate)
fi

report="$RUNNER_TEMP/r10-browser-$GITHUB_RUN_ID-$GITHUB_RUN_ATTEMPT-$BROWSER_PHASE.json"
...
CHECKED_SOURCE_SHA="$BROWSER_SOURCE_SHA" BROWSER_IMAGE_ID="$browser_image_id" PRECHECK_NODE="$precheck_node" \
  bash scripts/run-server-browser.sh "${browser_args[@]}" | tee "$report"
mapfile -t observed_live_ids < <(docker ps --filter 'name=^/arthello-direct-' --format '{{.ID}}')
test "${#observed_live_ids[@]}" -eq 1
test "$(docker inspect "${observed_live_ids[0]}" --format '{{.Id}}')" = "$observed_container_id"
...
BROWSER_REPORT_FILE="$report" OBSERVED_CANDIDATE_CONTAINER_ID="$observed_container_id" \
