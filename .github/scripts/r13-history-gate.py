"""Pure R13 history gate: accepted R12 success plus preserved candidate replay rules."""
import ast
import hashlib
import json
import os
...
ACCEPTED_SOURCE = '77f26ec9bcba7233f39d5e8cb9f59c276bc8c1ed'
...
ACCEPTED_PATH = '.github/workflows/deploy-arthello-recovery-r12-20260909.yml'
CONSUMER_PATH = '.github/workflows/deploy-arthello-recovery-r13-20260909.yml'
...
FORBIDDEN_SOURCES = {
    ACCEPTED_SOURCE, 'ee8f3080d941a12be357eec0fd1d902acd01a2f9',
...
FORBIDDEN_RUNS = {ACCEPTED_RUN, 34208952716, 34221458013, 34227595623,
                  34283447507, 34318973875, 34322039891, 34324235442}
ACCEPTED_JOBS = {'bundle': 102385622798, 'deploy': 102386111240}
LABELS = {'bundle': ['ubuntu-latest'], 'deploy': ['self-hosted', 'linux', 'x64', 'arthello-gateway']}
# Complete observed /attempts/1/jobs graph, independently read from accepted R12.
ACCEPTED_STEPS = {
  "bundle": [
    [
      "Set up job",
...
IDENTITY_STEP = 'Verify R13 identity and accepted R12 success history'
RESUME_STEP = 'Verify retained R13 candidate before any replay skip'
LIVE_STEP = 'Verify accepted R12 baseline and preserved School relay'
CUTOVER_STEP = 'Clone fresh snapshot and guarded R13 production cutover'
POST_STEP = 'School diagnostic and real browser acceptance after R13 cutover'
PRESTEPS = [
    'Set up job', IDENTITY_STEP, 'Wait for exact main Quality Proof and v52 verification',
    'Check out exact verified release', 'Verify R13 checkout and release contracts', RESUME_STEP, LIVE_STEP,
    'Check existing gateway Docker authority', 'Check current release and existing browser runtime',
    'Retire only the exact unused accepted R12 browser image',
    'Measure import capacity before downloading the verified archive',
    'Run actions/download-artifact@d3f86a106a0bac45b974a628896c90dbdf5c8093',
    'Verify archive, import off-host bundle and verify portable identity',
    'Read-only School diagnostic before candidate acceptance',
...
def accepted_success(run, jobs, commit):
    ...
    require(type(run.get('id')) is int and run['id'] == ACCEPTED_RUN
            ...
            and run.get('name') == 'Deploy ArtHello recovery R12 D086'
            and run.get('status') == 'completed' and run.get('conclusion') == 'success', 'R12_SUCCESS_REQUIRED')
    require(isinstance(commit, dict) and commit.get('sha') == ACCEPTED_SOURCE
            and commit.get('tree', {}).get('sha') == ACCEPTED_TREE, 'R12_TREE_INVALID')
    for job in complete_jobs(jobs):
        require(job['id'] == ACCEPTED_JOBS[job['name']] and job.get('run_id') == ACCEPTED_RUN
                ...
                and job.get('status') == 'completed' and job.get('conclusion') == 'success', 'R12_JOB_INVALID')
        steps = job.get('steps')
        require(isinstance(steps, list) and all(isinstance(step, dict) and step.get('status') == 'completed'
                and type(step.get('number')) is int for step in steps), 'R12_STEP_INVALID')
        observed = [[step.get('name'), step.get('number'), step.get('conclusion')] for step in steps]
        require(observed == ACCEPTED_STEPS[job['name']], 'R12_COMPLETE_STEP_GRAPH_INVALID')
...
def validate_gate(accepted_run, accepted_jobs, accepted_commit, runs, get_attempt_jobs, get_candidate_logs,
    ...
    try:
        ...
        require(isinstance(release, str) and re.fullmatch(r'[a-f0-9]{40}', release)
                ...
                and 1 <= current_attempt <= 50, 'FRESH_R13_IDENTITY_REQUIRED')
            ...
            for job in complete_jobs(value):
                ...
                if job['name'] == 'deploy' and steps:
                    require(isinstance(steps, list) and all(isinstance(step, dict) for step in steps)
                            ...
                            'R13_COMPLETE_STEP_GRAPH_INVALID')
        ...
        return {'schemaVersion': 13, 'history': 'verified', 'mode': 'resume_candidate' if resume else 'fresh',
...
def cli_context(environment):
    ...
    require(re.fullmatch(r'[a-f0-9]{40}', release) and re.fullmatch(r'[1-9][0-9]*', run)
            ...
            and release not in FORBIDDEN_SOURCES and int(run) not in FORBIDDEN_RUNS, 'FRESH_R13_IDENTITY_REQUIRED')
...
def main(frozen_functions=None):
    try:
        ...
        print(json.dumps({'schemaVersion': 13, 'history': 'refused', 'code': str(error)}))
    except Exception:
        print(json.dumps({'schemaVersion': 13, 'history': 'refused', 'code': 'HISTORY_EVIDENCE_UNAVAILABLE'}))
    return 2
