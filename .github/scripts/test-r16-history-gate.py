...
path = Path(__file__).with_name('r16-history-gate.py')
spec = importlib.util.spec_from_file_location('history', path)
gate = importlib.util.module_from_spec(spec)
spec.loader.exec_module(gate)
# Sanitized actual GitHub metadata for accepted R15, not generated from gate expectations.
ACTUAL = json.loads((path.parent / 'fixtures/r16-accepted-r15-history.json').read_text())
SOURCE = 'a' * 40
CURRENT = 90000000001
