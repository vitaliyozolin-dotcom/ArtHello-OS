...
m = load('r14_controller_test', 'r17-backup-controller.py')
fixture = load('r14_adoption_fixtures', 'test-r17-backup-adoption.py')
REFUSED = (m.r7.Refused, m.historical.r7.Refused, m.historical.historical.r7.Refused, fixture.r7.Refused, OSError, ValueError, KeyError, TypeError)
...
class ControllerTests(unittest.TestCase):
    ...
    def live_context(self):
        context = {'releaseSha': m.adoption.LIVE_SHA, 'sourceTree': m.adoption.LIVE_TREE,
                   ...
                   'previousContainerId': m.adoption.HISTORICAL_APP_ID,
                   'previousName': 'arthello-direct-' + m.adoption.HISTORICAL_RUN + '-1'}
        ...
        r13 = dict(context, releaseSha=m.adoption.HISTORICAL_SHA, sourceTree=m.adoption.HISTORICAL_TREE,
                   runId=m.adoption.HISTORICAL_RUN, candidateContainerId=m.adoption.HISTORICAL_APP_ID,
                   candidateName='arthello-direct-' + m.adoption.HISTORICAL_RUN + '-1',
                   imageId=m.adoption.HISTORICAL_IMAGE, previousContainerId=m.adoption.OLDER_APP_ID,
                   previousName='arthello-direct-' + m.adoption.OLDER_RUN + '-1')
        self.r15_historical = dict(schemaVersion=1, phase='public-started', context=r13,
            contextSha256=hashlib.sha256((json.dumps(r13, sort_keys=True, separators=(',', ':')) + '\n').encode()).hexdigest())
        self.r15_historical_path = self.root / ('candidate-acceptance-' + m.adoption.HISTORICAL_SHA + '.json')
        private(self.r15_historical_path, self.r15_historical)
        r13_patch = patch.object(m.historical, 'LIVE_CONTEXT_SHA256', self.r15_historical['contextSha256'])
        r13_patch.start()
        self.addCleanup(r13_patch.stop)
        r13 = dict(context, releaseSha=m.adoption.OLDER_SHA, sourceTree=m.adoption.OLDER_TREE,
                   runId=m.adoption.OLDER_RUN, candidateContainerId=m.adoption.OLDER_APP_ID,
                   candidateName='arthello-direct-' + m.adoption.OLDER_RUN + '-1',
                   imageId=m.adoption.OLDER_IMAGE, previousContainerId=m.adoption.ACCEPTED_APP_ID,
                   previousName='arthello-direct-' + m.adoption.ACCEPTED_RUN + '-1')
        self.r13_historical = dict(schemaVersion=1, phase='public-started', context=r13,
            contextSha256=hashlib.sha256((json.dumps(r13, sort_keys=True, separators=(',', ':')) + '\n').encode()).hexdigest())
        self.r13_historical_path = self.root / ('candidate-acceptance-' + m.adoption.OLDER_SHA + '.json')
        private(self.r13_historical_path, self.r13_historical)
        older_patch = patch.object(m.historical.historical, 'LIVE_CONTEXT_SHA256', self.r13_historical['contextSha256'])
        older_patch.start()
    ...
    def predecessor(self):
        ...
        original_patch = patch.object(m.historical, 'ACCEPTED_CONTEXT_SHA256', self.historical['contextSha256'])
        original_patch.start()
        self.addCleanup(original_patch.stop)
        older_patch = patch.object(m.historical.historical, 'ACCEPTED_CONTEXT_SHA256', self.historical['contextSha256'])
        older_patch.start()
        self.addCleanup(older_patch.stop)
    ...
    def retained_r15(self):
        value = self.app(old=True)
        value.update(Id=m.adoption.HISTORICAL_APP_ID,
                     Name='/arthello-direct-' + m.adoption.HISTORICAL_RUN + '-1', Image=m.adoption.HISTORICAL_IMAGE)
        value['Config']['Labels']['arthello.release.sha'] = m.adoption.HISTORICAL_SHA
        value['State'].update(Running=False, Paused=False)
        value['HostConfig']['RestartPolicy']['Name'] = 'no'
        self.f.docker.objects['container', value['Name'][1:]] = value
        self.f.docker.objects['image', value['Image']] = {
            'Id': value['Image'], 'Config': {'Labels': {'org.opencontainers.image.revision': m.adoption.HISTORICAL_SHA}}}
        return value

    def retained_r13(self):
        value = self.app(old=True)
        value.update(Id=m.adoption.OLDER_APP_ID,
                     Name='/arthello-direct-' + m.adoption.OLDER_RUN + '-1', Image=m.adoption.OLDER_IMAGE)
        value['Config']['Labels']['arthello.release.sha'] = m.adoption.OLDER_SHA
        value['State'].update(Running=False, Paused=False)
        value['HostConfig']['RestartPolicy']['Name'] = 'no'
        self.f.docker.objects['container', value['Name'][1:]] = value
        self.f.docker.objects['image', value['Image']] = {
            'Id': value['Image'], 'Config': {'Labels': {'org.opencontainers.image.revision': m.adoption.OLDER_SHA}}}
        return value

    def test_four_exact_r16_r15_r13_r12_context_links_preserve_every_stopped_ancestor(self):
        self.prepare()
        first = self.predecessor()
        r12 = self.retained_r12()
        r15 = self.retained_r15()
        r13 = self.retained_r13()
        before = copy.deepcopy([first,r12,r15,r13])
        files = [self.live_historical_path,self.r15_historical_path,self.r13_historical_path,self.historical_path]
        receipts = [file.read_bytes() for file in files]
        for phase in m.PHASES[:-1]:
            self.set_phase(phase)
            self.assertEqual(m.consumers(self.args,self.docker)['canonicalConsumers'],
                             7 if phase.startswith('candidate') else 6)
        self.set_phase('copyback')
        self.f.subject.quiesce()
        private(self.own,self.f.state.load())
        self.assertEqual(m.consumers(self.args,self.docker)['canonicalConsumers'],6)
        self.assertEqual(copy.deepcopy([first,r12,r15,r13]),before)
        self.assertEqual([file.read_bytes() for file in files],receipts)

    def test_r13_history_reader_and_each_link_cannot_drift_or_add_a_fifth_ancestor(self):
        self.prepare()
        self.predecessor()
        self.retained_r12()
        r15 = self.retained_r15()
        r13 = self.retained_r13()
        for file in [self.live_historical_path,self.r15_historical_path,self.r13_historical_path,self.historical_path]:
            original = file.read_bytes()
            file.write_text('{}')
            with self.assertRaises(REFUSED): m.consumers(self.args,self.docker)
            file.write_bytes(original)
        for change in [{'Running':True},{'Paused':True},{'Restarting':True}]:
            original = copy.deepcopy(r13['State'])
            r13['State'].update(change)
            with self.assertRaises(REFUSED): m.consumers(self.args,self.docker)
            r13['State'] = original
        r13['Mounts'][1]['RW'] = True
        with self.assertRaises(REFUSED): m.consumers(self.args,self.docker)
        r13['Mounts'][1]['RW'] = False
        unknown = copy.deepcopy(r13)
        unknown.update(Id='5'*64,Name='/arthello-direct-unbound-fourth')
        unknown['Mounts'] = unknown['Mounts'][:1]
        unknown['HostConfig']['Mounts'] = []
