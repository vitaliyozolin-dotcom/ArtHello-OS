...
source = File.read('.github/workflows/deploy-arthello-recovery-r2-20260908.yml')
workflow = YAML.safe_load(source, aliases: true)
trigger = workflow['on'] || workflow[true]
raise 'D064 trigger must be workflow_run only' unless trigger.keys == ['workflow_run']
raise 'D064 must consume only successful canonical Verify completion' unless trigger.fetch('workflow_run') == {'workflows'=>['Verify ArtHello v52 release'], 'types'=>['completed']}
env = workflow.fetch('env')
raise 'D064 release identity changed' unless env.fetch('EXPECTED_REPOSITORY') == 'vitaliyozolin-dotcom/ArtHello-OS' && env.fetch('EXPECTED_RELEASE_HEAD') == 'codex/school-arthello-recovery-r2-20260908' && env.fetch('EXPECTED_RELEASE_PR').to_s == '355' && env.fetch('PREVIOUS_RELEASE_SHA') == '68a159dc647f35cdd30ce586fc6d5beb93b89a51'
...
["github.event_name == 'workflow_run'", "head_branch == 'main'", "conclusion == 'success'", "actor.login == 'vitaliyozolin-dotcom'", "triggering_actor.login == 'vitaliyozolin-dotcom'", "number == fromJSON('355')", "codex/school-arthello-recovery-r2-20260908"].each {|token| raise 'Missing D064 trigger condition' unless condition.include?(token)}
...
['school-sso-readonly-diagnostic.sh', 'check-school-live-acceptance-r2.py', 'OBSERVED_LIVE_ARTHELLO_SHA', 'StrictHostKeyChecking=yes', 'SHA256:/kBNohTF+5g8U+jQt+PzOCoWZ9yCSFjBnEP3Oc3MwRI'].each {|token| raise 'Missing diagnostic or real acceptance gate' unless diagnostic.include?(token)}
...
['deploy/school/sso-relay', 'r2-school-repair-remote.py', 'EXPECTED_REPAIR_CONFIG_SHA256', 'EXPECTED_REPAIR_BUNDLE_SHA256', 'check-school-repair-receipt-r2.py', 'StrictHostKeyChecking=yes', "SHA256:/kBNohTF+5g8U+jQt+PzOCoWZ9yCSFjBnEP3Oc3MwRI"].each {|token| raise 'Missing exact protected School repair transport' unless repair.include?(token)}
...
root = 'deploy/school/sso-relay'
manifest = JSON.parse(File.read("#{root}/manifest.json"))
raise 'R2 configuration-only scope changed' unless manifest['schemaVersion'] == 1 && manifest['decisionId'] == 'D064' && manifest['school'] == {'container'=>'school-1-11','sourceSha'=>'54242340f2d9b6a9887d69ecc03520ddf9f7982c','imageId'=>'sha256:664c2c0c3e628a53ca492953803b420e0c4a44acab35eb250f5f899c10bc93df','network'=>'arthello-os_backend'}
...
raise 'R2 must preserve School application/data/shared network' unless manifest['scope'] == {'applicationBuildOnProduction'=>false,'schoolDataChanges'=>false,'schoolRuntimeChanges'=>false,'sharedNetworkInternalChanges'=>false,'tlsTermination'=>false}
...
['ruby .github/scripts/check-recovery-r2-contract.rb','python3 -I .github/scripts/test-recovery-r2-gates.py','node --test scripts/test/school-sso-relay.test.mjs','python3 -I scripts/test/school-sso-repair.test.py','bash scripts/test/school-sso-relay-docker-smoke.sh'].each {|token| raise 'Hosted R2 gate missing' unless verify_source.include?(token)}
puts 'ARTHELLO_D064_CONTRACT=VERIFIED'
