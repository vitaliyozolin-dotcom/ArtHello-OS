"""Read-only verification of an already held D080 candidate; never resumes by itself."""
import argparse
import hashlib
...
state = module('candidate_state', 'd080-candidate-state.py')
backup = module('candidate_backup', 'r7-backup-runtime.py')
...
class Docker(backup.Docker):
    def command(self, args, timeout=30):
        return command(['docker', *args]).strip()
...
def verify_backup(context, docker):
    ...
    saved = backup.StateFile(context['backupRuntimeStateFile'])
    try:
        runtime = backup.Runtime(args, docker, saved)
        runtime.state = saved.load()
        require(runtime.state is not None and runtime.state.get('schemaVersion') == 1
                and runtime.state.get('state') == 'verified' and runtime.state.get('pending') is None
                and runtime.state.get('identity') == runtime.identity)
        runtime.load_image()
        resources = runtime.state.get('resources', [])
        require(len(resources) == 5 and len({(item.get('kind'), item.get('role')) for item in resources}) == 5)
        for role in ('backups', 'control', 'activation'):
            entries = [item for item in resources if item.get('kind') == 'volume' and item.get('role') == role]
            require(len(entries) == 1)
            runtime.verify_volume(docker.inspect('volume', runtime.names[role]), role, entries[0])
        workers = [item for item in resources if item.get('kind') == 'container' and item.get('role') == 'worker']
        require(len(workers) == 1 and runtime.names['worker'] == context['backupWorker'])
        worker = docker.inspect('container', runtime.names['worker'])
        runtime.verify_container(worker, 'worker', workers[0]['id'])
        require(worker.get('State', {}).get('Running') is True and worker['State'].get('Paused') is False)
        proof = json.loads(docker.command(['container', 'exec', worker['Id'], 'python3', '-I',
                                         '/opt/arthello-backup/probe.py', '--require-initial-verified']))
        require(proof.get('schemaVersion') == 1 and proof.get('state') == 'verified' and proof.get('initialVerified') is True)
    finally:
        saved.close()
...
def verify(path, *, release, tree, run, attempt, durable_root, hold=False, docker=None, probe=True):
    ...
    require(previous.get('Name') == '/' + context['previousName'] and previous.get('State', {}).get('Running') is False
    ...
    if probe:
        command(['python3', '-I', str(HERE / 'd080-maintenance-route.py'), 'probe', '--nonce-file', context['gateNonceFile']])
    consumers = docker.command(['ps', '--no-trunc', '--filter', 'volume=' + context['dataVolume'], '--format', '{{.ID}}']).splitlines()
    for identity in consumers:
        item = docker.inspect('container', identity)
        data_mounts = [item for item in item.get('Mounts', []) if item.get('Name') == context['dataVolume']]
        require(data_mounts and (identity == context['candidateContainerId'] or all(item.get('RW') is False for item in data_mounts)))
