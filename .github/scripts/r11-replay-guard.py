...
CONSUMER_PATH = '.github/workflows/deploy-arthello-recovery-r11-20260909.yml'
...
def run():
    ...
    assert release not in (INSTALL_SHA, PREVIOUS_SHA, 'dbe630145a3828643fcfb9f202b1239e8cc54826', R8_SHA, R9_SHA, R10_SHA), 'R11 candidate identity must differ from preserved releases'
    ...
    print('ARTHELLO_D085_PRESERVED_R5_R8_R9_R10_ABORT_REPLAY_BOUNDARY=VERIFIED')
...
def validate_history(runs, get, repository, release, current_run, current_attempt, get_logs=None):
    resume = False
    assert release not in (R9_SHA, R10_SHA) and current_run not in (R9_RUN, R10_RUN), 'Frozen R9 or R10 cannot be rearmed as R11'
