...
acceptance = load('acceptance', 'check-school-live-acceptance-r3.py')
replay = load('replay', 'd065-replay-guard.py')
...
def step_script(step_id):
    path = Path(__file__).resolve().parents[1] / 'workflows/deploy-arthello-recovery-r3-20260908.yml'
...
class GateTests(unittest.TestCase):
    ...
    def test_bootstrap_rejects_root_or_changed_identity_before_capability_and_write(self):
        for real_uid,effective_uid,real_gid,effective_gid in [(0,0,0,0),(1002,1003,1002,1002),(1002,1002,1002,1003)]:
            with self.subTest(ids=(real_uid,effective_uid,real_gid,effective_gid)), mock.patch.object(transport.os,'getuid',return_value=real_uid), mock.patch.object(transport.os,'geteuid',return_value=effective_uid), mock.patch.object(transport.os,'getgid',return_value=real_gid), mock.patch.object(transport.os,'getegid',return_value=effective_gid), mock.patch.object(transport,'validated_environment') as environment:
                with self.assertRaises(ValueError):
                    transport.main()
                environment.assert_not_called()

    def test_bootstrap_uses_only_existing_readonly_regular_shared_lock(self):
        with tempfile.TemporaryDirectory() as temporary:
            path=Path(temporary)/'lock'
            with mock.patch.object(transport,'LOCK_PATH',path):
                with self.assertRaises(FileNotFoundError):
                    transport.checked_shared_lock(os.geteuid())
                self.assertFalse(path.exists())
                path.write_bytes(b'existing-coordination-inode\n')
                path.chmod(0o444)
                with mock.patch.object(transport.os,'open',wraps=os.open) as opened:
                    transport.checked_shared_lock(os.geteuid())
                    flags=opened.call_args.args[1]
                    self.assertEqual(flags & (os.O_CREAT|os.O_WRONLY|os.O_RDWR),0)
                    self.assertTrue(flags & os.O_NOFOLLOW)
                self.assertEqual(path.read_bytes(),b'existing-coordination-inode\n')
                path.chmod(0o666)
                with self.assertRaises(ValueError):
                    transport.checked_shared_lock(os.geteuid())
                path.unlink()
                target=Path(temporary)/'target';target.write_text('preserve')
                path.symlink_to(target)
                with self.assertRaises(OSError):
                    transport.checked_shared_lock(os.geteuid())
                self.assertEqual(target.read_text(),'preserve')

    def test_bootstrap_private_state_and_passwd_home_path_boundaries(self):
        with tempfile.TemporaryDirectory(dir=pwd.getpwuid(os.geteuid()).pw_dir) as temporary:
            home=Path(temporary)
            transport.checked_home(home,os.geteuid())
            transport.checked_private(home,os.geteuid())
            with self.assertRaises(ValueError):
                transport.checked_private(home,os.geteuid()+1)
            home.chmod(0o755)
            with self.assertRaises(ValueError):
                transport.checked_private(home,os.geteuid())
            for bad in [str(home)+',other',str(home)+'\nother',str(home)+'\0other']:
                with self.assertRaises(ValueError):
                    transport.checked_home(Path(bad),os.geteuid())
            link=home/'link';link.symlink_to(home,target_is_directory=True)
            with self.assertRaises(ValueError):
                transport.checked_home(link,os.geteuid())

    def test_bootstrap_real_nonroot_unpack_execute_and_cleanup_without_host_writes(self):
        # Real OS permissions exercise the full bootstrap. Only passwd and the
        # fixed lock location are fixture seams; no privilege-transition code is
        # introduced into production. Hosted CI already runs as an ordinary UID.
        with tempfile.TemporaryDirectory(dir=pwd.getpwuid(os.geteuid()).pw_dir) as temporary:
            work=Path(temporary);work.chmod(0o755)
            uid=os.geteuid() if os.geteuid()>0 else 1002
            gid=os.getegid() if os.geteuid()>0 else 1002
            home=work/'home';home.mkdir(mode=0o700)
            if os.geteuid()==0:
                if os.environ.get('GITHUB_ACTIONS')=='true':
                    self.fail('Hosted real non-root bootstrap gate must not run as root')
                self.skipTest('Local UID mapping has no ordinary caller; mandatory hosted CI executes this real non-root fixture')
            lock=work/'existing.lock';lock.write_text('unchanged');lock.chmod(0o444)
            fixture=b'import json,os; print(json.dumps({"actualUid":os.geteuid(),"fixture":True}))\n'
            raw=json.dumps({'schemaVersion':1,'files':{name:base64.b64encode(fixture if name=='repair.py' else b'fixture').decode() for name in transport.FILES}},sort_keys=True,separators=(',',':')).encode()
            environment=dict(os.environ,RELEASE_SHA='a'*40,RELEASE_RUN_ID='123',RELEASE_ATTEMPT='1',EXPECTED_REPAIR_CONFIG_SHA256='b'*64,EXPECTED_REPAIR_BUNDLE_SHA256=hashlib.sha256(raw).hexdigest())
...
m.LOCK_PATH=pathlib.Path(sys.argv[2]);home=sys.argv[3]
if os.geteuid()==0:
 os.setgroups([]);os.setgid(int(sys.argv[5]));os.setuid(int(sys.argv[4]))
m.pwd.getpwuid=lambda uid:types.SimpleNamespace(pw_dir=home)
m.main()
'''
            result=subprocess.run([os.sys.executable,'-I','-c',code,str(Path(transport.__file__)),str(lock),str(home),str(uid),str(gid)],input=raw,capture_output=True,env=environment)
            self.assertEqual(result.returncode,0,result.stderr.decode())
            self.assertEqual(json.loads(result.stdout),{'actualUid':uid,'fixture':True})
            state=home/'.arthello-school-sso-relay'
            self.assertEqual(state.stat().st_uid,uid)
            self.assertEqual(state.stat().st_mode & 0o777,0o700)
            self.assertEqual(list(state.iterdir()),[])
            self.assertEqual(lock.read_text(),'unchanged')

    def test_repair_transport_accepts_only_exact_bounded_reviewed_bundle(self):
        files={name:base64.b64encode(b'reviewed-fixture').decode() for name in transport.FILES}
        raw=json.dumps({'schemaVersion':1,'files':files}).encode()
        decoded=transport.decode_bundle(raw,hashlib.sha256(raw).hexdigest())
        self.assertEqual(set(decoded),transport.FILES)
        with self.assertRaises(ValueError):
            transport.decode_bundle(raw+b' ',hashlib.sha256(raw).hexdigest())
        escaped=dict(files,**{'../repair.py':base64.b64encode(b'bad').decode()})
        raw=json.dumps({'schemaVersion':1,'files':escaped}).encode()
        with self.assertRaises(ValueError):
            transport.decode_bundle(raw,hashlib.sha256(raw).hexdigest())
        files['repair.py']=base64.b64encode(b'x'*262145).decode()
        raw=json.dumps({'schemaVersion':1,'files':files}).encode()
        with self.assertRaises(ValueError):
            transport.decode_bundle(raw,hashlib.sha256(raw).hexdigest())
