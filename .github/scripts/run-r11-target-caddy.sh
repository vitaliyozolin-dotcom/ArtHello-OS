...
cleanup() {
  ...
  case "$work" in
    "$RUNNER_TEMP"/d083-target-caddy.*)
      [[ -d "$work" && ! -L "$work" && -O "$work" ]] || return 1
      # Only our fixed nested directory needs write permission to unlink its
      # public fixture. Refuse replacements; never chmod a symlink target.
      if [[ -e "$work/test-fixtures" || -L "$work/test-fixtures" ]]; then
        [[ -d "$work/test-fixtures" && ! -L "$work/test-fixtures" && -O "$work/test-fixtures" ]] || return 1
        chmod 0700 -- "$work/test-fixtures"
      fi
      chmod 0700 -- "$work"
      rm -rf -- "$work"
      ;;
