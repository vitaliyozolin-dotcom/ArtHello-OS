...
def validate(record, release_sha, school_sha, live_arthello_sha, repair_config_sha, repair_activated_at, now):
    ...
    assert record.get('schemaVersion') == 2
...
def main():
    ...
    url = 'https://api.github.com/repos/vitaliyozolin-dotcom/ArtHello-OS/contents/docs/acceptance/2026-09-08-school-live-acceptance-r2.json?ref=codex/recovery-evidence-20260907'
    ...
    try:
        ...
        assert receipt['state'] == 'verified' and receipt['repairConfigSha256'] == os.environ['SCHOOL_REPAIR_CONFIG_SHA256']
        validate(record, os.environ['RELEASE_SHA'], school[0], os.environ['OBSERVED_LIVE_ARTHELLO_SHA'], os.environ['SCHOOL_REPAIR_CONFIG_SHA256'], receipt['activatedAtUtc'], datetime.datetime.now(datetime.timezone.utc))
    except Exception:
