...
spec = importlib.util.spec_from_file_location('r14_contract', ROOT / 'deploy/v52/recovery-r14/verify-contract.py')
...
LOCAL_MAP = json.loads(Path(os.environ['R14_CONTRACT_TEST_SOURCE_MAP']).read_text()) if os.environ.get('R14_CONTRACT_TEST_SOURCE_MAP') else {}
...
class ContractTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.release = {'head': 'codex/r14-contract-test', 'pr': 999999, 'parentSha': 'a' * 40, 'prefix': contract.PREFIX}
    ...
    def ready_fixture(self):
        ...
        workflow = read_source(contract.VERIFICATION).replace(
            b'__R14_CONTRACT_SHA256_NONPUBLISHABLE_UNASSIGNED__', contract.digest(read_source(contract.CONTRACT)).encode())
        def read(path):
    ...
    def test_actual_23_fragments_and_semantics_match_reviewed_composition(self):
        self.assertEqual(sum(len(group['operations']) for group in self.recipe['groups']), 23)
        self.assertEqual(contract.digest(self.projection.encode()), '40eebc0aaa5addc4803333a1373b0b5d217531e58e1045fade812b511b6566ce')
    ...
    def test_history_source_literal_span_and_frozen_functions_are_independently_bound(self):
        ...
        self.assertEqual(operation['afterTemplate'].count('__R14_HISTORY_BYTES_REPR__'), 1)
        ...
        with self.assertRaisesRegex(contract.Refused, 'FROZEN_R13_CONTROLLER_CHANGED'):
        ...
        for key, value in (('beforeSha256', '0' * 64), ('afterSha256', '0' * 64),
                           ('afterTemplate', operation['afterTemplate'].replace('__R14_HISTORY_BYTES_REPR__', 'b"changed"'))):
    ...
    def test_pinned_helper_and_test_coverage_cannot_be_removed_or_changed(self):
        pins, read = self.ready_fixture()
        helper = '.github/scripts/r14-resume-candidate.py'
        ...
        del pins['sourceFiles']['.github/scripts/test-r14-backup-controller.py']
    ...
    def test_seal_copyback_consumer_and_before_after_identity_guards_are_required(self):
        for before, after in (
            ...
            ('python3 -I .github/scripts/run-r14-live-browser.py', 'true'),
        ):
    ...
    def test_hosted_verification_requires_all_behavior_gates_and_normalized_identity(self):
        ...
        for mutation in ('permission', 'test', 'pin', 'checkout'):
            ...
            if mutation == 'pin': job['env']['R14_CONTRACT_SHA256'] = '0' * 64
