#!/usr/bin/env python3
"""Read-only fixed-scope diagnostic for the failed D066 School repair.
...
def docker_command(arguments):
    ...
    if result.returncode != 0:
        ...
        network_absent = f'Error response from daemon: network {name} not found' if kind == 'network' else exact_absent
        if error in (exact_absent, alternate_absent, network_absent):
...
def diagnose():
    output = {'schemaVersion': 1, 'diagnostic': 'D066-fingerprint-read-only', 'startedAtUtc': now()}
...
SCHOOL_KEYS = ('Id', 'Image', 'Config', 'HostConfig', 'Mounts')
ENDPOINT_KEYS = ('NetworkID', 'IPAddress', 'GlobalIPv6Address', 'Aliases')
BACKEND_KEYS = ('Id', 'Name', 'Driver', 'Internal', 'EnableIPv6', 'IPAM', 'Options', 'Labels')
CONFIG_FIELDS = ('Hostname', 'Domainname', 'User', 'AttachStdin', 'AttachStdout',
                 'AttachStderr', 'ExposedPorts', 'Tty', 'OpenStdin', 'StdinOnce',
                 'Env', 'Cmd', 'Healthcheck', 'Image', 'Volumes', 'WorkingDir',
                 'Entrypoint', 'OnBuild', 'Labels', 'StopSignal', 'StopTimeout',
                 'Shell', 'ArgsEscaped')
HOST_FIELDS = ('Binds', 'ContainerIDFile', 'LogConfig', 'NetworkMode', 'PortBindings',
               'RestartPolicy', 'AutoRemove', 'VolumeDriver', 'VolumesFrom',
               'ConsoleSize', 'CapAdd', 'CapDrop', 'CgroupnsMode', 'Dns',
               'DnsOptions', 'DnsSearch', 'ExtraHosts', 'GroupAdd', 'IpcMode',
               'Cgroup', 'Links', 'OomScoreAdj', 'PidMode', 'Privileged',
               'PublishAllPorts', 'ReadonlyRootfs', 'SecurityOpt', 'UTSMode',
               'UsernsMode', 'ShmSize', 'Sysctls', 'Runtime', 'Isolation',
               'CpuShares', 'Memory', 'NanoCpus', 'CgroupParent', 'BlkioWeight',
               'BlkioWeightDevice', 'BlkioDeviceReadBps', 'BlkioDeviceWriteBps',
               'BlkioDeviceReadIOps', 'BlkioDeviceWriteIOps', 'CpuPeriod',
               'CpuQuota', 'CpuRealtimePeriod', 'CpuRealtimeRuntime', 'CpusetCpus',
               'CpusetMems', 'Devices', 'DeviceCgroupRules', 'DeviceRequests',
               'MemoryReservation', 'MemorySwap', 'MemorySwappiness',
               'OomKillDisable', 'PidsLimit', 'Ulimits', 'CpuCount',
               'CpuPercent', 'IOMaximumIOps', 'IOMaximumBandwidth',
               'MaskedPaths', 'ReadonlyPaths', 'Init', 'Mounts')
MOUNT_FIELDS = ('Type', 'Name', 'Source', 'Destination', 'Driver', 'Mode', 'RW', 'Propagation')
...
def old_school_value(school):
    # Identical to the reviewed R3 controller, including order-sensitive arrays.
    return {key: school[key] for key in SCHOOL_KEYS} | {
        'networks': {name: {key: endpoint.get(key) for key in ENDPOINT_KEYS}
                     for name, endpoint in school['NetworkSettings']['Networks'].items()}}


def old_backend_value(backend):
    return {key: backend.get(key) for key in BACKEND_KEYS}


def sorted_mounts(value):
    if not isinstance(value, list) or not all(isinstance(item, dict) for item in value):
        raise ValueError('invalid mount metadata')
    # Sorting complete entries preserves all fields, duplicates and actual values.
    return sorted(value, key=canonical)


def fingerprint_record(school, backend):
    old_school = old_school_value(school)
    mounts_sorted = sorted_mounts(old_school['Mounts'])
    normalized = old_school | {'Mounts': mounts_sorted}
    old_backend = old_backend_value(backend)
    return {'schoolExactSha256': digest(old_school),
            'schoolMountOrderNormalizedSha256': digest(normalized),
            'backendExactSha256': digest(old_backend),
            'schoolComponentSha256': {key: digest(old_school[key]) for key in (*SCHOOL_KEYS, 'networks')},
            'mountOrderNormalizedSha256': digest(mounts_sorted),
            'backendComponentSha256': {key: digest(old_backend[key]) for key in BACKEND_KEYS},
            'mountCount': len(old_school['Mounts']),
            'schoolIdentity': container_record(school, SCHOOL)}


def changed_object_fields(before, after, prefix, allowed):
    if before == after:
        return []
    if not isinstance(before, dict) or not isinstance(after, dict):
        return [prefix]
    # Never print unknown keys, label names, mount paths or environment entries.
    changed = [prefix + '.' + key for key in allowed
               if (key in before) != (key in after) or before.get(key) != after.get(key)]
    old_other = {key: value for key, value in before.items() if key not in allowed}
    new_other = {key: value for key, value in after.items() if key not in allowed}
    if old_other != new_other:
        changed.append(prefix + '.<other>')
    return changed


def compare_samples(previous_school, previous_backend, school, backend):
    before, after = old_school_value(previous_school), old_school_value(school)
    fields = []
    for key in ('Id', 'Image'):
        if before[key] != after[key]:
            fields.append('School.' + key)
    fields += changed_object_fields(before['Config'], after['Config'], 'School.Config', CONFIG_FIELDS)
    fields += changed_object_fields(before['HostConfig'], after['HostConfig'], 'School.HostConfig', HOST_FIELDS)
    mount_changed = before['Mounts'] != after['Mounts']
    mount_order_only = mount_changed and sorted_mounts(before['Mounts']) == sorted_mounts(after['Mounts'])
    if mount_changed:
        fields.append('School.Mounts.order' if mount_order_only else 'School.Mounts.content')
        if not mount_order_only:
            for index in range(min(len(before['Mounts']), len(after['Mounts']))):
                fields += changed_object_fields(before['Mounts'][index], after['Mounts'][index],
                                                'School.Mounts[' + str(index) + ']', MOUNT_FIELDS)
    if before['networks'] != after['networks']:
        fields.append('School.NetworkSettings.Networks')
        fields += changed_object_fields(before['networks'].get(BACKEND), after['networks'].get(BACKEND),
                                        'School.NetworkSettings.Networks.backend', ENDPOINT_KEYS)
    old_backend, new_backend = old_backend_value(previous_backend), old_backend_value(backend)
    fields += changed_object_fields(old_backend, new_backend, 'Backend', BACKEND_KEYS)
    school_exact_changed = digest(before) != digest(after)
    school_normalized_equal = digest(before | {'Mounts': sorted_mounts(before['Mounts'])}) == digest(after | {'Mounts': sorted_mounts(after['Mounts'])})
    return {'changedFieldPaths': fields,
            'schoolExactChanged': school_exact_changed,
            'backendExactChanged': old_backend != new_backend,
            'mountsOrderOnlyChanged': mount_order_only,
            'schoolChangeExplainedOnlyByMountOrder': school_exact_changed and school_normalized_equal,
            'normalizedSchoolChanged': not school_normalized_equal}
...
def sample_fingerprints():
    ...
    for index in range(SAMPLE_COUNT):
        school, school_error = docker_command(['docker', 'container', 'inspect', SCHOOL])
        backend, backend_error = docker_command(['docker', 'network', 'inspect', BACKEND])
        record = {'sampleIndex': index, 'observedAtUtc': now()}
        if school is None or backend is None:
            record.update(state='unavailable', schoolError=school_error, backendError=backend_error)
            previous = None
        else:
            try:
                record.update(state='observed', **fingerprint_record(school, backend))
                if previous is not None:
                    record['changeFromPrevious'] = compare_samples(*previous, school, backend)
                previous = school, backend
            except (KeyError, TypeError, ValueError, AttributeError):
                record.update(state='invalid', error='docker_metadata_invalid')
                previous = None
        observations.append(record)
        if index + 1 < SAMPLE_COUNT:
            time.sleep(SAMPLE_INTERVAL_SECONDS)
    return {'requestedSamples': SAMPLE_COUNT, 'samples': observations,
            'complete': len(observations) == SAMPLE_COUNT and all(item['state'] == 'observed' for item in observations),
            'normalization': 'Only Mounts array order; complete mount records and duplicate counts preserved',
            'limitation': 'Observations describe this diagnostic interval; original failed-setup object values were not retained'}


def full_diagnosis():
    baseline = diagnose()
    baseline['fingerprintSeries'] = sample_fingerprints()
    baseline['completedAtUtc'] = now()
    return baseline
...
if __name__ == '__main__':
    if len(sys.argv) != 1:
        ...
        sys.exit(2)
    try:
        print(json.dumps(full_diagnosis(), sort_keys=True, separators=(',', ':')))
