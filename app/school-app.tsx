...
const classLabel = (name: string) => /(?:^|\s)класс(?:\s|$)/i.test(name) ? name : `${name} класс`;
    ...
    <div className="l0-stage">
      <div className={cn("l0-app", `role-${snapshot.viewer.role}`)}>
        <aside className="l0-rail">
          ...
          <div className="rail-context" title={familyContext && snapshot.selectedStudent ? `${snapshot.selectedStudent.firstName}, ${classLabel(snapshot.selectedStudent.className)}` : `${snapshot.viewer.displayName}, ${roleLabels[snapshot.viewer.role]}`}>
            {familyContext && snapshot.selectedStudent ? <><Avatar name={snapshot.selectedStudent.fullName} color={snapshot.selectedStudent.avatarColor} size="sm" /><span><strong>{snapshot.selectedStudent.firstName}</strong><small>{classLabel(snapshot.selectedStudent.className)}</small></span></> : <><Avatar name={snapshot.viewer.displayName} size="sm" /><span><strong>{snapshot.viewer.displayName}</strong><small>{roleLabels[snapshot.viewer.role]}</small></span></>}
          </div>
        </aside>
...
function RankingBoard({ snapshot, compact = false }: { snapshot: SchoolSnapshot; compact?: boolean }) {
  ...
  return <section className={cn("content-card", "ranking-panel", compact && "ranking-compact")}>
    <div className="ranking-heading"><div><span className="eyebrow">{anonymous ? "Позиция в своём классе" : "Рейтинг по классам"}</span><h2>{anonymous ? classLabel(classRanking.className) : "Учебные результаты"}</h2><p>{anonymous ? "Другие дети показаны только образами животных" : "Поимённо, отдельно по каждому классу и предмету"}</p></div>{!anonymous && snapshot.rankings.classes.length > 1 ? <label className="ranking-class-select"><span>Класс</span><select value={classRanking.className} onChange={(event) => { setClassName(event.target.value); setMetricId("overall"); }}>{snapshot.rankings.classes.map((item) => <option key={item.className} value={item.className}>{classLabel(item.className)}</option>)}</select></label> : null}</div>
...
function StudentDashboard({ snapshot, onView }: { snapshot: SchoolSnapshot; onView: (view: View) => void }) {
  ...
  return <div className="page-shell student-dashboard">
    <section className="student-hero">
      ...
      <div className="student-score-orbit"><span>Средний балл</span><strong>{weightedAverage(ownGrades)}</strong><small>{student?.className ? classLabel(student.className) : ""}</small></div>
    </section>
...
function StudyPage({ snapshot, openAction, initialTab = "schedule" }: { snapshot: SchoolSnapshot; openAction: (kind: ActionKind, preset?: Record<string, string>) => void; initialTab?: "grades" | "homework" | "schedule" | "success" | "comments" }) {
  ...
  return (
    <div className="page-shell">
      ...
      {tab === "schedule" ? <section className="content-card study-panel schedule-editor">{scheduleClassSelector ? <div className="schedule-toolbar"><label><span>Класс</span><select value={scheduleClass} onChange={(event) => setScheduleClass(event.target.value)}>{snapshot.classes.map((schoolClass) => <option value={schoolClass.name} key={schoolClass.id}>{classLabel(schoolClass.name)}{schoolClass.homeroomTeacherName ? ` · ${schoolClass.homeroomTeacherName}` : ""}</option>)}</select></label>{scheduleEditor ? <div><button className="ghost-btn" onClick={() => openAction("lesson.copy-day", { className: scheduleClass, sourceWeekday: String(day) })}><Icon name="clipboard" size={17} />Скопировать день</button><button className="primary-btn" onClick={addLesson}><Icon name="calendar" size={17} />Добавить урок</button></div> : <StatusPill tone="blue">Только просмотр</StatusPill>}</div> : null}<div className="day-switch">{[1, 2, 3, 4, 5, 6].map((weekday) => <button key={weekday} className={cn(day === weekday && "active")} onClick={() => setDay(weekday)}><span>{weekdayShort[weekday]}</span><small>{snapshot.lessons.filter((lesson) => lesson.weekday === weekday && (!scheduleClassSelector || lesson.className === scheduleClass)).length} уроков</small></button>)}</div><SectionTitle title={weekdays[day]} subtitle={`${classLabel(scheduleClassSelector ? scheduleClass : snapshot.selectedStudent?.className ?? scheduleClass)}${scheduleEditor ? " · нажмите «Изменить» у нужного урока" : methodistMode ? " · методический просмотр" : ""}`} /><LessonList snapshot={snapshot} day={day} className={scheduleClassSelector ? scheduleClass : undefined} onEdit={scheduleEditor ? editLesson : undefined} /></section> : null}
...
function CalendarPage({ snapshot, openAction }: { snapshot: SchoolSnapshot; openAction: (kind: ActionKind, preset?: Record<string, string>) => void }) {
  ...
  const calendarItems = [
    ...
    ...snapshot.homework.map((item) => ({ id: item.id, type: "Задание", title: item.title, at: item.dueAt, meta: `${classLabel(item.className)} · ${item.subjectName}`, tone: "violet" })),
  ...
  return <div className="page-shell">
    ...
    {snapshot.programs.length ? <section className="program-grid">{snapshot.programs.map((program) => {
      ...
      return <article key={program.id} className={cn(program.id === selectedProgram?.id && "selected", program.unscheduledLessons > 0 && "has-deficit")}>
        ...
        <small>{classLabel(program.className)} · {program.subjectName}</small><strong>{program.title}</strong><p>{program.teacherName}</p>
    ...
    {selectedProgram ? <section className="content-card curriculum-detail">
      <div className="curriculum-detail-head"><SectionTitle title={`${classLabel(selectedProgram.className)} · ${selectedProgram.subjectName}`} subtitle={selectedProgram.sourceFileName ? `Источник: ${selectedProgram.sourceFileName}` : "Темы ещё не импортированы"} /><span className="curriculum-lock-note"><Icon name="lock" size={16} />Даты не сдвигаются автоматически</span></div>
...
export default function SchoolApp() {
  ...
  return (
    <>
      <AppShell snapshot={snapshot} activeView={activeView} onView={(view) => { setPreviewStudentId(null); setParentPreviewOpen(false); setPreviewSelection(""); navigate(view); }} onStudent={(nextStudent) => void switchStudent(nextStudent)} helpAction={helpAction} helpOverlayKey={helpOverlayKey} helpOverlayLabel={helpOverlayLabel} helpPortalTarget={helpPortalTarget}>
        {previewStudentId ? <ParentPreview key={previewStudentId} studentId={previewStudentId} onClose={() => setPreviewStudentId(null)} /> : <>
          {showLeadershipParentPreview && parentPreviewOpen ? <section className="page-shell parent-preview-tools" aria-label="Предпросмотр учебных сведений семьи"><div className="content-card"><span className="eyebrow">Инструмент руководителя</span><h2>Предпросмотр для родителя</h2><p>Выберите ребёнка, чтобы проверить сведения, доступные его семье.</p><div className="hero-actions"><label>Ребёнок<select aria-label="Ребёнок для предпросмотра" value={previewSelection} onChange={event => setPreviewSelection(event.target.value)}><option value="">Выберите ребёнка</option>{snapshot.students.map(student => <option key={student.id} value={student.id}>{student.fullName} · {classLabel(student.className)}</option>)}</select></label><button className="ghost-btn" disabled={!previewSelection} onClick={() => setPreviewStudentId(previewSelection)}>Посмотреть глазами родителя</button><button className="ghost-btn" onClick={() => { setParentPreviewOpen(false); setPreviewSelection(""); }}>Закрыть</button></div></div></section> : null}
          {content}
