...
function LessonList({ snapshot, day = todayWeekday(), compact = false, className, onEdit }: { snapshot: SchoolSnapshot; day?: number; compact?: boolean; className?: string; onEdit?: (lesson: SchoolSnapshot["lessons"][number]) => void }) {
  ...
  return <div className={cn("lesson-list", compact && "lesson-list-compact")}>{lessons.map((lesson, index) => <article className="lesson-line" key={lesson.id}><span className="lesson-order">{index + 1}</span><span className="lesson-time"><strong>{lesson.startsAt}</strong><small>{lesson.endsAt}</small></span><span className="subject-bar" style={{ background: lesson.subjectColor }} /><span className="lesson-copy"><strong>{lesson.subjectName}</strong><small>{lesson.room} · {lesson.teacherName ?? "Учитель не назначен"}</small>{lesson.note ? <em>{lesson.note}</em> : null}</span>{lesson.status === "moved" ? <StatusPill tone="warn">Изменение</StatusPill> : lesson.status === "completed" ? <StatusPill tone="good">Проведён</StatusPill> : null}{onEdit ? <button className="lesson-edit" onClick={() => onEdit(lesson)} aria-label={`Изменить урок ${lesson.subjectName}`}><Icon name="settings" size={16} />Изменить</button> : null}</article>)}</div>;
}
...
function LeadershipDashboard({ snapshot, onView }: { snapshot: SchoolSnapshot; onView: (view: View) => void }) {
  ...
  return <div className="page-shell">
    <div className="page-heading"><div><span className="eyebrow">ArtHello OS · Обучение</span><h1>Школа 1–11</h1><p>Достоверная картина учебного дня и конкретные точки внимания</p></div><button className="primary-btn" onClick={() => onView("calendar")}><Icon name="calendar" size={18} />Открыть календарь</button></div>
    <section className="metric-grid">
...
function StudyPage({ snapshot, openAction, initialTab = "schedule" }: { snapshot: SchoolSnapshot; openAction: (kind: ActionKind, preset?: Record<string, string>) => void; initialTab?: "grades" | "homework" | "schedule" | "success" | "comments" }) {
  const academicMode = snapshot.viewer.role === "teacher" || snapshot.viewer.role === "director" || snapshot.viewer.role === "deputy";
...
function ProgramsPage({ snapshot, openAction }: { snapshot: SchoolSnapshot; openAction: (kind: ActionKind, preset?: Record<string, string>) => void }) {
  const canEdit = snapshot.viewer.role === "teacher" || snapshot.viewer.role === "director" || snapshot.viewer.role === "deputy";
  const statusMeta: Record<string, { label: string; tone: "good" | "warn" | "blue" | "neutral" }> = {
    draft: { label: "Черновик", tone: "neutral" }, review: { label: "На проверке", tone: "blue" }, changes_requested: { label: "Нужны правки", tone: "warn" }, approved: { label: "Утверждена", tone: "good" }, active: { label: "Используется", tone: "good" }, archived: { label: "Архив", tone: "neutral" },
  };
  return <div className="page-shell">
    <div className="page-heading"><div><span className="eyebrow">Учебный год → класс → предмет</span><h1>{snapshot.viewer.role === "teacher" ? "Моя программа" : "Учебные программы"}</h1><p>Годовой план, поурочные темы, факт проведения и отклонение от плана</p></div>{canEdit ? <button className="primary-btn" onClick={() => openAction("program.upsert")}><Icon name="book" size={18} />Новая программа</button> : null}</div>
    <section className="metric-grid program-metrics"><MetricCard label="Программы" value={String(snapshot.programs.length)} caption="в системе" icon="book" /><MetricCard label="На проверке" value={String(snapshot.programs.filter((item) => item.status === "review").length)} caption="ожидают завуча" icon="clock" tone="blue" /><MetricCard label="Утверждены" value={String(snapshot.programs.filter((item) => ["approved", "active"].includes(item.status)).length)} caption="можно использовать" icon="check" tone="green" /><MetricCard label="Без программы" value={String(Math.max(0, snapshot.teacherAssignments.length - snapshot.programs.length))} caption="назначений требуют плана" icon="info" tone="amber" /></section>
    {snapshot.programs.length ? <section className="program-grid">{snapshot.programs.map((program) => { const meta = statusMeta[program.status] ?? statusMeta.draft; const progress = program.plannedLessons ? Math.min(100, Math.round(program.completedLessons / program.plannedLessons * 100)) : 0; return <button key={program.id} disabled={!canEdit} onClick={() => openAction("program.upsert", { programId: program.id, className: program.className, subjectId: program.subjectId, teacherUserId: program.teacherUserId, title: program.title, plannedLessons: String(program.plannedLessons), status: program.status })}><header><span style={{ background: `${snapshot.subjects.find((item) => item.id === program.subjectId)?.color ?? "#e84412"}20` }}><Icon name="book" /></span><StatusPill tone={meta.tone}>{meta.label}</StatusPill></header><small>{program.className} класс · {program.subjectName}</small><strong>{program.title}</strong><p>{program.teacherName}</p><div className="program-progress"><span><i style={{ width: `${progress}%` }} /></span><small>{program.completedLessons} из {program.plannedLessons} уроков</small></div></button>; })}</section> : <section className="content-card"><EmptyState title="Программы ещё не загружены" text="Учитель создаёт программу вручную или импортирует проверенный XLSX; PDF сам по себе программой не считается." icon="book" /></section>}
  </div>;
}
...
function ActionModal({ state, snapshot, close, submit }: { state: Exclude<ModalState, null>; snapshot: SchoolSnapshot; close: () => void; submit: (kind: ActionKind, values: Record<string, unknown>) => Promise<void> }) {
...
function RegistrationGate({ access, retry, activated }: { access: AccessState; retry: () => void; activated: () => void }) {
  ...
  if (access.code === "sign_in_required") return <div className="gate-stage"><section className="access-card"><Image src="/school-logo.svg" alt="" width={68} height={68} /><span className="eyebrow">Школа 1–11</span><h1>Войдите в дневник</h1><p>Используйте номер телефона и пароль, созданный при первом входе.</p><button className="primary-btn" onClick={signIn}><Icon name="lock" size={18} />Войти безопасно</button><small>Доступ выдаётся персонально и ограничивается назначенной ролью.</small></section></div>;
  ...
  if (access.code !== "registration_required") return <div className="gate-stage"><section className="access-card"><Image src="/school-logo.svg" alt="" width={68} height={68} /><span className="eyebrow">Школа 1–11</span><h1>Дневник временно недоступен</h1><p>{access.message}</p><button className="primary-btn" onClick={retry}>Попробовать снова</button></section></div>;
...
function InviteResultModal({ result, close }: { result: InviteResult; close: () => void }) {
  ...
  const copy = async () => { await navigator.clipboard.writeText(result.inviteLink); setCopied(true); };
  return <div className="modal-backdrop"><section className="action-modal invite-result" role="dialog" aria-modal="true"><header><div><span className="eyebrow">Приглашение готово</span><h2>Отправьте эту ссылку семье</h2></div><button onClick={close} aria-label="Закрыть"><Icon name="close" /></button></header><div className="invite-result-body"><span className="success-mark"><Icon name="check" /></span><p>Ссылка одноразовая или ограничена выбранным количеством регистраций. В сообщении не указывайте персональные данные ребёнка.</p><label><span>Ссылка для регистрации</span><input value={result.inviteLink} readOnly /></label><button className="primary-btn" onClick={() => void copy()}><Icon name="clipboard" size={18} />{copied ? "Ссылка скопирована" : "Скопировать ссылку"}</button><small>Действует до {formatDateTime(result.expiresAt)}</small></div></section></div>;
}
...
export default function SchoolApp() {
  ...
  const submit = async (kind: ActionKind, values: Record<string, unknown>) => {
    const response = await fetch("/api/school", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: kind, ...values }) });
    const payload = await response.json() as { error?: string; inviteLink?: string; inviteCode?: string; expiresAt?: string };
  ...
  const openAction = (kind: ActionKind, preset?: Record<string, string>) => setModal({ kind, preset });
