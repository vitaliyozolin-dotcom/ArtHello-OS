  ...
  return <div className="page-shell">
    ...
    {snapshot.programs.length ? <section className="program-grid">{snapshot.programs.map((program) => {
      ...
      return <article key={program.id} className={cn(program.id === selectedProgram?.id && "selected", program.unscheduledLessons > 0 && "has-deficit")}>
        ...
        <footer><button onClick={() => setSelectedProgramId(program.id)}>Темы и даты</button>{canEdit ? <button onClick={() => openAction("program.upsert", { programId: program.id, className: program.className, subjectId: program.subjectId, teacherUserId: program.teacherUserId, title: program.title, plannedLessons: String(program.plannedLessons), status: program.status })}>Статус</button> : null}</footer>
      </article>;
    ...
    {selectedProgram ? <section className="content-card curriculum-detail">
      <div className="curriculum-detail-head"><SectionTitle title={`${selectedProgram.className} класс · ${selectedProgram.subjectName}`} subtitle={selectedProgram.sourceFileName ? `Источник: ${selectedProgram.sourceFileName}` : "Темы ещё не импортированы"} /><button className="ghost-btn" disabled={!selectedTopics.length} onClick={() => openAction("program.reschedule", { programId: selectedProgram.id })}><Icon name="calendar" size={17} />Пересчитать даты</button></div>
      ...
      {selectedTopics.length ? <div className="curriculum-table" role="table" aria-label="Темы и даты программы"><div className="curriculum-row curriculum-head" role="row"><span role="columnheader">№</span><span role="columnheader">Дата</span><span role="columnheader">Тема урока</span><span role="columnheader">Домашнее задание</span></div>{selectedTopics.map((topic) => <div className={cn("curriculum-row", !topic.scheduledDate && "unscheduled")} role="row" key={topic.id}><span role="cell">{topic.sessionIndex > 1 ? `${topic.sequence}.${topic.sessionIndex}` : topic.sequence}</span><span role="cell">{topic.scheduledDate ? `${formatDate(topic.scheduledDate)}${topic.startsAt ? ` · ${topic.startsAt}` : ""}` : "Не распределено"}</span><strong role="cell">{topic.topic}</strong><span role="cell">{topic.homework || "—"}</span></div>)}</div> : <EmptyState title="Тем пока нет" text="Загрузите XLSX: система найдёт колонки «Тема», «Количество часов» и «Домашнее задание»." icon="clipboard" />}
    </section> : null}
...
const modalTitles: Record<ActionKind, string> = {
  ...
  "program.reschedule": "Пересчитать даты программы",
