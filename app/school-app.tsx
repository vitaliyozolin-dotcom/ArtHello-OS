...
function ProgramsPage({ snapshot, openAction }: { snapshot: SchoolSnapshot; openAction: (kind: ActionKind, preset?: Record<string, string>) => void }) {
  ...
  return <div className="page-shell">
    ...
    {selectedProgram ? <section className="content-card curriculum-detail">
      ...
      {selectedTopics.length ? <div className="curriculum-table" role="table" aria-label="Темы и даты программы"><div className="curriculum-row curriculum-head" role="row"><span role="columnheader">№</span><span role="columnheader">Дата</span><span role="columnheader">Тема урока</span><span role="columnheader">Домашнее задание</span><span role="columnheader">Действие</span></div>{selectedTopics.map((topic) => <div className={cn("curriculum-row", !topic.scheduledDate && "unscheduled")} role="row" key={topic.id}><span role="cell">{topic.sessionIndex > 1 ? `${topic.sequence}.${topic.sessionIndex}` : topic.sequence}</span><span role="cell">{topic.scheduledDate ? `${formatDate(topic.scheduledDate)}${topic.startsAt ? ` · ${topic.startsAt}` : ""}` : "Не распределено"}</span><strong role="cell">{topic.topic}</strong><span role="cell">{topic.homework || "—"}</span>{canEdit ? <button className="curriculum-edit" onClick={() => openAction("program.topic.update", { sessionId: topic.id, topic: topic.topic, homework: topic.homework })}><Icon name="settings" size={15} />Изменить</button> : <span role="cell">—</span>}</div>)}</div> : <EmptyState title="Тем пока нет" text="Загрузите XLSX: система найдёт колонки «Тема», «Количество часов» и «Домашнее задание»." icon="clipboard" />}
    </section> : null}
